 ...
 * primitive (the libchaos {@code .so} is unavailable on the current OS, the JVM agent isn't loaded,
 * the active connection-chaos backend is Toxiproxy and the verb requires libchaos-net, etc.).
 *
 ...
 * their environment. {@link #ABORT} is the opt-in for environments where the chaos library is known
 * to be sometimes-unavailable; tests show as the JUnit 5 ABORTED state (yellow in IDE, distinct in
 * CI reports), which is visible enough that nobody mistakes it for a clean pass.
 *
...
public enum OnMissingEnv {
   ...
   * printable reason. Opt-in for tests that must remain runnable on developer machines without the
   * chaos library installed.
   */
  ABORT
}
