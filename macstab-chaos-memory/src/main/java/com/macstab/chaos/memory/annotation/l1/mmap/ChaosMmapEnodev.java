...
/**
 * Injects {@code ENODEV} on every libchaos-memory-intercepted {@code mmap} call inside
 * the target container, making the call fail as if the kernel returned {@code ENODEV}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP}, errno = {@code ENODEV})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code ENODEV} is a valid POSIX result of {@code mmap}; the invalid combinations
 ...
 * in the test class. Use the repeatable form ({@code @ChaosMmapEnodevs}) to bind different
