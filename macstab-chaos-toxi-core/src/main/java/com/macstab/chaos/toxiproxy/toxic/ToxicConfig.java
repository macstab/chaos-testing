 ...
 *
 * <ol>
 *   <li><strong>Exhaustive switch:</strong> Code that switches on {@code ToxicConfig} subtypes can
 *       be verified by the compiler as exhaustive, eliminating defensive {@code default} branches
 *       and the associated dead-code risk.
 ...
 *
 * <p>The sealed chain is two-level: {@code ToxicConfig permits AbstractToxic}, and {@code
 * AbstractToxic permits LatencyToxic, TimeoutToxic, ...}. A single-level chain ({@code ToxicConfig
 * permits LatencyToxic, ...}) would force each concrete class to re-implement the common {@code
 * name}, {@code toxicity}, validation, and builder scaffold. The two-level chain deduplicates this
 * structure via {@link AbstractToxic} (Template Method pattern, GoF) while preserving the sealed
 ...
 * {@code "type"}, and {@code "toxicity"} — is assembled by {@link
 * com.macstab.chaos.toxiproxy.api.ToxiproxyApiClientImpl} in its {@code buildToxicJson()} method.
 * This separation is deliberate: the outer envelope structure is identical for all toxics; only the
 * attributes object is type-specific. Each class need only know its own domain.
 *
 * <h2>Toxicity Probability Model</h2>
 *
 * <p>Toxiproxy implements toxicity as a <em>Bernoulli trial per connection</em>. Each new
 * connection independently has probability {@code toxicity} of experiencing the fault. This means:
 *
 * <ul>
 *   <li>{@code toxicity=1.0} — every connection is affected (deterministic for testing)
 *   <li>{@code toxicity=0.3} — each connection has a 30% independent probability of being affected;
 *       over many connections, approximately 30% will be affected
 ...
 * values require a sufficient number of connections for statistical significance; a test that opens
 * only 1–2 connections with {@code toxicity=0.1} has a ~81% chance of seeing zero failures, making
 * the test non-deterministic.
 *
 * <h2>Available Implementations</h2>
 *
 * <ul>
 *   <li>{@link LatencyToxic} — adds fixed delay ± jitter to data chunks; simulates slow networks
 ...
 *   <li>{@link BandwidthToxic} — throttles throughput per connection in KB/s; simulates slow links
 ...
 *   <li>{@link LimitDataToxic} — closes the connection after a cumulative byte threshold; simulates
 *       partial reads, truncated responses, and reconnection pressure
 ...
 */
public sealed interface ToxicConfig permits AbstractToxic {
   ...
   * conflict. Within a single proxy, names must be unique — Toxiproxy uses the name as the key for
   * update ({@code PUT}) and delete ({@code DELETE}) operations. Using a duplicate name when
   * calling {@link com.macstab.chaos.toxiproxy.api.ToxiproxyApiClient#addToxic} will fail at the
   * Toxiproxy API level.
   *
   ...
   *
   * <p>Maps to one of Toxiproxy's registered fault types. The value must exactly match the string
   * Toxiproxy expects — a mismatch results in a Toxiproxy API error with HTTP 400. Known values as
   * of Toxiproxy 2.x: {@code "latency"}, {@code "timeout"}, {@code "bandwidth"}, {@code
   * "slow_close"}, {@code "limit_data"}, {@code "down"}.
   *
   * <p>Each concrete implementation returns a compile-time constant. The type string is part of the
   * public API contract — changing it is a breaking change.
   *
  ...
  /**
   * Serializes this toxic's type-specific attributes to a JSON object string for use as the {@code
   * "attributes"} value in the Toxiproxy REST API payload.
   *
   * <p><strong>Returns attributes only — NOT the full payload:</strong> The returned string is the
   * value of the {@code "attributes"} field, not the complete JSON body. For example, {@link
   * LatencyToxic} with 100 ms latency and 10 ms jitter returns:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p><strong>Why attributes-only:</strong> The outer envelope ({@code name}, {@code type}, {@code
   * toxicity}) is structurally identical for every toxic type and is assembled by {@link
   * com.macstab.chaos.toxiproxy.api.ToxiproxyApiClientImpl}. Separating responsibilities means each
   * implementation only encodes its own domain knowledge (the Toxiproxy attribute schema for its
   * type) and is not coupled to the REST wire format.
   *
   ...
   *
   * @return JSON object string representing the toxic's attributes; never null; may be empty object
   *     ({@code {}}) for toxics with no attributes
   */
  String toJson();
}
