...
/**
 * Injects {@code ENOTCONN} on every libchaos-intercepted {@code shutdown} call inside the target
 * container, making the call fail as if the kernel returned {@code ENOTCONN}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code ENOTCONN}) pair and has no runtime
 * selector-errno matrix to validate. The combination is safe by construction: this annotation class
 * exists only because {@code ENOTCONN} is a valid POSIX result of {@code shutdown}.
 *
 ...
 * interceptor returns {@code -1} and sets {@code errno = ENOTCONN} — from the application's
 * perspective this is indistinguishable from a real kernel-level failure. Specifically this
 * simulates: socket not connected — shutdown on idle socket race.
 *
 ...
 * class. Use the repeatable form ({@code @ChaosShutdownEnotconns}) to bind different probabilities
