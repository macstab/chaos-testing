 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code EXECVE}, errno = {@code E2BIG}, effect =
 ...
 * with the encoded errno until the rule is removed. This models resource-exhaustion scenarios where
 * capacity is finite and depletes deterministically rather than probabilistically.
 *
 ...
 *       intercepted call sets {@code errno = E2BIG} and returns {@code -1} without executing.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 7, {@code strerror}: "Argument
 *       list too long"; the counter remains tripped until the rule is removed or the container
 *       restarts.
 * </ol>
 *
 ...
 *       child processes until the underlying condition is resolved.
 ...
 *       requests that eventually causes queue overflow.
 ...
 *       the threshold should complete normally; requests submitted after should be rejected with a
 *       clear backpressure signal rather than timing out silently.
 * </ul>
 *
 * Production failure mode: a container orchestrator's argument-accumulation bug gradually grows the
 * environment size across successive re-deployments; after N successful starts the environment
 ...
 * on any call with probability {@code p}, so failures are scattered randomly throughout the call
 * sequence; FAIL_AFTER fires deterministically after exactly {@code N} successes, so failures
 * cluster at the end of the sequence. For {@code execve}, the deterministic model is more realistic
 * than the probabilistic one for argument-limit scenarios: the environment size grows monotonically
 * with each deployment and the limit is crossed at a predictable point.
 *
 * <p>The counter-trip semantic is permanent until rule removal — there is no auto-reset after the
 * container makes additional calls. This means the failing state persists for the lifetime of the
 * test unless the rule is explicitly removed via {@code AdvancedProcessChaos.remove}. Test methods
 * that share a class-scoped annotation must account for this: if {@code test1} exhausts the
 * counter, {@code test2} will also see failures from the first call. Use method- scoped annotations
 * to isolate counter state between test methods.
 *
 * <p>Setting {@link #successesBeforeFailure} to zero makes the very first {@code execve} call fail,
 * which is useful for testing startup-time argument validation — verifying that the application's
 * own launch sequence handles exec failure gracefully before any worker process is started. Setting
 * it to the expected number of worker spawns per test scenario exercises the exact saturation
 * boundary that production environments cross during overload.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
