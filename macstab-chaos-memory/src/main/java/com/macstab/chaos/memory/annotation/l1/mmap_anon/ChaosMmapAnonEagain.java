...
/**
 * Injects {@code EAGAIN} on every libchaos-memory-intercepted {@code mmap(MAP_ANONYMOUS)} call inside
 * the target container, making the call fail as if the kernel returned {@code EAGAIN}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP_ANON}, errno = {@code EAGAIN})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code EAGAIN} is a valid POSIX result of {@code mmap(MAP_ANONYMOUS)}; the invalid combinations
 ...
 * kernel-level failure. Specifically this simulates: temporary failure (would block) — surfaces in kernels under memory pressure or rlimit exhaustion.
 *
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-4 to 1e-3 to simulate transient pressure; high rates quickly saturate retry budgets.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every memory-chaos-capable container
 * in the test class. Use the repeatable form ({@code @ChaosMmapAnonEagains}) to bind different
