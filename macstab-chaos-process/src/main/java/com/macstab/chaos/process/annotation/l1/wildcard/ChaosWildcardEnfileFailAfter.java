 ...
 * {@code every interposed process syscall} calls succeed, then fail every subsequent call with {@code ENFILE}.
 *
 * <p><strong>What this simulates:</strong> resource exhaustion at the every interposed process syscall boundary — typical of fd-leak / thread-leak bugs that succeed indefinitely until the resource pool is drained, at which point every further call fails with ENFILE.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
