...
/**
 * <h2>What this is</h2>
 * <p>Simulates the scenario where G1 GC temporarily exceeds the configured {@code -Xmx} during
 * heap evacuation, pushing RSS above the cgroup memory limit. The kernel OOM killer terminates
 * the JVM with exit code 137 mid-GC — no Java {@code OutOfMemoryError} appears in logs.
 *
 * <h2>Composed of</h2>
 * <ul>
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Critical</strong><br>The pod is killed with no Java OOM in logs; GC log
 * shows "GC overhead limit exceeded" then silence. Engineers waste hours looking for a Java-level
...
@ChaosL3(composer = "com.macstab.chaos.kubernetes.testpack.l3.composers.K8sOomKillMidGcComposer", severity = Severity.CRITICAL)
