 ...
 * succeed, then injects {@code ESRCH} on every subsequent call until the rule is removed.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one
 * (selector = {@code WILDCARD}, errno = {@code ESRCH}, effect = FAIL_AFTER) tuple.
 ...
 * every subsequent call returns {@code -1} with {@code errno = ESRCH}, regardless of real
 ...
 * ({@code @ChaosWildcardEsrchFailAfter.Repeatable}) to apply different counters to different containers.
 *
