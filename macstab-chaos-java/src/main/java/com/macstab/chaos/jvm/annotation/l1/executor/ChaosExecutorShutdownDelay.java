 ...
 * {@link java.util.concurrent.ExecutorService#shutdown shutdown()} and {@link
 * java.util.concurrent.ExecutorService#shutdownNow shutdownNow()} call, stretching the time window
 * between when the application requests a shutdown and when the executor actually stops accepting
 * new tasks.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>An L1 JVM chaos primitive in the {@code EXECUTOR} selector family targeting the {@code
 * EXECUTOR_SHUTDOWN} operation with the {@code delay} effect. It intercepts every call to {@code
 * shutdown()} and {@code shutdownNow()} on {@code ExecutorService} implementations in the container
 * JVM and parks the calling thread for the configured duration before allowing the shutdown to
 * proceed. After the sleep, the executor transitions normally to the shutdown state: no new tasks
 * are accepted, and (for {@code shutdownNow()}) queued tasks are returned as a list.
 *
 ...
 *
 * <ul>
 *   <li>{@code executor.shutdown()} returns only after the sleep — graceful shutdown takes at least
 *       {@link #delayMs} ms longer than expected.
 ...
 * by lock contention or deregistration from a service registry) pushes the actual drain window past
 * the deadline, causing the orchestrator to SIGKILL the process mid-request.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p><strong>Interception point.</strong> The agent targets {@code
 * java.util.concurrent.ExecutorService#shutdown()} and {@code
 * java.util.concurrent.ExecutorService#shutdownNow()} via Byte Buddy retransformation. Concrete
 * implementations ({@code ThreadPoolExecutor}, {@code ScheduledThreadPoolExecutor}) are also
 * covered because they inherit or override these methods from the abstract base class.
 *
 ...
 * after the actual shutdown, where submissions throw {@code RejectedExecutionException}. Tests that
 * check for rejection must account for this window.
 *
 ...
 * delay, the time for running tasks to finish, and the time {@code awaitTermination} blocks. If the
 * application calls {@code awaitTermination(5, SECONDS)} immediately after the delayed {@code
 * shutdown()}, the awaitTermination budget begins after the sleep.
 *
