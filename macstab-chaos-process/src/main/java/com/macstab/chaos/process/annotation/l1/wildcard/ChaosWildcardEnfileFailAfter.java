 ...
 * system-wide kernel fd table saturation curve where aggregate fd usage across all processes on the
 * host crosses {@code fs.file-max} after N successful operations.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-process primitive — one (selector = {@code WILDCARD}, errno = {@code ENFILE}, effect
 ...
 *       subsequent calls return ENFILE permanently; assert that the application circuit-breaks all
 *       process-management attempts and escalates to the platform team — ENFILE is a host-level
 *       condition that cannot be resolved by in-process fd management alone.
 ...
 *       platform alert with the saturation metrics.
 *   <li>Assert that ENFILE is distinguished from EMFILE: ENFILE cannot be resolved by closing this
 *       process's fds (the kernel table is full system-wide); assert that the application does not
 *       attempt an in-process fd audit in response to ENFILE — the runbook is different.
 * </ul>
 *
 * Production failure mode: a multi-tenant host accumulates fd leaks across multiple containers; the
 * system-wide kernel fd table fills; all containers on the host start receiving ENFILE from
 * process-management operations; containers that attempt in-process fd recovery find no local leaks
 * and loop indefinitely; the platform team is not notified; the host remains saturated.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>ENFILE is the system-wide kernel fd table overflow — controlled by {@code
 * /proc/sys/fs/file-max} and observable via {@code /proc/sys/fs/file-nr} (used/free/max). Unlike
 * EMFILE (per-process, fixable in-process), ENFILE requires operator intervention: either raise
 * {@code fs.file-max} (requires root, takes effect immediately via sysctl) or identify and evict
 * the container(s) leaking the most fds (via {@code ls /proc/&#42;/fd | wc -l} per process). The
 * FAIL_AFTER threshold models the point at which the gradual saturation curve crosses the kernel
 * limit.
 *
 ...
 * phase (all process management blocked, platform escalation required). Set {@link
 * #successesBeforeFailure} to the number of calls during the normal operating phase.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
