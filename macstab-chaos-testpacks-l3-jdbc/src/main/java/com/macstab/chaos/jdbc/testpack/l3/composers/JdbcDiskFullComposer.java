...
public final class JdbcDiskFullComposer implements L3Composer<IncidentChaosJdbcDiskFull> {
    ...
    public List<Object> apply(
        ...
        final com.macstab.chaos.filesystem.api.RuleHandle writeHandle =
                CompositeFilesystemChaos.standard().advanced()
                        .apply(container, IoRule.errno(
                                dataPath, IoOperation.WRITE, Errno.ENOSPC, annotation.probability()));
        handles.add(writeHandle);

        final com.macstab.chaos.filesystem.api.RuleHandle fsyncHandle =
                CompositeFilesystemChaos.standard().advanced()
                        .apply(container, IoRule.errno(
                                dataPath, IoOperation.FSYNC, Errno.ENOSPC, annotation.probability()));
        ...
        final ChaosScenario scenario = ChaosScenario.builder(scenarioId)
                ...
                .effect(ChaosEffect.injectException("java.sql.SQLException", "disk full — write failed"))
        ...
        for (final Object h : handles) {
            if (h instanceof com.macstab.chaos.filesystem.api.RuleHandle ioRh) {
                ...
                try {
                    ...
                    log.warn("JdbcDiskFullComposer.removeAll: failed to remove JVM scenario {}", scenarioId, e);
                }
            }
        }
    }
    ...
    public List<String> describe(final IncidentChaosJdbcDiskFull annotation) {
        return List.of(
                ...
                "filesystem: WRITE ENOSPC probability=" + annotation.probability() + " on path=" + annotation.path(),
                "filesystem: FSYNC ENOSPC probability=" + annotation.probability() + " on path=" + annotation.path(),
