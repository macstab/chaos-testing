 ...
 * {@code posix_spawn} calls succeed, then fail every subsequent call with {@code EMFILE}.
 *
 * <p><strong>What this simulates:</strong> resource exhaustion at the posix_spawn boundary — typical of fd-leak / thread-leak bugs that succeed indefinitely until the resource pool is drained, at which point every further call fails with EMFILE.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
