 ...
 * ENOENT} on every subsequent call, causing the calling code to observe a no-such-file failure that
 ...
 *       failure that requires operator action, not in-process retry.
 ...
 *       assert that the application detects this deployment-integrity failure at the threshold.
 *   <li>Assert that the application does not call {@code waitpid} on an uninitialised pid after
 *       post-threshold ENOENT — POSIX does not define the pid value when spawn fails.
 * </ul>
 *
 * Production failure mode: a tool runner uses {@code posix_spawn} with a hardcoded binary path; a
 * rolling deployment updates the container image; the binary is temporarily unavailable during the
 * update window; after N successful spawns all subsequent spawns return ENOENT; the runner has no
 * deployment-integrity alert and operators cannot determine which binary is missing.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>FAIL_AFTER models the rolling-deployment binary-absence scenario where N spawns succeed before
 * the binary is removed. Real ENOENT from posix_spawn in this scenario is not probabilistic — it
 * occurs once and persists until the binary is restored. POSIX spawn returns the error code
 ...
 * scenarios; 0 means the binary is missing from the first spawn (cold deployment error).
 *
