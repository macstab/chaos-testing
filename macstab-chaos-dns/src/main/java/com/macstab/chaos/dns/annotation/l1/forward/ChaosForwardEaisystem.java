...
/**
 * Injects {@code EAI_SYSTEM} on every libchaos-intercepted {@code forward} call inside
 * the target container, making the call fail as if the kernel returned {@code EAI_SYSTEM}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EAI_SYSTEM}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EAI_SYSTEM} is a valid POSIX result of
 * {@code forward}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EAI_SYSTEM} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: resolver syscall failure — consult errno for detail; resolver-side fd exhaustion.
 *
 ...
 * @ChaosForwardEaisystem(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosForwardEaisystems}) to bind different probabilities to
