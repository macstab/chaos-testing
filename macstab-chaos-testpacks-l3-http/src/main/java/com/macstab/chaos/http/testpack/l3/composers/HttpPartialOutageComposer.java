 ...
 *
 * <p>Combines ETIMEDOUT on a fraction of connections, transient DNS EAI_AGAIN, and
 * ConnectException injection to simulate mixed healthy/failing backend conditions.
 *
...
public final class HttpPartialOutageComposer implements L3Composer<IncidentChaosHttpPartialOutage> {
    ...
    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosHttpPartialOutage ann) {
        final List<Object> handles = new ArrayList<>();

        final var adv = CompositeConnectionChaos.standard().advanced();
        handles.add(adv.apply(container,
                NetRule.errno(Endpoint.wildcard(), NetOperation.CONNECT, Errno.ETIMEDOUT, ann.toxicity())));
        ...
        final var scenario = ChaosScenario.builder(scenarioId)
                ...
                .selector(selector)
                .effect(ChaosEffect.injectException("java.net.ConnectException", "Partial outage — mixed healthy/failing"))
    ...
    public List<String> describe(final IncidentChaosHttpPartialOutage ann) {
        return List.of(
                ...
                "jvm: ConnectException(\"Partial outage — mixed healthy/failing\") on class prefix '" + ann.classPattern() + "' (METHOD_ENTER)",
