 ...
 *
 * <p>Applies a MetaspacePressure stressor with retained strong references to reproduce the compound
 * failure profile of a classloader leak exhausting Metaspace over hours while heap metrics remain
 * green.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class JvmMetaspaceGlacierComposer
    implements L3Composer<IncidentChaosJvmMetaspaceGlacier> {

  public JvmMetaspaceGlacierComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosJvmMetaspaceGlacier ann) {
  ...
  public List<String> describe(final IncidentChaosJvmMetaspaceGlacier ann) {
    return List.of(
        ...
        "jvm: MetaspacePressure "
            + ann.generatedClassCount()
            + " classes × "
            + ann.fieldsPerClass()
            + " fields (strong refs retained)",
