...
/**
 * Delays every libchaos-memory-intercepted {@code munmap} call by {@link #delayMs}
 ...
 * (selector = {@code MUNMAP}, effect = LATENCY) pair. Unlike the errno variants, the
 ...
 * starts. The shared library interposes the libc wrappers for the {@code MUNMAP} syscall
 ...
 * applies the rule to every capable container. Use the repeatable form ({@code @ChaosMunmapLatencys})
...
public @interface ChaosMunmapLatency {
  ...
  /**
   * Latency to inject before every matching {@code munmap} call, in milliseconds.
