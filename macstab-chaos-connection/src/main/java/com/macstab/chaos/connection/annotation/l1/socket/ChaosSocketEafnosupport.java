...
/**
 * Injects {@code EAFNOSUPPORT} on every libchaos-intercepted {@code socket} call inside
 * the target container, making the call fail as if the kernel returned {@code EAFNOSUPPORT}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EAFNOSUPPORT}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EAFNOSUPPORT} is a valid POSIX result of
 * {@code socket}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EAFNOSUPPORT} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: address family not supported — IPv4/IPv6 misconfiguration.
 *
 ...
 * @ChaosSocketEafnosupport(toxicity = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosSocketEafnosupports}) to bind different probabilities to
