 ...
 * {@link #probability}, modelling capability-restriction failures that affect all process lifecycle
 ...
 *       CAP_SYS_NICE; assert that the application falls back to SCHED_OTHER rather than logging
 *       a fatal error — EPERM from thread scheduling is a capability issue, not a thread-creation
 *       failure; the thread can still be created with the default scheduling policy.</li>
 *   <li>{@code fork()}/{@code posix_spawn()} return {@code -1}/{@code EPERM} when a seccomp
 *       policy or cgroup restriction blocks the clone syscall; assert that the application
 *       escalates immediately rather than retrying — EPERM from clone/fork indicates a permanent
 *       security policy restriction that cannot be resolved by the process itself.</li>
 *   <li>Assert that the application distinguishes EPERM from EACCES: EPERM means the operation
 *       is not permitted for this process regardless of credentials; EACCES means the operation
 *       is not allowed for this security context (MAC policy) — the escalation path differs.</li>
 ...
 * <p>{@code EPERM} from process-management syscalls has multiple capability-related sources:
 * {@code pthread_create} returns EPERM when the scheduling policy in {@code pthread_attr_t}
 * requires CAP_SYS_NICE (SCHED_FIFO priority {@literal >} 0 or SCHED_RR) and the process lacks
 * it; {@code fork()} returns EPERM under cgroup restrictions or certain clone flag combinations
 * that require CAP_SYS_ADMIN; {@code execve()} returns EPERM for set-uid binaries when the
 * filesystem has nosuid mount option. Unlike EACCES (MAC policy decisions from SELinux/AppArmor),
 * EPERM comes from DAC capability checks performed by the kernel before the MAC layer.
 *
 ...
 * (the thread is created) with lower scheduling priority than desired. Applications that treat EPERM
 * from thread creation as a fatal error lose all thread pool capacity unnecessarily.
 *
 ...
 * cannot be resolved by the application — it requires a security policy change by an operator.
 * The application's error classification must distinguish these two sources to route to the
 * correct escalation or recovery path.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 5e-3; EPERM from thread creation can be
 * handled gracefully via fallback, so moderate rates are acceptable; values above 0.1 will
 * prevent all real-time thread creation if the application does not implement fallback.
