...
/**
 * Intercepts {@code SocketChannel.write(ByteBuffer)} and throws the configured exception before any
 * bytes are copied to the kernel send buffer, simulating a mid-response write failure that causes
 * Netty or Undertow to close the channel and signal the pipeline's exception handler.
 *
 ...
 *
 * <ul>
 *   <li>Netty's {@code ChannelOutboundBuffer} is not drained; its pending bytes remain; assert that
 *       Netty closes the channel and releases all pending write buffers to avoid memory leaks in
 *       frameworks that track unacknowledged outbound data.
 *   <li>The remote peer receives an abrupt TCP RST (the channel is closed without draining the
 *       kernel send buffer); assert that the client side surfaces a {@code connection reset} {@code
 *       IOException} and that the client's retry logic fires.
 ...
 *       mid-response; the container's network namespace is destroyed; all active {@code
 *       SocketChannel.write()} calls throw {@code IOException: Broken pipe}; Netty fires {@code
 *       exceptionCaught} on every open channel simultaneously; if the application's handler
 ...
 * call {@code javaChannel().write(buf.nioBuffer())} in a loop until either all bytes are written or
 * the channel signals that the send buffer is full (returns 0 bytes). The chaos exception fires
 * inside this loop on the first call, causing {@code AbstractNioByteChannel.doWrite()} to catch it
 * and call {@code close(voidPromise())} on the channel.
 *
 * <p>When Netty closes the channel after a write exception, {@code channelInactive()} fires on the
 * pipeline. All pending {@code ChannelFuture}s in {@code ChannelOutboundBuffer} are failed with
 * {@code ClosedChannelException}. Applications using promise-based write patterns must register
 * listeners on every write future; unfuture-tracked writes cause silent data loss that only becomes
 * visible when the channel is closed.
 *
 ...
 *
 * <p>Unlike {@link ChaosNioChannelReadInjectException} which simulates an inbound fault seen by the
 * server, this annotation simulates an outbound fault seen by the client that sent the request.
 * Combined, they exercise both directions of a bidirectional NIO connection failure.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
