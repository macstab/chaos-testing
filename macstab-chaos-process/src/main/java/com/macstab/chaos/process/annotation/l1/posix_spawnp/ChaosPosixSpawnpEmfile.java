 ...
 *
 * <ul>
 *   <li>{@code posix_spawnp} returns {@code EMFILE}; no child process is created; assert that the
 *       application reports the current fd count in the diagnostic and does not call {@code
 *       waitpid} on an uninitialised pid after the failure.
 *   <li>The {@code $PATH} search in {@code posix_spawnp} opens directory fds to traverse PATH
 *       directories; when the fd table is nearly full, the PATH search itself may consume the last
 *       remaining slots — assert that the application detects fd exhaustion at or before the spawn
 *       call rather than only after seeing the EMFILE return value.
 *   <li>Assert that the application distinguishes {@code posix_spawnp}-EMFILE (24, per-process fd
 *       table full, fixable by closing leaked fds) from ENFILE (23, system-wide, requires platform
 *       escalation) — the operator runbook differs.
 * </ul>
 *
 ...
 * traversal phase (opening directories consumes fd slots) or from the spawn's internal parent-child
 * communication pipe allocation. The interposer fires at the API boundary, covering both cases.
 * POSIX spawn returns the error code directly — checking {@code if (ret < 0)} misses EMFILE (24).
 * Applications using {@code posix_spawnp} with pipe-based subprocess communication must close pipe
 * read-ends after each child exits to prevent fd leak accumulation. Unlike {@code posix_spawn}, the
 * PATH search in {@code posix_spawnp} adds fd consumption during the search itself, which means the
 * effective EMFILE threshold may be slightly lower for spawnp.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
