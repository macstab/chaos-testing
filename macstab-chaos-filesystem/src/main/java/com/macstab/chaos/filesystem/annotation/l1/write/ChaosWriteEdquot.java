...
import com.macstab.chaos.filesystem.annotation.l1.pwrite.ChaosPwriteEdquot;
import com.macstab.chaos.filesystem.model.Errno;
import com.macstab.chaos.filesystem.model.IoOperation;

/**
 * Injects {@code EDQUOT} into {@code write(2)}, causing the call to return {@code -1} with {@code
 * errno = EDQUOT} as if the user's disk quota on this filesystem has been exceeded and the kernel
 * cannot allocate additional blocks for the write operation.
 *
 ...
 * write} call; when it fires the interposer returns {@code -1} with {@code errno = EDQUOT} without
 ...
 *       {@code EDQUOT} requires quota increase or data deletion, while {@code ENOSPC} requires disk
 *       expansion or cleanup. Assert that the application's error message distinguishes "quota
 *       exceeded" from "disk full" to help operators apply the correct fix.
 ...
 *       user's identity, enabling operators to identify which tenant is consuming excessive
 *       storage.
 * </ul>
 *
 * <p>In production, {@code EDQUOT} from {@code write} occurs in multi-tenant environments where
 * filesystem quotas are enforced per user or per group ({@code quota(1)}, {@code repquota(8)}), in
 * Kubernetes environments that use project quotas on XFS or ext4 volumes to limit per-namespace
 * storage consumption, and in shared NFS environments where the NFS server enforces per-user
 * quotas.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>Disk quotas are enforced at two levels: block quotas (limits the total number of filesystem
 * blocks a user can allocate) and inode quotas (limits the number of files a user can create). When
 * {@code write} would cause the user's block usage to exceed the hard limit, the kernel returns
 * {@code EDQUOT}. If the user is between the soft limit and the hard limit, the write succeeds but
 * a grace timer starts; when the timer expires, writes return {@code EDQUOT} even if the hard limit
 * has not been reached.
 *
 * <p>The distinction between {@code EDQUOT} and {@code ENOSPC}: {@code EDQUOT} is per-user or
 * per-group, while {@code ENOSPC} is filesystem-wide. A user can receive {@code EDQUOT} even when
 * the filesystem has free space (because the user's quota is exhausted), and a user can receive
 * {@code ENOSPC} even when they have quota remaining (because the filesystem itself is full).
 * Applications that treat both as equivalent "cannot write" errors should use the {@code strerror}
 * message or the errno value to log the specific cause for operator diagnostics.
 *
 * <p>Java maps {@code EDQUOT} from {@code write} to an {@code IOException} with the message "Disk
 * quota exceeded". The same {@code IOException} type is used for all write-side IO errors;
 * application code that needs to distinguish quota exhaustion from disk exhaustion must inspect the
 * exception message or use a platform-specific API to query the quota status.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
