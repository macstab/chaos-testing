 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code WILDCARD}, errno = {@code EPERM}, effect =
 ...
 *       restore the removed capability.
 *   <li>FAIL_AFTER models the capability revocation scenario: N process-management calls succeed
 *       while CAP_SYS_NICE and related capabilities are held; a Kubernetes operator applies a new
 *       pod security policy that drops these capabilities; all subsequent thread and process
 *       creation calls return EPERM — assert that the application detects the capability change and
 *       alerts operators rather than spinning in a retry loop.
 ...
 *       EPERM should the application escalate.
 * </ul>
 *
 ...
 * SELinux/AppArmor), EPERM arises from the kernel's capability table check — more fundamental and
 * faster to check, but equally permanent for the lifetime of the process.
 *
 ...
 * dropped). The fallback path for EPERM from pthread_create (retry with SCHED_OTHER) must be tested
 * in the subsequent test method to verify the degraded-mode behavior is correct.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
