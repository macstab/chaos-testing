 ...
 *       sets {@code errno = EMFILE} and returns {@code -1} without issuing the real kernel call.</li>
 ...
 *       {@code RLIMIT_NOFILE}; the {@code dirfd} must be closed explicitly since no
 *       close-on-exec processing occurs for failed execs.</li>
 * </ol>
 *
 ...
 *       {@code dirfd} on every failure and reports the current fd count in the diagnostic to
 *       enable operator diagnosis of the leak source.</li>
 *   <li>Under FAIL_AFTER, each failed exec that does not close its dirfd leaks one fd slot and
 *       worsens the exhaustion — assert that the application does not enter a self-reinforcing
 *       loop where each failed exec attempt opens a fresh dirfd, fails with EMFILE, and leaks
 *       the dirfd, making the next attempt more likely to fail even earlier in the open phase.</li>
 ...
 *       the diagnostic must route to the correct operator runbook.</li>
 * </ul>
 * Production failure mode: a container runtime opens a {@code dirfd} to the container's rootfs
 * overlay for each {@code execveat(AT_EMPTY_PATH)} launch; leaked fds from previous failed
 * launches accumulate in the runtime's fd table; after N successful launches the table approaches
 * {@code RLIMIT_NOFILE}; subsequent exec attempts open a dirfd (consuming the last free slot),
 * then fail with {@code EMFILE} from the exec's internal binary-open, and leak the dirfd —
 * triggering {@code EMFILE} on the dirfd open itself on the very next attempt.
 *
 * <h2>Deep technical dive</h2>
 * <p>FAIL_AFTER models the progressive fd-leak accumulation pattern more accurately than
 * probabilistic ERRNO. Real {@code EMFILE} from {@code execveat} in container runtimes follows
 * a deterministic threshold: the runtime leaks N fds (one per failed exec from previous errors)
 * until the process's fd table reaches {@code RLIMIT_NOFILE}; then all subsequent exec attempts
 * fail. Setting {@link #successesBeforeFailure} to the observed exec count before the fd table
 * fills reproduces this threshold exactly.
 *
 ...
 * over time and the failure occurs when the process hits the limit, not as random individual events.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Threshold guidance:</strong> derive from the observed fd leak rate and
 * {@code RLIMIT_NOFILE}; for test purposes, values in the range 10–200 exercise the failure
 * mode efficiently; 0 means the fd table is full from the first exec attempt (tests the case
 * where the runtime starts with a nearly-full fd table from inherited fds).
