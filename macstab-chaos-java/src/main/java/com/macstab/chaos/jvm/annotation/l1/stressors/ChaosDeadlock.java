...
/**
 * Activates a self-driving JVM stressor that permanently deadlock N threads in a lock-acquisition
 * cycle inside the target container's JVM for the duration of the test class or test method.
 *
 ...
 *
 * <p><strong>What chaos this applies:</strong> the stressor permanently deadlock N threads in a
 * lock-acquisition cycle. The effect persists throughout the test and is not probabilistic — it
 * runs continuously at the configured intensity until the rule is removed.
 *
 ...
 * applies to every agent-capable container. Use the repeatable form ({@code @ChaosDeadlocks}) to
