 ...
 * {@code Cleaner} reference, preventing GC reclamation. The heap remains completely clean. The pod
 * is alive to liveness probes. Every new NIO, Netty, or gRPC request fails.
 *
 * <h2>Composed of</h2>
 *
 * <ul>
 *   <li>JVM: DirectBufferPressure allocating {@code totalMb} MB in {@code bufferSizeMb} MB chunks,
 *       retained without a Cleaner (leak mode)
 *   <li>JVM: {@code OutOfMemoryError("Direct buffer memory")} injection on {@code io.netty} classes
 *       at METHOD_EXIT
 * </ul>
 *
 * <h2>How bad it is</h2>
 ...
 * Heap is clean; liveness probes pass; every new NIO/Netty/gRPC allocation fails with {@code
 * OutOfMemoryError: Direct buffer memory}. Standard heap monitoring is completely blind to this
 * failure mode.
 *
 * <h2>Example</h2>
 *
...
@ChaosL3(
    composer = "com.macstab.chaos.system.testpack.l3.composers.SystemDirectMemoryLeakComposer",
