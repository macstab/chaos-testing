...
/**
 * Injects {@code EIO} into {@code fsync(2)}, causing the call to return {@code -1} with {@code
 * errno = EIO} as if the kernel could not flush all of the file's dirty pages to the storage device
 * because the device returned a hardware I/O error during the flush operation.
 *
 ...
 * fsync} call; when it fires the interposer returns {@code -1} with {@code errno = EIO} without
 ...
 *       guaranteed to have reached the storage device. Assert that the application does not
 *       consider the preceding writes durable and aborts any transaction or operation that depended
 *       on the fsync for its durability guarantee.
 ...
 *       on the temporary file and an {@code fsync} on the parent directory; assert that an {@code
 *       EIO} on the temporary file's fsync causes the rename to be skipped, leaving the original
 *       target file intact.
 ...
 * return until the storage device acknowledges the write — or until an I/O error is detected. When
 * the device returns an error for any of the dirty pages being flushed, the kernel returns {@code
 * EIO} for the {@code fsync} call and clears the error flag so that a subsequent {@code fsync} can
 * succeed (if the device error was transient). The pages that failed to flush remain dirty and will
 * be retried on the next writeback.
 *
 ...
 * fsync(2)} under the hood. When the underlying call returns {@code EIO}, Java throws a {@code
 * SyncFailedException} (from {@code FileDescriptor.sync()}) or an {@code IOException} (from {@code
 * FileChannel.force()}). Application code must propagate this exception to the transaction layer
 * rather than catching and suppressing it.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
