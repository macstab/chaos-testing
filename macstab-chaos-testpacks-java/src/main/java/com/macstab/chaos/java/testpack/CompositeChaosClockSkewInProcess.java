 ...
 * to every time reading. Unlike OS-level clock skew (which would affect all containers), this
 * is a targeted, in-process JVM interception that affects only the applications using the standard
 * Java time APIs. In production, clock skew causes JWT expiry validation failures, distributed
 * lock TTL miscalculations, and OAuth token invalidation.
 *
 * <h2>How bad it is</h2>
 *
 ...
 * linearisability violations in distributed systems. RFC 7519 (JWT) §4.1.4 ({@code exp} claim)
 * is sensitive to even 30-second clock differences between issuer and validator.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
