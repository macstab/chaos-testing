 ...
 *   <li>JVM: OutOfMemoryError injected on methods matching class prefix {@code "redis"} —
 *       simulates the Java heap impact of defensive copy-on-eviction read patterns
 * </ul>
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Combined memory pressure and connection instability
 * trigger cascading client failures; applications relying on Redis as a session store or
 * rate-limiter lose state atomicity during the eviction window.
 *
 * <h2>Industry references</h2>
 * <p>Redis maxmemory eviction storms are documented in Redis documentation (maxmemory-policy)
 * and operator post-mortems, particularly in deployments where client-side buffering amplifies
 * host memory pressure once Redis starts returning -OOM errors.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.redis.testpack.l3.composers.RedisOomEvictionComposer", severity = Severity.SEVERE)
