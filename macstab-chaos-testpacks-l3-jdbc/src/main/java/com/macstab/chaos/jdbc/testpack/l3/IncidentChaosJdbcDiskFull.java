 ...
 * <h2>Composed of</h2>
 * <ul>
 *   <li>Filesystem: WRITE → ENOSPC at {@code probability} on {@code path} — bulk inserts and
 *       WAL writes fail with "no space left on device"
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Partial data persisted before ENOSPC leaves tables in
 * an inconsistent state; WAL corruption risk exists if FSYNC fails mid-segment; operator
 * intervention to expand the volume or prune files is required to recover.
 *
 * <h2>Industry references</h2>
 * <p>Disk-full during bulk write and migration is a documented operational failure mode:
 * described in the PostgreSQL documentation §"Monitoring Disk Usage", the MySQL documentation
 * §"Disk Full in MySQL", and post-mortems from data-migration incidents where unexpected data
 * volume growth exhausted provisioned storage.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.jdbc.testpack.l3.composers.JdbcDiskFullComposer", severity = Severity.SEVERE)
