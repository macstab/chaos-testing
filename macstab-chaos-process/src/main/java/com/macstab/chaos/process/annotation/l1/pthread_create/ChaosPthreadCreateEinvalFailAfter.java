...
/**
 * Injects {@code ERRNO} on every libchaos-intercepted {@code pthread create} call inside
 ...
 * @ChaosPthreadCreateEinvalFailAfter(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosPthreadCreateEinvalFailAfters}) to bind different probabilities to
...
public @interface ChaosPthreadCreateEinvalFailAfter {
   ...
   * <p>Example:
   * <pre>{@code
   * @ChaosPthreadCreateEinvalFailAfter(id = "primary",  probability = 0.001)
   * @ChaosPthreadCreateEinvalFailAfter(id = "replica",  probability = 0.01)
