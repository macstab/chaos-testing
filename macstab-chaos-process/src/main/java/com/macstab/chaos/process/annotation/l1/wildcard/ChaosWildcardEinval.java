...
/**
 * Injects {@code EINVAL} ("Invalid argument") into every process-management syscall intercepted by
 ...
 *       errno = EINVAL} (22); {@code pthread_create}/{@code posix_spawn} return {@code EINVAL}
 ...
 *       the failure — EINVAL from thread creation is a programming error, non-retryable with the
 *       same attributes.
 ...
 *       means the options argument is wrong, and retrying with the same options will always fail.
 *   <li>{@code posix_spawn}/{@code posix_spawnp} return {@code EINVAL} when the {@code
 *       posix_spawnattr_t} contains an unsupported scheduling policy or flags value; assert that
 ...
 *       essential diagnostic information for root-cause analysis.
 * </ul>
 *
 ...
 * differs by syscall: for {@code pthread_create}, it is a {@code pthread_attr_t} field value (stack
 * size, scheduling policy, priority, detach state, guard size); for {@code waitpid}, it is a bit in
 * the options bitmask (any bit other than WNOHANG=1, WUNTRACED=2, WCONTINUED=8, WEXITED=4,
 * WNOWAIT=0x1000000); for {@code posix_spawn}, it is a {@code posix_spawnattr_t} or {@code
 * posix_spawn_file_actions_t} field.
 *
 * <p>The wildcard selector fires EINVAL across all process families. This validates that every
 * process-management path logs the specific argument values on EINVAL and treats it as
 * non-retryable. A common defect is a catch-all error handler that logs only "process error:
 * EINVAL" without recording which arguments were invalid — the wildcard variant surfaces this
 * diagnostic gap across all call sites simultaneously.
 *
 * <p>Return-value conventions differ: {@code fork()}/{@code waitpid()} return {@code -1} and set
 * {@code errno}; {@code pthread_create}/{@code posix_spawn} return the error code directly. Code
 * that checks only {@code if (ret == -1 && errno == EINVAL)} misses EINVAL from spawn and
 ...
 * monitoring causes zombie accumulation (children exit but cannot be reaped); EINVAL from {@code
 * pthread_create} causes thread starvation; EINVAL from POSIX spawn leaves child pids
