 ...
 *
 * <p>A JVM agent L1 chaos primitive targeting the {@code DATE_NEW} operation — one typed
 ...
 *
 * <ul>
 *   <li><strong>Legacy cookie and HTTP header timestamps wrong.</strong> {@code Date} is used
 *       by JAX-RS and Servlet APIs when setting {@code Set-Cookie: Expires=} and
 *       {@code Last-Modified:} headers; a skewed date causes browser caching and CDN TTL
 *       calculations to misfire. Assert that clients that respect these headers behave correctly.
 *   <li><strong>JDBC timestamp inserts incorrect.</strong> Frameworks that bind
 *       {@code new Date()} to a JDBC {@code Timestamp} parameter will insert a wrong value into the
 *       database; assert that date-range queries and audit records remain consistent.
 *   <li><strong>XML and SOAP timestamp fields corrupted.</strong> JAXB serialises
 *       {@code java.util.Date} fields as ISO 8601 strings; assert that downstream consumers
 *       validate and reject out-of-range dates.
 *   <li><strong>Production failure mode:</strong> legacy applications that use {@code new Date()}
 *       for pessimistic locking (e.g. "lock acquired at", "lock expires at = now + N seconds")
 *       will release or retain locks at the wrong wall-clock time, causing phantom locks or
 *       premature lock expiry.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code java.util.Date}'s no-arg constructor is defined as {@code new Date(System.currentTimeMillis())}.
 * The agent instruments the constructor's exit point (after the superclass chain has completed)
 * using a Byte Buddy constructor-exit interceptor and adjusts the internal {@code fastTime} field
 * via {@code Date.setTime()} before the constructor returns. This approach is preferred over
 * intercepting the call to {@code System.currentTimeMillis()} inside the constructor because it
 * leaves the millis call unaffected for other callers that have not opted into clock skew.
 *
 ...
 * particularly in JPA entity listeners or Hibernate types that still materialise
 * {@code java.util.Date} for JDBC compatibility. Applying both this annotation and
 * {@link ChaosInstantNowSkew} with matching offsets provides uniform skew across the entire
 * codebase; applying different offsets creates an inconsistency that can surface bugs in code that
