...
/**
 * Injects {@code ERRNO} on every libchaos-intercepted {@code wildcard} call inside
 ...
 * @ChaosWildcardEintrFailAfter(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosWildcardEintrFailAfters}) to bind different probabilities to
...
public @interface ChaosWildcardEintrFailAfter {
   ...
   * <p>Example:
   * <pre>{@code
   * @ChaosWildcardEintrFailAfter(id = "primary",  probability = 0.001)
   * @ChaosWildcardEintrFailAfter(id = "replica",  probability = 0.01)
