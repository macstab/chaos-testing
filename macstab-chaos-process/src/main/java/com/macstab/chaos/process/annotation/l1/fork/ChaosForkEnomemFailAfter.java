 ...
 * {@code fork} calls succeed, then fail every subsequent call with {@code ENOMEM}.
 *
 * <p><strong>What this simulates:</strong> resource exhaustion at the fork boundary — typical of fd-leak / thread-leak bugs that succeed indefinitely until the resource pool is drained, at which point every further call fails with ENOMEM.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
