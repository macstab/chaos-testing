 ...
 *       permission failure and closes the {@code dirfd} to avoid a file descriptor leak.</li>
 *   <li>Container runtimes using {@code AT_EMPTY_PATH} (exec-by-fd) to launch the container
 *       entrypoint must handle {@code EACCES} when the binary's execute permission bits are not
 *       set or an LSM policy denies exec on the open fd — assert that the runtime surfaces the
 *       permission error with the fd number and the binary path if available.</li>
 *   <li>Assert that the application distinguishes {@code EACCES} on {@code execveat} from
 *       {@code EACCES} on file open operations — an exec permission failure has different
 *       remediation (chmod, LSM policy) than an open permission failure (file ownership).</li>
 * </ul>
 ...
 * context change makes the fd non-executable under the new policy; the exec fails with
 * {@code EACCES} even though the binary file is executable — because the LSM check is on the
 * fd's current security context, not the file's inode permission bits.
 *
 * <h2>Deep technical dive</h2>
 * <p>{@code execveat} with {@code AT_EMPTY_PATH} uses the open file description referenced by
 * {@code dirfd} directly as the executable. The kernel's LSM hook ({@code security_bprm_check})
 * is applied to the already-open file description, not to a fresh path lookup. This means that
 * an SELinux or AppArmor policy change that takes effect after the file is opened but before
 * the exec is issued can cause {@code EACCES} on the exec without modifying the file's permission
 * bits — a subtle TOCTOU-style failure that is invisible in path-based permission checks.
 *
 ...
 * in production, so any non-zero probability exercises the error handling path.
