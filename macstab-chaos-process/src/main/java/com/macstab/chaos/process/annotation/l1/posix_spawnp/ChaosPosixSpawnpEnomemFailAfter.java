 ...
 * ENOMEM} on every subsequent call, causing the calling code to observe a persistent kernel
 * out-of-memory failure that models progressive slab allocator exhaustion on a Kubernetes node
 ...
 *       call returns {@code ENOMEM} directly (POSIX spawn returns the error code, not -1).
 *   <li>The calling code receives: return value {@code ENOMEM} (12); no child process is created;
 *       the calling process is in a clean state since no child resources were allocated.
 * </ol>
 *
 ...
 *       whereas EAGAIN self-heals when children exit.
 ...
 *       assert that the application detects this threshold and escalates to a memory-pressure
 *       alert.
 ...
 *       process is in a clean state and no child resources need to be reaped.
 * </ul>
 *
 ...
 * as each spawn allocates task_struct, kernel stack, mm_struct, and glibc's internal pipe buffers;
 * after N spawns the allocator is exhausted and all subsequent spawns fail with ENOMEM. POSIX spawn
 * returns the error code directly — checking {@code if (ret < 0)} silently misses ENOMEM (12).
 * ENOMEM (12) and EAGAIN (11) are adjacent integers; misclassification between them causes wrong
 * retry behaviour — EAGAIN calls for short exponential backoff; ENOMEM calls for longer backoff
 * with memory pressure alert.
 *
 * <p>The posix_spawnp PATH traversal adds a subtle memory pressure contribution not present in
 * posix_spawn: each PATH directory is opened with {@code opendir}, which allocates a {@code DIR}
 * structure and read buffer in the parent process's heap. Under extreme memory pressure, this PATH
 * traversal allocation itself may fail with ENOMEM before the fork phase. The interposer fires at
 * the spawn API boundary and covers both the traversal-allocation and the
 * spawn-internal-structure-allocation failure cases.
 *
 ...
 * SIGCHLD from killed children and avoid accumulating zombie processes during the retry window. If
 * the application itself is OOM-killed during the ENOMEM phase, the retry logic is moot — the
 * memory alert must be raised before the OOM killer activates.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
