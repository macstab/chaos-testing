...
/**
 * Injects {@code ENFILE} on every libchaos-memory-intercepted {@code mmap} call inside the target
 * container, making the call fail as if the kernel returned {@code ENFILE}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP}, errno = {@code ENFILE}) pair. The
 * combination is safe by construction: this annotation class exists only because {@code ENFILE} is
 ...
 * the interceptor returns {@code -1} and sets {@code errno = ENFILE} before the kernel call
 * completes — from the application perspective this is indistinguishable from a real kernel-level
 * failure. Specifically this simulates: system-wide file-descriptor limit reached — typical of
 * host-side fd exhaustion incidents.
 *
 ...
 * the test class. Use the repeatable form ({@code @ChaosMmapEnfiles}) to bind different
