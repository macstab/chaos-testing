 ...
 *       or directory"; the binary name was not found in any directory listed in {@code $PATH}; no
 ...
 *
 * <ul>
 *   <li>{@code posix_spawnp} returns {@code ENOENT}; no child process is created; assert that the
 *       application includes the binary name and the current {@code $PATH} value in the error
 *       diagnostic — operators cannot fix a missing binary without knowing which name was searched
 *       and which directories were checked.
 *   <li>{@code posix_spawnp}-ENOENT means the binary was not found in any {@code $PATH} directory
 *       (a deployment-integrity failure), which is fundamentally different from {@code
 *       posix_spawn}-ENOENT (the explicit path supplied to the call was wrong). Assert that the
 *       application distinguishes these two cases in its error message — both require operator
 *       action but the fix differs: PATH search failure may be resolved by adding the missing
 *       directory to {@code $PATH}; explicit-path failure requires fixing the configured path.
 *   <li>Assert that the application does not retry on ENOENT — binary absence is a deployment
 *       error, not a transient condition; retry is appropriate for EAGAIN but not for ENOENT; also
 *       assert that {@code waitpid} is not called on an uninitialised pid after ENOENT failure.
 * </ul>
 *
 ...
 * in {@code $PATH} (obtained from {@code getenv("PATH")}) for a file named {@code file} and none is
 * found, or {@code $PATH} is empty. The PATH search occurs in the parent process before the fork,
 * opening each directory with {@code opendir} and calling {@code stat} on candidate paths. ENOENT
 * is returned after the entire PATH list is exhausted. This is in contrast to {@code posix_spawn}
 * ENOENT, where the explicit path argument does not resolve to an existing file.
 *
 * <p>POSIX spawn returns the error code directly — checking {@code if (ret < 0)} or {@code if (ret
 * == -1)} silently misses ENOENT (2). Applications that use {@code if (ret != 0) handleError(ret)}
 * are correct; applications that check {@code errno} after a non-negative return are incorrect.
 * Because ENOENT (2) is numerically small, integer comparison errors that test {@code ret < 0} or
 * {@code ret > 100} can silently swallow the error.
 *
