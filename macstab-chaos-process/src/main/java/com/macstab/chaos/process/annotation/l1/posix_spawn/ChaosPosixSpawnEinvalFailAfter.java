 ...
 * EINVAL} on every subsequent call, causing the calling code to observe an invalid-argument failure
 ...
 *       retryable programming error and escalates to a configuration alert rather than retrying.
 ...
 *       regression at the threshold and does not mask it with silent retries.
 *   <li>Assert that the application does not call {@code waitpid} on an uninitialised pid after
 *       post-threshold EINVAL — POSIX does not define the pid value when spawn fails.
 * </ul>
 *
 ...
 * with EINVAL after N successful spawns (where N is the number of spawns before the updated library
 * code path is triggered); the manager retries EINVAL indefinitely, filling logs.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>FAIL_AFTER models a spawn attribute regression that manifests after N successful calls. Unlike
 * EAGAIN or ENOMEM (transient resource failures), EINVAL is a permanent programming error. POSIX
 * spawn returns the error code directly — checking {@code if (ret < 0)} misses EINVAL (22). The
 * counter does not reset between test methods at class scope, allowing a test suite to verify
