...
/**
 * Spawns a non-terminating background thread inside the target container's JVM that blocks the
 * JVM from shutting down gracefully if configured as a non-daemon thread, simulating the presence
 * of a misbehaving background task that refuses to stop on SIGTERM.
 *
 ...
 * The stressor starts a single named thread that parks itself in a tight {@code LockSupport.parkNanos()}
 * loop with a configurable heartbeat interval, ignoring interrupts, and runs until the chaos rule
 * is explicitly removed by the agent.
 *
 * <h2>What chaos this applies</h2>
 *
 * <ol>
 *   <li>The agent creates a single {@code Thread} named {@link #threadName()}, sets its
 *       {@link #daemon()} flag, and starts it.</li>
 ...
 *       protocol.</li>
 *   <li>As long as {@code daemon = false}, the JVM will not exit normally (via
 *       {@code Runtime.halt()} or the completion of all non-daemon threads) until this thread
 *       terminates. The JVM will wait indefinitely during the shutdown sequence, delaying or
 *       preventing graceful termination.</li>
 *   <li>At chaos rule removal, the agent forcibly terminates the thread (using the agent's
 *       internal control channel, not {@code Thread.stop()}), allowing the JVM to proceed with
 *       shutdown.</li>
 * </ol>
 *
 ...
 *       runs shutdown hooks and then waits for all non-daemon threads to terminate. With
 *       {@code daemon = false}, the keep-alive thread blocks this phase indefinitely; Kubernetes
 *       will eventually send SIGKILL after {@code terminationGracePeriodSeconds}; assert that the
 ...
 *       probe is removed promptly on SIGTERM so the load balancer stops routing traffic even if
 *       the JVM has not exited.
 ...
 *       application threads have stopped, the Prometheus endpoint may still respond (if served by
 *       a daemon thread) or not (if served by a non-daemon thread that has exited); assert that
 *       the scraper's last scrape before the pod is removed from service reflects the correct
 *       state.
 ...
 * {@code runFinalizersOnExit} is enabled (rare and deprecated), all uninvoked finalizers run;
 * (3) {@code Runtime.halt()} is called to stop the VM. A non-daemon thread that is still running
 * at the start of phase 1 keeps the JVM alive until the thread exits — shutdown hooks run, but
 * the JVM waits at the end of phase 1 for all non-daemon threads to finish before calling
 * {@code halt()}.
 *
 ...
 * affects monitoring, or consumes stack resources without affecting shutdown timing. Set
 * {@code daemon = false} explicitly to test shutdown-stall scenarios.
 *
 * <p>The thread ignores {@code Thread.interrupt()} by design, simulating a misbehaving background
 * task that does not honour the standard cooperative shutdown protocol. Application frameworks
 * that send {@code interrupt()} to background threads during shutdown and assume the threads
 * will stop will fail to shut down cleanly in this scenario.
 *
 ...
 * responsive to rule removal at the cost of slightly higher CPU wakeup frequency; the default
 * 1000 ms is sufficient for test scenarios where cleanup latency of up to one second is
 * acceptable.
 *
 * <h2>Example</h2>
 *
