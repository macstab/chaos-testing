 ...
 * <h2>Composed of</h2>
 * <ul>
 *   <li>JVM: SafepointStorm every {@code gcIntervalMs} ms with retransformation of
 *       {@code retransformClassCount} classes per cycle — each retransformation invalidates
 *       previously compiled native code and triggers deoptimisation safepoints
 * </ul>
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>2–5 s CPU spike followed by 80% throughput drop
 * during the recompilation phase; self-resolving after the JIT recompiles, but repeats on the
 * next cycle.
 *
...
@ChaosL3(composer = "com.macstab.chaos.jvm.testpack.l3.composers.JvmDeoptimizationStormComposer", severity = Severity.SEVERE)
