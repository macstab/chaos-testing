 ...
 *
 * <p>Applies a DirectBufferPressure stressor and {@code OutOfMemoryError} injection on
 * {@code io.netty} classes to reproduce the compound failure profile of Netty/gRPC off-heap
 * ByteBuffer exhaustion where the heap is clean but every new NIO allocation fails.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SystemDirectMemoryLeakComposer implements L3Composer<IncidentChaosSystemDirectMemoryLeak> {

    public SystemDirectMemoryLeakComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosSystemDirectMemoryLeak ann) {
        final List<Object> handles = new ArrayList<>();

        final String directId = JvmPlanAccumulator.instance().mintScenarioId("SystemDirectMemoryLeak-direct");
        final ChaosScenario directScenario = ChaosScenario.builder(directId)
                .selector(ChaosSelector.stress(ChaosSelector.StressTarget.DIRECT_BUFFER))
                .effect(ChaosEffect.directBufferPressure(
                        (long) ann.totalMb() * 1024L * 1024L,
                        ann.bufferSizeMb() * 1024 * 1024))
        ...
        final ChaosScenario oomScenario = ChaosScenario.builder(oomId)
                .selector(ChaosSelector.method(
                        ...
                        NamePattern.any()))
                .effect(ChaosEffect.injectException("java.lang.OutOfMemoryError", "Direct buffer memory"))
                .activationPolicy(ActivationPolicy.always())
                .build();
    ...
    public List<String> describe(final IncidentChaosSystemDirectMemoryLeak ann) {
        return List.of(
                ...
                "jvm: DirectBufferPressure " + ann.totalMb() + "MB in " + ann.bufferSizeMb() + "MB chunks (no Cleaner)",
