...
/**
 * Injects {@code EMFILE} into {@code open(2)}, causing the call to return {@code -1} with {@code
 * errno = EMFILE} as if the calling process has reached its per-process file descriptor limit
 * ({@code RLIMIT_NOFILE}) and the kernel cannot assign a new file descriptor for the opened file.
 *
 ...
 * open} call; when it fires the interposer returns {@code -1} with {@code errno = EMFILE} without
 ...
 *
 * <ul>
 *   <li>{@code EMFILE} from {@code open} indicates the process's file descriptor table is
 *       exhausted; no new files, sockets, pipes, or other resources can be opened until existing
 *       descriptors are closed. Assert that the application does not spin retrying the open — each
 *       retry consumes CPU without progress.
 ...
 *       {@code /proc/self/limits}, enabling operators to increase the container's {@code ulimit
 *       -n}.
 * </ul>
 *
 ...
 * hard upper bound of {@code RLIMIT_NOFILE}. The soft limit (the enforced one) is typically 1024 on
 * unmodified Linux systems; it can be raised up to the hard limit with {@code setrlimit(2)}. When
 * {@code open} is called and the lowest available file descriptor number exceeds the soft limit,
 * the kernel returns {@code EMFILE}.
 *
 ...
 * created by NIO frameworks consumes at least two descriptors (epoll fd + wakeup pipe); class
 * loading opens jar files; JMX uses sockets; GC logging and heap dump tools open files. The actual
 * fd usage of a production JVM process is often 3-5x higher than the number of application-visible
 * sockets and files.
 *
 * <p>Java maps {@code EMFILE} from {@code open} to a {@code FileNotFoundException} with the message
 * "Too many open files". The same message is used by some file-not-found errors on certain JVM
 * versions; application code that distinguishes fd exhaustion from genuine file-not-found
 * conditions must inspect the cause chain rather than the message text.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
