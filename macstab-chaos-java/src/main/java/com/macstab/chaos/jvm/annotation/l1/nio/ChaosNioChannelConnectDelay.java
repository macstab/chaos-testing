 ...
 * Netty, Undertow, and any other NIO-based async I/O framework.
 *
 ...
 *   <li>Control returns and the underlying {@code connect()} executes normally, initiating the TCP
 *       three-way handshake.
 * </ol>
 *
 ...
 *       before the SYN packet is sent; assert that Netty's connect-timeout handler fires when the
 *       injected delay exceeds {@code Bootstrap.option(ChannelOption.CONNECT_TIMEOUT_MILLIS)}.
 *   <li>Netty's event loop thread is blocked during the sleep if the connect call is made from the
 *       event loop — this stalls all other channels registered on the same selector; assert that
 *       the application does not make blocking connect calls from event loop threads.
 *   <li>Connection pool warm-up: frameworks that pre-warm connection pools at startup will take
 *       longer to complete; assert that the health-check endpoint returns STARTING rather than UP
 *       during the delay.
 *   <li><strong>Production failure mode:</strong> a BGP routing change increases RTT from 1 ms to
 *       400 ms; Netty client connect attempts block for 400 ms per SYN; the event loop thread is
 *       occupied and cannot process responses for existing connections — throughput drops to zero
 ...
 * initiates the handshake and returns immediately (the channel transitions to {@code
 * ConnectionPending} state); the completion is signalled via {@code SelectionKey.OP_CONNECT}
 * readiness on the next {@code Selector.select()} call. The chaos delay fires before the {@code
 * connect()} call itself, so it adds to the time before the SYN packet is sent, not to the TCP
 * handshake RTT.
 *
 * <p>In blocking mode (the channel's blocking mode is set to true), {@code connect()} blocks until
 * the handshake completes or the OS connection timeout fires. The chaos delay fires before the
 * OS-level connect, compounding with the OS timeout: if the injected delay is longer than the OS
 * timeout ({@code /proc/sys/net/ipv4/tcp_syn_retries} × RTT), the OS timeout fires during the delay
 * and the connect never actually starts.
 *
 ...
 * blocks the event loop, preventing all other channels on the same event loop from processing I/O —
 * a single-point-of-failure scenario unique to NIO event-loop architectures. This is distinct from
 * thread-pool-based I/O where only the connecting thread is blocked.
 *
 ...
 * partition where connection attempts are slow and the selector's ready-key notifications are also
 * delayed, fully blocking the NIO pipeline.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
