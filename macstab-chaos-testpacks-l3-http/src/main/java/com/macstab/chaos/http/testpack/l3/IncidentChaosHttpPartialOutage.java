 ...
 * <h2>What this is</h2>
 *
 * <p>Simulates a partial backend outage where a fraction of instances are unreachable while others
 * continue to serve traffic. DNS resolution transiently fails for affected pods, and connection
 * attempts time out, producing a mixed healthy/failing response profile that exposes load-balancer
 * and circuit-breaker logic to realistic split conditions.
 *
 * <h2>Composed of</h2>
 *
 * <ul>
 *   <li>Connection: CONNECT → ETIMEDOUT at toxicity {@code toxicity} — connection attempts to the
 *       affected fraction of backends time out without reaching the server
 ...
 * A 50% error rate under mixed healthy/failing backends is often harder to detect and route around
 * than a full outage; circuit breakers tuned for full-failure may not open, allowing sustained
 * error injection into user traffic.
 *
...
@ChaosL3(
    composer = "com.macstab.chaos.http.testpack.l3.composers.HttpPartialOutageComposer",
