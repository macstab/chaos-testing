 ...
 *   <li>The calling code receives: return value {@code -1}, {@code errno = EINTR} (4), {@code
 *       strerror(EINTR)}: "Interrupted system call"; a signal was caught during the wait and no
 *       child state changed while the signal was being handled.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li>{@code waitpid} returns {@code -1} with {@code errno == EINTR}; assert that the application
 *       retries the waitpid call after EINTR — EINTR means the signal handler ran and the wait may
 *       be retried immediately; the child has not yet changed state (it is not yet a zombie).
 *   <li>Applications using {@code waitpid} with blocking mode (not WNOHANG) must retry on EINTR to
 *       ensure the child is eventually reaped; assert that the retry loop is bounded and checks a
 *       shutdown flag so that SIGTERM during a wait does not cause an infinite EINTR loop.
 *   <li>Assert that the application's EINTR retry does not reset the accumulated wait time — the
 *       retry must continue waiting for the same child, not start a new timed wait from zero.
 * </ul>
 *
 ...
 *
 * <p>{@code EINTR} from {@code waitpid} occurs when the process is blocked in the wait and a signal
 * is delivered with a handler that does not use {@code SA_RESTART}. POSIX specifies that waitpid
 * may return EINTR when a signal is caught; the glibc implementation of waitpid does not
 * automatically restart the system call on EINTR (unlike read/write for SA_RESTART signals). The
 * WNOHANG flag prevents EINTR: a non-blocking waitpid returns 0 (no child ready) rather than EINTR
 * because it does not block and cannot be interrupted.
 *
 * <p>waitpid returns -1 on error and sets errno. Code that tests {@code if (ret == -1 && errno ==
 * EINTR)} is correct and must loop. A common bug is handling EINTR by returning an error to the
 * caller rather than retrying — the child becomes a zombie that is never reaped, accumulating
 * zombie entries in the process table until the parent exits.
 *
 * <p>Signal-safe EINTR handling for waitpid: the retry loop must be written as {@code do { ret =
 * waitpid(pid, &status, 0); } while (ret == -1 && errno == EINTR && !shutdown_requested)}. The
 * {@code shutdown_requested} flag must be set in the signal handler as an {@code atomic_bool} or
 * {@code volatile sig_atomic_t}. Without this flag, a SIGTERM during waitpid causes an EINTR loop
 * that ignores the termination signal.
 *
 * <p>EINTR from waitpid differs from EINTR from read/write: for I/O calls, SA_RESTART causes the
 * kernel to automatically restart the call; waitpid does not benefit from SA_RESTART in the same
 * way on all platforms — applications must implement the EINTR retry explicitly for waitpid.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
