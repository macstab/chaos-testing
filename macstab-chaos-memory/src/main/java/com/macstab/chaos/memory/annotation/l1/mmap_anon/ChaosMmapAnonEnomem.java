 ...
 * The combination is enforced at compile time: this annotation class exists only because
 * {@code ENOMEM} is a valid POSIX result of {@code mmap(MAP_ANONYMOUS)}; invalid selector/errno
 * pairings have no annotation class and therefore cannot be expressed.
 *
 ...
 *       {@code MAP_FAILED} ({@code (void*)-1}) without issuing the real kernel call.</li>
 ...
 *       and JVM direct allocators surface {@code OutOfMemoryError}.</li>
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 * <ul>
 *   <li>glibc {@code malloc} returns {@code NULL} for requests above {@code MMAP_THRESHOLD}
 *       (~128 KB by default) and sets {@code errno = ENOMEM} ({@code strerror}: "Out of memory").</li>
 *   <li>C++ {@code new} throws {@code std::bad_alloc}; Java {@code ByteBuffer.allocateDirect}
 *       and JNA native allocations throw {@code OutOfMemoryError: Direct buffer memory}.</li>
 *   <li>Application logs, metrics, or error responses should contain OOM-related messages or
 *       error codes — assert these rather than relying on return values alone.</li>
 * </ul>
 ...
 *
 * <p>The Linux kernel raises {@code ENOMEM} in {@code do_mmap} when the VMA count reaches
 * {@code sysctl_max_map_count} (default 65 536), when the requested region overlaps an existing
 * mapping and merging fails due to a fragmented VMA tree, or when the cgroup memory controller
 * rejects the charge. Under {@code vm.overcommit_memory=0} (heuristic, the default) the kernel
 * rarely raises {@code ENOMEM} for anonymous mappings but does so at very high allocation rates
 * on memory-constrained machines.
 *
 * <p>glibc wraps {@code mmap} with a thin veneer that translates the kernel {@code -ENOMEM}
 * return into a C {@code errno = ENOMEM} / return-{@code MAP_FAILED} pair; {@code malloc}
 * internally calls {@code mmap} for allocations above {@code MMAP_THRESHOLD} and propagates
 * {@code NULL} to the caller. The JVM uses anonymous mappings for code cache, metaspace, and
 * {@code DirectByteBuffer} — all three paths raise {@code OutOfMemoryError} when {@code mmap}
 * fails. JNA and Panama allocators share the same code path. Application frameworks that pool
 * native buffers (Netty, gRPC, Kafka clients) may attempt retries but will exhaust their budgets
 * quickly if the probability is too high.
 *
 * <p>Compared with sibling errnos: {@code EAGAIN} is transient (retry may succeed), whereas
 * {@code ENOMEM} indicates structural exhaustion — retrying without releasing memory is futile.
 * {@code EINVAL} indicates a programmer error (wrong flags or alignment); {@code ENOMEM} indicates
 * a resource limit. Test both to verify that the application handles each case distinctly.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-4 to 1e-3 mirrors realistic OOM rates in
 * production; values above 0.1 will typically prevent the container process from completing
 * startup, making the test fail at infrastructure level.
