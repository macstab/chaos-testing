 ...
 *       latency rises monotonically and that the application does not crash but recovers fully once
 ...
 *   <li><strong>Production failure mode:</strong> the JDK epoll selector bug (pre-Java 11 on Linux)
 *       causes the selector to return spurious wakeups indefinitely; without Netty's selector
 *       rebuild workaround the event loop spins at 100% CPU, starving OS threads scheduled for
 *       other processes on the same core; the application remains reachable via existing
 *       connections but throughput drops as the CPU is consumed by the spinning event loop; the
 *       issue appears as a CPU saturation alert with no corresponding increase in request rate.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The interception targets {@code sun.nio.ch.SelectorImpl#select(long)} (and the no-arg {@code
 * select()}) via Byte Buddy. Netty's {@code NioEventLoop.select(long)} drives the event loop and
 * uses a counter ({@code selectCnt}) to track consecutive select calls that return zero. When
 * {@code selectCnt} exceeds {@code SELECTOR_AUTO_REBUILD_THRESHOLD}, Netty calls {@code
 * rebuildSelector0()} which creates a new {@code Selector}, re-registers all channels on it, and
 * replaces the old selector. This is Netty's workaround for the JDK-6403933 epoll spinning bug on
 * Linux.
 *
 * <p>The spurious wakeup effect is distinct from a normal zero return: in non-blocking mode, {@code
 * select(0)} (i.e. {@code selectNow()}) legitimately returns zero when no channels are ready. Netty
 * specifically calls {@code selectNow()} during task-queue-draining phases and correctly handles a
 * zero return from {@code selectNow()} without incrementing the epoll-bug counter. The intercept on
 * the full {@code select(timeout)} variant targets the blocking select call that should only return
 * zero for a genuine epoll wakeup bug.
 *
 * <p>The spurious wakeup causes the event loop to enter its task-processing phase on every loop
 * iteration even when no tasks are queued. Netty's {@code runAllTasks(long timeoutNanos)} returns
 * quickly when the task queue is empty, so the overhead of spurious task processing phases is low;
 * the CPU cost is dominated by the rapid select-return-select cycle rather than task processing.
 *
 ...
 */
@Repeatable(ChaosNioSelectorSelectSpuriousWakeup.Repeatable.class)
...
public @interface ChaosNioSelectorSelectSpuriousWakeup {
  ...
  @interface Repeatable {
    ChaosNioSelectorSelectSpuriousWakeup[] value();
  }
}
