 ...
 * causing the calling code to observe a permission-denied failure when attempting to establish a
 * file-backed memory mapping.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-memory primitive — one (selector = {@code MMAP_FILE}, errno = {@code EACCES}) tuple.
 ...
 *       must fall back to conventional read/write I/O or surface a structured error.
 ...
 *       stochastically even on files opened with the correct mode.
 *   <li>Assert that no data corruption occurs on the fallback path and that the application logs a
 *       descriptive error rather than silently producing incorrect results.
 * </ul>
 *
 * Production failure mode: a security policy change (file ACL tightened, SELinux label changed, or
 * volume remounted read-only) causes all existing file-backed mappings to fail on the next access
 * and all new mapping attempts to return {@code EACCES} — typically observed in database engines
 * and log-structured storage systems.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX specifies {@code EACCES} for file-backed {@code mmap} when the file is not open for
 * reading (when {@code PROT_READ} is requested) or when the file is not open for writing and {@code
 * MAP_SHARED | PROT_WRITE} is requested. The kernel validates the fd's access mode against the
 * requested protection in {@code do_mmap_pgoff} before establishing the mapping.
 *
 * <p>In production, {@code EACCES} on file-backed mappings most commonly arises when a process
 * inherits a file descriptor from a parent that opened the file with insufficient permissions, or
 * when a filesystem is remounted read-only (e.g. due to an I/O error triggering the kernel's
 ...
 * journals must handle {@code EACCES} on their mapping calls — the alternative (mapping with {@code
 * MAP_PRIVATE} and syncing changes back via {@code write}) is a safe fallback but sacrifices the
 * zero-copy benefit. Many implementations do not implement this fallback.
 *
 * <p>Compared with {@code EPERM}: {@code EACCES} is a credentials check (this process does not have
 * the right to access this object in this mode); {@code EPERM} is a structural check (the operation
 * itself is disallowed for this process class regardless of the object). Both are non-transient for
