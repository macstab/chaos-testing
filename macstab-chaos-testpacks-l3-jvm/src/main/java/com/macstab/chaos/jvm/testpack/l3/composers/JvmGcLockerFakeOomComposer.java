 ...
 *
 * <p>Applies GcPressure, MonitorContention, and an OOM injection to reproduce the compound failure
 * profile of a GCLocker-induced spurious OutOfMemoryError where the heap is not actually exhausted.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class JvmGcLockerFakeOomComposer
    implements L3Composer<IncidentChaosJvmGcLockerFakeOom> {

  public JvmGcLockerFakeOomComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosJvmGcLockerFakeOom ann) {
    ...
    final String monitorId =
        JvmPlanAccumulator.instance().mintScenarioId("JvmGcLockerFakeOom-monitor");
    final ChaosScenario monitorScenario =
        ChaosScenario.builder(monitorId)
            .selector(ChaosSelector.stress(ChaosSelector.StressTarget.MONITOR_CONTENTION))
            .effect(
                ChaosEffect.monitorContention(
                    Duration.ofMillis(ann.lockHoldMs()), ann.contendingThreads()))
            .activationPolicy(ActivationPolicy.always())
            .build();
  ...
  public List<String> describe(final IncidentChaosJvmGcLockerFakeOom ann) {
    return List.of(
        ...
        "jvm: GcPressure "
            ...
            + "MB/s + MonitorContention "
            + ann.contendingThreads()
            + " threads × "
            + ann.lockHoldMs()
            + "ms",
