...
/**
 * Injects {@code ENOENT} into {@code unlink(2)}, causing the call to return {@code -1} with {@code
 * errno = ENOENT} as if the named file does not exist or a component of the path prefix does not
 * exist — simulating a TOCTOU race where the file was deleted by another process between the
 * application's existence check and its deletion attempt.
 *
 ...
 *       treats {@code ENOENT} from unlink as a successful deletion (the file is gone, which was the
 *       goal) and does not propagate it as an error to callers.
 ...
 *       assert that an {@code ENOENT} on the immediate unlink is treated as an error because the
 *       file should definitely exist at this point.
 ...
 * cleanup scenarios where multiple processes attempt to delete the same temporary file, and when a
 * file's containing directory is removed before the file itself is deleted.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code unlink(2)} removes a directory entry by decrementing the inode's link count and
 * removing the name from the directory. If the link count reaches zero and no process has the file
 * open, the inode and its data blocks are freed. If the link count reaches zero but processes still
 * have the file open, the directory entry is removed (the file becomes unlinked) but the inode and
 * data blocks are not freed until all file descriptors are closed.
 *
 * <p>{@code ENOENT} from {@code unlink} means the directory entry was not found; the file does not
 * exist or was already removed. Unlike most other error conditions, {@code ENOENT} from unlink
 * often indicates that the desired outcome (the file is gone) has already been achieved by another
 * means. Applications should distinguish between "expected to exist, does not" (error) and "may or
 * may not exist" (success if absent).
 *
 * <p>Java's {@code Files.delete(Path)} throws a {@code NoSuchFileException} (a subtype of {@code
 * IOException}) on {@code ENOENT}. {@code Files.deleteIfExists(Path)} returns {@code false} on
 * {@code ENOENT} without throwing, which is the appropriate idiom for idempotent deletion.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
