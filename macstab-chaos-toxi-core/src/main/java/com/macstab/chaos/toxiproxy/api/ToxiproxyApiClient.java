 ...
 * client running in the test JVM (which operates in the host network namespace) cannot reach
 * {@code localhost:8474} — that address resolves to the host's loopback interface, not the
 * container's. The only way to reach Toxiproxy's API is to execute HTTP commands <em>inside</em>
 * the container, where {@code localhost:8474} resolves correctly. This interface abstracts that
 * shell-based HTTP dispatch, hiding the container-side execution model from callers.
 *
 ...
 * components — {@link com.macstab.chaos.toxiproxy.lifecycle.ToxiproxyLifecycle},
 * {@link com.macstab.chaos.proxy.internal.operations.ProxyOperations},
 * {@link com.macstab.chaos.proxy.internal.operations.ToxicOperations} — depend on this interface,
 * never on Toxiproxy's REST wire format directly.
 *
 ...
 * {@link ContainerContext#http()} collaborators. No direct TCP connections from the test JVM to
 * any Toxiproxy port. The call path is:
 *
 * <pre>
 * Test JVM
 ...
 * Docker Desktop vs native Linux Docker). Operations that combine multiple API calls
 * ({@link #proxyExists} then {@link #createProxy}) are not atomic; the proxy state can change
 ...
 * internal state machine is not protected by this interface: two concurrent
 * {@link #createProxy} calls for the same proxy name will result in a Toxiproxy API error on the
 * second call (proxy already exists). Coordination at a higher level is the caller's
 * responsibility.
 *
 * <h2>Security Notes</h2>
 *
 * <p>Implementations that construct JSON payloads using {@code String.format()} without a
 * JSON library (the production case) are vulnerable to injection if proxy names or toxic names
 * contain special characters ({@code "}, {@code \}, etc.). Proxy names must be restricted to
 * alphanumeric characters and hyphens by callers. The {@code ToxiproxyApiClientImpl} does not
 * validate or sanitize names.
 *
 * <h2>Stable Contract</h2>
 *
 * <p>This interface is a stable internal API. {@link com.macstab.chaos.proxy.ProxyChaosProvider},
 * {@link com.macstab.chaos.connection.ToxiproxyConnectionChaos}, and test implementations depend
 * on it. Method signatures may not change in a backwards-incompatible way without a major version
 * bump.
 *
 * <h2>Non-Goals</h2>
 *
 * <ul>
 *   <li>This interface does not manage Toxiproxy lifecycle (install, start, stop) — see
 *       {@link com.macstab.chaos.toxiproxy.lifecycle.ToxiproxyLifecycle}.
 *   <li>This interface does not manage iptables redirect rules — see
 *       {@link com.macstab.chaos.toxiproxy.network.NetworkRedirect}.
 *   <li>This interface does not implement retry logic for transient failures.
 * </ul>
 *
 * <h2>Reference</h2>
 *
 * <p>Toxiproxy REST API: https://github.com/Shopify/toxiproxy — the {@code /proxies} and
 * {@code /proxies/{name}/toxics} endpoints are the only surfaces used by this interface.
 *
...
public interface ToxiproxyApiClient {
   ...
   * @return {@code true} if Toxiproxy's management API responded with HTTP 200; {@code false}
   *         for any error including container stopped, process not running, or network failure
   */
   ...
   * Toxiproxy responds with HTTP 200 (proxy found). Returns {@code false} for HTTP 404
   * (proxy not found). Throws {@link IOException} for unexpected errors (e.g., Toxiproxy process
   * crashed mid-call, Docker exec failure).
   *
   ...
   * time may be deleted before the caller acts on the result. Callers that require
   * create-or-skip semantics must not assume the result remains valid.
   *
   * @param ctx resolved container context; must reference a running container with Toxiproxy active
   * @param proxyName the Toxiproxy proxy name to check; must match the name used in
   *                  {@link #createProxy}; must not contain characters that break shell command
   *                  interpolation (restrict to {@code [a-zA-Z0-9_-]})
   * @return {@code true} if a proxy with this name is registered in Toxiproxy;
   *         {@code false} if it does not exist (HTTP 404)
   * @throws IOException if the shell command fails unexpectedly, or if Toxiproxy returns an
   *                     error other than HTTP 404
   ...
   * forwarding to {@code servicePort} at {@code localhost} inside the container. Throws
   * {@link IOException} if Toxiproxy rejects the request (e.g., duplicate proxy name, invalid
   * port, process not running) or if the Docker exec call fails.
   *
   ...
   *
   * <p><strong>Listen address:</strong> Always {@code 0.0.0.0:{proxyPort}} — Toxiproxy binds on
   * all interfaces so that iptables PREROUTING redirected traffic can reach it regardless of
   * source interface.
   *
   * <p><strong>Not idempotent:</strong> Calling this method with a name that already exists in
   * Toxiproxy's registry will fail with an error from Toxiproxy. Callers must check
   * {@link #proxyExists} first or use higher-level orchestration that handles idempotency.
   *
   * <p><strong>Listener readiness:</strong> Toxiproxy starts listening asynchronously after the
   * API call returns. A brief delay (typically a few milliseconds) may exist before the proxy
   * port actually accepts connections. Callers requiring immediate connectivity must poll.
   *
   * @param ctx resolved container context; must reference a running container with Toxiproxy active
   * @param config complete proxy configuration including name, service port, proxy port,
   *               and container hostname; see {@link ProxyConfiguration}
   ...
   * toxics.
   *
   * <p><strong>Contract:</strong> Issues {@code DELETE /proxies/{proxyName}}. On success,
   * Toxiproxy immediately stops listening on the proxy's port and removes all toxic configurations
   * from its in-memory registry. Throws {@link IOException} if the command fails.
   *
   * <p><strong>Toxics are removed atomically:</strong> Toxiproxy removes all toxics associated
   * with the proxy as part of proxy deletion. Callers do not need to remove toxics individually
   ...
   * also invoke
   * {@link com.macstab.chaos.toxiproxy.network.NetworkRedirect#removeRedirect(ContainerContext, int, int)}
   * to restore normal traffic flow. Higher-level orchestration (e.g.,
   * {@link com.macstab.chaos.proxy.internal.ToxiproxyOrchestrator#deleteProxy}) handles both
   * steps together.
   *
   * @param ctx resolved container context; must reference a running container with Toxiproxy active
   ...
   *
   * <p><strong>Contract:</strong> Fetches the toxic list via {@code GET /proxies/{proxyName}/toxics}
   * and performs a name-based membership check. Returns {@code false} on any error rather than
   * propagating, making it safe for conditional checks. Throws {@link IOException} only for
   * infrastructure failures (Docker exec failure, process not running).
   *
   * <p><strong>Implementation note:</strong> This method delegates to {@link #listToxics} and
   * performs a linear scan. For proxies with many toxics (unusual but possible), this is O(n).
   * In typical test scenarios a proxy has 1–3 toxics; the cost is negligible.
   *
   ...
   * no toxics are active. Throws {@link IOException} if the proxy does not exist or if the
   * command fails.
   *
   ...
   * adding a JSON library (Jackson, Gson) for a single parse operation in a test framework
   * is disproportionate. The regex is robust for well-formed Toxiproxy responses but will
   * misparse malformed or unexpected JSON structures. Toxiproxy's response format is stable
   * across versions 2.x.
   *
   * <p><strong>Snapshot semantics:</strong> The returned list reflects the state at the time of
   * the API call. Toxics added or removed concurrently are not reflected.
   *
   * @param ctx resolved container context
   ...
   * @throws IOException if the proxy does not exist, Toxiproxy is not running, or the Docker
   *                     exec call fails
   * @throws NullPointerException if ctx or proxyName is null
   */
   ...
   * fault to new and existing connections (depending on toxic type) immediately. Throws
   ...
   * </pre>
   *
   * <p>The {@code attributes} value must be a JSON object string produced by
   * {@link com.macstab.chaos.toxiproxy.toxic.ToxicConfig#toJson()}. The outer envelope
   * ({@code name}, {@code type}, {@code toxicity}) is assembled by the API client, not by the
   * toxic model. This separation enforces that the toxic model only knows its domain-specific
   * parameters while the API client knows the wire format.
   *
   * <p><strong>Duplicate name:</strong> Adding a toxic whose name already exists on the proxy
   * will fail with a Toxiproxy API error. Callers must ensure name uniqueness per proxy, or use
   * {@link #toxicExists} to guard the call.
   *
   ...
   * @param toxicType Toxiproxy type identifier: {@code "latency"}, {@code "timeout"},
   *                  {@code "bandwidth"}, {@code "slow_close"}, {@code "limit_data"}, or
   *                  {@code "down"}
   ...
   * affected by it.
   *
   * <p><strong>Contract:</strong> Issues {@code DELETE /proxies/{proxyName}/toxics/{toxicName}}.
   * On success, the proxy continues forwarding traffic but the removed fault is no longer applied
   * to new connections. In-flight connections that were already affected by the toxic may still
   ...
   *
   * <p><strong>Per-proxy scope:</strong> Toxic names are scoped to their proxy. The same toxic
   * name may exist on different proxies simultaneously without conflict.
   *
   * @param ctx resolved container context
