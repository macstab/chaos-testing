...
/**
 * Abstract sealed base for all {@link ToxicConfig} implementations, providing shared state,
 * shared validation, and a reusable builder scaffold via the Template Method pattern.
 *
 * <h2>Why This Class Exists: Eliminating Structural Duplication</h2>
 *
 * <p>All six concrete toxic implementations ({@link LatencyToxic}, {@link TimeoutToxic},
 * {@link BandwidthToxic}, {@link SlowCloseToxic}, {@link LimitDataToxic}, {@link DownToxic})
 * share an identical structural skeleton: two common fields ({@code name}, {@code toxicity}),
 * the same validation logic for those fields, the same builder pattern, and the same
 * {@link ToxicConfig} method implementations for {@link ToxicConfig#name()} and
 * {@link ToxicConfig#toxicity()}. Without this base class, each of the six implementations would
 * repeat ~80 lines of structurally identical code. The result: O(1) changes to shared logic
 * (e.g., toxicity range validation) instead of O(6).
 *
 * <h2>Pattern: Template Method (GoF)</h2>
 *
 * <p>This class embodies the Template Method pattern. The invariant structure (field storage,
 * common validation, common builder setters) is defined here. The type-specific behavior —
 * {@link ToxicConfig#type()} and {@link ToxicConfig#toJson()} — is delegated to concrete
 * subclasses as abstract methods. Neither {@code type()} nor {@code toJson()} has a sensible
 * default at this level, so they remain abstract.
 *
 ...
 * </pre>
 *
 * <p>This two-level sealed chain means: (1) only {@code AbstractToxic} may implement
 * {@code ToxicConfig}, and (2) only the six named classes may extend {@code AbstractToxic}.
 * The compiler enforces both constraints. External modules cannot add new toxic types without
 * modifying this module's source — an intentional framework integrity guarantee.
 *
 ...
 * type-specific setters. The concrete builder's {@code build()} method calls
 * {@code super(builder)}, which validates common fields and populates them.
 *
 * <h2>Immutability</h2>
 *
 * <p>All fields in this class and all concrete subclasses are {@code final}. Once constructed via
 * a builder, a toxic configuration is immutable. This is intentional: toxic configurations are
 * value objects that represent a desired fault state, not mutable entities. Immutability makes
 * them safe to share across threads and to reuse across multiple proxy operations.
 *
 * <h2>Thread Safety</h2>
 *
 * <p>Instances are unconditionally thread-safe due to full immutability. The builder itself
 * is NOT thread-safe — it must not be shared across threads during construction (standard Builder
 * pattern usage).
 *
 * <h2>Validation Lifecycle</h2>
 *
 * <p>Common-field validation (null check on {@code name}, range check on {@code toxicity}) runs
 * in the {@link AbstractToxic#AbstractToxic(AbstractBuilder)} constructor, which is called by
 * each concrete class's constructor via {@code super(builder)}. Type-specific validation
 * (e.g., {@code rateKbps > 0} for {@link BandwidthToxic}) runs in the concrete class's
 * constructor, after {@code super(builder)} returns. This ordering ensures all invariants are
 * checked before the object is fully constructed.
 *
...
public abstract sealed class AbstractToxic implements ToxicConfig
   ...
   *
   * <p>Called from each concrete subclass constructor via {@code super(builder)} immediately
   * before any type-specific field initialization. Validates {@code name} (not null) and
   * {@code toxicity} (in [0.0, 1.0]) and stores them in final fields. Any validation failure
   * throws before the object is half-constructed, ensuring instances are always fully valid.
   *
   ...
   *
   * <p>Final: the name is a core identity field and must not be overrideable by subclasses.
   ...
   *
   * <p>Called by this class's constructor for the common {@code toxicity} field. Exposed as
   * {@code protected static} so concrete subclasses could call it for secondary toxicity-like
   * fields if needed, though no current implementation requires this.
   *
   ...
   *
   * <p>Used by concrete subclasses for fields like {@code latencyMs}, {@code timeoutMs},
   * {@code delayMs} — values where zero is meaningful (no delay) but negative is nonsensical.
   *
   ...
   *
   * <p>Provides the common fields ({@code name}, {@code toxicity}) with defaults
   * ({@code toxicity = 1.0}) and fluent setter methods that return the concrete builder type
   * ({@code B}) via the self-referential generic bound.
   *
   ...
   * </pre>
   * With the self-referential generic, {@code name()} and {@code toxicity()} are declared to
   * return {@code B} (the concrete type), preserving the fluent chain:
   * <pre>
   * LatencyToxic.builder()
   ...
   * </pre>
   *
   * <p>The {@code @SuppressWarnings("unchecked")} on the cast {@code (B) this} is required
   * because the Java type system cannot prove {@code this} is of type {@code B} at compile time
   * (the self-referential generic is not reified at runtime). This is a standard Java Builder
   * pattern technique and the cast is safe provided subclasses correctly declare
   * {@code Builder extends AbstractBuilder<Builder>}.
   *
   * <h3>Thread Safety</h3>
   *
  ...
  public abstract static class AbstractBuilder<B extends AbstractBuilder<B>> {
     ...
     *
     * <p>Required field — not calling this before {@code build()} will cause
     * {@link AbstractToxic#AbstractToxic(AbstractBuilder)} to throw {@link NullPointerException}.
     *
     * <p>Name must not contain shell metacharacters or JSON special characters. Restrict to
     * {@code [a-zA-Z0-9_-]}.
     *
     ...
     * @throws IllegalArgumentException if toxicity is outside [0.0, 1.0] — validation deferred
     *                                  to {@link AbstractToxic} constructor
     */
