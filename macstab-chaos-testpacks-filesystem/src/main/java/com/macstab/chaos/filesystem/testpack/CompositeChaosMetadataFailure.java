 ...
 * simulating inode or filesystem-metadata read failure. The default probability is {@code 0.3}
 * (30 % of opens fail), modelling a filesystem where metadata structures are damaged or where
 * the underlying block device returns errors specifically on metadata blocks.
 *
 * <h2>How it's created</h2>
 *
 * <p>Injects {@code IoRule.errno(path, OPEN, EIO, 0.3)} via libchaos-io. Filesystem metadata
 * lives separately from file data in the inode table and directory tree. When the block device
 * returns EIO reading an inode, the kernel propagates EIO to the {@code open()} caller — the
 * application cannot determine whether the file exists at all, let alone read its data. In
 * production this happens during journal replay after an unclean shutdown, during fsck on a
 * partially-failed RAID member, or on a flash device with a worn-out inode region.
 *
 * <h2>How bad it is</h2>
 *
 ...
 * (e.g. configuration reload, plugin discovery) will see intermittent EIO errors that are
 * difficult to attribute without good logging. 30 % failure rate is high enough to produce
 * consistent test signal without total service outage.
 *
 ...
 * state produce sporadic EIO on metadata-heavy workloads before full error state is declared
 * (AWS EBS Volume Status Checks documentation).
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
