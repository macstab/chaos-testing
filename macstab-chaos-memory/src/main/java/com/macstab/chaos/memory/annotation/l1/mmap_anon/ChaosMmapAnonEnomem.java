...
/**
 * L1 chaos primitive: inject {@code ENOMEM} on every libchaos-memory-intercepted
 * {@code mmap(MAP_ANONYMOUS)} call inside the container, gated by {@link #probability}.
 *
 * <p><strong>What this simulates:</strong> out of memory — the canonical libc malloc()-allocation-failure code for sizes >= MMAP_THRESHOLD.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * production rates for {@code ENOMEM} on {@code mmap(MAP_ANONYMOUS)}; {@code 1.0} produces
