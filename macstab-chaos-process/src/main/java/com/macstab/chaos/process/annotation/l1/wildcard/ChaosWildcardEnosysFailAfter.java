 ...
 * succeed, then injects {@code ENOSYS} on every subsequent call until the rule is removed.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one
 * (selector = {@code WILDCARD}, errno = {@code ENOSYS}, effect = FAIL_AFTER) tuple.
 ...
 * every subsequent call returns {@code -1} with {@code errno = ENOSYS}, regardless of real
 ...
 * @ChaosWildcardEnosysFailAfter(successesBeforeFailure = 128)
 ...
 * ({@code @ChaosWildcardEnosysFailAfter.Repeatable}) to apply different counters to different containers.
 *
