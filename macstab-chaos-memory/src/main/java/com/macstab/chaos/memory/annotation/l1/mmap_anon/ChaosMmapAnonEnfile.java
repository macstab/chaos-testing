...
/**
 * Injects {@code ENFILE} on every libchaos-memory-intercepted {@code mmap(MAP_ANONYMOUS)} call inside
 * the target container, making the call fail as if the kernel returned {@code ENFILE}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP_ANON}, errno = {@code ENFILE})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code ENFILE} is a valid POSIX result of {@code mmap(MAP_ANONYMOUS)}; the invalid combinations
 ...
 * kernel-level failure. Specifically this simulates: system-wide file-descriptor limit reached — typical of host-side fd exhaustion incidents.
 *
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-4 to 1e-3; system-wide limit fires are rare — simulate near-limit conditions.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every memory-chaos-capable container
 * in the test class. Use the repeatable form ({@code @ChaosMmapAnonEnfiles}) to bind different
