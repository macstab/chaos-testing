 ...
 * of {@code nanosleep(2)} when the {@code req} or {@code rem} argument points to inaccessible
 * memory. No runtime selector-errno validation is needed.
 *
 ...
 *       corrupted buffer if the caller passes a null {@code rem} pointer that is later
 *       dereferenced.
 *   <li>Native wrappers around {@code nanosleep} in JNI code may throw {@link java.lang.Error} or
 *       produce undefined behavior if they do not guard against the {@code EFAULT} case.
 ...
 *
 * <p>The kernel validates both the {@code req} and (if non-null) the {@code rem} pointers during a
 * {@code nanosleep(2)} call via {@code copy_from_user} and {@code put_user}. If either pointer lies
 * outside the accessible virtual address space, the kernel returns {@code EFAULT} without sleeping
 * at all.
 *
 * <p>In practice, a null {@code req} pointer is the most common source; glibc does not guard
 * against it before calling the kernel, so a null pointer passed from a buggy JNI shim produces a
 * genuine {@code EFAULT}. {@code libchaos-time.so} injects the same error code synthetically,
