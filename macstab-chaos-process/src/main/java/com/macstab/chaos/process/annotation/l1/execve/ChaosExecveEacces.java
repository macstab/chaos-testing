 ...
 *       {@code strerror}: "Permission denied"; no new process image is loaded.</li>
 * </ol>
 *
 ...
 *       assuming the binary is absent (distinct from {@code ENOENT}).</li>
 ...
 *       implementation rather than propagating an unhandled exception.</li>
 ...
 *       messages; operators require different remediation steps for each: {@code EACCES} requires
 *       a {@code chmod} or SELinux/AppArmor policy change; {@code ENOENT} requires installing
 *       the missing binary.</li>
 * </ul>
 * Production failure mode: a Kubernetes pod drops capabilities (e.g. removes the execute bit
 * from a sidecar binary via an incorrect init container) or an SELinux/AppArmor policy update
 * denies {@code execve} to the application's service account; the application's subprocess
 * invocations fail with {@code EACCES} and the application silently loses external-tool
 * functionality without surfacing a clear diagnostic.
 *
 * <h2>Deep technical dive</h2>
 * <p>POSIX specifies {@code EACCES} for {@code execve} when: (1) the calling process does not
 * have execute permission on the file; (2) the file resides on a filesystem mounted with the
 * {@code noexec} flag; (3) the kernel's security module (SELinux, AppArmor, Landlock) denies
 * the exec operation based on policy. On Linux, the kernel checks these conditions in sequence
 * in {@code do_open_execat}: permission bits first, then mount flags, then LSM hooks.
 *
 * <p>The most operationally significant source of {@code EACCES} on {@code execve} in
 * containerised environments is LSM policy: SELinux and AppArmor policies that restrict which
 * binaries a container's process domain can execute are set at container startup. A policy
 * update that restricts the allowed exec targets will cause previously-successful {@code execve}
 * calls to fail with {@code EACCES} for the container's process domain. This failure is
 * invisible in integration tests that run without the LSM policy active.
 *
 * <p>A second important source is the {@code noexec} mount option: containers that mount volumes
 * (config maps, secrets, persistent volumes) with {@code noexec} cannot execute binaries from
 * those volumes. Orchestrators that auto-apply security hardening (e.g. mounting all volumes with
 * {@code noexec,nosuid,nodev} by default in a hardened cluster) may cause unexpected {@code EACCES}
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2; permission failures are non-transient
 * in production so even low rates usefully exercise the error handling path.
