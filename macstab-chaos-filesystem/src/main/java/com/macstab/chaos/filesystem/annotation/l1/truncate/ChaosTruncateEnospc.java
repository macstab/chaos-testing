...
import com.macstab.chaos.filesystem.annotation.l1.allocate.ChaosAllocateEnospc;
 ...
 * {@code errno = ENOSPC} as if the kernel could not allocate additional blocks needed to extend the
 * file to the requested size because the filesystem has no free blocks remaining.
 *
 ...
 *       the file to the expected mmap size before mapping it) must handle {@code ENOSPC}
 *       gracefully; assert that the application does not attempt to mmap a file that was not
 *       successfully sized.
 ...
 * filesystem is full and the application attempts to pre-allocate space (often as part of WAL file
 * creation or memory-mapped file setup), and on sparse files when the application sets the file
 * size to a very large value without having free blocks to back the sparse regions.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code truncate(2)} can both shrink and extend files. When shrinking a file (new length
 * smaller than current size), the kernel frees the blocks that back the truncated region and no new
 * allocation is needed; {@code ENOSPC} is not possible for shrink operations. When extending a file
 * (new length larger than current size), the kernel must allocate blocks to back the new region (or
 * create a sparse region on filesystems that support it). If the filesystem has no free blocks,
 * extending via {@code truncate} returns {@code ENOSPC}.
 *
 * <p>On filesystems with sparse file support (ext4, XFS, Btrfs), extending via {@code truncate} may
 * not allocate blocks immediately — the hole is represented as a sparse region and blocks are only
 * allocated when the sparse region is written. In this case, {@code truncate} may succeed even when
 * the filesystem is nearly full, and {@code ENOSPC} only appears when the application writes to the
 * sparse region. This injection bypasses the sparse file optimisation and injects {@code ENOSPC} at
 * the truncate call itself.
 *
