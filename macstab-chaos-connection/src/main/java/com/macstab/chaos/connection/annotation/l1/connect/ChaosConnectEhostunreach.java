...
/**
 * Injects {@code EHOSTUNREACH} on every libchaos-intercepted {@code connect} call inside
 * the target container, making the call fail as if the kernel returned {@code EHOSTUNREACH}.
 *
 ...
 * annotation class exists only because {@code EHOSTUNREACH} is a valid POSIX result of
 * {@code connect}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EHOSTUNREACH} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: host unreachable — ICMP unreachable; typical of route-table flaps.
 *
 ...
 * class. Use the repeatable form ({@code @ChaosConnectEhostunreachs}) to bind different probabilities to
