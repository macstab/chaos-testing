...
/**
 * Injects {@code ENODEV} on every libchaos-memory-intercepted {@code mmap(MAP_ANONYMOUS)} call inside
 * the target container, making the call fail as if the kernel returned {@code ENODEV}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP_ANON}, errno = {@code ENODEV})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code ENODEV} is a valid POSIX result of {@code mmap(MAP_ANONYMOUS)}; the invalid combinations
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = ENODEV} before the
 * kernel call completes — from the application perspective this is indistinguishable from a real
 * kernel-level failure. Specifically this simulates: no such device — typical of memory-backed device removal or hot-unplug.
 *
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-4 to 1e-3; device-removal faults are rare but catastrophic when unhandled.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every memory-chaos-capable container
 * in the test class. Use the repeatable form ({@code @ChaosMmapAnonEnodevs}) to bind different
