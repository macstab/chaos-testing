 ...
 *
 * <p>Slows log segment writes, injects EIO on fsync to simulate segment cleanup failure, and
 * adds SEND latency to reproduce the full backpressure chain from a Kafka broker whose disk
 * is under heavy pressure.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class KafkaStoragePressureComposer implements L3Composer<IncidentChaosKafkaStoragePressure> {

    public KafkaStoragePressureComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosKafkaStoragePressure ann) {
        final List<Object> handles = new ArrayList<>();

        final var fs = CompositeFilesystemChaos.standard().advanced();
        handles.add(fs.apply(container,
                IoRule.latency(PathPrefix.path(ann.path()), IoOperation.WRITE, Duration.ofMillis(ann.latencyMs()))));
        handles.add(fs.apply(container,
                IoRule.errno(PathPrefix.path(ann.path()), IoOperation.FSYNC,
                        com.macstab.chaos.filesystem.model.Errno.EIO, ann.probability())));

        final var conn = CompositeConnectionChaos.standard().advanced();
        handles.add(conn.apply(container,
                NetRule.latency(Endpoint.wildcard(), NetOperation.SEND, Duration.ofMillis(ann.latencyMs() / 2), 1.0)));
