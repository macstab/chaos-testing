...
/**
 * Load a hand-written jvm-agent plan json file (from the test classpath) and push it into the
 * target container via {@code CompositeJavaChaos}.
 *
 ...
 *
 * <p><strong>What chaos this applies:</strong> load a hand-written JVM-agent plan JSON file (from
 * the test classpath) and push it into the target container via {@code CompositeJavaChaos} inside
 * the JVM of the target container. The effect fires on every matching call. The rule is active from
 * {@code beforeAll} until {@code afterAll} (class-scope) or from {@code beforeEach} until {@code
 ...
 * string applies to every agent-capable container. Use the repeatable form ({@code @ChaosJvmPlans})
 * to apply different configurations to different containers.
 *
