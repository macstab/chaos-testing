 ...
 * {@code errno = ENOTCONN} as if the socket is not in a connected state and the kernel cannot shut
 * down a connection that was never fully established.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (operation = {@code SHUTDOWN}, errno = {@code
 * ENOTCONN}) tuple. A Bernoulli trial with probability {@link #toxicity} is run on each intercepted
 * {@code shutdown} call; when it fires the interposer returns {@code -1} with {@code errno =
 * ENOTCONN} without performing any real kernel operation. No runtime operation-errno validation is
 * needed.
 *
 ...
 *       condition in cleanup code. Assert that the application treats {@code ENOTCONN} from {@code
 *       shutdown} as a no-op and proceeds to close the socket without logging an error.
 *   <li>Connection pool teardown code that calls {@code shutdown} before evicting a connection must
 *       tolerate {@code ENOTCONN}; assert that the pool proceeds to close the socket and does not
 *       leave the file descriptor open when {@code shutdown} fails.
 *   <li>Graceful shutdown sequences that rely on {@code shutdown(SHUT_WR)} to signal the end of the
 *       write stream must handle {@code ENOTCONN} without assuming the remote peer received a FIN;
 *       assert that the application falls back to abrupt close or connection replacement.
 *   <li>Assert that {@code ENOTCONN} from {@code shutdown} does not propagate to application-layer
 *       callers as a connection error — the underlying connection was already gone, and the
 *       shutdown failure is a symptom, not a new fault.
 * </ul>
 *
 * <p>In production, {@code ENOTCONN} from {@code shutdown} occurs when a connection is terminated
 * by the remote peer (RST or FIN received) concurrently with the local application calling {@code
 * shutdown}: the kernel has already torn down the connection's transport state when the {@code
 * shutdown} syscall arrives, causing it to return {@code ENOTCONN}. It is also returned when {@code
 * shutdown} is called on a socket that was never connected, which can happen in accept-loop error
 * paths that attempt to shut down a socket from which {@code accept} returned an error.
 *
 ...
 * before the application's {@code shutdown} call arrives, creating a race window that is common in
 ...
 * shut down.
 *
 * <p>Java's {@code Socket.shutdownInput()} and {@code Socket.shutdownOutput()} methods throw {@code
 * SocketException} with the message "Socket is not connected" when the underlying {@code shutdown}
 * syscall returns {@code ENOTCONN}. Java's {@code Socket.close()} method suppresses the error
 * entirely because the JVM's close path is designed to be idempotent and tolerant of already-closed
 * or not-connected sockets.
 *
 ...
 * management — handle the case where the connection was already gone when the cleanup runs. These
 * paths are rarely exercised in unit tests because the error requires a race between two concurrent
 * operations.
 *
 * <h2>Example</h2>
 *
