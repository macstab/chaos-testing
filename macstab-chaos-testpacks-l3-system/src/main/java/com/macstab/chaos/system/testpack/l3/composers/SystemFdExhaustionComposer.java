 ...
 *
 * <p>Injects {@code EMFILE} on file opens and {@code ECONNREFUSED} on new socket connections
 * to reproduce the compound failure profile of per-process file descriptor limit exhaustion:
 * health probes pass, the pod is never restarted, but every new operation fails.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SystemFdExhaustionComposer implements L3Composer<IncidentChaosSystemFdExhaustion> {

    public SystemFdExhaustionComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosSystemFdExhaustion ann) {
        final List<Object> handles = new ArrayList<>();

        final var fsAdv = CompositeFilesystemChaos.standard().advanced();
        handles.add(fsAdv.apply(container,
                IoRule.errno(PathPrefix.wildcard(), IoOperation.OPEN,
                        com.macstab.chaos.filesystem.model.Errno.EMFILE, ann.toxicity())));
    ...
    public List<String> describe(final IncidentChaosSystemFdExhaustion ann) {
        ...
        return List.of(
                ...
                "connection: CONNECT ECONNREFUSED toxicity=" + ann.toxicity() + " (new socket creation fails)",
