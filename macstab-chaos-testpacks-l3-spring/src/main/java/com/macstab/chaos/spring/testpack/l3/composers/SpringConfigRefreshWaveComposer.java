 ...
 *
 * <p>Applies RECV latency, DNS EAI_AGAIN, and a JVM BeanCreationException to reproduce the compound
 * failure profile of a simultaneous {@code /refresh} wave — every refreshing pod drops in-flight
 * requests, cascading if the config server is slow. (Spring Cloud Config #2341)
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SpringConfigRefreshWaveComposer
    implements L3Composer<IncidentChaosSpringConfigRefreshWave> {

  public SpringConfigRefreshWaveComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosSpringConfigRefreshWave ann) {
    ...
    final var sc =
        ChaosScenario.builder(id)
            .description(
                "Spring Config refresh wave — bean destruction cascade during simultaneous /refresh")
            .selector(
