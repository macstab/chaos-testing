 ...
 */
public final class SpringConfigServerDownComposer
    implements L3Composer<IncidentChaosSpringConfigServerDown> {

  public SpringConfigServerDownComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosSpringConfigServerDown ann) {
    ...
    handles.add(
        conn.apply(
            container,
            NetRule.timeout(
                Endpoint.wildcard(), Duration.ofMillis(ann.timeoutMs()), ann.toxicity())));

    final String scenarioId =
        JvmPlanAccumulator.instance().mintScenarioId("SpringConfigServerDown");
    final var selector =
