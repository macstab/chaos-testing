 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code WILDCARD}, errno = {@code E2BIG}, effect =
 ...
 *       with {@code errno = E2BIG}.
 *   <li>The calling code receives: {@code execve}/{@code fork} return {@code -1} with {@code errno
 *       = E2BIG} (7); {@code posix_spawn} and {@code pthread_create} return {@code E2BIG} directly;
 ...
 *       the argument-too-long condition and logs the argument byte count for operator diagnosis.
 *   <li>FAIL_AFTER models the environment-accumulation scenario: N successful process launches each
 *       carry an incrementally larger environment; at call N+1 the environment has crossed ARG_MAX;
 *       all subsequent launches return E2BIG — assert that the application prunes the environment
 *       before retrying rather than retrying with the same oversized environment.
 ...
 *       child-tracking registry is not updated when the spawn fails.
 * </ul>
 *
 ...
 * specific scenario (e.g., the Nth exec fails), use a single-selector variant (e.g., {@code
 * ChaosExecveE2bigFailAfter}) so the counter only charges on the targeted syscall family. The
 * wildcard variant is appropriate when testing that the application correctly handles E2BIG
 ...
 * pre-regression phase); subsequent test methods exercise the E2BIG phase. Set {@link
 * #successesBeforeFailure} to the number of process-management calls the application makes during
 * its startup and first successful request handling phase.
 *
 * <p>Return-value conventions differ by function: {@code execve}/{@code fork} return {@code -1} and
 * set {@code errno}; {@code posix_spawn}/{@code posix_spawnp} return the error code directly;
 * {@code pthread_create} also returns the error code directly. Code that checks only {@code if (ret
 * == -1 && errno == E2BIG)} misses errors from spawn and thread-create paths. The FAIL_AFTER
 * counter charges across all families, so the test exercises all of these paths in sequence.
 *
 * <p>E2BIG is non-retryable with the same arguments. The application must either trim its argument
 * vector, prune the environment to reduce total byte count, or fall back to a minimal spawn that
 * avoids environment passthrough. Applications that retry on E2BIG without pruning will spin
 * indefinitely since the argument size does not change between attempts.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
