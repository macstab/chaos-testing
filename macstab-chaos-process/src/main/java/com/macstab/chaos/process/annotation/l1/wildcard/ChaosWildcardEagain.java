...
/**
 * Injects {@code EAGAIN} ("Resource temporarily unavailable") into every process-management syscall
 ...
 * #probability}, modelling transient kernel resource pressure across the entire process lifecycle.
 *
 ...
 *   <li>When the trial fires, the interposer sets {@code errno = EAGAIN} and returns {@code -1} (or
 ...
 *       (11); {@code pthread_create} and {@code posix_spawn} return {@code EAGAIN} directly; {@code
 ...
 *       (RLIMIT_NPROC) and retrying without delay will not resolve the pressure.
 ...
 *       created; assert that the child-tracking registry is not updated on a failed spawn.
 ...
 *       pressure that caused the original EAGAIN.
 * </ul>
 *
 ...
 *
 * <p>Return-value conventions differ by function: {@code fork()} returns {@code -1} and sets {@code
 * errno}; {@code posix_spawn}/{@code posix_spawnp} return the error code directly without setting
 * {@code errno}; {@code pthread_create} also returns the error code directly. Code that checks only
 * {@code if (ret == -1 && errno == EAGAIN)} misses EAGAIN from spawn and thread-create paths. The
 * wildcard variant exercises all of these simultaneously, exposing inconsistent error handling
 * across the process management abstraction layers.
 *
 * <p>EAGAIN is semantically retryable — unlike ENOMEM or ENOSYS — but the retry must be bounded and
 * back-off must be applied. The correct pattern is to retry after a brief delay (50–200 ms) with a
 * maximum retry count, then escalate with an alert and reduce load by rejecting incoming requests
 * rather than spawning more processes or threads.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 5e-3 exercises all error paths without blocking
 * startup; values above 0.05 will prevent the container init sequence from spawning the threads and
 * processes it needs; start with 1e-3 and confirm the container starts successfully before
 * increasing.
 *
