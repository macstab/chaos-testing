...
@DisplayName("L1 Connection annotation contract")
class L1ConnectionAnnotationContractTest {

    static Stream<Arguments> allL1Annotations() throws Exception {
        List<Class<?>> result = new ArrayList<>();
        String rootPackage = "com.macstab.chaos.connection.annotation.l1";
        String resourcePath = rootPackage.replace('.', '/');
        ClassLoader cl = L1ConnectionAnnotationContractTest.class.getClassLoader();
    ...
    static void scanDir(File dir, String pkg, List<Class<?>> out) {
        ...
        for (File f : dir.listFiles()) {
            if (f.isDirectory()) {
                ...
                try { out.add(Class.forName(className)); }
    ...
    void annotationContract(Class<?> annotationClass) throws Exception {
        ...
        Class<?> repeatableInner = Arrays.stream(annotationClass.getDeclaredClasses())
    ...
    void foundExpectedCount() throws Exception {
        ...
        assertThat(count).isGreaterThanOrEqualTo(47);
    }
}
