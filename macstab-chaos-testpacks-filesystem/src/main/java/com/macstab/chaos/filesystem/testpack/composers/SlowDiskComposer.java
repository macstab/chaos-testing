...
import com.macstab.chaos.filesystem.testpack.CompositeChaosSlowDisk;

/** L2 composer for {@link CompositeChaosSlowDisk}. */
public final class SlowDiskComposer implements L2Composer<CompositeChaosSlowDisk> {

  /** Public no-arg constructor required by the L2 composer contract. */
  public SlowDiskComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final CompositeChaosSlowDisk annotation) {
    ...
    handles.add(adv.apply(container, IoRule.latency(path, IoOperation.READ, delay)));
    handles.add(adv.apply(container, IoRule.latency(path, IoOperation.WRITE, delay)));
    return handles;
  }
