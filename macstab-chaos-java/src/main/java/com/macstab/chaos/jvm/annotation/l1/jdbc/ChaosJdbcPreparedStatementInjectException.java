 ...
 * {@code executeUpdate()} and throws the configured exception before parameterised SQL is sent,
 * enabling tests to verify ORM and Spring Data error-handling paths for specific JDBC failure types
 * that occur during parameterised query execution.
 *
 ...
 *   <li>No SQL is sent; the prepared statement's parameter bindings are discarded; the
 *       connection remains valid and is returned to the pool by the exception handler.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li>Every parameterised SQL execution throws the configured exception; Hibernate wraps it in
 *       a {@code JDBCException} subtype — assert the specific subtype ({@code JDBCConnectionException},
 *       {@code QueryTimeoutException}, etc.) to verify correct wrapping.
 *   <li>Spring Data JPA methods that call {@code save()} in a {@code @Transactional} context:
 *       the exception propagates through the flush, the transaction is rolled back, and the entity
 ...
 *       return stale data, and a concurrent write to the primary violates a unique constraint;
 *       the application receives {@code SQLIntegrityConstraintViolationException} and must decide
 ...
 * implementations. In Hibernate's flush pipeline, each dirty entity generates one or more
 * {@code PreparedStatement.executeUpdate()} calls. When this annotation fires, Hibernate's
 * {@code BatchingBatcher} or {@code NonBatchingBatcher} catches the exception and wraps it in a
 * {@code JDBCException}. The {@code Session} is then marked as closed/invalid by Hibernate's
 * exception handling contract, which means all subsequent operations on the same session will
 * throw {@code IllegalStateException}.
 *
 * <p>Spring's {@code SQLExceptionTranslator} chain (vendor-specific → SQL state → fallback)
 * is exercised by the injected exception's SQL state field. If the injected class is a plain
 * {@code SQLException} without a SQL state, the translator falls back to a generic
 * {@code UncategorizedSQLException}. To test a specific translation, use a subclass that provides
 * the appropriate SQL state in its constructor.
 *
 * <p>Optimistic locking in JPA is implemented via a versioned {@code UPDATE} that checks the
 * current version column; if the update affects 0 rows, JPA throws
 * {@code OptimisticLockException}. Injecting {@code SQLException} before the update fires tests
 * the code path where the version check itself fails due to a database error, which is a distinct
 * scenario from the normal optimistic lock conflict path.
 *
 ...
 *
 * <p>The connection is not invalidated by this annotation; the driver's connection remains in
 * the pool. However, if the application does not close the connection after the exception (e.g.
 * leaks a connection from a manually managed resource), the injected fault makes the leak
 * observable through pool statistics.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
