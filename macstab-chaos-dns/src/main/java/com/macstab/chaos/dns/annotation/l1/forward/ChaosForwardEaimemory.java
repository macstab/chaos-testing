 ...
 * EAI_MEMORY}) tuple. This annotation always fires on every intercepted forward lookup — there is
 * no per-call probability field. Use it when you need every resolution attempt to fail with an
 * out-of-memory condition so that the application's memory-pressure handling is exercised under a
 * DNS-failure scenario. No runtime selector-errno validation is needed.
 *
 ...
 *   <li>Java's {@code InetAddress.getByName()} maps {@code EAI_MEMORY} to an {@code
 *       UnknownHostException}; high-level frameworks that catch {@code OOM} separately will not
 *       intercept this — assert that the DNS path is handled independently.
 *   <li>Assert that the application does not silently swallow {@code EAI_MEMORY} and proceed with a
 *       null or stale address, which would produce a misleading connection error downstream.
 * </ul>
 *
 * <p>In production, {@code EAI_MEMORY} from {@code getaddrinfo} occurs when the process is near its
 * memory limit ({@code RLIMIT_AS} or cgroup memory limit) and cannot allocate the internal buffers
 * that the glibc resolver requires to parse the DNS response and build the {@code addrinfo} linked
 * list.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code EAI_MEMORY} is defined by POSIX as "memory allocation failure". The glibc resolver
 * returns it when an internal {@code malloc} call fails during the parsing of a DNS response or the
 * construction of the returned {@code addrinfo} chain. Unlike other {@code EAI_*} codes, this error
 * is entirely host-side — no DNS packet was malformed and no server was unreachable.
 *
 * <p>Because Java's JVM heap is separate from the native heap, a JVM process can be in a healthy
 * heap state while the native heap (used by glibc, libpq, libcurl) is exhausted. This scenario is
 * particularly common in JNI-heavy applications: the JVM garbage-collects its objects but the
 * native libraries' internal buffers fragment the native heap over time. Injecting {@code
 * EAI_MEMORY} tests whether the JNI boundary correctly surfaces this failure to the Java layer.
 *
 * <p>Applications that use asynchronous DNS resolution (e.g. via c-ares) will receive this error in
 * a callback rather than a synchronous return, which requires that the callback correctly handles
