...
/**
 * Injects {@code EBUSY} on every libchaos-intercepted {@code pthread create} call inside
 * the target container, making the call fail as if the kernel returned {@code EBUSY}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EBUSY}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EBUSY} is a valid POSIX result of
 * {@code pthread create}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EBUSY} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: device or resource busy — typical of pthread_attr exhaustion.
 *
 ...
 * @ChaosPthreadCreateEbusy(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosPthreadCreateEbusys}) to bind different probabilities to
