...
import com.macstab.chaos.dns.annotation.l1.reverse.ChaosReverseEaifail;
import com.macstab.chaos.dns.model.EaiErrno;

/**
 * Injects {@code EAI_FAIL} into both {@code getaddrinfo(3)} (forward lookup) and {@code
 * getnameinfo(3)} (reverse lookup), causing every DNS resolver call to return {@code EAI_FAIL} as
 * if the authoritative nameserver returned a permanent failure response.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (selectorKind = {@code WILDCARD}, errno = {@code
 * EAI_FAIL}) tuple. The {@code WILDCARD} selector matches both interposed DNS calls simultaneously
 * — equivalent to applying {@link ChaosForwardEaifail} and {@link ChaosReverseEaifail} in a single
 * annotation. This annotation always fires on every intercepted DNS call — there is no per-call
 * probability field. No runtime selector-errno validation is needed.
 *
 ...
 *       component that tries to open a connection to a hostname will receive a failure immediately,
 *       with no possibility of recovery through retries.
 ...
 * </ul>
 *
 * <p>In production, simultaneous {@code EAI_FAIL} on both forward and reverse DNS occurs when the
 * entire DNS infrastructure has returned a permanent error for the queried names — for example,
 ...
 * Unlike {@code EAI_AGAIN}, which signals a retriable transient failure, {@code EAI_FAIL} signals a
 * non-retriable permanent failure. This distinction is critical for application correctness: code
 * that retries on all {@code EAI_*} errors will spin indefinitely under {@code EAI_FAIL} injection,
 * revealing a missing retry-limit guard.
 *
 * <p>The wildcard form is more aggressive than the per-call forms because it prevents the
 * application from recovering by attempting a reverse lookup when a forward lookup fails (e.g. to
 * find a cached address via PTR record correlation). Both paths fail simultaneously, closing all
 * DNS-based recovery routes.
 *
