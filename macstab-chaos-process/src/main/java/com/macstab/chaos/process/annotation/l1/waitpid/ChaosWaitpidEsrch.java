...
/**
 * Injects {@code ESRCH} on every libchaos-intercepted {@code waitpid} call inside
 * the target container, making the call fail as if the kernel returned {@code ESRCH}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code ESRCH}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code ESRCH} is a valid POSIX result of
 * {@code waitpid}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = ESRCH} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: no such process — stale PID after the child has been reaped.
 *
 ...
 * @ChaosWaitpidEsrch(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosWaitpidEsrchs}) to bind different probabilities to
