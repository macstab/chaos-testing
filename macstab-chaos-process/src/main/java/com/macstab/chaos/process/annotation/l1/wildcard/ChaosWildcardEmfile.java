 ...
 *   <li>When the trial fires, the interposer sets {@code errno = EMFILE} and returns {@code -1} (or
 ...
 *       {@code errno = EMFILE} (24); {@code pthread_create}/{@code posix_spawn} return {@code
 *       EMFILE} directly; {@code strerror(EMFILE)}: "Too many open files".
 * </ol>
 *
 ...
 *       parent's file-descriptor table at fork time, requiring a duplicate slot for each fd; assert
 *       that the application triggers an in-process fd audit ({@code /proc/self/fd} inventory) when
 *       EMFILE fires from fork, and does not retry without closing at least one fd.
 *   <li>{@code posix_spawn}/{@code posix_spawnp} return {@code EMFILE} when the spawn helper cannot
 *       duplicate fds for the child; assert that the application does not call {@code waitpid} on
 *       an uninitialised pid — the child was never created.
 ...
 *       active-thread count and alerts when the pool falls below minimum size.
 *   <li>Assert that EMFILE is distinguished from ENFILE: EMFILE means this process's fd table is
 *       full (fixable in-process by closing leaked fds); ENFILE means the system-wide kernel fd
 *       table is full (requires platform team intervention, not in-process recovery).
 * </ul>
 *
 ...
 * all fork and spawn attempts return EMFILE; the application cannot launch health-check
 * subprocesses; monitoring reports the process as healthy while it silently fails to spawn new
 * workers.
 *
 ...
 * additional fd consumer per PATH element, making the effective EMFILE threshold slightly lower for
 * spawnp than for spawn or fork. {@code pthread_create} needs internal fds for thread
 * synchronisation (an eventfd or pipe depending on kernel version).
 *
 * <p>In-process EMFILE recovery requires: (1) inventorying {@code /proc/self/fd} to identify leaked
 * descriptors; (2) closing the leaked descriptors; (3) retrying the failed operation. Applications
 * that retry without closing any fds will always receive EMFILE again. The retry must close at
 * least one fd before retrying, otherwise it loops indefinitely.
 *
 * <p>EMFILE from the wildcard selector exercises all process-management paths simultaneously. This
 * reveals whether different paths in the application share the same EMFILE recovery code or each
 * implement their own — inconsistent recovery leads to different paths surviving EMFILE
 ...
 * container from spawning the threads it needs during startup; start with 1e-3 and confirm the
 * container starts before increasing.
 *
