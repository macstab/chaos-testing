 ...
 *       open files in system"; the kernel's global file-table ({@code fs.file-max}) has been
 *       exhausted system-wide; no child process is created.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li>{@code posix_spawnp} returns {@code ENFILE}; no child process is created; assert that the
 *       application checks the return value directly (not {@code errno} — POSIX spawn returns the
 *       error code, not -1) and does not call {@code waitpid} on an uninitialised pid.
 ...
 *       than attempting in-process recovery; in-process fd cleanup reduces EMFILE but not ENFILE.
 ...
 *       path differ fundamentally.
 * </ul>
 *
 * Production failure mode: a container runs a high-throughput command executor using {@code
 * posix_spawnp} to invoke utilities by name; the host node runs hundreds of containers with
 * subprocess-heavy workloads; the kernel's global file-table fills; spawnp returns ENFILE; the
 * executor retries on ENFILE with the same short backoff as for EAGAIN, which makes node saturation
 * worse; operators cannot distinguish the failure from EMFILE without checking {@code
 * /proc/sys/fs/file-nr}.
 *
 ...
 * == -1)} silently misses ENFILE (23). Checking {@code errno} after a non-negative return is
 * undefined behaviour. Code that tests {@code if (ret != 0)} is correct; code that tests {@code if
 * (ret == ENFILE)} after verifying {@code ret != 0} is correct. The distinction matters when ENFILE
 * (23) is one higher than EMFILE (24 — note: EMFILE=24, ENFILE=23 on Linux) — integer comparison
 * errors can cause misclassification.
 *
 * <p>The {@code /proc/sys/fs/file-nr} file reports three space-separated numbers: allocated file
 * handles, (unused — always 0 since kernel 2.6), and maximum file handles. Monitoring {@code
 * file-nr} during load tests reveals whether ENFILE from posix_spawnp is approaching a real
 * threshold. The {@code $PATH} traversal in posix_spawnp opens directory fds during the search,
 * which can push a near-full system table over the edge; this makes ENFILE slightly more likely for
 * posix_spawnp than for posix_spawn under the same node load.
 *
