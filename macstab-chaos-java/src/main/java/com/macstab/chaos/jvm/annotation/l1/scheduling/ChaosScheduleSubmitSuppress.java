 ...
 * {@code suppress} effect applied to the {@code SCHEDULE_SUBMIT} operation. It intercepts the
 * submission path of {@link java.util.concurrent.ScheduledExecutorService} and discards the call
 * before the task's {@code ScheduledFutureTask} is constructed or enqueued. The annotation is
 ...
 *       constructed.
 *   <li>The suppress effect discards the call — a pre-cancelled {@code ScheduledFuture} is
 *       returned to the caller (or {@code null} depending on the suppress translator's
 *       implementation) without touching the executor's delay queue.
 ...
 *
 * <ul>
 *   <li>The executor's queue size ({@code executor.getQueue().size()}) does not increase after
 *       the suppressed submit — assert it remains zero.
 ...
 *   <li>No execution timestamp is ever recorded inside the task's {@code Runnable} — assert that
 *       a counter or latch inside the task is never incremented or counted down.
 ...
 * </ul>
 *
 * <p><strong>Production failure mode this simulates:</strong> a scheduler whose thread pool is
 * shut down prematurely (e.g. by a misconfigured graceful-shutdown sequence), causing all
 * subsequent {@code schedule()} calls to be silently rejected — a Spring {@code @Scheduled}
 * lease-renewal task is never registered, the lease expires, and the node loses distributed
 * leadership, causing split-brain until the operator restarts the service.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p><strong>Interception point.</strong> The agent intercepts
 * {@code ScheduledThreadPoolExecutor#schedule} before the internal {@code delayedExecute} helper
 * is called. {@code delayedExecute} is the method that constructs a {@code ScheduledFutureTask},
 * sets its trigger time, and calls {@code super.execute} to enqueue it. By intercepting before
 * this point, the suppress effect ensures no internal scheduler state is modified.
 *
 * <p><strong>Return value semantics.</strong> The {@code schedule} methods return
 * {@code ScheduledFuture<?>}. The suppress translator returns a synthetic future that is
 * immediately cancelled ({@code isCancelled() == true}, {@code isDone() == true}). Callers that
 * call {@code get()} on the returned future receive a {@code CancellationException} immediately
 * rather than hanging.
 *
 ...
 *
 * <p><strong>Periodic tasks.</strong> For {@code scheduleAtFixedRate} and
 * {@code scheduleWithFixedDelay}, suppressing the initial submission prevents all subsequent
 * executions because periodic re-scheduling only occurs from within a successfully running task.
 * Suppressing the submit therefore silences the entire recurring task, not just the first
 * execution.
 *
 * <h2>Example</h2>
 *
