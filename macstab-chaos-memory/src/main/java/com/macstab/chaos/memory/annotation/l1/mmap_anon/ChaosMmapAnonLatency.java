...
/**
 * Delays every libchaos-memory-intercepted {@code mmap(MAP_ANONYMOUS)} call by {@link #delayMs}
 ...
 * = {@code MMAP_ANON}, effect = LATENCY) pair. Unlike the errno variants, the latency primitive
 * always delegates to the kernel — it only adds wall-clock cost before doing so.
 *
 * <p><strong>What chaos this applies:</strong> every {@code mmap(MAP_ANONYMOUS)} call intercepted
 * by libchaos-memory blocks for {@link #delayMs} ms before the kernel call is issued. This
 * simulates the wall-clock cost increase that surfaces under memory-pressure stall events,
 ...
 * starts. The shared library interposes the libc wrappers for the {@code MMAP_ANON} syscall family.
 ...
 * ({@code @ChaosMmapAnonLatencys}) to set different delays on different containers simultaneously.
 *
