 ...
 * ConnectException during the config refresh cycle — causing the service to continue with
 * stale configuration, potentially diverging from the expected feature flag state.
 *
 * <h2>Composed of</h2>
 * <ul>
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Config Server down → scheduled refresh fails → stale
 * config persists → feature flag divergence between pods; if fail-fast is enabled and the
 * application cannot start without fresh config, a CrashLoopBackOff ensues.
 *
...
@ChaosL3(composer = "com.macstab.chaos.springboot.testpack.l3.composers.SpringConfigServerDownComposer", severity = Severity.SEVERE)
