 ...
 *   <li>Time: MONOTONIC clock skew of {@code -skewMs} ms at probability 1.0 — slow monotonic makes
 *       deadline checks fire late, masking imminent expiry until the budget is fully consumed
 ...
 * Deadline cascade means every service in the call chain simultaneously receives DEADLINE_EXCEEDED;
 * retry storms compound the latency; the entire fan-out of the affected RPC path becomes
 * unavailable until deadlines expire and retries are exhausted.
 *
 * <h2>Industry references</h2>
 *
 * <p>gRPC deadline cascade is described in the gRPC documentation §"Deadlines", the Google SRE book
 * §"Handling Overload", and post-mortems from microservice deployments where a single slow
...
@ChaosL3(
    composer = "com.macstab.chaos.grpc.testpack.l3.composers.GrpcDeadlinePropagationComposer",
