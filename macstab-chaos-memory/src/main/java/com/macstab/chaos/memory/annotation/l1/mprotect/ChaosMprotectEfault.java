...
/**
 * Injects {@code EFAULT} on every libchaos-memory-intercepted {@code mprotect} call inside
 * the target container, making the call fail as if the kernel returned {@code EFAULT}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MPROTECT}, errno = {@code EFAULT})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code EFAULT} is a valid POSIX result of {@code mprotect}; the invalid combinations
 ...
 * in the test class. Use the repeatable form ({@code @ChaosMprotectEfaults}) to bind different
