 ...
 * {@code errno = ECONNRESET} as if the remote peer sent a TCP RST segment in response to data being
 * sent on the connection, terminating the connection mid-transfer.
 *
 ...
 *       socket and discard any data that was in the send buffer — the remote peer did not receive
 *       it.
 ...
 * </ul>
 *
 * <p>In production, {@code ECONNRESET} from {@code send} occurs when the remote server sends RST in
 * response to data it cannot process (e.g., after a timeout, after a protocol violation, or during
 * server restart), when a load balancer terminates a connection while data is in transit, and when
 * a firewall rule change causes packets to be rejected with RST after some data has already been
 * forwarded.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The kernel can receive a RST at any time on an established TCP connection. If the RST arrives
 * while the application is blocked in {@code send}, the kernel unblocks the call and returns {@code
 * -1} with {@code errno = ECONNRESET}. If the RST arrives between calls, it is queued and the error
 * is returned on the next socket operation (send or recv). The data that was in the local send
 * buffer when the RST was received is discarded by the kernel without being transmitted.
 *
