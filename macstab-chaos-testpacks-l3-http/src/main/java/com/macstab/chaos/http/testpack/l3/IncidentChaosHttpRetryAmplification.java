 ...
 * cause HTTP clients to retry, multiplying the effective load on the already-failing upstream until
 * the full request fan-out saturates every available connection.
 *
 * <h2>Composed of</h2>
 *
 * <ul>
 ...
 *   <li>JVM: IOException injected at METHOD_ENTER on class prefix {@code classPattern} — forces the
 *       client library's retry path to activate, amplifying the load multiplier
 * </ul>
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Critical</strong><br>
 * A 50% initial error rate becomes 100% saturation once retries are factored in; with exponential
 * backoff disabled or misconfigured the upstream is destroyed within seconds of the first failure.
 *
...
@ChaosL3(
    composer = "com.macstab.chaos.http.testpack.l3.composers.HttpRetryAmplificationComposer",
