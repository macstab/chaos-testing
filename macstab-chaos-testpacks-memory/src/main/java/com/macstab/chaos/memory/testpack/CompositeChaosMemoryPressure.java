 ...
 *
 * <p>Applies {@code AdvancedMemoryChaos#simulateMemoryPressure(container, 0.05)} via
 * libchaos-memory. This injects a {@code mmap:ERRNO:ENOMEM@0.05} rule — one in twenty anonymous or
 * file-backed mmap calls will fail. In production this class of pressure arises when a host
 * approaches its memory high-watermark, when a noisy-neighbour cgroup is competing, or when memory
 * fragmentation prevents large contiguous allocations even though total free memory is non-zero.
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Moderate</strong><br>
 * A well-written service handles these failures: memory allocators fall back to smaller pools,
 * thread creation is retried, and glibc's arena machinery absorbs most pressure. Services that do
 * not check allocator return values or propagate {@code ENOMEM} correctly will exhibit {@code
 * NullPointerException}, silent data loss, or corrupted state. At 5% rate the impact is noticeable
 * but typically self-correcting if retries are implemented.
 *
 ...
 * Chaos Engineering (2016) and Google's Site Reliability Engineering book (chapter 22) both
 * advocate for low-rate fault injection as a standard pre-production gate.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
