 ...
 *
 * <ul>
 *   <li>Time: REALTIME offset of {@code skewMs} ms at probability {@code probability} — shifts the
 *       application clock forward, causing TTL calculations and script deadlines to diverge from
 *       Redis server time
 *   <li>Connection: SEND latency of {@code latencyMs} ms — adds wire delay that amplifies apparent
 *       skew for time-sensitive MULTI/EXEC and WATCH() transactions
 * </ul>
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Moderate</strong><br>
 * Keys expire earlier than expected from the application's perspective; WATCH()-guarded
 * transactions see elevated CAS failure rates, leading to retry amplification in high-concurrency
 * pipelines.
 *
 * <h2>Industry references</h2>
 *
 * <p>Redis EVAL script TTL drift and WATCH() CAS failures under clock skew are documented in the
 * Redis FAQ and observed in distributed systems where NTP is not tightly configured — common in
 * containerised deployments running on shared hypervisors.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
@ChaosL3(
    composer = "com.macstab.chaos.redis.testpack.l3.composers.RedisClockDriftComposer",
