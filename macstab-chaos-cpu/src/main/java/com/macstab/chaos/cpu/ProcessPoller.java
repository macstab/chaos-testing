 ...
 *
 * <p>{@code /proc/[0-9]&#42;/comm} is a kernel-provided interface available on all Linux
 * containers without any installed utilities. {@code ps} is not available on Alpine minimal
 * images and BusyBox {@code ps} does not support {@code aux}. The {@code /proc} approach
 * has zero dependency on installed tools.
 *
 ...
 * process will never be in zombie state. Zombie filtering is only needed in observability
 * queries ({@link CpuObservability#isStressed}, {@link CpuObservability#isThrottled}) where
 * previously killed processes may linger.
 *
 * <h2>Poll interval</h2>
 *
 * <p>The 10ms poll interval aligns with Docker {@code execInContainer} round-trip latency
 * (5–15ms on Moby/macOS, <5ms in DinD). With tini as init, processes are reaped within
 * milliseconds — the first poll after SIGKILL typically finds no process.
 *
...
final class ProcessPoller {
   ...
   *
   * <p>10ms aligns with Docker exec round-trip latency. With tini as PID 1, processes
   * are reaped in <5ms after SIGKILL — the first poll after kill typically succeeds.
   */
   ...
   *
   * <p>Uses a simple {@code grep} (no zombie filter) — with tini as init, processes are
   * fully reaped ({@code /proc} entry removed) within milliseconds of SIGKILL.
   *
   * <p>If the timeout expires without the process disappearing, a warning is logged and
   * the method returns normally. The caller may verify state via the observability API.
   *
  ...
  static void waitUntilGone(
    ...
    final String check = String.format(
        "grep -rl '^%s' /proc/[0-9]*/comm 2>/dev/null | grep -q .", commName);
   ...
   *
   * <p>Background processes ({@code cmd &}) return exit 0 from the shell immediately
   * but may not yet be visible in {@code /proc} when the exec returns. This method
   * bridges that gap event-driven rather than with a blind sleep.
   *
   ...
   * @param exitWhenFound  {@code true} = stop when exit 0 (waitUntilStarted);
   *                       {@code false} = stop when exit non-0 (waitUntilGone)
