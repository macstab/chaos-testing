...
/**
 * Production implementation of {@link ToxiproxyLifecycle} — manages the Toxiproxy server
 * process lifecycle inside a container with explicit idempotency, polling-based startup,
 * and correct interrupt handling.
 *
 * <h2>Design: No Internal Platform Detection</h2>
 *
 * <p>This class receives a pre-resolved {@link ContainerContext} on every method call. It
 * never calls {@link com.macstab.chaos.core.platform.PlatformDetector} itself. The orchestrator
 * that creates a {@code ContainerContext} via {@link ContainerContext#of(GenericContainer)}
 * performs platform detection exactly once per top-level operation. This design is enforced by
 * the API contract: all methods accept {@code ContainerContext}, not {@code GenericContainer<?>}.
 *
 * <h2>Constructor Variants</h2>
 *
 * <p>Two constructors are provided:
 * <ul>
 *   <li><strong>Single-arg constructor</strong> (production): wires default collaborators —
 *       {@link ToxiproxyInstaller} and {@link ToxiproxyApiClientImpl} constructed with the
 *       API URL from config. This is the constructor used by
 *       {@link com.macstab.chaos.proxy.internal.ToxiproxyOrchestrator} and
 *       {@link com.macstab.chaos.connection.ToxiproxyConnectionChaos}.
 *   <li><strong>Three-arg constructor</strong> (testability): accepts pre-built collaborators
 *       for unit testing. Allows injecting mock {@link ToxiproxyInstaller} and
 *       {@link com.macstab.chaos.toxiproxy.api.ToxiproxyApiClient} without Docker.
 * </ul>
 *
 * <h2>ensureRunning Idempotency</h2>
 *
 * <p>The {@link #ensureRunning(ContainerContext)} method is idempotent by construction: it calls
 * {@link com.macstab.chaos.toxiproxy.api.ToxiproxyApiClient#isApiReady(ContainerContext)} first
 * and returns immediately if the API is healthy. Multiple modules may safely call
 * {@code ensureRunning} on the same container in sequence or interleaved without coordination.
 *
 * <h2>Startup Polling Loop</h2>
 *
 * <p>After launching the Toxiproxy background process, the implementation polls the API at
 * intervals of {@code config.pollIntervalMs()} (default: 100 ms) until either the API responds
 * or the deadline ({@code config.startupTimeoutMs()}) is exceeded. The deadline is computed as
 * a wall-clock epoch millisecond value ({@code System.currentTimeMillis() + timeout}). Each
 * poll iteration costs one Docker API round trip. The loop is not real-time precise — JVM
 * scheduling jitter may cause individual poll intervals to exceed {@code pollIntervalMs}.
 *
 * <h2>Thread Safety</h2>
 *
 * <p>This class is effectively stateless after construction: all state is in the injected
 * collaborators and in the {@code ContainerContext} passed per-call. The collaborators
 * ({@link ToxiproxyInstaller} and {@link com.macstab.chaos.toxiproxy.api.ToxiproxyApiClient})
 * are themselves stateless. Concurrent calls from multiple threads are safe at the Java level
 * but may race on Toxiproxy process startup (see {@link ToxiproxyLifecycle} class-level notes).
 *
...
public final class ToxiproxyLifecycleManager implements ToxiproxyLifecycle {
   ...
   *
   * <p>Constructs a {@link ToxiproxyInstaller} and a {@link ToxiproxyApiClientImpl} using the
   * API URL from {@code config}. This is the production constructor used by orchestrators
   * that create this manager from a {@link ToxiproxyConfig}.
   *
   ...
   *
   * <p>Accepts pre-built {@link ToxiproxyInstaller} and
   * {@link com.macstab.chaos.toxiproxy.api.ToxiproxyApiClient} instances, enabling injection
   * of mocks or stubs. Use this constructor in tests to verify lifecycle behavior without
   ...
   *
   * <p>Executes {@code toxiproxy-server -host 0.0.0.0 >/dev/null 2>&1 &} via the container
   * shell. The {@code -host 0.0.0.0} flag is required so that Toxiproxy's API (port 8474) and
   * proxy listeners are reachable from outside the container — the default bind address
   * ({@code localhost}) would make them invisible to iptables PREROUTING redirected traffic
   * originating from external clients.
   *
   * <p>The trailing {@code &} detaches the process from the shell session. Without it, the
   * {@code execInContainer} call would block indefinitely.
   *
   * <p>stdout and stderr are redirected to {@code /dev/null} to prevent the Toxiproxy process
   * from holding the exec session open via the output stream.
   *
   ...
   * than a fixed iteration count, so JVM scheduling jitter does not systematically under-wait.
   * Each poll iteration reuses the same {@code ContainerContext} — no additional platform
   * detection per iteration. The loop exits as soon as
   * {@link com.macstab.chaos.toxiproxy.api.ToxiproxyApiClient#isApiReady(ContainerContext)}
   * returns {@code true}.
   *
   * <p>On timeout: throws {@link IOException} (not unchecked), consistent with the
   * {@link #ensureRunning} throws clause. This ensures timeout failures are not silently
   * swallowed by callers that catch only checked exceptions.
   *
   * <p>On interrupt: restores the interrupt flag via {@code Thread.currentThread().interrupt()}
   * and throws {@link IOException}. Callers that catch {@link IOException} and continue should
   * re-check interrupt status if cancellation via interruption is required.
   *
