 ...
 *       sets {@code errno = ENFILE} and returns {@code -1} without issuing the real kernel call.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 23, {@code strerror}: "Too many
 *       open files in system"; the kernel's global file table ({@code fs.file-max}) is exhausted;
 *       no process on the system can open any new file; the {@code dirfd} must be closed explicitly
 *       since no close-on-exec processing occurs for failed execs; closing in-process fds will not
 ...
 *       process's fds will not restore capacity when the system-wide kernel table is exhausted.
 ...
 *       system-wide condition and makes the next exec attempt more likely to fail.
 *   <li>Assert that the application distinguishes {@code ENFILE} (23, system-wide, requires
 *       platform escalation to raise {@code fs.file-max} or reduce aggregate fd usage) from {@code
 *       EMFILE} (24, per-process, fixable by closing leaked fds in this process) — the diagnostic
 *       must route to the correct runbook and must not instruct the operator to close application
 *       fds when the problem is system-wide.
 * </ul>
 *
 * Production failure mode: a high-density Kubernetes node runs container runtimes that each open a
 * dirfd to the container's rootfs overlay for {@code execveat(AT_EMPTY_PATH)} exec; each failed
 ...
 * system-wide fd exhaustion follows a capacity curve: aggregate fd usage across all runtimes on the
 * node rises with each container launch; after N launches the cumulative usage crosses {@code
 * fs.file-max}; all subsequent launches fail. FAIL_AFTER with threshold N reproduces this
 ...
 * pattern requires opening a dirfd before calling exec. Under FAIL_AFTER, the first N execs succeed
 * and the dirfd is closed by the kernel (via {@code FD_CLOEXEC}). After the counter trips, every
 * subsequent exec attempt opens a dirfd (consuming one slot from the system-wide table), then
 * receives {@code ENFILE} from the interposer. If the application does not close the dirfd in the
 * error path, each failed launch contributes one more fd to the system-wide table — the failing
 * launches become the cause of the next launch's failure, creating a self-reinforcing cascade that
 * plain ERRNO testing cannot reveal.
 *
 * <p>The operational distinction from {@code EMFILE} is critical: {@code EMFILE} means this
 * process's fd table is full (fixable in-process by closing leaked fds); {@code ENFILE} means the
 * system-wide kernel file table is exhausted (requires the platform team to raise {@code
 * fs.file-max} or reduce aggregate fd usage across all processes on the node). Applications that
 * treat {@code ENFILE} as {@code EMFILE} will instruct operators to close application fds — an
 * action that does not resolve the system-wide condition and wastes incident response time.
 *
 ...
 * number of container launches before the node's fd table reaches {@code fs.file-max}; for test
 * purposes, values in the range 10–200 exercise the saturation pattern efficiently; 0 means the
 * system-wide table is exhausted from the first exec attempt (tests cold-start node saturation).
 *
