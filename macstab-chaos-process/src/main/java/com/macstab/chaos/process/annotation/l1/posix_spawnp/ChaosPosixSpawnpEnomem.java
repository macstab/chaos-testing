...
/**
 * L1 chaos primitive: inject {@code ENOMEM} on every libchaos-process-intercepted
 * {@code posix_spawnp} call inside the container, gated by {@link #probability}.
 *
 * <p><strong>What this simulates:</strong> out of memory — kernel cannot allocate process structures; precedes OOM-killer activation.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * @ChaosPosixSpawnpEnomem(probability = 0.01)
 * class MyTest { ... }
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> {@code 1e-3} to {@code 1e-2} mirrors realistic
 * production rates for {@code ENOMEM} on {@code posix_spawnp}; {@code 1.0} typically
