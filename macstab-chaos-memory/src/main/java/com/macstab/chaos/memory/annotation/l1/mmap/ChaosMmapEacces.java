 ...
 * L1 libchaos-memory primitive — one (selector = {@code MMAP}, errno = {@code EACCES}) tuple.
 * The {@code MMAP} selector covers both anonymous and file-backed {@code mmap} calls; use
 * {@code ChaosMmapAnonEacces} or {@code ChaosMmapFileEacces} for narrower fault isolation.
 ...
 *       allocator and file-mapping paths are denied simultaneously.</li>
 *   <li>glibc {@code malloc} propagates {@code NULL}; file-mapping code should fall back to
 *       {@code read}/{@code write} or surface a structured error — assert that no silent
 *       data loss occurs.</li>
 *   <li>Assert that the application produces a permission-related diagnostic and does not
 ...
 * incompatible with the requested protection. For file-backed mappings, this arises when the
 * file is opened read-only and {@code PROT_WRITE | MAP_SHARED} is requested. For anonymous
 * mappings, {@code EACCES} arises from LSM hooks that deny the operation based on security
 * policy rather than file permissions.
 *
 * <p>The broad {@code MMAP} selector injects {@code EACCES} on both code paths simultaneously.
 * This stresses applications more aggressively than the narrower selectors: a process that
 * handles allocation failure gracefully may still crash if its memory-mapped file path also
 * fails at the same time with a different errno than expected.
 *
 * <p>glibc's internal allocator (anonymous path), the dynamic linker (shared-library loading),
 * and any application code that uses {@code mmap} for file I/O are all affected. On JVM
 * workloads, class-loading via memory-mapped JAR files will fail — usually manifesting as
 * {@code ClassNotFoundException} or {@code NoClassDefFoundError} rather than a memory error.
 *
 * <p>Compared with {@code EPERM}: {@code EACCES} is a DAC/MAC credentials check failure
 * (the object is accessible in principle but not to this process); {@code EPERM} is a
 * structural capability check failure (the operation is globally disallowed for this process
 * regardless of the object). Both are non-transient: retry will not succeed without a
