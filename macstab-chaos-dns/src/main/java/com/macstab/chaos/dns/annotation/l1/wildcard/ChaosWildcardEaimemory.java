...
/**
 * Injects {@code EAI_MEMORY} on every libchaos-intercepted {@code wildcard} call inside
 * the target container, making the call fail as if the kernel returned {@code EAI_MEMORY}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EAI_MEMORY}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EAI_MEMORY} is a valid POSIX result of
 * {@code wildcard}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EAI_MEMORY} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: resolver memory allocation failure — host-side memory pressure during resolution.
 *
 ...
 * @ChaosWildcardEaimemory(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosWildcardEaimemorys}) to bind different probabilities to
