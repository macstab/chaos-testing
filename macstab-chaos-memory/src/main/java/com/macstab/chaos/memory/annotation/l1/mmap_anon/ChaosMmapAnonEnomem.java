...
/**
 * Injects {@code ENOMEM} on every libchaos-memory-intercepted {@code mmap(MAP_ANONYMOUS)} call inside
 * the target container, making the call fail as if the kernel returned {@code ENOMEM}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP_ANON}, errno = {@code ENOMEM})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code ENOMEM} is a valid POSIX result of {@code mmap(MAP_ANONYMOUS)}; the invalid combinations
 ...
 * kernel-level failure. Specifically this simulates: out of memory — the canonical libc malloc()-allocation-failure code for sizes >= MMAP_THRESHOLD.
 *
 ...
 * in the test class. Use the repeatable form ({@code @ChaosMmapAnonEnomems}) to bind different
