 ...
 *       buffer allocation can starve the I/O thread of buffers, causing it to block and leaving all
 *       channels unreadable until the allocation completes — effectively halting the service.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code ByteBuffer.allocateDirect(int)} calls {@code new DirectByteBuffer(int)}, which invokes
 * the unsafe off-heap memory allocator via {@code Bits.reserveMemory()} and {@code
 * unsafe.allocateMemory()}. On the fast path this is a native {@code malloc} call and returns
 * quickly; on the slow path (when the JVM's direct-memory limit tracked by {@code java.nio.Bits} is
 * close to {@code -XX:MaxDirectMemorySize}) the JVM triggers a GC and may retry several times
 * before either succeeding or throwing {@code OutOfMemoryError: Direct buffer memory}.
 *
 * <p>The agent intercepts at the {@code ByteBuffer.allocateDirect(int)} level using Byte Buddy
 * method-entry advice. The delay fires before the actual allocation, so it adds to the total wall
 * time of the allocation regardless of whether the slow path is taken. This simulates the
 * conditions seen when the OS page allocator is under pressure (e.g. on a host with high memory
 * fragmentation or NUMA imbalance) and {@code malloc} returns slowly even though the JVM's logical
 * direct-memory limit is not yet reached.
 *
 ...
 *
 * <p>This annotation is distinct from the {@link
 * com.macstab.chaos.jvm.annotation.l1.stressors.ChaosDirectBufferPressure} stressor: the delay
