 ...
 * <h2>Composed of</h2>
 * <ul>
 *   <li>Time: REALTIME clock skew of {@code skewMs} ms at probability {@code probability} —
 *       broker and client wall-clocks diverge; timestamp-indexed segments become inconsistent
 *       and compaction decisions based on wall time may evict live data
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Moderate</strong><br>Messages land in wrong partition segments under
 * timestamp-based routing; offset commit drift is possible when wall-clock mismatches cause
 * log compaction to remove messages that consumers have not yet processed.
 *
 * <h2>Industry references</h2>
 * <p>Kafka timestamp routing under clock skew — messages landing in wrong partition segments
 * and offset commit drift — is documented in KIP-32 and operator post-mortems involving
 * NTP desynchronisation on broker or client hosts.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.kafka.testpack.l3.composers.KafkaClockDriftComposer", severity = Severity.MODERATE)
