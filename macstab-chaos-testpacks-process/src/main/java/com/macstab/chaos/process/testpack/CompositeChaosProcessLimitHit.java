 ...
 * a kernel process-table slot or {@code RLIMIT_NPROC} exhaustion. From the application's
 * perspective the OS is refusing to create new processes: server-side workers cannot be spawned,
 * CGI-style process-per-request servers silently drop requests, and credential-isolation daemons
 * that fork for privilege separation return error to the caller.
 *
 ...
 * runtime or sidecar process burst can push the uid process count to {@code RLIMIT_NPROC} and cause
 * every subsequent fork on any container sharing that uid to fail.
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Severe</strong><br>
 * At {@code toxicity = 0.9} nine out of ten fork attempts fail. Applications that treat fork-EAGAIN
 * as a hard error (vs. retrying with backoff) will immediately begin returning errors to callers.
 * Without a process-table health check in readiness probes the service appears healthy to the
 * scheduler while silently rejecting work. Manual intervention or node-level process-count
