 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Clients must detect the GOAWAY and retry on a new
 * connection; if retries are not implemented, requests fail during the entire drain window;
 * channel warm-up latency on new connections can extend the effective unavailability.
 *
 * <h2>Industry references</h2>
 * <p>Rolling deploy server GOAWAY drain is described in the gRPC documentation §"Server
 * Reflection" and §"Graceful Server Shutdown", the Envoy documentation §"Draining", and
 * post-mortems from Kubernetes rolling update incidents where clients did not handle GOAWAY
 * correctly.
 *
 * <h2>Example</h2>
 * <pre>{@code
...
@ChaosL3(composer = "com.macstab.chaos.grpc.testpack.l3.composers.GrpcConnectionDrainComposer", severity = Severity.SEVERE)
