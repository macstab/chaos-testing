 ...
 * eviction on an unrelated key blocks on the same lock, causing up to 1,400 threads to wait.
 * Cache throughput drops to zero and a restart is required to recover.
 *
 * <h2>Composed of</h2>
 * <ul>
 ...
 *       {@code classPattern} at METHOD_EXIT — loader fails after holding the lock, amplifying
 *       the thread pile-up
 * </ul>
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Cache throughput drops to zero; all threads waiting on
 * unrelated keys are blocked. A process restart is required to recover.
 *
 * <h2>Industry references</h2>
 * <p>Caffeine GitHub issue #672 documents this behaviour: a single slow loader acquires
 * {@code evictionLock} and blocks all eviction-driven operations across the entire cache instance.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.cache.testpack.l3.composers.CaffeineEvictionDeadlockComposer", severity = Severity.SEVERE)
