 ...
 * giving the test precise control over exactly when the consumer is allowed to dequeue the next
 * item.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>An L1 JVM chaos primitive in the {@code QUEUE} selector family targeting the {@code
 * QUEUE_TAKE} operation with the {@code gate} effect. Unlike {@link ChaosQueueTakeDelay}, which
 * parks the consumer thread for a fixed random duration and releases automatically, the gate blocks
 * the consumer thread indefinitely on an internal barrier until the test explicitly releases it —
 * or until the {@link #maxBlockMs} safety timeout fires.
 *
 ...
 * queue depth metric reaches a specific value, that the producer applies back-pressure, or that a
 * watchdog thread fires when the consumer stops draining.
 *
 ...
 *       MILLISECONDS)}.
 *   <li>The test calls the agent's gate-release API; the latch counts down, unblocking the consumer
 *       thread.
 ...
 *
 * <ul>
 *   <li>{@code queue.take()} does not return until the gate is released or {@link #maxBlockMs} ms
 *       pass — the consumer thread is fully blocked.
 ...
 *   <li>Producer threads that are blocked on a full-queue {@code put} call remain blocked while the
 *       consumer gate is held, because no space is freed.
 ...
 *
 * <p><strong>Queue fill dynamics.</strong> While the consumer gate is held, items accumulate in the
 * queue at the producer's rate. If the queue is bounded, it fills in {@code capacity /
 * producer_rate} time; once full, producer {@code put} calls start blocking too. This creates a
 * two-stage chain: consumer freeze → queue full → producer freeze — testing the full back-pressure
 * propagation path.
 *
 * <p><strong>Gate release semantics.</strong> A single release signal unblocks all consumer threads
 * currently parked at the gate. Newly arriving consumer threads after the release pass through
 * immediately. To re-gate future {@code take} calls, reset the gate via the agent API.
 *
