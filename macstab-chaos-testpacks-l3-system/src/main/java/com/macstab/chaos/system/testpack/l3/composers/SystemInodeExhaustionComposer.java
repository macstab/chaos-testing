 ...
 *
 * <p>Injects {@code ENOSPC} on every {@code open()} call to reproduce the compound failure
 * profile of inode exhaustion where {@code df -h} shows plenty of disk space but all file
 * operations fail.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SystemInodeExhaustionComposer implements L3Composer<IncidentChaosSystemInodeExhaustion> {

    public SystemInodeExhaustionComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosSystemInodeExhaustion ann) {
        ...
        handles.add(fsAdv.apply(container,
                IoRule.errno(PathPrefix.wildcard(), IoOperation.OPEN,
                        com.macstab.chaos.filesystem.model.Errno.ENOSPC, ann.toxicity())));
    ...
    public List<String> describe(final IncidentChaosSystemInodeExhaustion ann) {
        return List.of(
                ...
                "filesystem: OPEN ENOSPC toxicity=" + ann.toxicity() + " (inode slots exhausted, df looks fine)",
