 ...
 *       {@link #probability}.
 *   <li>When the trial fires, the interposer returns {@code EBUSY} directly (pthread_create returns
 *       the error code, not -1; it does not set errno).
 *   <li>The calling code receives: return value {@code EBUSY} (16), {@code strerror(EBUSY)}:
 *       "Device or resource busy"; NPTL's internal thread stack cache is locked by a concurrent
 *       thread stack reclamation operation; no thread is created.
 * </ol>
 *
 ...
 *       -1; it does not set {@code errno}) and treats EBUSY as a transient condition warranting a
 *       brief retry — the stack cache lock contention self-resolves when the concurrent reclamation
 *       completes.
 *   <li>EBUSY from pthread_create is rare in production but can occur under extreme thread churn:
 *       many threads being created and destroyed concurrently cause NPTL's cached stack pool to be
 *       under concurrent access; assert that the application's thread-creation retry path handles
 *       EBUSY distinctly from EAGAIN — both warrant retry but EBUSY resolves faster (typically
 *       sub-millisecond) than EAGAIN (which requires thread stack reclamation).
 *   <li>Assert that the application does not spin-loop on EBUSY without a yield or sleep; tight
 *       spin loops on EBUSY worsen the contention they are waiting for.
 * </ul>
 *
 ...
 * be under contention; pthread_create returns EBUSY; the server treats EBUSY as a permanent failure
 * and rejects the request with a 500 error rather than retrying the thread creation after a brief
 * yield.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code EBUSY} from {@code pthread_create} is specific to the NPTL (Native POSIX Thread
 * Library) implementation in glibc. NPTL maintains a stack cache — a pool of recently-freed thread
 * stacks that can be reused to avoid repeated mmap/munmap cycles. When a thread exits, its stack is
 ...
 * -1)} or {@code if (errno == EBUSY)} after pthread_create silently misses EBUSY (16). Code that
 * tests {@code if (ret != 0)} is correct. EBUSY is not listed in the POSIX specification for
 * pthread_create; it is a glibc/NPTL extension error that can appear in production when thread
 * churn is high enough to cause cache lock contention.
 *
 * <p>The distinction between EBUSY and EAGAIN from pthread_create is operationally important: EBUSY
 * indicates a transient internal lock contention that resolves in microseconds to milliseconds;
 * EAGAIN indicates that the system lacks resources to create any new thread, which may persist for
 * the lifetime of threads currently running. Retry strategy should be a brief sleep (1–10 ms) for
 * EBUSY and exponential backoff for EAGAIN. Many applications handle only EAGAIN from
 * pthread_create and silently fail on EBUSY by treating it as an unknown error.
 *
