 ...
 *
 * <p>Injects connection-refused errors and IOException at the HTTP client entry point to
 * activate retry logic and amplify load on the failing upstream.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class HttpRetryAmplificationComposer implements L3Composer<IncidentChaosHttpRetryAmplification> {

    public HttpRetryAmplificationComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosHttpRetryAmplification ann) {
        ...
        final String scenarioId = JvmPlanAccumulator.instance().mintScenarioId("HttpRetryAmplification");
        ...
        final var scenario = ChaosScenario.builder(scenarioId)
                .description("HTTP retry amplification — inject IOException to trigger client retry storm")
                .selector(selector)
                .effect(ChaosEffect.injectException("java.io.IOException", "HTTP upstream failure triggering retries"))
    ...
    public List<String> describe(final IncidentChaosHttpRetryAmplification ann) {
        return List.of(
                ...
                "jvm: IOException(\"HTTP upstream failure triggering retries\") on class prefix '" + ann.classPattern() + "' (METHOD_ENTER)",
