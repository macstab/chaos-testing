 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code EXECVE}, errno = {@code ENOENT}, effect =
 ...
 * with {@code errno = ENOENT} until the rule is removed. This models scenarios where a binary is
 * deleted, a volume is unmounted, or a symlink target is removed mid-run after the helper binary
 * has already been successfully invoked a number of times.
 *
 ...
 *       intercepted call sets {@code errno = ENOENT} and returns {@code -1} without executing.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 2, {@code strerror}: "No such
 *       file or directory"; the counter remains tripped until removal.
 * </ol>
 *
 ...
 *       fail with {@code ENOENT} — the application must detect the binary-gone transition and fail
 *       gracefully rather than assuming the binary was never installed.
 ...
 *       {@code ENOENT} after previously succeeding, rather than logging a confusing "binary not
 *       found" error that suggests a deployment problem on a running system.
 *   <li>Assert that the application does not use the initial exec success as permanent proof that
 *       the binary exists — subsequent exec calls must not skip the return-value check based on a
 *       cached "binary is present" assumption established during startup.
 * </ul>
 *
 * Production failure mode: a shared volume providing a helper binary is unmounted during a rolling
 * deployment; existing containers that previously exec'd the binary from the volume successfully
 * now see {@code ENOENT}; the application continues accepting requests that require the helper but
 * silently fails to invoke it, producing incorrect output without surfacing the binary-gone
 * condition as an operational alert.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The FAIL_AFTER effect for {@code ENOENT} captures the binary-disappearance failure mode that
 * cannot be tested with the probabilistic ERRNO effect alone: if the binary is truly gone from the
 * path, every exec call will fail — not just a random fraction. The N-success-then-fail pattern
 * simulates the transition point where the binary becomes unavailable mid-run, with N representing
 * the number of successful invocations before the volume unmount or file deletion.
 *
 * <p>The distinction from the probabilistic variant is operationally significant: random ENOENT
 * (from the ERRNO effect) tests error handling for transient VFS lookup failures; permanent ENOENT
 * (from the FAIL_AFTER effect after threshold) tests graceful degradation when a dependency becomes
 * permanently unavailable. Applications must implement both: transient handling (retry or fallback
 * for random failures) and permanent degradation (disable the feature when the binary is
 * persistently absent).
 *
 * <h2>Example</h2>
 *
