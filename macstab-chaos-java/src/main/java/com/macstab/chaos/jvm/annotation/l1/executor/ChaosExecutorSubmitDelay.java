 ...
 * {@link java.util.concurrent.ExecutorService#submit ExecutorService.submit} and {@link
 * java.util.concurrent.ForkJoinPool#submit ForkJoinPool.submit} call, stretching the wall-clock
 * time of task hand-off without preventing the task from eventually being queued.
 *
 ...
 *       maxDelayMs]} when {@code maxDelayMs > delayMs}), parking the submitting thread.
 ...
 *   <li>Bounded queues that the caller fills before consuming may trigger {@code
 *       RejectedExecutionException} sooner than expected, because the delay stretches the window
 *       during which the queue is full.
 * </ul>
 *
 ...
 * scheduler cause queue buildup that triggers back-pressure or rejection policies earlier than load
 * tests predicted.
 *
 ...
 * java.util.concurrent.ExecutorService} (and concrete implementations such as {@code
 * ThreadPoolExecutor} and {@code ScheduledThreadPoolExecutor}) as well as {@code
 * java.util.concurrent.ForkJoinPool#submit(ForkJoinTask)}. Because these are JDK classes, the agent
 * retransforms them via the bootstrap instrumentation channel.
 *
 * <p><strong>Thread parking mechanics.</strong> {@code Thread.sleep} is used for the delay. The
 * submitting thread holds no executor-internal locks during the sleep (the lock is only acquired
 * inside the queue's {@code offer} or {@code put} call, which happens after the sleep). If the
 * submitting thread is a virtual thread (Java 21+), the sleep unmounts it from the carrier,
 * allowing other virtual threads to run during the delay.
 *
 * <p><strong>Interaction with rejection policies.</strong> If the executor uses a bounded queue and
 * the {@code RejectedExecutionHandler} is configured to block (e.g., a caller-runs policy), the
 * delay compounds with any existing back-pressure: the submitting thread sleeps during the chaos
 * delay, then potentially blocks again waiting for queue space.
 *
 * <p><strong>Cascading effects.</strong> In request-handling architectures where the request thread
 * submits tasks and then waits for their futures, the delay inflates end-to-end request latency by
 * at least {@link #delayMs} ms per submission. With multiple submissions per request, the total
 * latency impact multiplies.
 *
 * <p><strong>Distinguishing from siblings.</strong> {@link ChaosExecutorSubmitReject} throws {@code
 * RejectedExecutionException} instead of delaying. {@link ChaosExecutorSubmitGate} blocks the
 * submission thread until the test releases it explicitly, enabling precise timing control. {@link
 * ChaosExecutorWorkerRunDelay} targets the worker thread at execution time, not the submitter at
 * submission time.
 *
 * <h2>Example</h2>
 *
