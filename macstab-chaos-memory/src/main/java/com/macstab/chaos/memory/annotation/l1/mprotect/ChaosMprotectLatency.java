...
/**
 * Delays every libchaos-memory-intercepted {@code mprotect} call by {@link #delayMs}
 ...
 * (selector = {@code MPROTECT}, effect = LATENCY) pair. Unlike the errno variants, the
 * latency primitive always delegates to the kernel — it only adds wall-clock cost before doing so.
 *
 * <p><strong>What chaos this applies:</strong> every {@code mprotect} call intercepted by
 ...
 * starts. The shared library interposes the libc wrappers for the {@code MPROTECT} syscall
 ...
 * @ChaosMprotectLatency(delayMs = 50)
 ...
 * applies the rule to every capable container. Use the repeatable form ({@code @ChaosMprotectLatencys})
...
public @interface ChaosMprotectLatency {
  ...
  /**
   * Latency to inject before every matching {@code mprotect} call, in milliseconds.
