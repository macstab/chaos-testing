 ...
 * <h2>What this is</h2>
 *
 * <p>Combines a long shutdown-hook registration delay ({@link #hangMs()} ms) with rejection of new
 * shutdown-hook registrations, simulating a JVM whose graceful-shutdown subsystem has seized —
 ...
 *
 * <p>Applies a long delay on {@code SHUTDOWN_HOOK_REGISTER} operations via the JVM chaos agent. The
 * combined effect simulates a shutdown sequence that hangs indefinitely, causing Kubernetes or the
 * process supervisor to SIGKILL the container before hooks complete. In production, this happens
 * when a shutdown hook acquires a lock held by a deadlocked thread, or when it attempts a network
 * call to an already-unavailable service.
 *
 * <h2>How bad it is</h2>
 *
 ...
 * release) are never cleaned up. The process is force-killed, leaving in-flight requests without a
 * response, open file handles, and unreleased distributed locks. Data loss is possible if WAL or
 * transaction logs were not flushed.
 *
 * <h2>Industry references</h2>
 *
 * <p>Kubernetes §"Termination of Pods": the {@code terminationGracePeriodSeconds} (default 30 s) is
 * the window before SIGKILL. Spring Boot graceful shutdown documentation explicitly warns that
