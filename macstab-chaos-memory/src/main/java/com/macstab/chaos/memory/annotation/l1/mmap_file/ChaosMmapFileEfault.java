...
/**
 * Injects {@code EFAULT} on every libchaos-memory-intercepted {@code mmap (file-backed)} call inside
 * the target container, making the call fail as if the kernel returned {@code EFAULT}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP_FILE}, errno = {@code EFAULT})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code EFAULT} is a valid POSIX result of {@code mmap (file-backed)}; the invalid combinations
 ...
 * kernel-level failure. Specifically this simulates: bad address — typical of unhandled SIGSEGV-adjacent edge cases in JIT or native code.
 *
 ...
 * @ChaosMmapFileEfault(probability = 0.001)
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> very low rates (1e-5 to 1e-4); EFAULT at high rate produces unmaskable crashes.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every memory-chaos-capable container
 * in the test class. Use the repeatable form ({@code @ChaosMmapFileEfaults}) to bind different
