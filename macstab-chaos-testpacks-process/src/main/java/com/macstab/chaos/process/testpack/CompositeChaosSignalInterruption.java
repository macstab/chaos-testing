 ...
 * that rely on {@code SA_RESTART} to silently restart interrupted syscalls will surface latent bugs
 * in their EINTR-handling code paths.
 *
 * <h2>How it's created</h2>
 *
 * <p>Applies {@code ProcessRule.errno(ProcessSelector.WAITPID, ProcessErrno.EINTR, toxicity)} via
 * libchaos-process. In production {@code EINTR} from {@code waitpid} occurs when a signal
 ...
 * loops absorb these transparently. The scenario exercises the retry path without causing sustained
 * failures — the service degrades only if its EINTR-retry logic is absent or broken. A
 * correctly-implemented service shows no user-visible impact.
 *
 * <h2>Industry references</h2>
 *
 * <p>The requirement to restart {@code waitpid} on {@code EINTR} is documented in POSIX.1-2017
 * (System Interfaces, {@code waitpid}) and in W. Richard Stevens, <em>Advanced Programming in the
 * UNIX Environment</em> (3rd ed.), §10.5: "Interrupted System Calls". The pattern of languages and
 * runtimes silently swallowing EINTR rather than retrying is a well-known source of latent bugs in
