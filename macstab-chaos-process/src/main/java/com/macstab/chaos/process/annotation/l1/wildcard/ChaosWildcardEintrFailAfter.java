...
/**
 * Lets the first {@link #successesBeforeFailure} libchaos-intercepted {@code every interposed process syscall} calls
 * succeed, then injects {@code EINTR} on every subsequent call until the rule is removed.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one
 * (selector = {@code WILDCARD}, errno = {@code EINTR}, effect = FAIL_AFTER) tuple.
 ...
 * {@code every interposed process syscall} calls. After {@link #successesBeforeFailure} successes the counter trips and
 * every subsequent call returns {@code -1} with {@code errno = EINTR}, regardless of real
 ...
 * {@code every interposed process syscall} calls the application is expected to make before hitting the limit.
 ...
 * ({@code @ChaosWildcardEintrFailAfter.Repeatable}) to apply different counters to different containers.
 *
...
@ProcessFailAfterBinding(selector = ProcessSelector.WILDCARD, errno = ProcessErrno.EINTR)
public @interface ChaosWildcardEintrFailAfter {
   ...
   * <p>Example:
   * <pre>{@code
   * @ChaosWildcardEintrFailAfter(id = "primary",  successesBeforeFailure = 64)
   * @ChaosWildcardEintrFailAfter(id = "replica",  successesBeforeFailure = 128)
  ...
  @interface Repeatable {
    ChaosWildcardEintrFailAfter[] value();
  }
}
