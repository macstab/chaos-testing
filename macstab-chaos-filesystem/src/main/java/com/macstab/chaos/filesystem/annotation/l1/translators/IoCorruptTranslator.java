...
/**
 * Parameterised L1 translator for IO-corrupt L1 annotations.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class IoCorruptTranslator implements L1Translator<Annotation> {

  /** Public no-arg constructor required by the L1 translator contract. */
  public IoCorruptTranslator() {}
  ...
  static IoRule buildRule(final Annotation annotation) {
    final IoCorruptBinding binding =
        annotation.annotationType().getAnnotation(IoCorruptBinding.class);
    if (binding == null) {
      throw new IllegalStateException(
          "@IoCorruptBinding meta-annotation missing on " + annotation.annotationType().getName());
    }
    return IoRule.corrupt(PathPrefix.wildcard(), binding.operation(), readProbability(annotation));
  }
