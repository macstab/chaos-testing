...
/**
 * Immutable value object bundling a target container with its resolved execution context (platform,
 * shell, HTTP command builder), created once per operation entry point and passed down the call
 * chain.
 *
 ...
 * {@link com.macstab.chaos.toxiproxy.network.NetworkRedirectManager}, {@link
 * com.macstab.chaos.toxiproxy.lifecycle.ToxiproxyInstaller}) would independently cache {@code
 * (container, platform)} state — duplicating state management, introducing cache invalidation risk,
 * and creating thread-safety complexity.
 *
 * <p>{@code ContainerContext} eliminates this entirely: the orchestrator calls {@link
 * #of(GenericContainer)} once at the start of each operation, and passes the resolved context as a
 * plain argument down the entire call chain. All managers receive pre-detected platform data at
 * zero additional cost.
 *
 * <h2>Immutability Guarantee</h2>
 *
 * <p>All three fields are {@code private final}. The container reference itself is not copied — the
 * underlying container state can still change externally (container stopped, port changed) without
 * {@code ContainerContext} reflecting it. {@code ContainerContext} provides a consistent snapshot
 * of the <em>execution strategy</em> (which shell to use, which HTTP command builder) at the time
 * of creation. It should not be cached across operations; create a new instance per operation to
 * ensure the platform is re-evaluated if the container is restarted.
 *
 * <h2>Thread Safety</h2>
 *
 ...
 * multiple threads during {@code execInContainer} calls; Docker's Java client (docker-java) handles
 * concurrent exec calls safely via its connection pool.
 *
 ...
 *
 * <ul>
 *   <li>{@link #of(GenericContainer)} — production path; performs real platform detection via a
 *       Docker API call.
 *   <li>{@link #of(GenericContainer, Platform, Shell)} — test path; bypasses platform detection by
 *       accepting pre-resolved components. Use with mock {@code Platform} and {@code Shell} in unit
 *       tests to avoid Docker dependency and Docker API latency.
 * </ul>
 *
...
public final class ContainerContext {
   ...
   *
   * <p><strong>Cost:</strong> One Docker API call ({@code execInContainer("cat",
   * "/etc/os-release")}) to detect the Linux distribution. Expected latency: 5–100 ms on native
   * Linux Docker, 50–300 ms on Docker Desktop. Call this once per operation; pass the resulting
   * context to all collaborators.
   *
   ...
   * back to package manager detection ({@code which apt-get}, {@code which apk}, {@code which
   * dnf}). Supported distributions: Debian, Ubuntu, Alpine, RHEL/CentOS/Rocky/AlmaLinux.
   *
   ...
   * @throws com.macstab.chaos.core.platform.UnsupportedPlatformException if the container's Linux
   ...
   *
   * <p><strong>Primary use: unit testing.</strong> Inject a mock {@link Platform} and {@link Shell}
   * to test components that consume {@code ContainerContext} without requiring a running Docker
   * container. This avoids the {@code mockStatic(PlatformDetector.class)} approach, which is
   * fragile and adds test framework overhead.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param container the target container (need not be running for unit test purposes, but any
   *     {@code execInContainer} calls will fail if it is not)
  ...
  /**
   * Returns the detected platform, providing access to platform-specific command builders and
   ...
   *
   * <p>Used to execute multi-word commands that cannot be passed as a single string to {@code
   * execInContainer}. The shell is responsible for parsing the command string and expanding
   * arguments.
   *
   ...
   *
   * <p>Convenience shortcut for {@code platform().getHttpCommandBuilder()}. The builder generates
   * {@code curl} or {@code wget} commands appropriate for the container's package manager
   * ecosystem. Alpine images may not have {@code curl} pre-installed and may fall back to {@code
   * wget}; the builder abstracts this difference.
   *
