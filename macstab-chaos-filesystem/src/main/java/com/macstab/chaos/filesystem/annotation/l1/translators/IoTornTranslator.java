...
/**
 * Parameterised L1 translator for IO-torn L1 annotations.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class IoTornTranslator implements L1Translator<Annotation> {

  /** Public no-arg constructor required by the L1 translator contract. */
  public IoTornTranslator() {}
  ...
  static IoRule buildRule(final Annotation annotation) {
    final IoTornBinding binding = annotation.annotationType().getAnnotation(IoTornBinding.class);
    if (binding == null) {
      throw new IllegalStateException(
          "@IoTornBinding meta-annotation missing on " + annotation.annotationType().getName());
    }
    return IoRule.torn(PathPrefix.wildcard(), binding.operation(), readProbability(annotation));
  }
