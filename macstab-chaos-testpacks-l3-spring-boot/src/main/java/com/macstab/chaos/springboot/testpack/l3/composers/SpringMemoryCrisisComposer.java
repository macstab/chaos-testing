 ...
 *
 * <p>Fails both anonymous and file-backed mmap allocations at the configured rate and injects
 * an OutOfMemoryError via the JVM chaos layer to reproduce the compound failure profile of a
 * Spring Boot service exhausting its heap just before an OOM kill.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SpringMemoryCrisisComposer implements L3Composer<IncidentChaosSpringMemoryCrisis> {

    public SpringMemoryCrisisComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosSpringMemoryCrisis ann) {
        ...
        final var scenario = ChaosScenario.builder(scenarioId)
                ...
                .effect(ChaosEffect.injectException(
                        ...
                        "heap exhaustion — OOM kill imminent"))
