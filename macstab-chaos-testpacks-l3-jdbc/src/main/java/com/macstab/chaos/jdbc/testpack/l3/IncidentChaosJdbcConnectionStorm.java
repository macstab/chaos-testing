...
/**
 * <h2>What this is</h2>
 * <p>Simulates a JDBC connection pool exhaustion storm: a load spike causes every connection-acquire
 * attempt to be refused at the network level while the JVM layer simultaneously reports pool
 * exhaustion, driving request queues to saturation.
 *
 * <h2>Composed of</h2>
 * <ul>
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Critical</strong><br>Connection-acquire blocks, pool queue fills, and every
 * inbound request stacks up waiting for a connection that will never arrive; the service degrades
 * to complete unavailability within seconds.
 *
 * <h2>Industry references</h2>
 * <p>JDBC pool exhaustion under load spikes is a well-documented failure mode: described in HikariCP
 * documentation §"Pool sizing", the Percona blog "Diagnosing Connection Pool Exhaustion", and
 * numerous post-mortems from high-traffic e-commerce deployments.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.jdbc.testpack.l3.composers.JdbcConnectionStormComposer", severity = Severity.CRITICAL)
