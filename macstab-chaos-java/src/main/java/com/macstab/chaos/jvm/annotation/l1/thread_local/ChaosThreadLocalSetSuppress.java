 ...
 * transaction bindings, Hibernate session holders, security contexts, and SLF4J MDC absent from the
 * thread even after the framework attempts to install them.
 *
 ...
 *       calls {@code ThreadLocal.set()} to bind a datasource connection to the current thread; with
 *       suppress active, the binding does not happen; the next call to {@code getResource(key)}
 *       returns {@code null}; Spring creates a new connection for each repository call instead of
 *       reusing the transaction's connection; assert that multiple repository calls within a
 *       {@code @Transactional} method do not execute in separate database transactions.
 *   <li>Spring Security's {@code SecurityContextHolder.setContext(SecurityContext)} stores the
 *       security context via a {@code ThreadLocal}; a suppressed set means the security context is
 *       lost immediately after being set; downstream calls to {@code getAuthentication()} return
 *       {@code null}; assert that the application correctly returns HTTP 401 or 403 rather than
 *       NullPointerException.
 *   <li>SLF4J's {@code MDC.put(key, value)} stores keys in a {@code ThreadLocal<Map>}; a suppressed
 *       set means MDC keys are never stored; log events lack correlation IDs; assert that the
 *       monitoring system flags the missing trace context.
 *   <li><strong>Production failure mode:</strong> a reactive framework's context propagation
 *       mechanism uses a library that bridges reactive context to {@code ThreadLocal} via {@code
 *       ThreadLocal.set()} at the start of each operator; if the bridge is incorrectly configured
 *       (e.g. wrong {@code Context} key), the set calls complete normally but the wrong value is
 *       stored — a different failure than suppression, but with the same downstream observable
 *       effect: thread-local-dependent libraries see wrong or absent context.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The interception targets {@code java.lang.ThreadLocal#set(T)} and performs a no-op instead of
 * the real set. The JVM's {@code ThreadLocal.set()} calls {@code
 * Thread.currentThread().threadLocals} (a {@code ThreadLocal.ThreadLocalMap}) and either updates
 * the existing entry for the given {@code ThreadLocal} instance or creates a new entry. With
 * suppress active, neither update nor creation happens; the map state after the intercept is
 * identical to the state before the intercepted set call.
 *
 ...
 *
 * <p>This annotation and {@link ChaosThreadLocalGetSuppress} are complementary: set-suppress tests
 * whether the application correctly handles the case where context propagation fails at the moment
 * of propagation; get-suppress tests whether it handles the case where the context disappears after
 * it was propagated. The former simulates a propagation bug; the latter simulates a context loss
 * bug.
 *
 * <h2>Example</h2>
 *
