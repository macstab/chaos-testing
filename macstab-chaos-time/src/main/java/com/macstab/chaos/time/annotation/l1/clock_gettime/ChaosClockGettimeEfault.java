 ...
 * EFAULT}) tuple. The tuple is safe by construction — {@code EFAULT} is a documented POSIX result
 * of {@code clock_gettime(2)} when the {@code tp} pointer passed by the caller points outside the
 * accessible address space. No runtime selector-errno validation is needed.
 *
 ...
 *
 * <ul>
 *   <li>{@code clock_gettime} returns {@code -1}; the {@code timespec} struct is not written, so
 *       any subsequent read of its fields produces undefined results.
 ...
 *   <li>Java's {@code System.nanoTime()} may return a stale cached value or throw from native code
 *       depending on how the JVM handles the failed JNI call.
 ...
 * corrupted processes, stack-overflow scenarios where the {@code timespec} buffer is on a guard
 * page, or containers with unusual memory layout from custom allocators.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX (IEEE Std 1003.1-2017, {@code clock_gettime}) mandates {@code EFAULT} when the {@code
 * tp} argument points to an inaccessible memory location. In the kernel this is detected by the
 ...
 *
 * <p>This errno is adjacent to memory-safety bugs: a real {@code EFAULT} during {@code
 * clock_gettime} almost always indicates a corrupted stack or heap. Using it as a chaos target
 * reveals whether error-handling paths perform safe cleanup (not reading the struct) rather than
 * cascading into further undefined behavior. Frameworks such as Netty's {@code HashedWheelTimer}
 * and {@code java.util.concurrent.ScheduledThreadPoolExecutor} read the monotonic clock frequently;
 ...
 * ChaosClockGettimeEperm} targets capability failures; {@link ChaosClockGettimeOffset} skews the
 * returned timestamp instead of failing the call outright.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
