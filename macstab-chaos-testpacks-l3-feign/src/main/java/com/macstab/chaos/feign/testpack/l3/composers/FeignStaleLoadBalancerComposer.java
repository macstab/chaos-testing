 ...
 *
 * <p>Injects ECONNREFUSED on CONNECT at the configured toxicity to simulate dead pod IPs served by
 * the Spring Cloud LoadBalancer stale cache during a rolling deploy.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class FeignStaleLoadBalancerComposer
    implements L3Composer<IncidentChaosFeignStaleLoadBalancer> {

  public FeignStaleLoadBalancerComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosFeignStaleLoadBalancer ann) {
  ...
  public List<String> describe(final IncidentChaosFeignStaleLoadBalancer ann) {
    return List.of(
        ...
        "connection: CONNECT ECONNREFUSED toxicity="
            + ann.toxicity()
            + " (dead pod IPs from stale LB cache)",
