 ...
 * JVM chaos layer to reproduce the application-level symptom of heap exhaustion preceding an
 * OOM kill.
 *
 * <h2>Composed of</h2>
 * <ul>
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Critical</strong><br>Sustained memory leak leads to OOM kill; pod restarts;
 * in-flight requests are lost; if the circuit breaker (Resilience4j) does not open in time,
 * upstream callers accumulate failed requests and may cascade to their own thread pool exhaustion.
 *
 * <h2>Industry references</h2>
 * <p>Sustained memory leak → OOM kill → pod restart with in-flight request loss → circuit breaker
 * opens is a documented failure mode for long-running Spring Boot services without heap size
 * caps or proper native memory accounting.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.springboot.testpack.l3.composers.SpringMemoryCrisisComposer", severity = Severity.CRITICAL)
