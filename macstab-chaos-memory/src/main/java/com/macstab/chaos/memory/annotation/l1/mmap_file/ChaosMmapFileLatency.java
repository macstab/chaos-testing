 ...
 * (selector = {@code MMAP_FILE}, effect = LATENCY) pair. Unlike the errno variants, the
 ...
 * starts. The shared library interposes the libc wrappers for the {@code MMAP_FILE} syscall
 ...
 * applies the rule to every capable container. Use the repeatable form ({@code @ChaosMmapFileLatencys})
...
public @interface ChaosMmapFileLatency {
  ...
  /**
   * Latency to inject before every matching {@code mmap (file-backed)} call, in milliseconds.
