 ...
 *
 * <ul>
 *   <li><strong>Incoming RMI calls slow.</strong> Java RMI deserialises arguments on receipt; slow
 *       deserialisation blocks the RMI dispatcher thread, reducing the effective throughput of the
 *       server; assert that the client's call-timeout is reached gracefully.
 ...
 *   <li><strong>Production failure mode:</strong> complex object graphs (many fields, deep nesting,
 *       cyclical references) require significant CPU and heap during deserialisation; a delay
 *       annotation amplifies this cost and can cause GC pressure or heap exhaustion on busy nodes
 *       that process many inbound serialised messages concurrently.
 * </ul>
 *
 ...
 * resolves class names via the context class loader, allocates objects, and invokes {@code
 * readObject()} callbacks recursively for each sub-object. The agent intercepts at the top-level
 * {@code readObject()} call; nested calls during graph traversal are also intercepted, accumulating
 * delays proportional to the depth of the object graph.
 *
 ...
 *
 * <p>Deserialisation also poses a security risk: the JVM executes {@code readResolve()} and {@code
 * readObject()} methods on arbitrary objects read from the stream. The delay fires before any of
 * those callbacks run, so the total time from interception to method return includes both the delay
 * and any custom deserialisation logic. Tests should account for this.
 *
