...
/**
 * Injects {@code ENOMEM} into {@code socket(2)}, causing the call to return {@code -1} with
 * {@code errno = ENOMEM} as if the kernel's memory allocator could not allocate the internal
 * socket data structures ({@code sock}, {@code inet_sock}, protocol-specific buffers) needed to
 * create the socket.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (operation = {@code SOCKET}, errno = {@code ENOMEM})
 ...
 *
 * <ul>
 *   <li>{@code ENOMEM} from {@code socket} indicates kernel memory exhaustion; unlike
 *       {@code EMFILE} (file descriptor table full) or {@code ENFILE} (system-wide fd limit),
 *       {@code ENOMEM} means the kernel's slab allocator could not satisfy a memory allocation
 *       request for the socket's internal structures. Assert that the application treats this as
 *       a transient resource shortage and backs off before retrying.
 ...
 *   <li>Assert that the application does not conflate kernel-side {@code ENOMEM} with
 *       {@code OutOfMemoryError} from the JVM heap — the JVM heap may be healthy while the kernel's
 *       slab allocator is exhausted, and the remediation paths (GC tuning vs. kernel memory tuning)
 *       are different.
 ...
 * allocates a {@code sock} structure using {@code sk_alloc()}, which calls {@code kmem_cache_alloc()}
 * on the protocol-specific slab cache. If the slab allocator returns NULL (due to memory pressure
 * or a cgroup memory limit), {@code sk_alloc} returns NULL and {@code inet_create} propagates
 * {@code ENOMEM} to the caller.
 *
 ...
 * {@code ENOMEM} from {@code send}/{@code recv}, since socket creation buffer allocations are
 * small and typically satisfied before per-packet buffer allocations fail.
 *
 ...
 * calls; application code should log the operation context (socket creation vs. send/recv) to
 * help distinguish which operation failed. The JVM does not translate kernel {@code ENOMEM} into
 * a JVM {@code OutOfMemoryError} — these are entirely separate error domains.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
