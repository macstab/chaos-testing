...
@Slf4j
public final class GrpcDeadlinePropagationComposer implements L3Composer<IncidentChaosGrpcDeadlinePropagation> {
    ...
    public List<Object> apply(
        ...
        final com.macstab.chaos.time.api.RuleHandle timeHandle = CompositeTimeChaos.standard().advanced()
                .apply(container, TimeRule.offset(
                        TimeClock.MONOTONIC,
                        Duration.ofMillis(-annotation.skewMs()),
                        1.0));
        ...
        final ChaosScenario scenario = ChaosScenario.builder(scenarioId)
                .description("L3 gRPC deadline propagation — RECV latency + monotonic skew + DEADLINE_EXCEEDED")
                ...
                .effect(ChaosEffect.injectException(
                        "io.grpc.StatusRuntimeException", "DEADLINE_EXCEEDED: deadline propagation failure"))
        ...
        for (final Object h : handles) {
            if (h instanceof RuleHandle netRh) {
                try {
                    ...
                    log.warn("GrpcDeadlinePropagationComposer.removeAll: failed to remove connection rule", e);
                }
                ...
                try {
                    ...
                    log.warn("GrpcDeadlinePropagationComposer.removeAll: failed to remove JVM scenario {}", scenarioId, e);
                }
            }
        }
    }
