 ...
 * reproduce the compound failure profile of {@code @Transactional(REQUIRES_NEW)} pool exhaustion —
 * all threads freeze, no DB deadlock is visible. (Spring #26250)
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SpringTransactionalPoolDeadlockComposer
    implements L3Composer<IncidentChaosSpringTransactionalPoolDeadlock> {

  public SpringTransactionalPoolDeadlockComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosSpringTransactionalPoolDeadlock ann) {
    ...
    final String id =
        JvmPlanAccumulator.instance().mintScenarioId("SpringTransactionalPoolDeadlock-exc");
    final var sc =
        ChaosScenario.builder(id)
            .description(
                "Spring @Transactional pool deadlock — REQUIRES_NEW exhausts connection pool")
            .selector(
