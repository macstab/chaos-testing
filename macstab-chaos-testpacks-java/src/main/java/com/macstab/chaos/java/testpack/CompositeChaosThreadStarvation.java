 ...
 *
 * <p>Applies a {@code THREAD_LEAK} stressor scenario via the JVM chaos agent. The agent creates
 * the requested number of daemon threads that park indefinitely. In production, thread leaks arise
 * from thread pools that are never shut down (e.g. a request-scoped executor created but not
 * closed), or from frameworks that spawn per-connection threads without a lifecycle bound.
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Severe</strong><br>
 * Once the OS thread limit is approached, new thread-creation calls throw
 * {@code OutOfMemoryError: unable to create new native thread}. The application's thread pools
 * stop accepting work, health checks time out, and operator intervention (restart) is required.
 *
 * <h2>Industry references</h2>
 *
 * <p>OS-level thread limits ({@code /proc/sys/kernel/threads-max} on Linux) and JVM thread
 * creation failure are documented in the HotSpot internals. Netflix Engineering blog "Thread
 * Starvation in Production" describes real incidents caused by leaked threads in HTTP connection
 * handling.
 *
 * <h2>Example</h2>
 *
