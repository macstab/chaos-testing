 ...
 * succeed, then injects {@code ENOMEM} on every subsequent call until the rule is removed.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one
 * (selector = {@code WILDCARD}, errno = {@code ENOMEM}, effect = FAIL_AFTER) tuple.
 ...
 * every subsequent call returns {@code -1} with {@code errno = ENOMEM}, regardless of real
 ...
 * @ChaosWildcardEnomemFailAfter(successesBeforeFailure = 128)
 ...
 * ({@code @ChaosWildcardEnomemFailAfter.Repeatable}) to apply different counters to different containers.
 *
