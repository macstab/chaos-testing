 ...
 * NPTL stack-cache lock stall scenario where a preempted thread holds the lock indefinitely after N
 * successful thread creations, causing all subsequent process-management operations to report
 * "Device or resource busy".
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-process primitive — one (selector = {@code WILDCARD}, errno = {@code EBUSY}, effect =
 ...
 *       preempted lock holder, not a momentary contention spike.
 *   <li>FAIL_AFTER models the sustained stall scenario: N process-management calls succeed; a
 *       thread holding the NPTL stack-cache lock is preempted (e.g., by a CPU scheduler change or
 *       priority inversion); all subsequent calls return EBUSY — assert that the application bounds
 *       its EBUSY retry count and escalates after the retry budget is exhausted.
 ...
 *       escalate rather than continue backing off and retrying indefinitely.
 * </ul>
 *
 * Production failure mode: a high-throughput thread pool hits the NPTL stack-cache lock under heavy
 * concurrency; a low-priority thread holding the lock is preempted by the OS scheduler while
 * holding the lock; all concurrent pthread_create calls return EBUSY; the pool's retry logic
 * applies exponential back-off without a maximum retry count; the pool stops expanding and starts
 * dropping requests while consuming scheduler cycles in the back-off loop.
 *
 ...
 * subsequent pthread_create calls to return EBUSY until the preempted thread is rescheduled. This
 * distinguishes the "brief yield and retry" scenario (transient) from the "alert operators"
 * scenario (sustained).
 *
 ...
 * This tests whether the application's cross-family error handling escalates correctly when EBUSY
 * affects the entire process lifecycle, not just thread creation.
 *
