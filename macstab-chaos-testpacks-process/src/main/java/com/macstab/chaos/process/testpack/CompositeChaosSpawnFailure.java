 ...
 *
 * <p>Applies {@code ProcessRule.errno(ProcessSelector.POSIX_SPAWN, ProcessErrno.ENOMEM,
 * toxicity)} via libchaos-process. In production this happens during memory-pressure events on
 * nodes running many containers, or when a container's memory cgroup limit is nearly exhausted
 * and the kernel cannot satisfy the allocation requests required for a new task_struct and its
 * associated kernel stack.
 *
 * <h2>How bad it is</h2>
 *
 ...
 * degrade — some succeed, some fail — creating inconsistent behaviour that is harder to detect
 * than a total failure. Applications without per-feature circuit breakers may accumulate spawned
 * but failed tasks silently, leading to resource leaks.
 *
...
public @interface CompositeChaosSpawnFailure {
  ...
  /**
   * Probability in {@code (0.0, 1.0]} that {@code ENOMEM} fires on each {@code posix_spawn()}
   * call. Defaults to {@code 0.5} (half of spawn attempts fail).
   */
