 ...
 *   <li>Filesystem: FSYNC → EIO at probability {@code probability} on path {@code path} —
 *       segment cleanup and index fsync fail; unclean segment closure triggers recovery on restart
 *   <li>Connection: SEND latency of {@code latencyMs/2} ms — produce requests are delayed from
 *       the network layer, compounding the storage-side backpressure seen by producers
 * </ul>
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Broker disk fills cause log segment cleanup lag,
 * which increases retention beyond configured limits; producers see backpressure; consumers
 * rebalance as produce blocks propagate through the pipeline.
 *
 * <h2>Industry references</h2>
 * <p>Broker disk fills → log segment cleanup lag → producer backpressure → consumer rebalance
 * is a documented Kafka operational hazard. Multiple post-mortems cite unmonitored disk growth
 * from high-retention topics as the root cause.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.kafka.testpack.l3.composers.KafkaStoragePressureComposer", severity = Severity.SEVERE)
