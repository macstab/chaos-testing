 ...
 * allocatable memory for the child — a more severe condition that may persist until the OOM
 * killer has freed memory.
 *
 ...
 * Applications that treat fork-ENOMEM as permanent (vs. transient) will stop spawning workers
 * even after memory pressure subsides. Unlike fork-EAGAIN, ENOMEM is not always transient —
 * the OOM killer may still be active — so the correct recovery is to wait and retry with
 * exponential backoff rather than immediately retrying. Applications without this distinction
 * in their error-handling path will behave identically for both errnos and may recover too
 * aggressively under ENOMEM.
 *
 * <h2>Industry references</h2>
 *
 * <p>The difference between {@code EAGAIN} and {@code ENOMEM} from {@code fork} is documented
 * in the Linux {@code fork(2)} man-page. Google's OOM-kill post-mortems (SRE book, Chapter 17)
 * emphasise the importance of distinguishing transient from permanent resource failures in
 * error-handling code; fork-ENOMEM is the canonical example where the distinction changes
 * recovery behaviour.
 *
 * <h2>Example</h2>
 *
