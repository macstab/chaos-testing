...
/**
 * Injects {@code EINVAL} on every libchaos-memory-intercepted {@code mmap(MAP_ANONYMOUS)} call inside
 * the target container, making the call fail as if the kernel returned {@code EINVAL}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP_ANON}, errno = {@code EINVAL})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code EINVAL} is a valid POSIX result of {@code mmap(MAP_ANONYMOUS)}; the invalid combinations
 ...
 * in the test class. Use the repeatable form ({@code @ChaosMmapAnonEinvals}) to bind different
