 ...
 *       {@code strerror}: "Out of memory"; no child process is created; the calling process is
 *       in a clean state since no child resources were allocated before the interposer fired.</li>
 * </ol>
 *
 ...
 *       longer backoff than the transient EAGAIN case.</li>
 *   <li>Assert that the application does not call {@code waitpid} on the uninitialised pid output
 *       parameter after an ENOMEM failure — POSIX does not define the pid value when spawn fails.</li>
 *   <li>Assert that the application distinguishes {@code posix_spawn}-ENOMEM (kernel memory
 *       structures unavailable, requires node-level memory metric check) from EAGAIN (process
 *       table or uid quota exhausted, self-healing when children exit) — the two require different
 *       retry strategies and different operator actions.</li>
 * </ul>
 ...
 * glibc spawn helper cannot allocate its internal communication pipe or argument-copy buffer.
 * The glibc implementation of {@code posix_spawn} on Linux uses a helper thread or helper process
 * to perform the file-actions and exec; if kernel memory is insufficient to create this helper,
 * ENOMEM is returned before any fork occurs.
 *
 ...
 * ENOMEM as the return value, consistent with the POSIX specification. Applications that mix
 * {@code fork}/{@code exec} error-handling patterns with spawn error-handling will fail silently
 * when ENOMEM is returned.
 *
 * <p>Unlike ENOMEM from {@code execve} or {@code fork}, ENOMEM from {@code posix_spawn} leaves
 * the calling process in a completely clean state: no child was created, no fds were allocated for
 * the spawn communication, and no argument structures were partially initialised in the child. The
 * only cleanup required is to release any application-level resources allocated in preparation for
 * the spawn (argument arrays, attribute structures), which should be destroyed with
 * {@code posix_spawnattr_destroy} and {@code posix_spawn_file_actions_destroy} regardless of
 ...
 * should implement a longer initial backoff for ENOMEM (e.g. 5–30 seconds) with a cap on the
 * number of retries, rather than treating it as equivalent to EAGAIN.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
