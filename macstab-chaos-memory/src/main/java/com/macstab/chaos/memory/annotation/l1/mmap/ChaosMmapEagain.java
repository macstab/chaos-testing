...
/**
 * Injects {@code EAGAIN} on every libchaos-memory-intercepted {@code mmap} call inside the target
 * container, making the call fail as if the kernel returned {@code EAGAIN}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MMAP}, errno = {@code EAGAIN}) pair. The
 * combination is safe by construction: this annotation class exists only because {@code EAGAIN} is
 ...
 * the interceptor returns {@code -1} and sets {@code errno = EAGAIN} before the kernel call
 * completes — from the application perspective this is indistinguishable from a real kernel-level
 * failure. Specifically this simulates: temporary failure (would block) — surfaces in kernels under
 * memory pressure or rlimit exhaustion.
 *
 ...
 * the test class. Use the repeatable form ({@code @ChaosMmapEagains}) to bind different
