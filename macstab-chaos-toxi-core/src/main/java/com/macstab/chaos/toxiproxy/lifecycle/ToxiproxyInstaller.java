...
/**
 * Installs the Toxiproxy binary and its system-level dependencies into a container on demand, with
 * idempotency and architecture-aware binary selection.
 *
 ...
 * Dockerfile patches for every service image used in testing — an unacceptable maintenance burden.
 * Instead, this class installs Toxiproxy at runtime, the first time it is needed, directly into a
 * running container. The installation is idempotent: subsequent calls are no-ops after a {@code
 * which toxiproxy-server} check confirms the binary is present.
 *
 ...
 *       <ul>
 *         <li>{@code ca-certificates} — required for TLS verification when downloading from GitHub
 *             over HTTPS. Installed with {@code verify=false} because the ca-certificates package
 *             provides no binary.
 *         <li>{@code curl} (or platform-appropriate HTTP client) — used to download the binary.
 *         <li>{@code iptables} — required by {@link
 *             com.macstab.chaos.toxiproxy.network.NetworkRedirectManager} for port redirection.
 *             Installed here as part of the Toxiproxy setup, since the installer runs before any
 *             proxy is created.
 *       </ul>
 ...
 *   <li><strong>Binary download</strong>: downloads the architecture-specific {@code
 *       toxiproxy-server} binary from GitHub Releases using the platform HTTP command builder
 *       ({@code curl -sL ...}). Writes to {@code /usr/local/bin/toxiproxy-server}.
 *   <li><strong>chmod</strong>: makes the binary executable ({@code chmod +x}).
 * </ol>
 *
 * <h2>Network Dependency</h2>
 *
 * <p>Step 4 requires the container to have outbound internet access to {@code
 * https://github.com/Shopify/toxiproxy/releases/}. In air-gapped or network-restricted
 * environments, this will fail. Workaround: pre-install the binary in the container image, or mount
 * it as a volume. Once installed, the binary persists for the container's lifetime.
 *
 * <h2>Fixed Version</h2>
 *
 ...
 *
 * <p>This class uses {@link ContainerContext#http()} for HTTP commands and {@link
 * ContainerContext#platform()} for package name resolution. Hardcoded tool names ({@code curl},
 * {@code apt-get}) would break on Alpine (uses {@code wget}/{@code apk}). The platform abstraction
 * ensures the same installer works across Debian, Ubuntu, Alpine, and RHEL-family images without
 * conditional logic in this class.
 *
 * <h2>Internal API</h2>
 *
 * <p>This class is an internal implementation detail consumed exclusively by {@link
 * ToxiproxyLifecycleManager}. It is not a stable public API. Callers outside the {@code lifecycle}
 * package should not reference this class directly.
 *
...
public final class ToxiproxyInstaller {
   ...
   *
   * <p><strong>Idempotency:</strong> The first operation is {@code which toxiproxy-server}. If exit
   * code 0, returns immediately. This check costs one Docker API round trip (~5–100 ms) and is the
   * common path after the first invocation per container.
   *
   * <p><strong>Side effects on success:</strong> The following are installed in the container: the
   * {@code ca-certificates} package, the platform-specific {@code curl} package, the
   * platform-specific {@code iptables} package, and the {@code toxiproxy-server} binary at {@code
   * /usr/local/bin/toxiproxy-server} (mode 755).
   *
   * <p><strong>Failure behavior:</strong> Any step failure throws {@link
   * ChaosOperationFailedException} (unchecked). This is consistent with the caller ({@link
   * ToxiproxyLifecycleManager#ensureRunning}), which wraps this in a {@code try/catch} and
   * re-throws as {@link java.io.IOException}.
   *
   ...
   * Testcontainers containers are ephemeral by design, so this is acceptable: each test run starts
   * fresh containers and the installation happens at most once per container per run.
   *
   * @param ctx resolved container context; the container must be running and have outbound internet
   *     access for the first-time installation path
   * @throws NullPointerException if ctx is null
   * @throws ChaosOperationFailedException if package installation, binary download, or chmod fails;
  ...
  private void installDependencies(final ContainerContext ctx) {
    PackageInstaller.ensureInstalled(
        ctx.container(), Tool.CA_CERTIFICATES, Tool.CURL, Tool.IPTABLES);
  }
