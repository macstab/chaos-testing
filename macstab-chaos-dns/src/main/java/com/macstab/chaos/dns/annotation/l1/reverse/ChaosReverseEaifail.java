...
/**
 * Injects {@code EAI_FAIL} into every {@code getnameinfo(3)} call (reverse DNS lookup), causing
 * the call to return {@code EAI_FAIL} as if the reverse DNS authoritative server returned a
 * permanent failure response for the queried IP address.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (selectorKind = {@code REVERSE}, errno =
 * {@code EAI_FAIL}) tuple. This annotation always fires on every intercepted reverse lookup —
 * there is no per-call probability field. Use it when you need every reverse resolution attempt to
 * fail permanently so that the application's hard-failure handling for PTR records is exercised.
 ...
 *
 * <ul>
 *   <li>Every reverse lookup attempt fails permanently; the application must treat the absence of
 *       a PTR record as a normal operating condition and fall back to using the raw IP address
 *       for logging, audit trails, and access control decisions.
 ...
 *       receive empty or null hostname fields; assert that the enrichment logic handles this without
 *       throwing or recording malformed spans.
 * </ul>
 *
 * <p>In production, {@code EAI_FAIL} from {@code getnameinfo} occurs when no PTR record exists
 * for the IP address (common for cloud ephemeral IPs), when the reverse DNS zone is misconfigured
 * or delegated to a non-responsive nameserver, or when the authoritative server for the reverse
 * zone returns {@code SERVFAIL}.
 *
 ...
 * instances do not have PTR records; calls to {@code getnameinfo} for their IPs return
 * {@code EAI_NONAME} or {@code EAI_FAIL}. This is a normal operating condition that application
 * code must handle, but it is rarely exercised in development environments where PTR records exist
 ...
 * returns the IP address string when the reverse lookup fails, without surfacing the
 * {@code EAI_FAIL} error to the caller. Application code that uses {@code getHostName()} will
 * therefore never observe this injection; only code that calls native resolver APIs directly or
 * via JNI will see the returned error code.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
