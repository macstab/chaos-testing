 ...
 * The stressor creates {@link #threadCount()} platform threads, all of which loop calling {@code
 * LockSupport.parkNanos()} to yield CPU while staying alive, and holds them for the duration of the
 * rule.
 *
 ...
 *       {@link #namePrefix()} prefix followed by a sequential index, sets their {@link #daemon()}
 *       flag, and starts them.
 *   <li>Each thread enters an infinite loop that parks itself for a short interval ({@code
 *       LockSupport.parkNanos()}), consuming negligible CPU but maintaining the OS kernel thread as
 *       alive. The thread never returns from its {@code run()} method unless interrupted.
 ...
 *       Linux) plus a JVM-side {@code JavaThread} struct.
 ...
 *       entries from its thread table.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li><strong>Thread count metric spike.</strong> Monitoring dashboards that track {@code
 *       java.lang:type=Threading/ThreadCount} will show a sudden increase of {@link #threadCount()}
 *       threads; assert that the alerting threshold is set appropriately and that the alert fires
 *       within an acceptable detection window.
 *   <li><strong>Native stack reservation exhaustion.</strong> Each leaked thread consumes {@code
 *       -Xss} of virtual address space; with many threads, the process may exhaust addressable
 *       stack space or the OS's thread limit ({@code ulimit -u} on Linux); assert that the JVM
 *       throws {@code OutOfMemoryError: Unable to create new native thread} rather than crashing
 *       silently.
 ...
 *   <li><strong>Thread-dump pollution.</strong> Operational tools that capture thread dumps ({@code
 *       jstack}, async-profiler) will include all leaked threads; assert that your
 *       incident-response runbook can filter out chaos-injected threads (identifiable by the {@link
 *       #namePrefix()}).
 *   <li><strong>Production failure mode:</strong> a framework that creates a new thread per request
 *       (or per scheduled task) without bounds will gradually exhaust the OS thread limit; the
 *       first symptom is an {@code OutOfMemoryError: Unable to create new native thread} on the
 *       request path, causing the server to stop accepting new connections.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>Platform threads in HotSpot are 1:1 mapped to OS kernel threads (pthreads on Linux, Win32
 * threads on Windows). Creating a thread allocates: a native stack (mmap'd, lazy-committed on Linux
 * by default), a JVM-internal {@code JavaThread} object on the C heap, and an entry in the JVM's
 * thread list. The thread is registered with the OS scheduler and receives a thread ID.
 *
 * <p>The OS imposes limits on the number of threads per process ({@code
 * /proc/sys/kernel/threads-max} and the process's {@code RLIMIT_NPROC}) and per system (the same
 * kernel parameter divided by running processes). In containerised environments these limits are
 * often set lower than on bare metal; the stressor exercises what happens when the application
 * approaches those limits.
 *
 ...
 * effective regardless of whether the application uses virtual threads. To stress the
 * virtual-thread scheduler specifically, use {@link ChaosVirtualThreadCarrierPinning}.
 *
