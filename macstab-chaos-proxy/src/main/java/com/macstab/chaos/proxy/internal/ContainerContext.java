 ...
 * single immutable value. Created once at the entry point of each operation (typically in
 * {@link ToxiproxyOrchestrator}) and passed down the call chain.
 *
 * <h2>Motivation</h2>
 *
 * <p>Platform detection ({@link PlatformDetector#detect}) requires an {@code execInContainer} call
 * to read {@code /etc/os-release}. Without this DTO, each manager class independently cached
 * {@code cachedPlatform} / {@code cachedContainer} fields — duplicating state, introducing
 * thread-safety risk, and violating DRY.
 *
...
public final class ContainerContext {
   ...
   *
   * <p>Performs platform detection via {@link PlatformDetector#detect}. This executes one
   * {@code cat /etc/os-release} inside the container — call once per operation, not per method.
   *
   ...
   *
   * <p>Bypasses platform detection. Intended for use in tests where platform and shell are
   * already known (e.g., from mocks), avoiding the need for {@code mockStatic(PlatformDetector)}.
   *
