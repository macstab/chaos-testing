 ...
 *
 * <p>Injects TCP-level faults via Toxiproxy and data-level faults via backend-specific CLI. No
 * application code changes required — all chaos is applied transparently at the network and data
 * layer.
 * <!-- ═══════════════════════════════════════════════════════════════════ -->
 *
 ...
 * <!-- ═══════════════════════════════════════════════════════════════════ -->
 *
 * <p>Implementations that use Toxiproxy share a single Toxiproxy process with other modules on the
 * same container. Cleanup must be done carefully:
 *
 * <ul>
 *   <li>✅ {@link #reset} on this interface is <strong>targeted</strong>: only the cache proxy is
 *       removed. The Toxiproxy process and all other proxies stay alive. Use in {@code @AfterEach}.
 *   <li>⛔ <strong>Nuclear reset</strong> (kills Toxiproxy + ALL iptables rules) must be done via
 *       the underlying {@code ProxyChaosProvider.reset(container)} in {@code @AfterAll}.
 * </ul>
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li>{@link #slowResponse} — add fixed latency to all operations
 *   <li>{@link #injectConnectionFailures} — drop TCP connections probabilistically
 *   <li>{@link #limitThroughput} — cap transfer rate (simulates slow network)
 *   <li>{@link #truncateResponses} — close connection mid-response (broken protocol framing)
 *   <li>{@link #removeFault} — remove a named fault
 *   <li>{@link #removeAllFaults} — remove all faults, proxy stays active
 * </ul>
 *
 * <p><strong>Data-level faults</strong> (via backend CLI — bypass proxy):
 *
 * <ul>
 *   <li>{@link #forceEviction} — delete percentage of keys (simulate eviction pressure)
 *   <li>{@link #limitMemory} — set maxmemory limit (trigger real eviction)
 *   <li>{@link #setEvictionPolicy} — change eviction policy
 *   <li>{@link #disconnectClients} — kill all connected clients
 *   <li>{@link #flushAll} — wipe entire cache (use in teardown)
 * </ul>
 *
 * <!-- ═══════════════════════════════════════════════════════════════════ -->
 *
 ...
 * <!-- ═══════════════════════════════════════════════════════════════════ -->
 *
 * <p>TCP faults are registered in Toxiproxy with fixed names. Use these names with {@link
 * #removeFault}:
 *
 * <table border="1">
...
public interface CacheChaos extends ChaosProvider {
   ...
   *
   * <p>Simulates slow networks or a heavily loaded cache backend. Creates a {@code "latency"} toxic
   * in Toxiproxy.
   *
   * <pre>{@code
   ...
   *
   * <p>Simulates flaky networks, intermittent cache outages, or network partitions. Creates a
   * {@code "timeout"} toxic (instant close) in Toxiproxy.
   *
   * <p><strong>Note:</strong> This is a TCP-level connection drop — not a protocol-level cache
   * miss. Clients observe a connection exception, not a null/miss return value.
   *
   * <pre>{@code
   ...
   *
   * <p>Simulates a bandwidth-constrained network link or a slow cache response path. Creates a
   * {@code "bandwidth"} toxic in Toxiproxy.
   *
   * <pre>{@code
   ...
   *
   * <p>Injects broken protocol framing — the client receives a partial response and must handle the
   * truncation gracefully. Creates a {@code "limit_data"} toxic in Toxiproxy.
   *
   * <pre>{@code
   ...
   *
   * <p>Restores normal throughput and latency without removing the proxy itself. Use {@link #reset}
   * to delete the proxy entirely.
   *
   * @param container target container
   */
   ...
   *
   * <p>Simulates eviction pressure — the application's cache hit ratio drops immediately. Operates
   * directly on the backend (bypasses the proxy) so it works under active TCP faults.
   *
   * <pre>{@code
   ...
   *
   * <ul>
   *   <li>{@code noeviction} — return error when limit reached (default)
   *   <li>{@code allkeys-lru} — evict least-recently used among all keys
   *   <li>{@code volatile-lru} — evict LRU among keys with TTL only
   *   <li>{@code allkeys-lfu} — evict least-frequently used among all keys
   *   <li>{@code allkeys-random} — evict random keys
   * </ul>
   *
   ...
   *
   * <p>Simulates a sudden cache restart — all active connections are dropped. Applications relying
   * on persistent connection pools observe reconnection behaviour.
   *
   * @param container target container
   */
   ...
   *
   * <p>⚠️ Destructive — wipes the entire cache. Intended for test setup/teardown, not chaos
   * injection in running assertions.
   *
   * @param container target container
   */
   ...
   *
   * <p>Surgical cleanup: only the named cache proxy is deleted from Toxiproxy. The Toxiproxy
   * process and all other proxies on the container stay alive. Use this in {@code @AfterEach}.
   *
   * <p>No-op if the container is not running.
   *
   * <p>The proxy name removed is determined by the implementation's configuration (e.g., {@code
   * "redis_cache"} for {@code RedisCacheChaosProvider}).
   *
   * @param container target container
   */
  @Override
  ...
  /**
   * @deprecated Use {@link #injectConnectionFailures(GenericContainer, double)} instead. This
   *     method drops TCP connections — not protocol-level cache misses. The name was misleading;
   *     the new method name reflects the actual behaviour.
   * @param container target container
