...
/**
 * L1 chaos primitive: inject {@code EINTR} on every libchaos-process-intercepted
 * {@code waitpid} call inside the container, gated by {@link #probability}.
 *
 * <p><strong>What this simulates:</strong> interrupted system call — signal delivery during syscall; canary for signal-handling correctness.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * @ChaosWaitpidEintr(probability = 0.01)
 * class MyTest { ... }
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> {@code 1e-3} to {@code 1e-2} mirrors realistic
 * production rates for {@code EINTR} on {@code waitpid}; {@code 1.0} typically
