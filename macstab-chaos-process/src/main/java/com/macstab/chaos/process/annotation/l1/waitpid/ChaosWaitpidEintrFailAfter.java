...
/**
 * After {@link #successesBeforeFailure} successful {@code waitpid} calls, injects {@code EINTR} on
 * every subsequent call, modelling a sustained signal-storm scenario where the process receives
 * continuous signals that interrupt every subsequent wait.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-process primitive — one (selector = {@code WAITPID}, errno = {@code EINTR}, effect =
 ...
 *       returns {@code -1} with {@code errno = EINTR}.
 *   <li>The calling code receives: return value {@code -1}, {@code errno = EINTR} (4); the wait was
 *       interrupted by a signal on every attempt.
 * </ol>
 *
 ...
 *       return EINTR permanently; assert that the application's EINTR retry loop is bounded — an
 *       unbounded retry loop on EINTR will spin indefinitely if signals are delivered continuously.
 ...
 *       blocking, or escalates the sustained interruption after a retry budget is exhausted.
 *   <li>Assert that the application checks its shutdown flag on each EINTR retry — a sustained
 *       EINTR triggered by SIGTERM should cause the application to exit the wait loop and proceed
 *       with graceful shutdown, not loop forever on EINTR.
 * </ul>
 *
 ...
 *
 * <p>FAIL_AFTER models a sustained signal-storm where every waitpid attempt is interrupted. In the
 * transient EINTR case (ERRNO variant), the signal is occasional and the retry succeeds quickly.
 * FAIL_AFTER models the sustained case: a continuously firing signal source interrupts every wait;
 * the application must detect this and fall back to non-blocking waitpid (WNOHANG) or implement a
 * retry budget with circuit-breaker escalation.
 *
 * <p>waitpid returns -1 on error and sets errno. The EINTR retry pattern: {@code do { ret =
 * waitpid(pid, &status, 0); retries++; } while (ret == -1 && errno == EINTR && retries < MAX &&
 * !shutdown)} is correct for bounded retry. The counter does not reset between test methods when
 * the annotation is at class scope. Set {@link #successesBeforeFailure} to the number of waits
 * expected to succeed before the signal storm begins; 0 means the storm begins immediately from
 * startup.
 *
 ...
 * succession. Applications that use libraries installing their own signal handlers (JVM, Go
 * runtime, Python GIL) may be especially susceptible to sustained EINTR from waitpid.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
