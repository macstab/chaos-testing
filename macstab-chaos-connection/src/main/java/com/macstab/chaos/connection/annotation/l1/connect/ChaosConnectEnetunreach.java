...
/**
 * Injects {@code ENETUNREACH} on every libchaos-intercepted {@code connect} call inside
 * the target container, making the call fail as if the kernel returned {@code ENETUNREACH}.
 *
 ...
 * annotation class exists only because {@code ENETUNREACH} is a valid POSIX result of
 * {@code connect}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = ENETUNREACH} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: network unreachable — split-brain or partition event.
 *
 ...
 * class. Use the repeatable form ({@code @ChaosConnectEnetunreachs}) to bind different probabilities to
