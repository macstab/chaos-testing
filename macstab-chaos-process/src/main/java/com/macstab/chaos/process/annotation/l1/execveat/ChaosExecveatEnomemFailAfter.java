 ...
 *       sets {@code errno = ENOMEM} and returns {@code -1} without issuing the real kernel call.</li>
 *   <li>The calling code receives: {@code -1} return, {@code errno} 12,
 *       {@code strerror}: "Out of memory"; the calling process remains in its current state and
 *       the {@code dirfd} must be closed explicitly since no close-on-exec processing occurs for
 *       failed execs; in fork+exec patterns the child must call {@code _exit()} to avoid zombies.</li>
 * </ol>
 *
 ...
 *       treating the failure as a permanent binary-unavailability error.</li>
 *   <li>In fork+exec patterns using {@code execveat}, the child process that receives
 *       post-threshold {@code ENOMEM} must call {@code _exit()} (not {@code exit()}) so that
 *       the parent can detect the exec failure via {@code waitpid} without a zombie accumulating
 *       for each failed launch — assert that the runtime's fork+exec child exits cleanly with
 *       a specific exit code that the parent maps to an ENOMEM error.</li>
 ...
 *       for each exec attempt before the kernel rejects the call.</li>
 * </ul>
 ...
 * OOM-protected workloads whose actual RSS exceeds their requests; after N successful launches
 * the exec's internal kernel memory allocations fail; the runtime's fork+exec children do not
 * call {@code _exit()} on exec failure, producing zombie processes that consume pid table space
 * and worsening the OOM condition.
 *
 ...
 * sufficient memory for the first N exec calls (each of which allocates kernel data structures
 * for the new process image); once the node's free memory falls below the allocation requirement
 * the exec fails and continues to fail until memory is reclaimed. Setting
 * {@link #successesBeforeFailure} to the observed exec count before the node crosses the exhaustion
 ...
 * present in the ERRNO variant: under FAIL_AFTER, each of the first N successful execs closes
 * the dirfd via {@code FD_CLOEXEC} on exec success. After the counter trips, every subsequent
 * call opens a fresh dirfd, then receives {@code ENOMEM} from the interposer. The dirfd is now
 * open but the exec has not proceeded — the application must close it explicitly. Applications
 * that omit the explicit close accumulate one leaked fd per failed exec attempt under memory
 * pressure, progressively adding to the system-wide fd count.
 *
 * <p>The fork+exec zombie risk is amplified under FAIL_AFTER because all fork+exec calls after
 * the counter trips will fail in the child. In the probabilistic ERRNO variant, only a fraction
 * of child processes fail and each failure is independent; under FAIL_AFTER, every fork+exec
 * child after the threshold fails in the exec and must call {@code _exit()}. Applications that
 * use {@code exit()} rather than {@code _exit()} in the failed-exec child run C++ destructors
 * and flush stdio buffers inherited from the parent, which can corrupt the parent's shared state
 ...
 * number of successful exec calls before the memory exhaustion threshold is crossed; values in
 * the range 10–100 cover most container-density scenarios; 0 exercises the cold-start OOM path
 * where memory is exhausted before the first launch succeeds.
