 ...
 *
 * <ul>
 *   <li>JVM: AsyncRequestTimeoutException injected at METHOD_ENTER on class prefix {@code
 *       classPattern} — active requests that straddle the shutdown boundary exceed their async
 *       timeout and are terminated with an error response
 ...
 * Rolling deploy shutdown under live traffic — SIGTERM initiates the drain window but the grace
 * period expires before all active requests complete, leaving clients with reset connections and
 * potentially incomplete idempotent operations.
 *
 * <h2>Industry references</h2>
 *
 * <p>Rolling deploy graceful shutdown races are documented in the Spring Boot lifecycle docs and
 * Kubernetes operator guides. The drain window (spring.lifecycle.timeout-per-shutdown-phase) must
 * exceed the longest request latency; this scenario tests whether the application correctly handles
...
@ChaosL3(
    composer = "com.macstab.chaos.springboot.testpack.l3.composers.SpringGracefulShutdownComposer",
