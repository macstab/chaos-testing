...
/**
 * After {@link #successesBeforeFailure} successful {@code execveat} calls, injects {@code E2BIG} on
 * every subsequent call, causing the calling code to observe an argument-list-too-large failure
 ...
 *       sets {@code errno = E2BIG} and returns {@code -1} without issuing the real kernel call.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 7, {@code strerror}: "Argument
 *       list too long"; the {@code dirfd} must be closed by the caller to avoid an fd leak since no
 *       close-on-exec processing occurs for failed execs.
 * </ol>
 *
 ...
 *       return {@code -1} with {@code errno = E2BIG}; assert that the counter threshold aligns with
 *       the application's expected exec budget so the failure fires at a predictable point in the
 *       test scenario.
 *   <li>Container runtimes using {@code execveat} with {@code AT_EMPTY_PATH} must close the open
 *       {@code dirfd} in the {@code E2BIG} error path — assert that every exec failure closes the
 *       dirfd before propagating the error, since failed execs do not trigger {@code FD_CLOEXEC}
 *       processing on the dirfd.
 ...
 *       tracing context, and sidecar env vars; the cumulative argument vector is well below {@code
 *       ARG_MAX} for the first N launches but crosses the limit deterministically after enough
 *       webhook transformations — assert that the runtime's argument pruning logic or error surface
 *       activates at the expected call count, not at a random trial.
 * </ul>
 *
 ...
 * ({@code OTEL_*}, {@code AWS_*}, service-mesh metadata); after N container launches the cumulative
 * environment size crosses {@code ARG_MAX}, all subsequent launches fail with {@code E2BIG}, and
 * the runtime's error path leaks the open {@code dirfd} it prepared for exec.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The FAIL_AFTER counter models the progressive accumulation of environment variables across a
 * series of exec calls more accurately than a probabilistic ERRNO. In real {@code E2BIG} incidents,
 ...
 *
 * <p>The {@code execveat} context adds a layer of complexity absent from plain {@code execve}: the
 * caller must open a {@code dirfd} before calling exec. Under FAIL_AFTER, the first N calls open
 * and exec correctly (dirfd closed by kernel on success with {@code FD_CLOEXEC}). After the counter
 * trips, every subsequent call opens the dirfd, then receives {@code E2BIG} from the interposer —
 * the kernel never sees the call, so no close-on-exec processing occurs, and the dirfd remains
 * open. Applications that omit an explicit {@code close(dirfd)} in the {@code E2BIG} error path
 * leak one fd per failed exec, progressively exhausting {@code RLIMIT_NOFILE} and creating a
 * compounding failure where the argument-size problem is followed by an fd-exhaustion problem.
 *
 * <p>The counter does not reset between test methods when the annotation is placed at class scope.
 * This is intentional: it allows a test suite to exercise the "first N succeed" phase in early test
 * methods and the "all subsequent fail" phase in later methods without re-deploying the container.
 * When per-method isolation is needed, place the annotation at method scope so the counter
 * re-initialises before each test.
 *
 * <p>Contrast with {@code ChaosExecveatE2big} (ERRNO, probabilistic): the ERRNO variant fires with
 * probability {@code p} on each call independently, making it suited for testing transient
 ...
 * production; values in the range 10–200 cover most container-launch scenarios; 0 means the very
 * first exec fails, which tests cold-start failure handling.
 *
