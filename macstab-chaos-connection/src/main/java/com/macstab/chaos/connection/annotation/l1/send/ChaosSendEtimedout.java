...
/**
 * Injects {@code ETIMEDOUT} on every libchaos-intercepted {@code send} call inside
 * the target container, making the call fail as if the kernel returned {@code ETIMEDOUT}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code ETIMEDOUT}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code ETIMEDOUT} is a valid POSIX result of
 * {@code send}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = ETIMEDOUT} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: operation timeout — peer unresponsive or routing black-holed.
 *
 ...
 * @ChaosSendEtimedout(toxicity = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-2 to 1e-1 for timeout testing; combine with retry-budget exhaustion tests.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every capable container in the test
 * class. Use the repeatable form ({@code @ChaosSendEtimedouts}) to bind different probabilities to
