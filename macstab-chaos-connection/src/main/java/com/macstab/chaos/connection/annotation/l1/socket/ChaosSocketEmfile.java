...
/**
 * Injects {@code EMFILE} on every libchaos-intercepted {@code socket} call inside
 * the target container, making the call fail as if the kernel returned {@code EMFILE}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EMFILE}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EMFILE} is a valid POSIX result of
 * {@code socket}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EMFILE} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: per-process file-descriptor limit reached — typical of fd-leaks in connection pools.
 *
 ...
 * @ChaosSocketEmfile(toxicity = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosSocketEmfiles}) to bind different probabilities to
