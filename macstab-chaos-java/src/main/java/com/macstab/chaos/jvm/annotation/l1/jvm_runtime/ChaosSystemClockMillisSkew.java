 ...
 *   <li><strong>Scheduled-job misfires.</strong> Cron-driven and delay-queue tasks keyed to
 *       {@code currentTimeMillis} may fire at the wrong time; assert idempotency and
 *       correct catch-up behaviour.
 ...
 *
 * <p>{@code System.currentTimeMillis()} is a {@code native} method declared in
 * {@code java.lang.System}. Intercepting it requires a Byte Buddy native-method delegation stub: the
 * agent introduces a Java-visible wrapper that the JVM calls instead of the raw native, allowing the
 * interceptor to add the skew before returning. This differs from plain bytecode advice and requires
 * the agent to be loaded at JVM startup via {@code -javaagent} so that the bootstrap class loader
 * can see the stub.
 *
 * <p>Wall-clock skew is qualitatively different from monotonic-clock skew (see
 * {@link ChaosSystemClockNanosSkew}). {@code currentTimeMillis()} reflects the system wall clock,
 * which is used for timestamps, token expiry, and calendar scheduling. Its value can jump forward
 * or backward (NTP slew), making it inherently unreliable for measuring elapsed time. Distributed
 ...
 *
 * <p>In Raft-based systems, the leader periodically renews its lease by comparing its local
 * {@code currentTimeMillis()} against the lease duration. If the leader's clock is skewed backward
 * by more than the lease duration it believes the lease has not expired when followers have already
 * timed it out, resulting in a window with two nodes that believe they are leader. Zookeeper session
 ...
 *
 * <p>The {@code DRIFT} mode increases the offset at a configurable rate, allowing tests to
 * observe how the system reacts to gradual skew growth — the kind caused by a VM whose NTP daemon
 * has stopped or whose hypervisor clock is running slow. The {@code FREEZE} mode pins the clock at
 * the activation instant, useful for testing time-based caches that are expected to refresh when
