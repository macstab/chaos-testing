 ...
 * <h2>What this is</h2>
 *
 * <p>Simulates a JDBC primary-failover event: the primary database goes down, replica promotion is
 * in progress, and DNS-based failover causes a transient name-resolution flap while connections are
 * still being refused by the new primary that has not yet fully taken over.
 *
 * <h2>Composed of</h2>
 *
 * <ul>
 ...
 *       in progress")} on classes matching {@code classPattern} at METHOD_ENTER — surfaces failover
 *       state at the Java layer
 * </ul>
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Critical</strong><br>
 * All clients lose connectivity for the duration of the promotion window; retry storms against the
 * DNS resolver and connection pool are common side effects, extending the unavailability window
 * beyond the actual failover time.
 *
...
@ChaosL3(
    composer = "com.macstab.chaos.jdbc.testpack.l3.composers.JdbcPrimaryFailoverComposer",
