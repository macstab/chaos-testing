 ...
 * only the time taken increases. Compile-time safety: invalid selector/effect combinations have
 * no annotation class.
 *
 ...
 *       calling thread before issuing the real syscall — the calling thread is stalled for the
 *       full delay period before the kernel creates the child process.</li>
 *   <li>After the sleep the real {@code fork} syscall is issued; the kernel processes it normally
 *       and returns either the child pid (parent) or 0 (child) on success, or -1 on failure.</li>
 *   <li>The calling code receives: the real kernel return value, after a wall-clock delay of at
 *       least {@link #delayMs} ms; no spurious errno is injected; the fork either succeeds or
 *       fails for a genuine kernel reason such as EAGAIN or ENOMEM.</li>
 * </ol>
 *
 ...
 *       will fire before the request is complete if the fork is delayed.</li>
 ...
 *       scheduling pressure rather than assuming instantaneous fork.</li>
 ...
 *       the fork will fire {@link #delayMs} ms prematurely under latency injection.</li>
 * </ul>
 * Production failure mode: a CGI-style application server forks a child process to handle each
 * HTTP request; the host kernel is under scheduling pressure from co-located workloads; fork stalls
 * for 150ms in the kernel's copy-on-write page table setup; the application's request timeout is
 * 200ms measured from request arrival; the forked child has only 50ms remaining to produce a
 * response, causing intermittent timeouts that are attributed to the application rather than to
 * the fork scheduling stall.
 *
 ...
 * copy-on-write marking, credential copying, signal handler copying) runs at kernel speed after
 * the sleep. This models the scenario where the delay is in the userspace-to-kernel transition
 * and CPU scheduling, not in the kernel's fork implementation.
 *
 * <p>Fork latency is particularly significant in copy-heavy workloads because the kernel must
 * walk the parent's page table to mark all pages as copy-on-write, even under CoW. For processes
 * with large virtual address spaces (e.g. JVM workers forking for subprocess execution), the
 * page table walk alone can take tens of milliseconds. The LATENCY primitive allows testing the
 * application's response to this latency without requiring a large-memory process or a loaded
 * kernel.
 *
 ...
 * faults as it diverges from the parent's pages. Under memory pressure, these faults can stall
 * the child for much longer than the fork call itself. The LATENCY primitive isolates the fork
 * call latency; to model the CoW fault latency in the child, combine this annotation with
 * memory-level chaos annotations.
 *
 ...
 * deadlines. If the credential delivery system sets a timeout from the token request to the
 * process launch, fork latency reduces the budget available for the actual credential handshake.
 * Assert that the credential delivery timeout is set relative to child process readiness, not
 ...
 * expose timeout calibration gaps; 200ms+ simulates heavily loaded nodes with many context switches.
