 ...
 * process syscall} calls succeed, then injects {@code ENFILE} on every subsequent call until the
 * rule is removed.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one (selector
 * = {@code WILDCARD}, errno = {@code ENFILE}, effect = FAIL_AFTER) tuple. FAIL_AFTER is the process
 ...
 * the counter trips and every subsequent call returns {@code -1} with {@code errno = ENFILE},
 ...
 * ({@code @ChaosWildcardEnfileFailAfter.Repeatable}) to apply different counters to different
 * containers.
 *
