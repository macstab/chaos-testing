 ...
 *
 * <p>Separated from {@link CgroupsCpuChaos} to keep mutation (inject chaos) and observation (query
 * state) in distinct classes. All methods are stateless — they read container state via /proc and
 * shell commands, never modify it.
 *
...
final class CpuObservability {
  ...
  int getAvailableCores(final GenericContainer<?> container) throws Exception {
    final var result =
        container.execInContainer(Shell.SH, Shell.FLAG_C, cmd.buildGetCoreCountCommand());
    if (result.getExitCode() == 0 && !result.getStdout().isBlank()) {
      return Integer.parseInt(result.getStdout().trim());
    }
    final var fallback =
        container.execInContainer(Shell.SH, Shell.FLAG_C, cmd.buildGetCoreCountFallbackCommand());
  ...
  boolean isThrottled(final GenericContainer<?> container) {
    try {
      return container
              .execInContainer(
                  Shell.SH, Shell.FLAG_C, cmd.buildIsRunningByCommExactCommand("cpulimit"))
              .getExitCode()
          == 0;
    } catch (final Exception e) {
      return false;
  ...
  boolean isStressed(final GenericContainer<?> container) {
    try {
      return container
              .execInContainer(
                  Shell.SH, Shell.FLAG_C, cmd.buildIsRunningByCommPrefixCommand("stress-ng"))
              .getExitCode()
          == 0;
    } catch (final Exception e) {
      return false;
   ...
   *
   * <p>For {@code cores < 64}: {@code (1L << cores) - 1} — e.g., 12 cores → {@code 0xfff}. For
   * {@code cores >= 64}: {@code -1L} ({@code 0xffff...ffff}) — all bits set, avoids {@code 1L <<
   * 64} overflow (Java shifts are mod 64).
   *
  ...
  private long[] readCpuStat(final GenericContainer<?> container) throws Exception {
    final var result =
        container.execInContainer(Shell.SH, Shell.FLAG_C, cmd.buildReadCpuStatCommand());
    if (result.getExitCode() != 0) {
  ...
  /**
   * Parses the aggregate cpu line from /proc/stat into tick counters. Package-private for unit
   * testing.
   */
  ...
  /** Computes CPU busy percentage from two /proc/stat samples. Package-private for unit testing. */
