 ...
 *
 * L1 libchaos-memory primitive — one (selector = {@code MADVISE}, errno = {@code EFAULT}) tuple.
 ...
 *       address"; the hint is not applied.
 * </ol>
 *
 ...
 *       the application must not assume the advised behaviour is active and must not depend on the
 *       hint having taken effect for correctness.
 *   <li>Memory managers that advise address ranges computed from allocation metadata must validate
 *       page alignment before issuing {@code madvise}; assert that {@code EFAULT} triggers a
 *       diagnostic that includes the address and length so that stale-pointer bugs can be
 *       attributed from logs.
 *   <li>Assert that a failed {@code madvise} is treated as a best-effort hint failure — the
 *       application must not abort, retry indefinitely, or degrade correctness in response to a
 *       failed hint.
 * </ul>
 *
 * Production failure mode: a native memory manager advises address ranges for performance
 * optimisation (e.g. {@code MADV_WILLNEED} for a segment it is about to access); a concurrent
 * {@code munmap} in another thread removes the mapping before the {@code madvise} call completes,
 * causing the kernel to return {@code EFAULT} for the now-unmapped range — a race that is invisible
 * in single-threaded testing.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX specifies {@code EFAULT} for {@code madvise} when the address range {@code [addr,
 * addr+len)} extends outside the process's accessible address space. On Linux, the kernel walks the
 * VMA list in {@code do_madvise} and returns {@code -EFAULT} if it encounters a gap in the VMA tree
 * within the requested range — meaning part of the range is not mapped. This is distinct from
 * {@code EINVAL} (which is returned for misaligned addresses or invalid advice values).
 *
 * <p>The most common production path to {@code EFAULT} from {@code madvise} is a TOCTOU race: a
 * thread computes an address range (which is valid at that moment) and then passes it to {@code
 * madvise} after another thread has unmapped the range. Between the computation and the syscall,
 * the VMA is gone. The kernel sees a gap in the VMA tree and returns {@code EFAULT}. This race is
 * difficult to reproduce without fault injection.
 *
 * <p>A second path is incorrect range arithmetic: a memory manager that tracks region sizes in
 * 32-bit integers and promotes them to 64-bit without sign-extension can produce {@code addr + len}
 * that exceeds the process's virtual address space, causing {@code EFAULT} from the overflow check.
 * This is the same integer-width bug that can affect {@code munmap} (see {@code
 * ChaosMunmapEfault}).
 *
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> very low (1e-5 to 1e-4) to simulate the TOCTOU race;
 * higher rates are safe since madvise failures are non-fatal.
 *
