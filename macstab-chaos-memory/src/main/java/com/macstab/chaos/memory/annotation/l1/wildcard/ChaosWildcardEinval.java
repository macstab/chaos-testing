...
/**
 * Injects {@code EINVAL} on every libchaos-memory-intercepted {@code every interposed VM syscall} call inside
 * the target container, making the call fail as if the kernel returned {@code EINVAL}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code WILDCARD}, errno = {@code EINVAL})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code EINVAL} is a valid POSIX result of {@code every interposed VM syscall}; the invalid combinations
 * simply have no annotation class, so the selector × errno matrix cannot be violated at compile
 * time.
 *
 * <p><strong>What chaos this applies:</strong> on every {@code every interposed VM syscall} call that the
 ...
 * kernel-level failure. Specifically this simulates: invalid argument — bad length, alignment, or flags; the universal canary errno.
 *
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2 as a canary; 1.0 will block all mapped I/O and crash the JVM.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every memory-chaos-capable container
 * in the test class. Use the repeatable form ({@code @ChaosWildcardEinvals}) to bind different
