...
/**
 * Injects {@code EAI_AGAIN} on every libchaos-intercepted {@code wildcard} call inside
 * the target container, making the call fail as if the kernel returned {@code EAI_AGAIN}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EAI_AGAIN}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EAI_AGAIN} is a valid POSIX result of
 * {@code wildcard}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EAI_AGAIN} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: temporary DNS resolution failure — resolver timeout or transient network issue.
 *
 ...
 * @ChaosWildcardEaiagain(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-2 to 1e-1 for DNS retry testing; 1.0 blocks all forward resolution.
 *
 * <p><strong>Scope:</strong> {@link #id()} binds this rule to a single container by its declared
 * {@code id}; the default empty string applies the rule to every capable container in the test
 * class. Use the repeatable form ({@code @ChaosWildcardEaiagains}) to bind different probabilities to
