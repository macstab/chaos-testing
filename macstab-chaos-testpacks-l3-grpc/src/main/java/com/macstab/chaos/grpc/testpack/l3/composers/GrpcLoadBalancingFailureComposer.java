...
@Slf4j
public final class GrpcLoadBalancingFailureComposer
    implements L3Composer<IncidentChaosGrpcLoadBalancingFailure> {
  ...
  public List<Object> apply(
    ...
    final ChaosScenario scenario =
        ChaosScenario.builder(scenarioId)
            .description(
                "L3 gRPC LB failure — EAI_AGAIN + DNS latency + ECONNREFUSED + UNAVAILABLE")
            .selector(
            ...
            .effect(
                ChaosEffect.injectException(
                    ...
                    "UNAVAILABLE: load balancer name resolution failure"))
    ...
    for (final Object h : handles) {
      if (h instanceof com.macstab.chaos.dns.api.RuleHandle dnsRh) {
        ...
        try {
          ...
          log.warn(
              "GrpcLoadBalancingFailureComposer.removeAll: failed to remove connection rule", e);
        }
        ...
        try {
          ...
          log.warn(
              "GrpcLoadBalancingFailureComposer.removeAll: failed to remove JVM scenario {}",
              scenarioId,
              e);
        }
      }
