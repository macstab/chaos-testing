 ...
 *
 * <p>Applies a SafepointStorm with class retransformation to reproduce the compound failure
 * profile of a JIT deoptimisation storm caused by JVMTI retransformation invalidating compiled
 * code.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class JvmDeoptimizationStormComposer implements L3Composer<IncidentChaosJvmDeoptimizationStorm> {

    public JvmDeoptimizationStormComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosJvmDeoptimizationStorm ann) {
        ...
        final ChaosScenario scenario = ChaosScenario.builder(id)
                ...
                .effect(new ChaosEffect.SafepointStormEffect(Duration.ofMillis(ann.gcIntervalMs()), ann.retransformClassCount()))
    ...
    public List<String> describe(final IncidentChaosJvmDeoptimizationStorm ann) {
        return List.of(
                ...
                "jvm: SafepointStorm every " + ann.gcIntervalMs() + "ms retransforming " + ann.retransformClassCount() + " classes per cycle",
