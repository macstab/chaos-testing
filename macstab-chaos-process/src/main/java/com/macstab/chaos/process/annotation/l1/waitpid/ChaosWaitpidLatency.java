 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code WAITPID}, effect = LATENCY) tuple. Unlike
 * errno variants, LATENCY always delegates to the real kernel call after the delay — no child state
 * is missed and no errno is injected; only wall-clock cost is added. Compile-time safety: invalid
 ...
 *   <li>On each {@code waitpid} call the interposer sleeps for {@link #delayMs} milliseconds before
 *       issuing the real kernel call.
 *   <li>The real {@code waitpid} call is then issued and its result (pid, status, errno) is
 *       returned unchanged to the caller — no errno is injected, no child state is lost.
 *   <li>The calling code observes: correct return value and status, but the call returns {@link
 *       #delayMs} ms later than expected; children that exited during the delay remain zombies for
 *       the duration of the delay on every wait cycle.
 * </ol>
 *
 ...
 *       for waitpid latency when calibrating their cleanup deadlines.
 *   <li>Children that exit while a previous waitpid call is delayed remain zombies for up to {@link
 *       #delayMs} ms; assert that the application's zombie accumulation monitoring does not
 *       false-alert during the delay window — short-lived bursts of zombies are expected when child
 *       reaping is delayed.
 ...
 *   <li>Assert that the application does not assume waitpid is instantaneous when computing child
 *       cleanup timeouts; a cleanup deadline of {@code child_exit_time + cleanup_budget} must add
 *       the maximum observed waitpid latency to the budget to be correct.
 * </ul>
 *
 ...
 * wait-queue notification path); NUMA effects where the calling thread and the child process ran on
 * different memory nodes (cross-node wait-queue wakeup is slower than local); CPU frequency scaling
 * that delays the scheduler on power-constrained hosts; and cgroup throttling that suspends the
 * calling thread's CPU slice mid-wait. All of these are correct-result-but-slow failures that no
 * errno can model.
 *
 * <p>The delay is injected before the real {@code waitpid} call, not after. This means that during
 * the delay the child has not yet been reaped even if it has already exited; the child remains a
 * zombie for the delay duration. Under LATENCY injection, the zombie window per child is extended
 * by {@link #delayMs} ms beyond the child's natural exit time. Applications that monitor zombie
 * counts via {@code /proc} must tolerate elevated zombie counts during high child exit rates when
 * waitpid latency is significant.
 *
 * <p>A key correctness concern for blocking waitpid callers: applications that call {@code
 * waitpid(pid, &status, 0)} (blocking, no WNOHANG) while holding resources (connections, locks,
 * file descriptors) expose those resources to the full waitpid delay. Under LATENCY injection, this
 * tests whether the resource hold time is bounded independently of child-reaping speed. The correct
 * pattern is to release resources before waiting, or to use a non-blocking loop ({@code
 * waitpid(pid, &status, WNOHANG)}) with a poll interval that fits within the resource hold budget.
 *
 * <p>Latency is applied uniformly to every intercepted waitpid call regardless of the pid argument.
 * This means group waits ({@code waitpid(-pgid, ...)}) and any-child waits ({@code waitpid(-1,
 * ...)}) are delayed identically to specific-pid waits. Applications that use a SIGCHLD handler
 * calling {@code waitpid(-1, WNOHANG)} to drain all zombies will observe each individual {@code
 * waitpid} call delayed; the total drain time for N zombie children is approximately N × {@link
 * #delayMs} ms under full injection.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
