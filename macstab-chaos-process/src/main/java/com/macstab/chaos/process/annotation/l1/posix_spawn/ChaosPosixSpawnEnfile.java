 ...
 *   <li>The calling code receives: return value {@code ENFILE} (23), {@code strerror}: "Too many
 *       open files in system"; the kernel's global file table ({@code fs.file-max}) is exhausted;
 *       no child process is created; closing this process's fds will not resolve the condition.
 * </ol>
 *
 ...
 *       application surfaces a platform-capacity alert rather than attempting in-process
 *       remediation — closing this process's fds will not restore capacity when the system-wide
 *       kernel file table is exhausted by other processes.
 ...
 *       fds in this process) — the diagnostic must route to the correct operator runbook.
 *   <li>Assert that the application does not call {@code waitpid} on the uninitialised pid output
 *       parameter after an ENFILE failure — POSIX does not define the pid value on spawn failure.
 * </ul>
 *
 * Production failure mode: a Kubernetes node runs multiple container runtimes that each use {@code
 * posix_spawn} to launch sidecar processes; accumulated fd leaks across all runtimes push the
 * node's aggregate fd usage towards {@code fs.file-max}; spawn intermittently returns ENFILE; the
 * runtimes do not distinguish ENFILE from EMFILE and instruct operators to check per-process fd
 * counts — operators close application fds but the system-wide count is unchanged.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code ENFILE} from {@code posix_spawn} occurs when the kernel's system-wide open-file table
 * ({@code fs.file-max}, visible in {@code /proc/sys/fs/file-max}) is full. Every open fd in every
 * process on the node counts against this limit. The glibc {@code posix_spawn} implementation
 * internally allocates fds for the parent-child communication pipe used to report exec errors from
 * the child back to the parent; if the system-wide table is full, this pipe allocation fails with
 * ENFILE before the fork even begins.
 *
 ...
 * process; ENFILE (23) means the system-wide kernel file table is exhausted and requires either a
 * platform-level increase in {@code fs.file-max} or a coordinated reduction in aggregate fd usage
 * across all processes on the node. Applications that emit a single "too many files" error for both
 * cases will receive incorrect remediation instructions from operators.
 *
 * <p>The diagnostic for ENFILE from spawn should include the node-level metric ({@code
 * /proc/sys/fs/file-nr} shows used, free, and max) to help operators distinguish a single-process
 * fd leak (EMFILE pattern) from a multi-process cumulative leak (ENFILE pattern). Including this
 * metric in the spawn failure log entry enables operators to take the correct action without
 * requiring kernel tracing tools.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
