 ...
 *       {@link #probability}.
 *   <li>When the trial fires, the interposer returns {@code EPERM} directly (pthread_create returns
 *       the error code, not -1; it does not set errno).
 *   <li>The calling code receives: return value {@code EPERM} (1), {@code strerror(EPERM)}:
 *       "Operation not permitted"; the caller does not have the privilege ({@code CAP_SYS_NICE})
 *       required to set the requested real-time scheduling policy in the thread attribute; no
 *       thread is created.
 * </ol>
 *
 ...
 *       requested in {@code pthread_attr_t} requires {@code CAP_SYS_NICE} which the container does
 *       not have; neither retry nor backoff is appropriate.
 ...
 *       and logs a diagnostic that includes the requested policy and the capability requirement.
 ...
 *       retry loops that consume resources without any possibility of success.
 * </ul>
 *
 * Production failure mode: a low-latency messaging component configures its I/O threads with
 * SCHED_FIFO via pthread_attr; the Kubernetes pod security policy drops CAP_SYS_NICE;
 * pthread_create returns EPERM; the component treats EPERM as a transient failure, retries the
 * thread creation in a tight loop, logs EPERM thousands of times per second, and never surfaces a
 * diagnostic that identifies the missing capability.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code EPERM} from {@code pthread_create} is returned when the {@code pthread_attr_t}
 * specifies a real-time scheduling policy (SCHED_FIFO or SCHED_RR) and the calling thread does not
 * have {@code CAP_SYS_NICE}. On Linux, setting a real-time scheduling policy for a new thread
 * requires the same privilege as {@code sched_setscheduler}: either the effective user is root (uid
 * 0) or the process has {@code CAP_SYS_NICE} in its effective capability set. Standard Kubernetes
 * pod security policies drop all capabilities by default; a container that requires real-time
 * scheduling must explicitly add {@code CAP_SYS_NICE} via the pod's {@code
 ...
 * -1)} or {@code if (errno == EPERM)} silently misses EPERM (1). Code that tests {@code if (ret !=
 * 0)} is correct. The scheduling-policy privilege check happens inside the kernel's {@code
 * sched_fork} path, before the new thread is added to the scheduler; the failure is clean (no
 * thread state to clean up).
 *
 * <p>On some kernel and glibc versions, invalid scheduling policy or priority in pthread_attr
 * returns EINVAL rather than EPERM; the distinction depends on the order of validation: EINVAL is
 * returned for structurally invalid values (policy integer not a valid constant) while EPERM is
 * returned for valid but unprivileged requests (valid SCHED_FIFO but no CAP_SYS_NICE). Applications
 * should handle both EINVAL and EPERM as non-retryable errors from pthread_create.
 *
 * <p>The correct fallback strategy for pthread_create-EPERM in a capability-dropped container is to
 * retry with {@code pthread_attr_setschedpolicy(attr, SCHED_OTHER)} and {@code
 * pthread_attr_setschedparam(attr, &default_param)} — this removes the privilege requirement and
 * allows thread creation to succeed with best-effort scheduling. Applications should log the
