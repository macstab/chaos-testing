 ...
 *
 * <ul>
 *   <li>Spring AOP creates CGLIB proxy subclasses via {@code defineClass()} for every proxied bean
 *       at application context startup; the delay inflates Spring context startup time
 *       proportionally to the number of proxied beans; assert that the startup readiness probe is
 *       configured with sufficient timeout.
 *   <li>Hibernate's bytecode enhancer generates enhanced entity subclasses via {@code
 *       defineClass()} for each entity class found during scanning; the delay inflates JPA {@code
 *       EntityManagerFactory} initialisation time; assert that the JPA container does not time out
 ...
 *       increasing time on metaspace GC; the {@code defineClass()} call itself takes longer as the
 *       JVM scans metaspace for space to allocate the new class descriptor; eventually {@code
 *       defineClass()} throws {@code OutOfMemoryError: Metaspace}; the chaos delay models the
 *       intermediate slow-defineClass symptom before the fatal OOM.
 * </ul>
 *
 ...
 * bytecode array, adding a predictable delay to the time the class definition is held in memory as
 * a raw byte array rather than as a resolved {@code Class} object.
 *
 * <p>Spring AOP's CGLIB-based proxying ({@code ObjenesisCglibAopProxy}) generates proxy subclasses
 * using ASM bytecode manipulation and then calls {@code defineClass()} to register them. This
 * happens once per proxied bean type during application context refresh. The proxy class is cached
 * after the first definition; subsequent beans of the same type reuse the cached proxy class
 * without calling {@code defineClass()} again.
 *
 * <p>Hibernate's {@code ByteBuddyProxyFactory} generates entity proxies using Byte Buddy, which
 * internally calls {@code defineClass()} for each generated proxy type. The delay fires during the
 * first access to each proxied entity type. In applications with many entity classes (50+), the
 * cumulative delay across all proxy definitions can significantly inflate startup time.
 *
 * <p>The distinguish from {@link ChaosClassLoadDelay}: load-delay adds time to finding and reading
 * an existing class from the classpath; define-delay adds time to registering a dynamically
 * generated class. Frameworks that generate classes at runtime (AOP proxies, code generators) are
 * primarily affected by define-delay; frameworks that load pre-compiled classes are primarily
 * affected by load-delay.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
