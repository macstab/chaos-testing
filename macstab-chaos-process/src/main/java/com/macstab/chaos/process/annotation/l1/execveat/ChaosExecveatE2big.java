...
/**
 * Injects {@code E2BIG} on every libchaos-intercepted {@code execveat} call inside
 * the target container, making the call fail as if the kernel returned {@code E2BIG}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code E2BIG}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code E2BIG} is a valid POSIX result of
 * {@code execveat}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = E2BIG} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: argument list too long — argv/envp exceeds ARG_MAX.
 *
 ...
 * @ChaosExecveatE2big(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosExecveatE2bigs}) to bind different probabilities to
