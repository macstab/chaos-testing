...
/**
 * Injects {@code EPERM} on every libchaos-intercepted {@code pthread create} call inside
 * the target container, making the call fail as if the kernel returned {@code EPERM}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EPERM}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EPERM} is a valid POSIX result of
 * {@code pthread create}.
 *
 * <p><strong>What chaos this applies:</strong> on every {@code pthread create} call that the
 * libchaos interceptor sees, a Bernoulli trial with probability {@link #probability} is run.
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EPERM} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: operation not permitted — capability-dropped containers performing privileged operations.
 *
 ...
 * @ChaosPthreadCreateEperm(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosPthreadCreateEperms}) to bind different probabilities to
