 ...
 *
 * <ul>
 *   <li>{@code mmap} returns {@code MAP_FAILED}; {@code errno = EPERM} (1); the application should
 *       treat this as a non-transient privilege failure, not as a transient OOM.
 *   <li>glibc {@code malloc} propagates {@code NULL}; JVM allocators raise {@code
 *       OutOfMemoryError}. The distinction from {@code EACCES} is lost at the Java level but is
 *       preserved in native logs.
 *   <li>Assert that the application surfaces a privilege-related diagnostic and that it does not
 *       retry indefinitely — {@code EPERM} is permanent without a privilege change.
 * </ul>
 *
 * Production failure mode: Kubernetes Pod Security Admission or a seccomp profile applied to a
 * newly deployed revision can block {@code mmap(PROT_EXEC)} for JIT-compiled runtimes (JVM, V8,
 * LuaJIT), causing {@code EPERM} to appear in the allocator stack immediately on startup.
 *
 ...
 * a Yama LSM restriction blocks low-address executable mappings; and when the process is subject to
 * a {@code RLIMIT_MEMLOCK} limit and attempts to create a locked anonymous mapping with {@code
 * MAP_LOCKED | MAP_ANONYMOUS} without the {@code CAP_IPC_LOCK} capability.
 *
 * <p>In containerised environments {@code EPERM} on anonymous mappings most commonly arises when a
 * JVM or other JIT runtime requests {@code PROT_EXEC} for its code cache on a host where {@code
 * vm.mmap_min_addr} is set to a non-zero value and the container runs without {@code
 * CAP_SYS_ADMIN}. This causes the JVM to fall back to interpreted mode, producing a severe
 * performance regression rather than an outright failure.
 *
 ...
 * structural capability check that the process can never satisfy without privilege elevation. Both
 * surface as allocation failure at the libc level. Well-designed error handlers should log the
 * specific errno rather than a generic "allocation failed" message.
 *
 * <p>glibc {@code malloc} never requests {@code MAP_LOCKED} and uses only {@code PROT_READ |
 * PROT_WRITE} for anonymous allocations, so it never triggers {@code EPERM} from the kernel. JVM
 * internal allocations for code cache ({@code PROT_EXEC}) are at risk; Netty's {@code
 * PooledByteBufAllocator} is not ({@code PROT_READ | PROT_WRITE} only).
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
