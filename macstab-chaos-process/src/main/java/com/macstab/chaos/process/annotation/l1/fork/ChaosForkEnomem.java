 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code FORK}, errno = {@code ENOMEM}) tuple. The
 ...
 *       memory"; no child process is created and the calling process continues in its current state
 *       — no cleanup of child resources is required since no child was allocated.
 * </ol>
 *
 ...
 *       stack) — assert that the application treats ENOMEM as a potentially persistent failure that
 *       may require backoff longer than the transient EAGAIN case.
 ...
 *       initialised resources from the pre-fork phase.
 ...
 *       pressure, alert the memory-management team); EAGAIN (11) means the pid table or {@code
 *       RLIMIT_NPROC} is exhausted (self-healing when children exit).
 * </ul>
 *
 ...
 * fork returns ENOMEM; the service's fork failure path does not return a proper error to the
 * caller, leaving the request pending indefinitely — the service appears healthy to its health
 * check but all requests silently queue without completion.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code ENOMEM} from {@code fork} occurs when the kernel fails to allocate the internal data
 * structures required to represent the child process. The primary structures are {@code
 * task_struct} (the process descriptor, several kilobytes), the kernel stack (typically 16 KiB on
 * 64-bit Linux), and {@code mm_struct} (the memory management descriptor). Copy-on-write page table
 * entries are not duplicated immediately — the actual page table duplication is deferred — so
 * ENOMEM from fork is not directly related to the size of the parent's virtual address space.
 *
 ...
 * full, which self-heals when children are reaped. Applications should implement exponential
 * backoff with a limit for ENOMEM and a shorter-timeout retry for EAGAIN.
 *
 ...
 * This makes the error-handling code simpler: there is no dirfd to close, no zombie to reap, and no
 * partial child state. The only requirement is that the parent surfaces the error correctly and
