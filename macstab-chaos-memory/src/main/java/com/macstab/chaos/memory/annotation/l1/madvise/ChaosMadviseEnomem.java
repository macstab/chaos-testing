...
/**
 * Injects {@code ENOMEM} on every libchaos-memory-intercepted {@code madvise} call inside
 * the target container, making the call fail as if the kernel returned {@code ENOMEM}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector = {@code MADVISE}, errno = {@code ENOMEM})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code ENOMEM} is a valid POSIX result of {@code madvise}; the invalid combinations
 ...
 * in the test class. Use the repeatable form ({@code @ChaosMadviseEnomems}) to bind different
