 ...
 *   <li>The calling code receives: return value {@code -1}, {@code errno = EINVAL} (22), {@code
 *       strerror(EINVAL)}: "Invalid argument"; the {@code options} argument contains an invalid or
 *       unsupported flag combination; no child state change is returned.
 * </ol>
 *
 ...
 *       contains an invalid flag that must be corrected in code, not retried.
 ...
 *       includes the options value in the diagnostic to enable rapid identification of the invalid
 *       flag.
 *   <li>Assert that the application distinguishes waitpid-EINVAL (programming error, options flag
 *       invalid) from waitpid-EINTR (signal interrupt, retry required) and from waitpid-ECHILD (no
 *       child, expected loop termination) — the three error codes require completely different
 *       responses and are adjacent in numeric value ({@code EINVAL=22}, {@code ECHILD=10}, {@code
 *       EINTR=4}).
 * </ul>
 *
 ...
 * waitpid returns EINVAL; the supervisor retries in a loop treating EINVAL as a transient error,
 * never reaping the child, which accumulates as a zombie; the supervisor's retry loop does not log
 * the options value, making root cause analysis require kernel changelog review.
 *
 ...
 * Platform-specific extension flags that exist on one kernel version may be removed or renamed in
 * another, making dynamic flag construction fragile.
 *
 * <p>waitpid returns -1 on error and sets errno. Code that tests {@code if (ret == -1 && errno ==
 * EINVAL)} is correct and must not retry. The distinction between EINVAL and ECHILD matters: ECHILD
 * (-1, errno=10) is expected in reap loops while EINVAL (-1, errno=22) is never expected in correct
 * code. An error handler that dispatches only on the -1 return value without checking errno will
 * silently conflate all three error conditions.
 *
 ...
 *
 * <p>A subtle source of EINVAL: calling {@code waitpid} with {@code options = 0} on a stopped child
 * without {@code WUNTRACED} will not return EINVAL — it will simply block until the child exits.
 * EINVAL fires only on genuinely invalid bit patterns, not on valid flags that happen to not match
