...
/**
 * Injects {@code ECHILD} on every libchaos-intercepted {@code wildcard} call inside
 * the target container, making the call fail as if the kernel returned {@code ECHILD}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code ECHILD}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code ECHILD} is a valid POSIX result of
 * {@code wildcard}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = ECHILD} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: no child processes — waitpid with no eligible child; typical of double-waits.
 *
 ...
 * @ChaosWildcardEchild(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosWildcardEchilds}) to bind different probabilities to
