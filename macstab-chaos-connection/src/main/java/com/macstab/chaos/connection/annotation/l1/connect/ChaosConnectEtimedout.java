 ...
 * errno = ETIMEDOUT} without performing any real kernel operation. No runtime operation-errno
 ...
 *       requires waiting through multiple SYN retransmissions (typically 75–127 seconds with
 *       default Linux settings); this injection delivers the error immediately, allowing tests to
 *       verify timeout handling without waiting.
 ...
 *       the possibility of {@code ETIMEDOUT}; assert that pool acquisition timeouts are shorter
 *       than the kernel's SYN retry period so that the pool fails fast and the thread is not
 *       blocked.
 ...
 * </ul>
 *
 * <p>In production, {@code ETIMEDOUT} from {@code connect} occurs when a firewall silently drops
 * TCP SYN packets (rather than rejecting them with RST), when a host is partially available
 * (accepting ICMP ping but not TCP connections), during asymmetric routing failures where SYN
 * packets reach the destination but SYN-ACK packets are lost on the return path, and when a load
 * balancer or NAT device fails and all packets to a destination are silently discarded.
 *
 ...
 * black-hole routes or waiting for the kernel's SYN retry timeout to expire. For tests that need to
 * verify that the application's connect timeout fires before the kernel's SYN timeout, use {@link
 * ChaosConnectLatency} with a delay longer than the application's configured timeout instead.
 *
 * <p>Java maps {@code ETIMEDOUT} from {@code connect} to a {@code SocketTimeoutException} if a
 * {@code SO_TIMEOUT} was set on the socket, or to a generic {@code SocketException} with the
 * message "Connection timed out" if no timeout was set and the kernel's SYN retry limit was
 * reached. Application code that uses {@code SO_TIMEOUT} to interrupt long connects will receive a
 * {@code SocketTimeoutException}; code that relies on the kernel timeout will receive a {@code
 * SocketException}. Both must be handled correctly.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
