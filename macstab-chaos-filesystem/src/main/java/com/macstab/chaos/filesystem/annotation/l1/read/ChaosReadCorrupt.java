 ...
 *       {@link #probability} is conducted for each byte in the returned buffer; when it fires for
 *       a given byte, a random bit in that byte is flipped before the buffer is returned to the
 *       caller.
 * </ol>
 *
 ...
 *       correct byte count with no error indication. Assert that the application uses a
 *       checksum or hash to detect data corruption rather than trusting the read return value alone.
 ...
 *   <li>Binary file formats with magic-number headers will appear invalid when the header bytes
 *       are corrupted; assert that the application validates the magic number and rejects corrupt
 *       files with a clear error rather than proceeding with garbled data.
 ...
 * </ul>
 *
 * <p>In production, data corruption on {@code read} occurs due to failing DRAM (bit flips in
 * the page cache), storage controller firmware bugs that produce corrupt data without an error
 * signal, and filesystem bugs in the page cache consistency path. Unlike hardware EIO errors,
 * silent data corruption (SDC) is particularly dangerous because the application receives
 * incorrect data without any indication that something went wrong.
 *
 ...
 * user buffer), which can occur due to DRAM bit flips or DMA errors. The actual on-disk data is
 * not modified; a subsequent read of the same file offset may return different (uncorrupted) data
 * if the page cache entry is evicted and re-read from disk.
 *
 * <p>The per-byte probability model means that for a {@link #probability} of {@code p}, the
 * expected number of corrupted bytes in a buffer of size N is N × p. For a 4 KB page with
 * {@code p = 0.001}, approximately 4 bytes are expected to be flipped per read. For structured
 * binary formats with checksums or integrity codes, even a single bit flip in the right location
 * (a length field, a record type byte, or a checksum field itself) can cause catastrophic parse
 * failures.
 *
 * <p>Java's NIO {@code FileChannel.read(ByteBuffer)} and classic {@code FileInputStream.read(byte[])}
 * both use the same underlying {@code read(2)} syscall path and will receive the corrupted buffer.
 * Memory-mapped files ({@code FileChannel.map()}) bypass the {@code read(2)} path and are not
 * affected by this injection — the corruption is injected at the syscall interposition layer, not
 * at the page cache level.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
