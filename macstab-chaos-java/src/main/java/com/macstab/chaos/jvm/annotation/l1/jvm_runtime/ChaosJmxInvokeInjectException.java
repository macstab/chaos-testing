 ...
 * failed JMX management operation such as an MBean that is deregistered or an operation that throws
 * at its implementation level.
 *
 ...
 *
 * <ul>
 *   <li><strong>Remote management operations fail.</strong> Scripts and admin consoles that trigger
 *       JMX operations (cache invalidation, thread-pool resize, log-level change) will receive
 *       exceptions; assert that the management tooling reports the error clearly and does not
 *       assume the operation succeeded.
 *   <li><strong>Automated remediation broken.</strong> Systems that use JMX invocations to trigger
 *       self-healing actions (e.g. clearing a poison-message queue via JMX) will fail; assert that
 *       the remediation system has a fallback path.
 *   <li><strong>Metrics reset operations skipped.</strong> Operations that reset MBean counters on
 *       a schedule will be skipped; assert that counter overflow or indefinite accumulation does
 *       not break downstream consumers that assume bounded counter values.
 *   <li><strong>Production failure mode:</strong> Kubernetes Operator controllers that manage
 *       application state via JMX invoke operations will see every control action fail, leaving the
 *       application in its current state indefinitely and causing the Operator to keep retrying
 *       until it enters an error backoff.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The natural exception types for JMX invoke failures are: {@code
 * javax.management.InstanceNotFoundException} (MBean not registered), {@code
 * javax.management.ReflectionException} (method not found on the MBean), and {@code
 * javax.management.MBeanException} (operation threw an application exception). The last wraps the
 * application exception as its cause, so the caller typically needs to unwrap it with {@code
 * getCause()} to identify the root cause.
 *
 ...
 *
 * <p>Because the exception is thrown at the {@code MBeanServer} level, it is indistinguishable from
 * a real MBean-level failure as seen by the caller. Remote callers connected via JMX/RMI will
 * receive the exception serialised over the wire as if the server had thrown it. This correctly
 * simulates the observable behaviour of a broken management infrastructure.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
