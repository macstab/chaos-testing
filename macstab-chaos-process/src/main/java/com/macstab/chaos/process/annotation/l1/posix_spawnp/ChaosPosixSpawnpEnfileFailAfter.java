 ...
 * {@code ENFILE} on every subsequent call, causing the calling code to observe a persistent
 * system-wide file-table exhaustion failure that models node-level fd saturation.
 *
 ...
 *       the kernel's global open-file table ({@code fs.file-max}) has been exhausted system-wide.</li>
 * </ol>
 *
 ...
 *       than attempting in-process recovery — ENFILE cannot be resolved by closing application-owned
 *       fds alone; assert that {@code /proc/sys/fs/file-nr} metrics are included in the alert.</li>
 *   <li>FAIL_AFTER models the node saturation curve: N spawns contribute to the system-wide file
 *       count via their internal pipes; after N spawns the kernel's global table reaches
 *       {@code fs.file-max}; subsequent spawns from any process on the node fail with ENFILE —
 *       assert that the application raises a circuit-breaker and stops spawning rather than
 *       retrying indefinitely into a saturated node.</li>
 ...
 *       cleanup resolves EMFILE but not ENFILE.</li>
 * </ul>
 * Production failure mode: a container runs a high-throughput command executor using
 * {@code posix_spawnp} to invoke utilities by name; aggregate pipe fd allocations across all
 * containers on the node fill the kernel's global file table; after N successful spawns all
 * subsequent spawns return ENFILE; the executor retries with the same interval as for EAGAIN,
 * which is inappropriate for ENFILE and worsens node saturation; the platform team is not alerted
 * because the application does not distinguish ENFILE from EMFILE.
 *
 * <h2>Deep technical dive</h2>
 * <p>FAIL_AFTER models the node saturation curve: each successful posix_spawnp allocates file-table
 * entries for the spawn's internal error-reporting pipe and for the process's own subprocess-capture
 * pipes. The kernel's global file table (bounded by {@code fs.file-max}) is shared across all
 * processes on the host. As the aggregate count of open file descriptions rises across all
 * containers, the global table fills; after N spawns from the container under test the table is
 * exhausted and ENFILE is returned from the next spawn. POSIX spawn returns the error code directly
 * — checking {@code if (ret < 0)} silently misses ENFILE (23).
 *
 ...
 * simulating normal operation during node saturation buildup); subsequent test methods exercise
 * the ENFILE path automatically. Set {@link #successesBeforeFailure} based on the estimated
 * {@code fs.file-max} minus the current node file count divided by the per-spawn file-table entry
 * contribution.
 *
 ...
 * saturation curves; 0 means the node is saturated before the first spawn (pre-existing saturation).
