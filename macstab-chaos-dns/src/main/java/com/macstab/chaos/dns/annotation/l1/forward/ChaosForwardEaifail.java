...
/**
 * Injects {@code EAI_FAIL} into every {@code getaddrinfo(3)} call (forward DNS lookup), causing the
 * call to return {@code EAI_FAIL} as if the authoritative name server returned a permanent failure
 * response.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (selectorKind = {@code FORWARD}, errno = {@code
 * EAI_FAIL}) tuple. This annotation always fires on every intercepted forward lookup — there is no
 * per-call probability field. Use it when you need every resolution attempt to fail permanently so
 * that the application's hard-failure code paths are exercised. No runtime selector-errno
 ...
 *       applications that treat all DNS errors as transient will over-retry and exhaust their retry
 ...
 * {@code SERVFAIL} responses that the resolver treats as authoritative. It also occurs when the DNS
 * search domain is misconfigured and no matching zone exists.
 *
 ...
 * permanent error — specifically when {@code h_errno} is set to {@code NO_RECOVERY}. Unlike {@code
 * EAI_NONAME} (which signals "host not found"), {@code EAI_FAIL} indicates a resolver-level
 * infrastructure failure rather than a legitimate NXDOMAIN response.
 *
 * <p>Java's {@code InetAddress.getByName()} maps {@code EAI_FAIL} to an {@code
 * UnknownHostException} with a message that does not distinguish it from {@code EAI_NONAME}.
 * Application code that relies solely on the exception type cannot tell permanent resolution
 * failures from transient ones. This injection makes that limitation visible: an application that
 * retries {@code UnknownHostException} indefinitely will loop forever under this injection,
 * revealing a missing max-retry guard.
 *
 * <p>Kubernetes pods that use DNS for service discovery (e.g. connecting to {@code
 * my-service.default.svc.cluster.local}) will receive {@code EAI_FAIL} if the in-cluster DNS
 * infrastructure (CoreDNS) is unable to answer the query at all. Injecting this error at the
