 ...
 * {@code madvise} — with a single rule. Use narrower selectors ({@code MMAP_ANON}, {@code
 * MMAP_FILE}, {@code MPROTECT}, {@code MUNMAP}, {@code MADVISE}) for targeted fault isolation; use
 * {@code WILDCARD} when you need to verify that a process handles comprehensive, simultaneous VM
 * argument validation failures across all memory-management paths.
 *
 ...
 *   <li>On each intercepted call the interposer runs a Bernoulli trial with probability {@link
 *       #probability}.
 *   <li>When the trial fires, the interposer sets {@code errno = EINVAL} and returns {@code -1} (or
 *       {@code MAP_FAILED} for {@code mmap}) without issuing the real kernel call.
 *   <li>The calling code receives: {@code -1} / {@code MAP_FAILED} return, {@code errno} 22, {@code
 *       strerror}: "Invalid argument"; no memory operation is performed.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li>Every intercepted VM syscall returns {@code EINVAL} with the configured probability; the
 *       application must handle the failure on every memory-management path simultaneously rather
 *       than only the specific syscall under test.
 *   <li>Applications that perform heap allocation ({@code mmap}), segment rotation ({@code
 *       munmap}), JIT code protection ({@code mprotect}), and readahead hints ({@code madvise}) in
 *       the same code path will encounter cascading {@code EINVAL} failures; assert that no
 *       single-path failure causes unhandled exceptions that mask failures on other paths.
 ...
 *       exercises all paths and exposes missing per-syscall error attribution.
 * </ul>
 *
 * Production failure mode: a kernel upgrade tightens argument validation (e.g. Linux 4.17 made
 * {@code mmap} reject negative {@code length} values explicitly; Linux 5.10 tightened {@code
 * mprotect} flags validation) causing previously-accepted calls to return {@code EINVAL}; this
 * wildcard annotation simulates that environment-wide validation change for all VM syscalls
 ...
 * rule that is evaluated for every intercepted function call before selector-specific rules. This
 * means a wildcard rule with {@code probability = 0.001} has a 0.1% chance of firing on every
 * {@code mmap}, {@code munmap}, {@code mprotect}, and {@code madvise} call independently. If the
 * process makes 1000 VM syscalls per second — typical for a JVM during GC warmup — the expected
 * fault rate is 1 per second distributed across all four syscall types.
 *
 ...
 * that allocates memory with {@code mmap}, immediately calls {@code madvise(MADV_WILLNEED)} on the
 * region, and then calls {@code mprotect} to set execute permissions will encounter all three
 ...
 * broadest available scope — it is most useful for initial coverage sweeps (verifying that at least
 * some error handling exists on all paths) before moving to targeted selectors for specific failure
 * mode validation.
 *
 * <h2>Example</h2>
 *
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2 for initial coverage sweeps; values above
 * 0.1 will cause cascading failures across all VM paths simultaneously and may prevent the process
 * from starting (mmap during dynamic linking may fail). Values at 1.0 will crash the process
 * immediately during the dynamic linker's own memory allocations.
 *
