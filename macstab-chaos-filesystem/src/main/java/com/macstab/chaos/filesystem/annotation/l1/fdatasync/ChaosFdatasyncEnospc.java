 ...
 * {@code errno = ENOSPC} as if the kernel ran out of disk space while allocating blocks needed
 * to flush the file's dirty data pages to the storage device.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (operation = {@code FDATASYNC}, errno = {@code ENOSPC})
 * tuple. A Bernoulli trial with probability {@link #probability} is run on each intercepted
 * {@code fdatasync} call; when it fires the interposer returns {@code -1} with {@code errno = ENOSPC}
 ...
 *   <li>Database engines that use {@code fdatasync} for WAL record durability must handle
 *       {@code ENOSPC} by aborting all transactions that were waiting for this fdatasync; assert
 *       that the engine does not acknowledge any commits when the fdatasync returned {@code ENOSPC}.
 ...
 *       alert and transitions to a read-only or error state, preventing new writes from accumulating
 *       unacknowledged data.
 * </ul>
 *
 * <p>In production, {@code ENOSPC} from {@code fdatasync} occurs on copy-on-write filesystems
 * (Btrfs, ZFS) where the fdatasync triggers the allocation of new on-disk blocks for CoW pages
 * that were deferred at write time, and on filesystems with delayed allocation where the block
 ...
 * deferred allocation fails because the filesystem has no free blocks, the fdatasync returns
 * {@code ENOSPC} even though the preceding write calls succeeded.
 *
 * <p>This deferred allocation ENOSPC is the most common real-world scenario for fdatasync
 * {@code ENOSPC}: the application writes data, the filesystem accepts it into the page cache
 * without allocating blocks (relying on free space estimates that may be optimistic), and then
 * the fdatasync fails when the actual block allocation is attempted. Applications must handle
 * this case even if they check free disk space before writing.
 *
 * <p>Java's {@code FileChannel.force(false)} calls {@code fdatasync(2)} on Linux. When the
 * underlying call returns {@code ENOSPC}, Java throws an {@code IOException} with the message
 * "No space left on device". Application code must propagate this to the transaction layer.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
