 ...
 * {@code every interposed process syscall} calls succeed, then fail every subsequent call with {@code EINTR}.
 *
 * <p><strong>What this simulates:</strong> deterministic post-warmup failure on every interposed process syscall — useful as a test fixture for failure paths that only trigger after the application has settled past startup.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
