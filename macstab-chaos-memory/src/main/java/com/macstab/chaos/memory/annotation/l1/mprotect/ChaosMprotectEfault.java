...
/**
 * Injects {@code EFAULT} into {@code mprotect} calls intercepted by libchaos-memory, causing the
 * calling code to observe a bad-address failure when attempting to change the protection attributes
 * of a memory region.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-memory primitive — one (selector = {@code MPROTECT}, errno = {@code EFAULT}) tuple.
 ...
 *       not access the region with the assumed (but not granted) protection.
 ...
 *       rather than a crash.
 *   <li>Assert that the application surfaces a structured error message that includes the address
 *       and length passed to {@code mprotect} so operators can diagnose stale address calculation
 *       bugs from logs.
 * </ul>
 *
 * Production failure mode: a native code generator or JIT that caches the base address of a code
 * arena passes a stale address to {@code mprotect} after the arena has been unmapped and
 * reallocated by the runtime; the kernel returns {@code EFAULT} because the stale address no longer
 * refers to a live mapping, causing the protection change to silently fail.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX specifies {@code EFAULT} for {@code mprotect} when the {@code addr} parameter points to
 * an address not in the process's accessible address space, or when {@code addr + len} overflows
 * the address space. On Linux, the kernel validates the range in {@code mprotect_fixup} by walking
 * the VMA list; if any part of {@code [addr, addr+len)} is not covered by a VMA, the kernel returns
 * {@code -EFAULT}.
 *
 * <p>Real {@code EFAULT} from {@code mprotect} is rare in well-structured code because the caller
 * always obtains the address from a prior {@code mmap} return value. The failure mode arises when
 * the address is computed indirectly: a JIT that stores the start address of a code region in a
 * field and later passes it to {@code mprotect} for the W-to-X flip can produce a stale address if
 * a concurrent GC or arena reset unmapped the region between the store and the {@code mprotect}
 * call. This race is extremely difficult to reproduce in normal testing.
 *
 * <p>An arithmetic path to {@code EFAULT} exists when {@code addr + len} wraps around the 64-bit
 * address space: if {@code addr} is near {@code ULONG_MAX} and {@code len} is large, the addition
 * overflows, and the kernel returns {@code EFAULT}. This is a latent integer overflow in the
 * address-range computation that only manifests with very large mappings. Languages that use 32-bit
 * integers for memory sizes and promote them to 64-bit without sign-extension are at risk.
 *
 * <p>Compared with {@code EINVAL}: {@code EFAULT} occurs when the address range is not accessible
 * (structural VMA gap or address overflow); {@code EINVAL} occurs when the protection flags are
 * structurally invalid (e.g. {@code PROT_GROWSDOWN | PROT_EXEC} on an architecture that doesn't
 * support it). Both are non-transient and indicate a programming error in the calling code.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * denies all protection transitions, causing process abort during JVM startup when the JIT code
 * cache cannot be made executable.
 *
