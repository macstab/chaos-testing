 ...
 * ENOMEM} on every subsequent call, causing the calling code to observe an out-of-memory failure
 ...
 *       application detects this threshold and escalates to a memory-pressure alert.
 *   <li>Assert that the application does not call {@code waitpid} on an uninitialised pid after
 *       post-threshold ENOMEM — POSIX does not define the pid value when spawn fails.
 * </ul>
 *
 * Production failure mode: a job runner uses {@code posix_spawn} to launch worker processes; the
 * Kubernetes node is under memory pressure from OOM-protected workloads; after N successful spawns
 * the kernel slab allocator cannot satisfy the spawn's internal structure allocation; the runner
 * applies the same short retry interval as for EAGAIN, which is insufficient for the duration of
 * the memory pressure event, causing repeated failed retries that worsen the load.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>FAIL_AFTER models the kernel slab exhaustion threshold: the slab allocator's free lists drain
 * as the system creates more spawn-internal structures; after N spawns the allocator is exhausted
 * and all subsequent spawns fail with ENOMEM. POSIX spawn returns the error code directly —
 * checking {@code if (ret < 0)} misses ENOMEM (12). ENOMEM from posix_spawn leaves the calling
 * process in a clean state (no child resources allocated). Retry strategy must be longer than for
 * EAGAIN: ENOMEM may persist for minutes during node-level OOM reclaim, while EAGAIN self-heals
 * when children exit. The counter does not reset at class scope, enabling sequential testing of the
 * success and ENOMEM-with-longer-backoff phases.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
