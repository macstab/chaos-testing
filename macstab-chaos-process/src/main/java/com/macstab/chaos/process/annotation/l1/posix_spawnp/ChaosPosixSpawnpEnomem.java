 ...
 *   <li>The calling code receives: return value {@code ENOMEM} (12),
 *       {@code strerror}: "Cannot allocate memory"; glibc's internal spawn helper or the kernel
 *       could not allocate the structures required to create a new process; no child is created
 *       and the calling process is in a clean state.</li>
 * </ol>
 *
 ...
 *       waitpid on an uninitialised pid may block indefinitely or wait on an unrelated process.</li>
 ...
 *       warrants short exponential backoff; ENOMEM warrants longer intervals with memory alert.</li>
 * </ul>
 ...
 * glibc's spawn helper cannot allocate the internal communication pipe; posix_spawnp returns ENOMEM;
 * the service applies the same short retry as for EAGAIN, issuing repeated failed spawn attempts
 * that consume memory in their own right and worsen the node's memory pressure.
 *
 * <h2>Deep technical dive</h2>
 * <p>{@code ENOMEM} from {@code posix_spawnp} originates from the same glibc internal structure
 * as {@code posix_spawn}: glibc's helper allocates an internal parent-child error-reporting pipe
 * and various working buffers before the fork; when the slab allocator is exhausted, these
 * allocations fail with ENOMEM and the spawn returns the error code directly. The {@code $PATH}
 * search that distinguishes spawnp from spawn occurs before these allocations — if the PATH search
 * exhausts memory via its directory traversal, the error may also appear as ENOMEM, though the
 * PATH search is typically lightweight.
 *
 * <p>POSIX spawn returns the error code directly — checking {@code if (ret < 0)} or
 * {@code if (ret == -1)} silently misses ENOMEM (12). Code that tests {@code if (ret != 0)} is
 * correct. ENOMEM (12) and EAGAIN (11) are adjacent integers; off-by-one errors in switch
 * statements can cause misclassification between these two errnos.
 *
 * <p>Unlike EAGAIN (which self-heals when children exit), ENOMEM persists until the kernel's
 * memory reclaim makes free memory available. Retry intervals should be substantially longer than
 * for EAGAIN — a minimum of several seconds before the first retry, with exponential backoff
 * capped at a value that does not worsen node memory pressure. Under real node OOM conditions,
 * the kernel OOM killer may terminate processes concurrently with the application's retries;
 * the application must handle SIGKILL of child processes during this window.
 *
 ...
 * using posix_spawnp do not need fork-error cleanup; they only need to avoid calling waitpid on
 * the uninitialised pid parameter.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
