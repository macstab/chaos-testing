 ...
 *   <li>The calling code sees the same result it would from a real security-policy denial: {@code
 *       MAP_FAILED} return, {@code errno} 13, {@code strerror}: "Permission denied".
 * </ol>
 *
 ...
 *       check the return value log "Permission denied" or equivalent.
 *   <li>Allocators backed by anonymous mappings ({@code malloc}, JVM direct allocators) treat
 *       {@code EACCES} the same as {@code ENOMEM} — allocation returns {@code NULL} / {@code
 *       OutOfMemoryError}.
 *   <li>Assert that the application returns a structured error rather than a crash or silent data
 *       corruption.
 * </ul>
 *
 * Production failure mode: LSM policies (SELinux, AppArmor) or seccomp profiles applied to a newly
 * deployed container revision can silently deny {@code mmap} for anonymous mappings, causing the
 * process to crash or behave incorrectly instead of reporting a clear error.
 *
 ...
 * AppArmor, SMACK) denies the {@code mmap_file} hook even for anonymous regions or when the process
 * capability set is insufficient for the requested protection (e.g. {@code PROT_EXEC} on a system
 * with strict W^X policies via {@code vm.mmap_min_addr} or Yama restrictions).
 *
 * <p>In production, {@code EACCES} on anonymous {@code mmap} is most commonly observed when a new
 * container image ships a policy change that tightens seccomp or LSM rules, or when a Kubernetes
 * PodSecurityPolicy / Pod Security Admission controller drops capabilities that the runtime
 * previously relied on. The failure is silent unless the application explicitly checks the return
 * code of allocator primitives.
 *
 * <p>glibc's {@code malloc} propagates the failure to the caller as a {@code NULL} return; the JVM
 * raises {@code OutOfMemoryError} for direct buffers and code-cache expansions. Application
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-4 to 1e-3 mirrors realistic LSM-denial rates; 1.0
 * prevents the container process from completing startup.
 *
