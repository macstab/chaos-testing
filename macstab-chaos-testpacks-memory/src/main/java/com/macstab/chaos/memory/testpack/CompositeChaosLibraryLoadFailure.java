 ...
 * file-backed {@code mmap} calls. When the dynamic linker ({@code ld-linux}) calls
 * {@code mmap(MAP_PRIVATE, fd)} to map a shared library's ELF segments, a {@code EBADF} return
 * causes {@code dlopen()} to fail with {@code "invalid argument"} or a libc-internal error,
 * propagating as a {@code java.lang.UnsatisfiedLinkError} in the JVM or a {@code null} return from
 * native {@code dlopen()} in C/C++ applications.
 *
 ...
 * plugin loaders, JNI libraries, Python extension modules, and any component that lazily loads shared
 * libraries at runtime will all fail. Services with static dependencies that are loaded at start-up
 * may fail to start entirely. Operator intervention (correcting the library configuration or
 * restarting with a patched image) is required.
 *
 ...
 * described in the System V ABI and the GNU libc dynamic linker source
 * ({@code elf/dl-load.c}). JNI library loading failure is documented in the Java Native Interface
 * Specification (section 2.4). Plugin-loader error-path testing is a recurring theme in the Netflix
 * Chaos Engineering corpus.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
