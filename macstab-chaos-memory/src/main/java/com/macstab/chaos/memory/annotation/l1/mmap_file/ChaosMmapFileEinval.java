 ...
 * causing the calling code to observe an invalid-argument failure when attempting to establish a
 * file-backed memory mapping.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-memory primitive — one (selector = {@code MMAP_FILE}, errno = {@code EINVAL}) tuple.
 ...
 *       {@code EINVAL} is treated as fatal for the mapping attempt, not as a transient failure.
 *   <li>Assert that the file descriptor is closed and any partially allocated resources are
 *       released before propagating the error — resource leaks after {@code EINVAL} are common.
 * </ul>
 *
 ...
 * {@code flags} contains both {@code MAP_PRIVATE} and {@code MAP_SHARED} (or neither); {@code addr}
 * is not a multiple of the page size (when non-zero); or {@code offset} is not a multiple of the
 * page size. The kernel validates all four conditions in {@code do_mmap_pgoff} before allocating
 * the VMA.
 *
 ...
 * the offset from file size, record counts, or block numbers must explicitly align down to the page
 * boundary with {@code offset & ~(PAGE_SIZE - 1)}. Arithmetic underflow or off-by-one in this
 * computation can produce a non-page-aligned value, causing {@code EINVAL} on the first mapping
 * that exercises the computed offset.
 *
 * <p>Kernel version differences in {@code EINVAL} triggering are significant: Linux 4.x added
 * stricter flag validation for {@code MAP_HUGETLB} with file-backed mappings, requiring the file to
 * have been opened with the hugepage filesystem flag. Applications that attempt to map regular
 * files with {@code MAP_HUGETLB} will receive {@code EINVAL} on kernel 4.x and later but may have
 * succeeded on earlier kernels. This class of regression is invisible in development environments
 * running older kernels.
 *
 * <p>Compared with {@code EBADF}: {@code EINVAL} is an argument-validity failure (the parameters to
 * the call are wrong); {@code EBADF} is a descriptor-validity failure (the fd itself is wrong).
 * Both are non-transient for the given arguments; recovery requires correcting the mapping
 * parameters rather than the fd. Unlike {@code EBADF}, {@code EINVAL} typically indicates a
 * programming error in the calling code rather than a runtime race.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2 to exercise the argument-validation error
 * path; rates above 0.1 will prevent shared-library loading and cause process abort.
 *
