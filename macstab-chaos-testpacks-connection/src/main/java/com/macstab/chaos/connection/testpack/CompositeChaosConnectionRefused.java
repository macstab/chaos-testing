 ...
 * perspective the service is completely down: no connection is ever established, and every attempt
 * is rejected immediately with a TCP RST. Applications that rely on connection-pool warm-up or that
 * do not implement circuit breakers will spam repeated connect calls until their thread pool is
 * exhausted.
 *
 * <h2>How it's created</h2>
 *
 * <p>Applies {@code NetRule.errno(Endpoint.wildcard(), NetOperation.CONNECT, Errno.ECONNREFUSED,
 * toxicity)} via libchaos-net. In production this happens when the target process has crashed but
 * its container is still scheduled, when a firewall sends TCP RST instead of silently dropping
 * packets, or when a Kubernetes {@code Service} has no ready endpoints and the kernel rejects the
 * SYN immediately.
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Severe</strong><br>
 * Every connection attempt fails instantly. Connection pools cannot warm, health checks fail, and
 * circuit breakers must open to prevent cascading retry storms. Services without circuit breakers
 * will exhaust their thread pools within seconds. Operator intervention — or automatic
 * circuit-break recovery — is required.
 *
 * <h2>Industry references</h2>
 *
 * <p>TCP RST on connect as a standard connection-refused scenario is documented in W. Richard
 * Stevens, <em>Unix Network Programming</em> (3rd ed.), §5.4: "Connection Refused". ECONNREFUSED is
 * the canonical kernel response when no process is listening on the target port.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
