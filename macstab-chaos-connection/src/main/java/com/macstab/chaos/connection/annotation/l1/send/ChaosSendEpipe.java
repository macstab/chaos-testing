...
/**
 * Injects {@code EPIPE} on every libchaos-intercepted {@code send} call inside
 * the target container, making the call fail as if the kernel returned {@code EPIPE}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EPIPE}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EPIPE} is a valid POSIX result of
 * {@code send}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EPIPE} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: broken pipe — write to closed peer; very common in disconnect scenarios.
 *
 ...
 * class. Use the repeatable form ({@code @ChaosSendEpipes}) to bind different probabilities to
