 ...
 * {@code EINVAL} on every subsequent call, causing the calling code to observe a persistent
 ...
 *       programming error — spawn attribute or file-action structure contains an invalid value
 *       that must be fixed in code, not retried.</li>
 *   <li>FAIL_AFTER models the attribute configuration regression: N successful spawns occur using
 *       a valid configuration; after N spawns a hot-reload or dynamic reconfiguration produces an
 ...
 *       the invalid attribute values for operator debugging.</li>
 ...
 *       that no retry loop is applied to EINVAL failures.</li>
 * </ul>
 * Production failure mode: a command executor builds {@code posix_spawnattr_t} structures from
 * a serialised configuration; a configuration hot-reload changes the scheduling policy field to
 * an integer that is not a valid POSIX scheduling policy; after the reload all subsequent spawns
 ...
 * EINVAL. Real EINVAL from posix_spawnp is not probabilistic — it fires deterministically once
 * the invalid attribute is present and persists until the attribute is corrected. POSIX spawn
 * returns the error code directly — checking {@code if (ret < 0)} silently misses EINVAL (22).
 *
 * <p>Sources of EINVAL in posix_spawnp include: invalid scheduling policy in {@code posix_spawnattr_t}
 * (value not SCHED_OTHER, SCHED_FIFO, or SCHED_RR); scheduling parameter out of range for the
 * policy; invalid signal number in the signal mask; invalid fd number in
 * {@code posix_spawn_file_actions_t}; invalid flags in the file-action sequence. The {@code $PATH}
 * search that distinguishes spawnp from spawn does not itself produce EINVAL; EINVAL comes from
 * the attribute/action validation step that occurs after the binary is found.
 *
 ...
 * failing spawn, enabling rapid identification of the invalid field after a configuration regression.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * spawns expected before the configuration regression takes effect; values 5–50 cover most hot-reload
 * scenarios; 0 means every spawn is invalid from the first call.
