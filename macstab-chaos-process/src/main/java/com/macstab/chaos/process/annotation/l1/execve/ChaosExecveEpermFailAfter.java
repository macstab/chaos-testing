 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code EXECVE}, errno = {@code EPERM}, effect =
 ...
 * with {@code errno = EPERM} until the rule is removed. This models scenarios where a security
 * policy tightening (seccomp profile update, capability drop via {@code prctl}, no-new-privs
 * enforcement) takes effect after the container has already successfully executed some privileged
 * binaries.
 *
 ...
 *       intercepted call sets {@code errno = EPERM} and returns {@code -1} without executing.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 1, {@code strerror}: "Operation
 *       not permitted"; the counter remains tripped until removal.
 * </ol>
 *
 ...
 *       and disable all features that require privileged exec rather than retrying indefinitely.
 ...
 *       when the helper binary starts returning {@code EPERM}, ceases to queue new spawn requests
 *       that will fail, and surfaces a clear capability-denied alert.
 *   <li>Assert that the application does not cache a "privileged exec is permitted" assumption from
 *       the early successful calls — each exec attempt must check the return value and respond to
 *       {@code EPERM} even if previous exec calls to the same binary succeeded.
 * </ul>
 *
 * Production failure mode: a Kubernetes operator applies a stricter security context mid-run (e.g.
 * patching the pod's security policy to set {@code allowPrivilegeEscalation: false} via a
 * MutatingWebhook); the running container receives the new {@code prctl(PR_SET_NO_NEW_PRIVS)}
 ...
 *
 * <p>The FAIL_AFTER effect for exec-EPERM captures the policy-hardening mid-run scenario where a
 * security constraint is applied to a running container after it has already successfully completed
 * some privileged exec calls. This is distinct from the probabilistic ERRNO variant (which fires
 * randomly, simulating intermittent policy enforcement) and from a startup-time configuration error
 * (which the zero-threshold variant captures).
 *
 * <p>The POSIX {@code PR_SET_NO_NEW_PRIVS} prctl bit, once set, is inherited across fork and exec
 * and cannot be cleared. This makes the FAIL_AFTER model accurate: once the no-new-privs bit is
 * set, all subsequent exec calls that would escalate privileges fail with {@code EPERM} — exactly
 * the permanent-failure-after-N-successes pattern that FAIL_AFTER encodes.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
