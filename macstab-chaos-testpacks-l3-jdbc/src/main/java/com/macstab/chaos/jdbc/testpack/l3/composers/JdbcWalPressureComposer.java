...
public final class JdbcWalPressureComposer implements L3Composer<IncidentChaosJdbcWalPressure> {
    ...
    public List<Object> apply(
        ...
        final com.macstab.chaos.filesystem.api.RuleHandle fsyncHandle =
                CompositeFilesystemChaos.standard().advanced()
        ...
        final RuleHandle connHandle = CompositeConnectionChaos.standard().advanced()
                .apply(container, NetRule.latency(Endpoint.wildcard(), NetOperation.RECV, fsyncDelay, 1.0));
        ...
        final ChaosScenario scenario = ChaosScenario.builder(scenarioId)
                .description("L3 JDBC WAL pressure — FSYNC/WRITE latency + RECV back-pressure + SQLTimeoutException")
                .selector(ChaosSelector.method(EnumSet.of(OperationType.METHOD_ENTER), cls, NamePattern.any()))
                .effect(ChaosEffect.injectException("java.sql.SQLTimeoutException", "WAL sync timeout exceeded"))
        ...
        for (final Object h : handles) {
            if (h instanceof com.macstab.chaos.filesystem.api.RuleHandle ioRh) {
                ...
                try {
                    ...
                    log.warn("JdbcWalPressureComposer.removeAll: failed to remove JVM scenario {}", scenarioId, e);
                }
            }
        }
    }
    ...
    public List<String> describe(final IncidentChaosJdbcWalPressure annotation) {
        return List.of(
                ...
                "filesystem: FSYNC latency=" + annotation.fsyncDelayMs() + "ms on path=" + annotation.dataPath(),
                "filesystem: WRITE latency=" + (annotation.fsyncDelayMs() / 2) + "ms on path=" + annotation.dataPath(),
                "connection: RECV latency=" + annotation.fsyncDelayMs() + "ms toxicity=1.0",
                "jvm: injectException(java.sql.SQLTimeoutException) on class='" + annotation.classPattern() + "'",
