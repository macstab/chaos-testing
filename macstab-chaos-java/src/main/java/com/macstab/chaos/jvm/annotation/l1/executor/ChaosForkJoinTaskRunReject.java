 ...
 * java.util.concurrent.ForkJoinPool} worker the moment it begins executing a {@link
 * java.util.concurrent.ForkJoinTask} — the task's compute method never runs, and the task completes
 * exceptionally with the injected exception.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>An L1 JVM chaos primitive in the {@code EXECUTOR} selector family targeting the {@code
 * FORK_JOIN_TASK_RUN} operation with the {@code reject} effect. It intercepts the internal {@code
 * ForkJoinTask.doExec()} entry point and throws {@code RejectedExecutionException} with the
 * configured {@link #message()} before the task's {@code exec()} or {@code compute()} method is
 * called. The exception is stored as the task's exceptional result — any caller blocked on {@code
 * ForkJoinTask.get()} receives {@code ExecutionException} wrapping the injected exception.
 *
 * <p>This is the ForkJoin-specific analogue of {@link ChaosExecutorWorkerRunReject}. For {@code
 * ThreadPoolExecutor}-based pools, use that annotation instead.
 *
 ...
 *   <li>The reject effect throws {@code new RejectedExecutionException(message)} before any of the
 *       task's own code runs.
 ...
 *
 * <ul>
 *   <li>{@code task.get()} throws {@link java.util.concurrent.ExecutionException} whose {@code
 *       getCause()} is a {@code RejectedExecutionException} with the configured message.
 *   <li>{@code task.join()} throws {@link java.util.concurrent.CancellationException} or re-throws
 *       the exception directly, depending on how {@code ForkJoinTask} surfaces it.
 *   <li>The task's compute or action method never executes — no side effects occur.
 *   <li>Parallel streams backed by the common pool will throw {@code RuntimeException} wrapping the
 *       injected exception when the stream terminal operation is collected.
 *   <li>All subtasks forked by the failing task are also affected if they are submitted to the same
 *       pool — each subtask independently triggers the interceptor and is also rejected.
 * </ul>
 *
 * <p><strong>Production failure mode:</strong> a JVM that encounters an out-of-memory condition
 * during a parallel computation has its {@code ForkJoinWorkerThread}s throw {@code
 * OutOfMemoryError} from inside task bodies; caller code that only handles {@code
 * ExecutionException} (not {@code Error}) propagates the failure incorrectly — this annotation
 * injects a recoverable exception version to test the error-propagation path without requiring
 * actual OOM.
 *
 ...
 * bootstrap class loader channel is required. The method is called by {@code
 * ForkJoinWorkerThread.run()} when the worker picks up a task, and also by {@code
 * ForkJoinTask.join()} when a caller thread executes the task inline (helping mode). Both paths are
 * intercepted.
 *
 ...
 * exception handling in {@code ForkJoinTask} stores the exception via {@code
 * recordExceptionalCompletion}, which CAS-sets the status to {@code EXCEPTIONAL}. This propagates
 * to all tasks that join on this one, making the failure cascade through recursive fork/join
 * decompositions.
 *
 ...
 * allows the task to run. This annotation prevents the task from running at all. {@link
 * ChaosExecutorWorkerRunReject} does the same thing but targets {@code ThreadPoolExecutor} workers
 * — it does not affect {@code ForkJoinPool} tasks. Use this annotation specifically when the
 * application relies on {@code ForkJoinPool}.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
