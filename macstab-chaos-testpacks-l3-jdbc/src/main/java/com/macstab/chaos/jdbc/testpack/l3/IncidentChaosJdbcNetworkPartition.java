 ...
 *
 * <ul>
 *   <li>Connection: timeout on wildcard endpoint with duration {@code timeoutMs} ms at {@code
 *       toxicity} — POLL timeout simulates a network partition cutting the TCP path
 *   <li>Connection: SEND → EPIPE at {@code toxicity} — broken-pipe errors on active send operations
 *       model severed connections inside open transactions
 ...
 * In-doubt 2PC transactions block row locks; applications cannot commit or roll back; split-brain
 * risk grows with partition duration; manual DBA intervention may be required to resolve in-doubt
 * transactions.
 *
 * <h2>Industry references</h2>
 *
 * <p>Network partition during active 2PC is described in the distributed systems literature (Gray
 * &amp; Lamport "Consensus on Transaction Commit"), PostgreSQL documentation §"Two-Phase
...
@ChaosL3(
    composer = "com.macstab.chaos.jdbc.testpack.l3.composers.JdbcNetworkPartitionComposer",
