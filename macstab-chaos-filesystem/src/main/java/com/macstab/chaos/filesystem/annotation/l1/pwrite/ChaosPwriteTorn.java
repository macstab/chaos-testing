 ...
 * {@code pwrite} call; when it fires the interposer passes a randomly-chosen prefix of the buffer
 ...
 *       issues the real {@code pwrite} syscall with only the prefix bytes at the original offset,
 *       and returns the partial count to the caller. The remaining bytes are not written.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li>A torn {@code pwrite} returns a positive byte count smaller than the requested count at
 *       the requested file offset, which is valid POSIX behaviour. Applications that do not check
 *       the return value and loop to write the remaining bytes will silently leave a partial write
 *       in the file at a known offset. Assert that every {@code pwrite} loop correctly advances
 *       the offset and repeats until all bytes are written.
 ...
 *       engine verifies the full page was written (either by checking the return count or by
 *       using {@code fsync} and then re-reading the page) before marking the write as complete.
 ...
 *       does not consider the record durable until all its bytes are confirmed at the target offset.
 ...
 * </ul>
 *
 * <p>In production, short {@code pwrite} returns occur on NFS mounts when the server's write
 * window limits how many bytes can be committed in a single RPC, when an interrupt is delivered
 * during a large write and the kernel restarts with a partial count, and when a signal is delivered
 * to the writing thread and the kernel returns the partial count before restarting the operation.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX specifies that {@code pwrite(2)} may return a count smaller than the requested count
 * (a "short write") for any file descriptor type, without this being an error. The returned count
 ...
 *
 * <p>The torn write injection specifically targets the {@code pwrite}-based page I/O path used
 * by storage engines. Unlike {@code write}, which updates the file position on each call (making
 * retry loops straightforward), {@code pwrite} requires the caller to explicitly advance the
 * offset on each retry. Storage engines that copy the offset from the page number calculation
 * and reuse it without adjustment on short-write retries will write the remaining bytes at the
 * wrong offset, overwriting previously-written data.
 *
 ...
 * count always equals {@code buf.remaining()} will silently leave partial data in the file.
 * The {@code WritableByteChannel} contract documents that the count may be less than the buffer
 * limit, but many callers ignore this because local filesystem writes rarely produce short counts
