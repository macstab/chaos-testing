 ...
 *
 * <p>Injects RECV-path latency and SocketTimeoutException at METHOD_ENTER on Redis client
 * methods to reproduce the application-level experience of a Redis slow-log command backlog.
 *
...
public final class RedisSlowlogComposer implements L3Composer<IncidentChaosRedisSlowlog> {
    ...
    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosRedisSlowlog ann) {
        ...
        final var scenario = ChaosScenario.builder(scenarioId)
                .description("Redis slow-log backlog — inject SocketTimeoutException on Redis client entry")
                .selector(selector)
                .effect(ChaosEffect.injectException("java.net.SocketTimeoutException", "Redis command timeout"))
    ...
    public List<String> describe(final IncidentChaosRedisSlowlog ann) {
        return List.of(
                ...
                "jvm: SocketTimeoutException(\"Redis command timeout\") on class prefix '" + ann.classPattern() + "' (METHOD_ENTER)",
