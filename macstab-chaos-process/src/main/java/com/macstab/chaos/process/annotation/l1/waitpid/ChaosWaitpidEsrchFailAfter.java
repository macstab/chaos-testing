...
/**
 * After {@link #successesBeforeFailure} successful {@code waitpid} calls, injects {@code ESRCH} on
 * every subsequent call, modelling the scenario where a process group dissolves after N successful
 * waits and all subsequent group waits find a non-existent process group.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-process primitive — one (selector = {@code WAITPID}, errno = {@code ESRCH}, effect =
 ...
 *       returns {@code -1} with {@code errno = ESRCH}.
 *   <li>The calling code receives: return value {@code -1}, {@code errno = ESRCH} (3); the process
 *       or process group being waited for does not exist as a child of the caller.
 * </ol>
 *
 ...
 *       that the pid being tracked has become stale — the process or group has already been reaped
 *       by another waiter or has exited before the call.
 ...
 *       that the application detects this dissolution and cleans up the group's registry entry.
 *   <li>Assert that the application does not retry on ESRCH — the process/group does not exist and
 *       will not reappear; assert that the stale pid or pgid is logged for diagnostic purposes so
 *       operators can trace the premature dissolution.
 * </ul>
 *
 * Production failure mode: a process pool reaps workers using a group waitpid loop; the pool tracks
 * the process group id in a registry; after N workers are reaped the group dissolves; subsequent
 * waits return ESRCH; the pool does not clean the registry entry and continues waiting for the
 * dissolved group; the registry entry remains stale indefinitely, preventing the pool from creating
 * a new group with the same id.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>FAIL_AFTER models the process-group-dissolution threshold: N group members are reaped; after N
 * successful waits all members have exited and their zombies have been collected; the group
 * dissolves; subsequent group waits return ESRCH. Real ESRCH from this source is deterministic — it
 * fires on the (N+1)th wait and persists. waitpid returns -1 and sets errno; checking {@code if
 * (ret == -1 && errno == ESRCH)} is correct.
 *
 ...
 * exists and has members); subsequent test methods exercise the ESRCH-group-dissolved phase. Set
 * {@link #successesBeforeFailure} to the number of group members expected to be reaped before the
 * group dissolves.
 *
 * <p>ESRCH from waitpid on a specific positive pid is distinct from ESRCH on a negative pgid. For a
 * specific pid, ESRCH means the process does not exist at all (it may have been reaped by a
 * concurrent waiter). For a pgid, ESRCH means no process in the group is a child of the caller.
