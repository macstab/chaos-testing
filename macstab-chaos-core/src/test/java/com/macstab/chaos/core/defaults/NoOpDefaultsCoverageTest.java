 ...
 *
 * <p>Each NoOp throws {@link ChaosProviderNotFoundException} for active operations and returns
 * false/no-op for isSupported/installTools/reset.
 *
...
class NoOpDefaultsCoverageTest {
  ...
  class NoOpNetworkChaosTest {
    ...
    @Test
    void injectLatency_throws() {
      assertThatThrownBy(() -> sut.injectLatency(container, Duration.ofMillis(100)))
          .isInstanceOf(ChaosProviderNotFoundException.class);
    }

    @Test
    void injectLatencyWithJitter_throws() {
      assertThatThrownBy(
              () ->
                  sut.injectLatencyWithJitter(
                      container, Duration.ofMillis(100), Duration.ofMillis(10)))
          .isInstanceOf(ChaosProviderNotFoundException.class);
    }

    @Test
    void injectPacketLoss_throws() {
      assertThatThrownBy(() -> sut.injectPacketLoss(container, 0.1))
          .isInstanceOf(ChaosProviderNotFoundException.class);
    }

    @Test
    void injectCorrelatedPacketLoss_throws() {
      assertThatThrownBy(() -> sut.injectCorrelatedPacketLoss(container, 0.1, 0.5))
          .isInstanceOf(ChaosProviderNotFoundException.class);
    }

    @Test
    void limitBandwidth_throws() {
      assertThatThrownBy(() -> sut.limitBandwidth(container, "100mbit"))
          .isInstanceOf(ChaosProviderNotFoundException.class);
    }

    @Test
    void partitionFrom_throws() {
      assertThatThrownBy(() -> sut.partitionFrom(container, container))
          .isInstanceOf(ChaosProviderNotFoundException.class);
    }

    @Test
    void installTools_noOp() {
      assertThatCode(() -> sut.installTools(container)).doesNotThrowAnyException();
    }

    @Test
    void reset_noOp() {
      assertThatCode(() -> sut.reset(container)).doesNotThrowAnyException();
    }
  ...
  class NoOpCacheChaosTest {
    ...
    @Test
    void injectMisses_noOp() {
      assertThatCode(() -> sut.injectMisses(container, "*", 0.5)).doesNotThrowAnyException();
    }

    @Test
    void slowResponse_noOp() {
      assertThatCode(() -> sut.slowResponse(container, Duration.ofMillis(100)))
          .doesNotThrowAnyException();
    }

    @Test
    void forceEviction_noOp() {
  ...
  class NoOpFilesystemChaosTest {
    ...
    @Test
    void injectPermissionErrors_noOp() {
      assertThatCode(() -> sut.injectPermissionErrors(container, "/tmp", 0.5))
          .doesNotThrowAnyException();
    }
  ...
  class NoOpConnectionChaosTest {
    ...
    @Test
    void addLatency_noOp() {
      assertThatCode(() -> sut.addLatency(container, "db", Duration.ofMillis(100)))
          .doesNotThrowAnyException();
    }

    @Test
    void dropPackets_noOp() {
      assertThatCode(() -> sut.dropPackets(container, "db", 0.1)).doesNotThrowAnyException();
    }

    @Test
    void limitBandwidth_noOp() {
      assertThatCode(() -> sut.limitBandwidth(container, "db", 1_000_000L))
          .doesNotThrowAnyException();
    }

    @Test
    void timeoutConnections_noOp() {
      assertThatCode(() -> sut.timeoutConnections(container, "db", Duration.ofSeconds(1)))
          .doesNotThrowAnyException();
    }

    @Test
    void slowClose_noOp() {
      assertThatCode(() -> sut.slowClose(container, "db", Duration.ofMillis(500)))
          .doesNotThrowAnyException();
    }

    @Test
    void rejectConnections_noOp() {
