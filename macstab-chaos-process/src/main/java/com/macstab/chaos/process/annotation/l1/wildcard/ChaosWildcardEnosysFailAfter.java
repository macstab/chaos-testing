 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code WILDCARD}, errno = {@code ENOSYS}, effect
 ...
 *       with {@code errno = ENOSYS}.
 *   <li>The calling code receives: {@code fork()}/{@code execve()} return {@code -1} with {@code
 *       errno = ENOSYS} (38); {@code posix_spawn}/{@code pthread_create} return {@code ENOSYS}
 *       directly; {@code strerror(ENOSYS)}: "Function not implemented".
 * </ol>
 *
 ...
 *       ENOSYS is permanent and non-retryable; no amount of waiting will restore the syscall.
 *   <li>FAIL_AFTER models a seccomp-profile tightening scenario: N process-management calls succeed
 *       while the old seccomp profile is active; a hot seccomp profile reload blocks all
 *       process-management syscalls; all subsequent calls return ENOSYS — assert that the
 *       application detects the sudden ENOSYS onset and alerts operators immediately.
 ...
 *       stops and the pool degrades to existing threads only; assert that health checks that spawn
 *       subprocesses return a DEGRADED status rather than a HEALTHY status.
 * </ul>
 *
 * Production failure mode: a container runtime applies a hot seccomp profile update that blocks the
 * clone syscall used by fork and pthread_create; the application receives ENOSYS from all
 * process-management operations starting at the Nth call; it does not distinguish ENOSYS from EPERM
 * and applies a retry loop; the retry loop spins at high rate consuming CPU without ever
 ...
 * (implemented but blocked by MAC policy). Real ENOSYS from process-management syscalls occurs on:
 * WSL1 (which does not implement the full Linux syscall table), seccomp-BPF policies that return
 * ENOSYS rather than EPERM for blocked syscalls (to avoid leaking information about policy scope),
 * and QEMU user-mode emulation of cross-architecture binaries where the host kernel lacks a
 * translated syscall implementation.
 *
 * <p>The WILDCARD FAIL_AFTER counter is shared across all intercepted syscall families. For N fork
 * calls + M pthread_create calls + P posix_spawn calls, the counter charges all of them together.
 * Set {@link #successesBeforeFailure} to the total number of process-management calls (across all
 * families) that the application makes during its startup and first-request phase, so that the
 * ENOSYS phase begins during steady-state operation.
 *
 * <p>The counter does not reset between test methods when the annotation is at class scope. This
 * enables sequential testing: the first test method exercises N successful process-management calls
 * (the pre-restriction phase); subsequent test methods exercise the ENOSYS phase where all process
 * management is blocked. The test class should verify both that the application operates correctly
 * before the restriction and that it degrades gracefully after.
 *
 * <p>ENOSYS is permanent: unlike EAGAIN (retryable) or ENOMEM (retryable after GC), ENOSYS will not
 * resolve. The application must detect the ENOSYS onset, stop all process-management attempts,
 ...
 * init + first-request phases; 0 means ENOSYS fires on the very first process call (blocks startup
 * entirely — useful for testing startup-failure handling).
 *
