 ...
 * EXECUTOR_AWAIT_TERMINATION} operation with the {@code gate} effect. Unlike {@link
 * ChaosExecutorAwaitTerminationDelay}, which applies a fixed random sleep and then releases
 ...
 * in-flight tasks are still running, that metrics are emitted, or that health checks transition to
 * a specific state before the drain is allowed to complete.
 *
 ...
 *   <li>Alternatively, if {@link #maxBlockMs} elapses, the latch times out and the thread proceeds
 *       automatically.
 ...
 *       or {@link #maxBlockMs} ms pass — the call is synchronously blocked before its timeout even
 *       starts counting.
 ...
 * during rolling deployment, causing the application's shutdown hook to call {@code
 * awaitTermination} before the mesh has deregistered the pod — requests continue arriving after the
 * drain window starts, extending actual termination time beyond the configured budget.
 *
 ...
 * budget begins at gate-release time, not at call time — the total block can exceed {@code timeout
 * + maxBlockMs} if both fire at their maximum.
 *
 * <p><strong>Gate release and multiple callers.</strong> If multiple threads call {@code
 * awaitTermination} concurrently and all hit the gate, a single release signal unblocks all of them
 * simultaneously (the latch counts down to zero). If precise per-thread release control is needed,
 * configure separate rules with distinct {@link #id()} values.
 *
 * <p><strong>Distinguishing from siblings.</strong> {@link ChaosExecutorAwaitTerminationDelay}
 * applies a fixed sleep and releases automatically — no test coordination is required. The gate
 * requires an explicit release but enables the test to assert on state between the shutdown and the
 * drain — the key use-case where precise timing matters.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
