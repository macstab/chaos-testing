 ...
 *   <li>Circuit breakers that open on {@code IOException} will trip; assert that the open state is
 *       correctly propagated to callers (fallback response, metric increment).
 *   <li><strong>Production failure mode:</strong> a downstream service refuses connections (e.g.
 *       firewall rule tightened at 02:00); the application's circuit breaker opens, the fallback
 *       path activates, but if the fallback also calls the same service the error cascades — an
 *       incident begins.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The reject effect intercepts {@code jdk.internal.net.http.HttpClientImpl#send} via Byte Buddy
 * before the method creates or acquires a connection. Because the throw happens before any socket
 * work, the JDK's internal connection pool is untouched: no connections are borrowed, no half-open
 * TCP sockets are left behind, and no file descriptors are leaked. This makes the fault perfectly
 * idempotent and safe to fire repeatedly in the same JVM process.
 *
 ...
 * wraps the IOException in a framework-specific exception (e.g. Spring's {@code
 * ResourceAccessException}), assert against that wrapper type in tests.
 *
 * <p>The difference between {@code Reject} and {@link ChaosHttpClientSendInjectException} is
 * intent: reject uses the operation's canonical exception type (always {@code IOException} for HTTP
 * send) while inject-exception lets the test author specify an arbitrary class, useful for testing
 * handlers that distinguish between timeout exceptions and connection-refused exceptions.
 *
 ...
 * the connection attempt. This means connect-timeout logic in the application is not exercised; use
 * {@link ChaosHttpClientSendDelay} with a very long delay to test that path.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
