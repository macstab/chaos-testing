 ...
 *       accepting; all JDBC connection pool acquisition attempts fail immediately; pool exhaustion
 *       follows as borrowing threads wait for a connection that never arrives
 ...
 *   <li>JVM: DataAccessResourceFailureException injected at METHOD_ENTER on class prefix {@code
 *       classPattern} — reproduces the Spring Data exception raised when the data source is
 *       unreachable, exercising the circuit breaker and readiness indicator logic
 * </ul>
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Critical</strong><br>
 * DB gone → Resilience4j circuit breaker opens → readiness probe fails → pod removed from load
 * balancer rotation; if the database recovers before the circuit half-opens, backlog of queued
 * requests is served in a burst.
 *
 * <h2>Industry references</h2>
 *
 * <p>Database outage → circuit breaker pattern → readiness probe failure is the canonical
 * Resilience4j + Spring Boot Actuator failure scenario, documented in the Resilience4j Spring Boot
 * starter guide and Spring Boot Actuator health indicator documentation.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
@ChaosL3(
    composer = "com.macstab.chaos.springboot.testpack.l3.composers.SpringDatabaseOutageComposer",
