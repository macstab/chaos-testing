 ...
 *       temporarily unavailable"; no child process is created and the pid output parameter is not
 ...
 *
 * <ul>
 *   <li>{@code posix_spawnp} returns {@code EAGAIN}; no child process is created; assert that the
 *       application checks the return value (not errno — POSIX spawn returns the error code
 ...
 *   <li>Assert that the application distinguishes {@code posix_spawnp}-EAGAIN (process table or uid
 *       quota exhausted, transient) from ENOENT (binary not found in any {@code $PATH} directory,
 *       deployment error) — retry is appropriate for EAGAIN but not for ENOENT.
 * </ul>
 *
 * Production failure mode: a pipeline tool uses {@code posix_spawnp} to invoke helper utilities by
 * name; a burst of parallel pipeline runs exhausts the uid's {@code RLIMIT_NPROC}; spawnp returns
 * EAGAIN; the tool does not check the return value and calls {@code waitpid} on the uninitialised
 * pid, which may block indefinitely or wait on an unrelated process.
 *
 ...
 * window not present in {@code posix_spawn}: the PATH search occurs in the parent before the fork,
 * which means multiple fds may be opened for directory traversal during the search. If the fd table
 * is nearly full, the PATH search itself may fail with EMFILE before the spawn's EAGAIN would
 * occur. The chaos annotation fires at the spawn API boundary, covering the case where resources
 * are exhausted after the PATH search succeeds but before the fork completes.
 *
 ...
 * implement a bounded retry with exponential backoff and apply load-shedding if the EAGAIN persists
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2; EAGAIN from posix_spawnp is transient and
