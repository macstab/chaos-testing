...
/**
 * Injects {@code EAGAIN} into {@code send(2)}, causing the call to return {@code -1} with {@code
 * errno = EAGAIN} as if the socket's send buffer is full and the kernel cannot accept any more data
 * for transmission without blocking.
 *
 ...
 * send} call; when it fires the interposer returns {@code -1} with {@code errno = EAGAIN} without
 ...
 *       assert that the back-off reduces the send rate and does not cause the sender to drop
 *       messages.
 ...
 * </ul>
 *
 * <p>In production, {@code EAGAIN} from {@code send} occurs when a slow receiver does not drain its
 * receive buffer fast enough, causing TCP's flow control to reduce the sender's window until the
 * send buffer fills. This is the normal TCP back-pressure mechanism and is a correct and expected
 * operating condition for high-throughput data transfer.
 *
 ...
 * non-blocking {@code send} returns {@code EAGAIN}. A blocking {@code send} would block until space
 * becomes available; the injection simulates the non-blocking case regardless of the actual socket
 * mode.
 *
 * <p>Event-driven frameworks (Netty, Vert.x) use edge-triggered {@code epoll} (EPOLLET) for high
 * performance; on edge-triggered sockets, {@code EAGAIN} is the signal to stop sending and wait for
 * the next EPOLLOUT event. Frameworks that use level-triggered epoll receive EPOLLOUT continuously
 * while the buffer has space, which requires a different handling strategy. This injection tests
 * that the framework's write path correctly handles the non-blocking constraint regardless of the
 * epoll mode.
 *
 * <p>Java's NIO {@code SocketChannel.write()} returns 0 when the kernel would return {@code
 * EAGAIN}, rather than throwing an exception. Application code that calls {@code write()} in a loop
 * until the buffer is consumed must handle the 0-return case by registering for {@code OP_WRITE}
