 ...
 * intercepted by libchaos-process — {@code execve}, {@code execveat}, {@code posix_spawn}, {@code
 * posix_spawnp}, and their variants — simultaneously, gated by {@link #probability}, causing any
 * process-launch call to fail as if the kernel rejected an oversized argument vector.
 *
 ...
 *   <li>When the trial fires, the interposer sets {@code errno = E2BIG} and returns {@code -1} (or
 *       the errno value directly for {@code pthread_create} and POSIX spawn functions) before the
 *       real kernel call executes.
 *   <li>The calling code receives: {@code execve}/{@code fork} return {@code -1} with {@code errno
 *       = E2BIG} (7); {@code pthread_create} and {@code posix_spawn} return {@code E2BIG} directly;
 *       {@code strerror(E2BIG)}: "Argument list too long".
 * </ol>
 *
 ...
 *       argv/envp is essential for fixing an ARG_MAX regression.
 *   <li>{@code posix_spawn}/{@code posix_spawnp} return {@code E2BIG} directly (not {@code -1});
 *       assert that the calling code checks {@code if (ret != 0)} and does not call {@code waitpid}
 *       on an uninitialised pid — the child was never created.
 *   <li>{@code pthread_create} returns {@code E2BIG} directly; assert that the application does not
 *       treat {@code E2BIG} from thread creation as an argument-too-long error — this would
 *       indicate a misconfigured stack attribute, not an oversized argument vector.
 *   <li>Assert that the application does not retry on {@code E2BIG} with the same argument vector —
 *       the kernel limit will not change between calls; the argument vector must be trimmed or the
 *       environment cleaned before retrying.
 * </ul>
 *
 ...
 * bytes must not exceed {@code ARG_MAX} (typically 131072 bytes on Linux, queryable via {@code
 * getconf ARG_MAX}). The check is: {@code sum(strlen(argv[i])+1) + sum(strlen(envp[j])+1)} plus
 * pointer overhead must be below the kernel limit. Environment variables from the parent process
 * accumulate silently over long-running processes, and each new {@code exec} carries the full
 * environment; this is the primary source of unexpected {@code E2BIG} in production.
 *
 ...
 * when the errno is unexpected for the specific call. Single-selector variants (e.g., {@code
 * ChaosExecveE2big}) are preferable when testing specific exec paths precisely.
 *
 * <p>The ARG_MAX limit is per-exec, not per-process: repeated {@code fork} calls without exec are
 * not affected by ARG_MAX. Only the transition to a new executable (via exec or spawn) triggers the
 * check. Applications that use large environment passthrough to child processes — common in CI/CD
 * pipelines, container entrypoints, and cloud-function runtimes — are most at risk.
 *
 ...
 * without setting {@code errno}; {@code pthread_create} also returns the error code directly. Code
 * that checks {@code if (ret == -1 && errno == E2BIG)} silently misses errors from spawn and
 * pthread_create — the correct check is {@code if (ret != 0)} for the latter two families.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2 exercises the error path without blocking
 * startup; values above 0.1 will prevent the container init sequence from spawning any
