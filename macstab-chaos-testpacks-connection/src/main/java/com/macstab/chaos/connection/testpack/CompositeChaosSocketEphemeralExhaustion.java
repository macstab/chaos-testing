 ...
 * happens when the kernel's ephemeral port range ({@code /proc/sys/net/ipv4/ip_local_port_range})
 * is saturated — typically at 28,232 simultaneous connections per destination — or during TIME_WAIT
 ...
 * partial and hard to diagnose: established pool connections work while new connection attempts
 * fail silently. Connection pools that do not health-check on borrow surface as cascading {@code
 * ConnectException} storms when the pool becomes exhausted and new connections are needed.
 *
 * <h2>Industry references</h2>
 *
 * <p>Ephemeral port exhaustion is documented in AWS re:Post ("TCP port exhaustion on EC2
 * instances") and in the Linux kernel networking FAQ. The TIME_WAIT accumulation variant is
 * described in Cloudflare's blog "How TCP sockets work" and in the NGINX performance tuning guide.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
