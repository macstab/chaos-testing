 ...
 * (selector = {@code MMAP_ANON}, effect = LATENCY) pair. Unlike the errno variants, the
 * latency primitive always delegates to the kernel — it only adds wall-clock cost before doing so.
 *
 * <p><strong>What chaos this applies:</strong> every {@code mmap(MAP_ANONYMOUS)} call intercepted by
 ...
 * starts. The shared library interposes the libc wrappers for the {@code MMAP_ANON} syscall
 ...
 * applies the rule to every capable container. Use the repeatable form ({@code @ChaosMmapAnonLatencys})
...
public @interface ChaosMmapAnonLatency {
  ...
  /**
   * Latency to inject before every matching {@code mmap(MAP_ANONYMOUS)} call, in milliseconds.
