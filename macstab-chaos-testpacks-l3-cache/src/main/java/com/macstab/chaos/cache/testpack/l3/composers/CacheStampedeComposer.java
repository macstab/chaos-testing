...
public final class CacheStampedeComposer implements L3Composer<IncidentChaosCacheStampede> {
    ...
    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosCacheStampede ann) {
        final List<Object> handles = new ArrayList<>();

        final var adv = CompositeConnectionChaos.standard().advanced();
        handles.add(adv.apply(container,
                NetRule.latency(Endpoint.wildcard(), NetOperation.RECV, Duration.ofMillis(ann.latencyMs()), ann.toxicity())));
    ...
    public List<String> describe(final IncidentChaosCacheStampede ann) {
        return List.of(
                ...
                "connection: RECV latency=" + ann.latencyMs() + "ms toxicity=" + ann.toxicity() + " (DB overload)",
