 ...
 *       processing path; rename should be performed asynchronously or with a deadline that
 *       protects the calling thread from unbounded blocking.
 * </ul>
 *
 ...
 * returns, on overloaded storage backends under heavy write pressure, and when the directory's
 * data blocks are not cached and must be fetched from disk to locate and modify the entry.
 *
 ...
 * completes in microseconds — it requires only in-memory directory updates and a journal entry.
 * The latency injection simulates the case where the directory blocks are cold or the storage
 * device is under pressure, making the journal commit take orders of magnitude longer.
 *
 * <p>On NFS, the rename requires a round-trip to the server for the directory modification; the
 * client blocks until the server acknowledges the atomic update. NFS server-side rename latency
 * can easily reach hundreds of milliseconds under load. Applications that assume rename is
 * "instant" because it is instant on local storage will fail timeouts in NFS-mounted environments.
 *
 ...
 * {@code rename(2)} directly and returns only after the kernel call completes. The injected delay
 * adds directly to the wall-clock time of {@code Files.move()}.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
