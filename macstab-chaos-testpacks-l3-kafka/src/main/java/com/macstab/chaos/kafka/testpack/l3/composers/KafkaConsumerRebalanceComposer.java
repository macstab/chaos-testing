 ...
 * the broker-side symptom of a consumer that stops sending heartbeats during a long pause,
 * causing the group coordinator to trigger a consumer group rebalance.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class KafkaConsumerRebalanceComposer implements L3Composer<IncidentChaosKafkaConsumerRebalance> {

    public KafkaConsumerRebalanceComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosKafkaConsumerRebalance ann) {
        final List<Object> handles = new ArrayList<>();

        final String scenarioId = JvmPlanAccumulator.instance().mintScenarioId("KafkaConsumerRebalance");
        ...
        final var scenario = ChaosScenario.builder(scenarioId)
                ...
                .effect(ChaosEffect.injectException(
                        ...
                        "simulated GC pause — poll deadline exceeded"))
        ...
        handles.add(conn.apply(container,
                NetRule.latency(Endpoint.wildcard(), NetOperation.RECV, Duration.ofMillis(ann.gcPauseMs()), 1.0)));
