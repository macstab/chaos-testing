 ...
 *
 * <p>Injects DataIntegrityViolationException at METHOD_EXIT on JDBC connection acquire paths to
 * reproduce the application-level symptom of Postgres sequence pre-allocation gaps after primary
 * failover reconnect.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
@Slf4j
public final class JdbcSequenceIdJumpComposer
    implements L3Composer<IncidentChaosJdbcSequenceIdJump> {
  ...
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosJdbcSequenceIdJump ann) {
    ...
    final ChaosScenario scenario =
        ChaosScenario.builder(scenarioId)
            .description(
                "L3 JDBC sequence ID jump — Postgres pre-allocation gap after failover reconnect")
            .selector(
    ...
    for (final Object h : handles) {
      if (h instanceof String scenarioId) {
        try {
          ...
          log.warn(
              "JdbcSequenceIdJumpComposer.removeAll: failed to remove JVM scenario {}",
              scenarioId,
              e);
        }
      }
  ...
  public List<String> describe(final IncidentChaosJdbcSequenceIdJump ann) {
    return List.of(
        ...
        "jvm: DataIntegrityViolationException on class prefix '"
            + ann.classPattern()
            + "' (METHOD_EXIT)",
