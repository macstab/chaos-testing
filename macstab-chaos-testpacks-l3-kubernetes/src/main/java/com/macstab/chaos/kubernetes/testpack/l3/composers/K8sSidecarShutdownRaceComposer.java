 ...
 * failures caused by Envoy/istio-proxy closing its listeners before the application container
 * has finished draining during pod shutdown.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class K8sSidecarShutdownRaceComposer implements L3Composer<IncidentChaosK8sSidecarShutdownRace> {

    public K8sSidecarShutdownRaceComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosK8sSidecarShutdownRace ann) {
        final List<Object> handles = new ArrayList<>();
        final var adv = CompositeConnectionChaos.standard().advanced();
        handles.add(adv.apply(container, NetRule.errno(Endpoint.wildcard(), NetOperation.CONNECT, Errno.ECONNREFUSED, ann.toxicity())));
        return handles;
    }
