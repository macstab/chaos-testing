 ...
 * chaos unit. It encodes exactly one (selector = {@code MMAP_FILE}, errno = {@code EBADF})
 * pair. The combination is safe by construction: this annotation class exists only because
 * {@code EBADF} is a valid POSIX result of {@code mmap (file-backed)}; the invalid combinations
 ...
 * in the test class. Use the repeatable form ({@code @ChaosMmapFileEbadfs}) to bind different
