...
/**
 * L1 chaos primitive: inject {@code EMFILE} on every libchaos-process-intercepted
 * {@code posix_spawnp} call inside the container, gated by {@link #probability}.
 *
 * <p><strong>What this simulates:</strong> per-process fd limit reached — typical of fd-leaks in pre-fork connection pools.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * @ChaosPosixSpawnpEmfile(probability = 0.01)
 * class MyTest { ... }
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> {@code 1e-3} to {@code 1e-2} mirrors realistic
 * production rates for {@code EMFILE} on {@code posix_spawnp}; {@code 1.0} typically
