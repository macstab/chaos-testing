 ...
 * — no such file or directory. The default probability is {@code 0.5}, simulating a race where
 * the source file disappears between the caller's {@code stat()} check and the actual
 * {@code rename()} system call.
 *
 * <h2>How it's created</h2>
 *
 * <p>Injects {@code IoRule.errno(path, RENAME_FROM, ENOENT, 0.5)} via libchaos-io's
 * {@code RENAME_FROM} operation. In production, atomic rename races occur in: write-ahead-log
 * segment rotation (producer renames the closed segment while the cleaner concurrently deletes it),
 * SSTable compaction in LSM-tree stores (RocksDB, LevelDB, Cassandra SSTables), and tmpfile +
 * rename patterns used by configuration management systems and package installers.
 *
 * <h2>How bad it is</h2>
 *
 ...
 * compaction atomicity via rename + ENOENT retry is described in the RocksDB Wiki (File
 * Operations) and the Cassandra CEP-7 architectural document. Apache Kafka's log segment
 * compaction relies on rename atomicity and explicitly handles ENOENT in its error path (KAFKA-6246).
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
public @interface CompositeChaosRenameRace {
  ...
  /**
   * Probability that a matched {@code rename()} source-path lookup returns {@code ENOENT}.
   * Default {@code 0.5} creates an observable but survivable rename-race condition.
   */
