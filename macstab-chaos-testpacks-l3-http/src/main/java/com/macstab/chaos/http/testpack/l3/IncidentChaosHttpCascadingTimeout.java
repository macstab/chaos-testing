...
/**
 * <h2>What this is</h2>
 * <p>Simulates a timeout cascade across a service call chain: each hop exhausts its deadline
 * budget waiting for the next, so a single slow upstream propagates full timeout failures
 * through every caller in the chain.
 *
 * <h2>Composed of</h2>
 * <ul>
 ...
 *   <li>DNS: forward lookup latency of {@code latencyMs / 2} ms — pre-connection DNS delay
 *       further reduces the effective timeout margin available for actual request processing
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Critical</strong><br>Every service in the call chain fails with timeouts;
 * thread pools saturate with blocked request threads; the outage blast radius spans the entire
...
@ChaosL3(composer = "com.macstab.chaos.http.testpack.l3.composers.HttpCascadingTimeoutComposer", severity = Severity.CRITICAL)
