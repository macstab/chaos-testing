...
/**
 * Silently suppresses every {@code System.exit()} call, preventing the application from terminating
 * the JVM process even when it explicitly requests shutdown.
 *
 ...
 *
 * <ul>
 *   <li><strong>Application enters undefined state.</strong> Code paths after a suppressed {@code
 *       System.exit()} call are normally unreachable; the application may attempt to continue
 *       initialisation in a partially constructed state, producing NPEs or state-machine
 ...
 *   <li><strong>Production failure mode:</strong> a process that ignores its own exit request will
 *       hold locks indefinitely, block cluster rebalancing, and consume resources while appearing
 *       healthy to observers that only check process liveness and not logical health.
 * </ul>
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code System.exit(int)} performs three steps: it calls the registered security manager's
 * {@code checkExit()} (if any), runs all registered shutdown hooks in parallel, then calls {@code
 * Runtime.halt()} to terminate the JVM. Suppression at the {@code System.exit()} level prevents all
 * three steps; the calling thread unwinds back to its caller as if exit had not been requested.
 *
 ...
 * if it is the bottom of the call stack, the thread will simply complete and die. If all non-daemon
 * threads die, the JVM will exit naturally even with the suppression active.
 *
