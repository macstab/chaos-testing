 ...
 * <h2>Composed of</h2>
 * <ul>
 *   <li>DNS: EAI_AGAIN on every forward lookup — DNS name resolver retry storm; every
 *       {@code getaddrinfo()} call fails transiently, forcing the gRPC resolver to retry
 ...
 *   <li>Connection: CONNECT → ECONNREFUSED at {@code toxicity} — backends refuse connections
 *       during the period when the DNS-based LB is converging to new backend addresses
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>All backends become temporarily unreachable; the gRPC
 * channel enters TRANSIENT_FAILURE; client-side load balancing stalls until DNS resolves and new
 * connections are established; the warm-up latency of new channels extends the unavailability.
 *
...
@ChaosL3(composer = "com.macstab.chaos.grpc.testpack.l3.composers.GrpcLoadBalancingFailureComposer", severity = Severity.SEVERE)
