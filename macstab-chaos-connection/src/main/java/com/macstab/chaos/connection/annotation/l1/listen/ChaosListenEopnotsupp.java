...
/**
 * Injects {@code EOPNOTSUPP} on every libchaos-intercepted {@code listen} call inside the target
 * container, making the call fail as if the kernel returned {@code EOPNOTSUPP}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EOPNOTSUPP}) pair and has no runtime
 * selector-errno matrix to validate. The combination is safe by construction: this annotation class
 * exists only because {@code EOPNOTSUPP} is a valid POSIX result of {@code listen}.
 *
 ...
 * interceptor returns {@code -1} and sets {@code errno = EOPNOTSUPP} — from the application's
 * perspective this is indistinguishable from a real kernel-level failure. Specifically this
 * simulates: operation not supported on socket — e.g. listen on UDP socket.
 *
 ...
 * class. Use the repeatable form ({@code @ChaosListenEopnotsupps}) to bind different probabilities
