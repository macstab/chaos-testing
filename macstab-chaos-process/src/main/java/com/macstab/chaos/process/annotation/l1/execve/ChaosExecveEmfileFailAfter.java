 ...
 * then injects {@code EMFILE} on every subsequent call until the rule is removed.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one (selector
 * = {@code EXECVE}, errno = {@code EMFILE}, effect = FAIL_AFTER) tuple. FAIL_AFTER is the process
 ...
 * subsequent call returns {@code -1} with {@code errno = EMFILE}, regardless of real kernel
 ...
 * ({@code @ChaosExecveEmfileFailAfter.Repeatable}) to apply different counters to different
 * containers.
 *
