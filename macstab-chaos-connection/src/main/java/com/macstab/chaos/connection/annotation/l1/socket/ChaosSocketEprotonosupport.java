...
/**
 * Injects {@code EPROTONOSUPPORT} on every libchaos-intercepted {@code socket} call inside
 * the target container, making the call fail as if the kernel returned {@code EPROTONOSUPPORT}.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive — the smallest declarative
 * chaos unit. It encodes exactly one (selector, errno = {@code EPROTONOSUPPORT}) pair and has no
 * runtime selector-errno matrix to validate. The combination is safe by construction: this
 * annotation class exists only because {@code EPROTONOSUPPORT} is a valid POSIX result of
 * {@code socket}.
 *
 * <p><strong>What chaos this applies:</strong> on every {@code socket} call that the
 * libchaos interceptor sees, a Bernoulli trial with probability {@link #toxicity} is run.
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EPROTONOSUPPORT} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: protocol not supported — kernel feature missing (e.g. SCTP not enabled).
 *
 ...
 * @ChaosSocketEprotonosupport(toxicity = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosSocketEprotonosupports}) to bind different probabilities to
