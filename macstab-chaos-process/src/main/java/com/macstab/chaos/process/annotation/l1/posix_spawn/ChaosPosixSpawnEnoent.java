...
/**
 * L1 chaos primitive: inject {@code ENOENT} on every libchaos-process-intercepted
 * {@code posix_spawn} call inside the container, gated by {@link #probability}.
 *
 * <p><strong>What this simulates:</strong> executable not found — typical of PATH misconfiguration in container init.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * @ChaosPosixSpawnEnoent(probability = 0.01)
 * class MyTest { ... }
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> {@code 1e-3} to {@code 1e-2} mirrors realistic
 * production rates for {@code ENOENT} on {@code posix_spawn}; {@code 1.0} typically
