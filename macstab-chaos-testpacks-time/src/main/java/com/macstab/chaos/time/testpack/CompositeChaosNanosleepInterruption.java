 ...
 * simulating signal delivery that interrupts the sleep syscall. On Linux, {@code nanosleep} returns
 * {@code -1} with {@code errno = EINTR} when a signal is received during the sleep; POSIX requires
 * callers to check the remaining time and restart. This scenario exercises whether the
 * application's sleep loop correctly handles the interrupted case.
 *
 * <h2>How it is created</h2>
 *
 * <p>Applies one libchaos-time rule: {@code nanosleep:ERRNO:EINTR@<toxicity>}. The probability is
 * set to {@link #toxicity} — at the default 0.4, roughly 40 % of {@code nanosleep} calls return
 * {@code EINTR} instead of completing. The remaining 60 % succeed normally, so the application is
 * not completely starved of sleep — it must correctly retry the interrupted ones.
 *
 * <h2>How bad it is</h2>
 *
 ...
 * dropped retry budgets, or incorrect backoff calculations. In production this failure mode occurs
 * when an async-signal-safe signal handler (SIGTERM, SIGHUP, SIGUSR1) is delivered to a thread
 * blocked in {@code nanosleep}.
 *
 ...
 * The "Fallacies of Distributed Computing" list implicitly covers signal handling as an out-of-band
 * communication channel that callers must anticipate.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
public @interface CompositeChaosNanosleepInterruption {
  ...
  /**
   * Probability that a given {@code nanosleep()} call returns {@code EINTR} instead of completing.
   * Must be in {@code (0.0, 1.0]}. Defaults to {@code 0.4} (40 % interruption rate).
   *
