 ...
 * causing the calling code to observe a transient resource-unavailable failure on anonymous memory
 * allocation.
 *
 ...
 *   <li>The calling code receives the transient-failure signal: {@code MAP_FAILED} return, {@code
 *       errno} 11, {@code strerror}: "Resource temporarily unavailable".
 * </ol>
 *
 ...
 *       should retry with back-off or report a transient error rather than crashing.
 *   <li>glibc {@code malloc} does not retry on {@code EAGAIN} from {@code mmap} — it propagates
 *       {@code NULL}; the JVM raises {@code OutOfMemoryError} for the affected allocation.
 *   <li>Assert that the application tolerates transient allocation failures without data
 *       corruption, and that retry logic (where present) does not loop infinitely.
 * </ul>
 *
 * Production failure mode: kernels under severe memory pressure combined with {@code RLIMIT_AS}
 * limits, or a cgroup memory controller near its high-watermark, can return {@code EAGAIN} instead
 * of {@code ENOMEM} for anonymous mappings — a transient condition that catches callers who treat
 * all allocation failures as fatal.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX allows {@code mmap} to return {@code EAGAIN} when the kernel has insufficient resources
 * to complete the request but the condition is transient. On Linux this arises when {@code
 * mmap_sem} (now {@code mmap_lock}) cannot be acquired in write mode within the kernel's internal
 * retry budget, or when a memory cgroup is operating in soft-limit mode and the page reclaim loop
 * yields {@code -ENOMEM} through the {@code EAGAIN} propagation path in specific kernel versions
 * (3.x/4.x era). Under normal conditions the kernel prefers {@code ENOMEM} for permanent exhaustion
 * and avoids {@code EAGAIN} for anonymous mappings; its occurrence in production is therefore rare
 * but not impossible.
 *
 * <p>Because {@code EAGAIN} semantically means "try again", well-designed allocators may loop when
 * they receive it. This creates a subtle risk: if the probability is high, the retry loop spins
 * without ever succeeding, consuming CPU and causing watchdog timeouts. Design retry budgets
 * carefully and pair this annotation with a timeout assertion.
 *
 ...
 * matters only in native C/C++ allocators and in frameworks that wrap {@code mmap} directly (e.g.
 * RocksDB's memory table allocator, MemSQL's native storage engine).
 *
 * <p>Compared with siblings: {@code ENOMEM} signals structural exhaustion (retry is futile); {@code
 * EAGAIN} signals transience (retry may succeed). {@code EINVAL} signals a programmer error. Test
 * both {@code EAGAIN} and {@code ENOMEM} to verify the application handles each code path
 * correctly.
 *
 * <h2>Example</h2>
 *
 ...
 * 0.01 exhaust retry budgets quickly and produce cascading failures.
 *
