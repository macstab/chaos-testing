...
class MixedTopologyTest {
  ...
  static void verifyAllInstancesStarted(
      final List<SentinelCluster> sentinels, final List<RedisConnectionInfo> standalones) {
    ...
    final SentinelCluster haCluster = sentinels.get(0);
    assertThat(haCluster.getMasterName()).isEqualTo("master-ha");
    ...
    assertThat(haCluster.getMasterContainer().isRunning()).isTrue();
  ...
  @AfterEach
  void cleanup(final List<SentinelCluster> sentinels, final List<RedisConnectionInfo> standalones) {
  ...
  void shouldIsolateDataAcrossAllInstances() {
    ...
    final SentinelCluster haCluster = RedisSentinel.INSTANCE.get("ha-cluster");
  ...
  void shouldSupportDifferentUseCases(
      final List<SentinelCluster> sentinels, final List<RedisConnectionInfo> standalones) {
    final SentinelCluster haCluster = sentinels.get(0);
    final RedisConnectionInfo cache = standalones.get(0);
    final RedisConnectionInfo session = standalones.get(1);
  ...
  void shouldAccessByIdProgrammatically() {
    ...
    assertThat(haCluster.getMasterPort()).isNotEqualTo(cache.getPort());
    assertThat(haCluster.getMasterPort()).isNotEqualTo(session.getPort());
    assertThat(cache.getPort()).isNotEqualTo(session.getPort());
  }

  // ==================== Helper Methods ====================

  private RedisClient createClientForSentinel(final SentinelCluster cluster) {
    final String uri =
        String.format("redis://%s:%d", cluster.getMasterHost(), cluster.getMasterPort());
    return RedisClient.create(uri);
  }

  private RedisClient createClientForStandalone(final RedisConnectionInfo info) {
