 ...
 * {@code posix_spawnp} calls succeed, then fail every subsequent call with {@code EAGAIN}.
 *
 * <p><strong>What this simulates:</strong> resource exhaustion at the posix_spawnp boundary — typical of fd-leak / thread-leak bugs that succeed indefinitely until the resource pool is drained, at which point every further call fails with EAGAIN.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
