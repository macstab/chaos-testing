 ...
 *
 * <p>Used by {@link PackageInstaller#ensureInstalled} to express the relationship between
 * the binary that must exist on the container's {@code PATH} ({@code tool}) and the package
 * that provides it ({@code packageName}).
 *
...
public record ToolPackage(String tool, String packageName) implements ToolDefinition {
   ...
   *
   * <p>Convenience factory for the common case where installing the package provides a binary
   * with the same name (e.g. {@code stress-ng}, {@code cpulimit}).
   *
