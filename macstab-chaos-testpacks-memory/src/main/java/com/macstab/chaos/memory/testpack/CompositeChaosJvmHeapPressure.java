 ...
 * probability. The JVM's garbage collector expands the heap by calling {@code mmap(MAP_ANONYMOUS)} to
 * commit new pages; when this call fails the JVM must either trigger a full GC, shrink survivor
 * regions, or throw {@code java.lang.OutOfMemoryError: Java heap space}. This scenario exercises the
 * code path that connects the OS-level {@code ENOMEM} to the JVM's heap-expansion error handler —
 ...
 * musl's mallocng for all sizes. In production JVM heap expansion failures arise from cgroup
 * {@code memory.max} limits being reached, from JVM {@code -Xmx} exhaustion, or from physical
 ...
 * that allocate large objects frequently will see elevated GC pause times and may throw
 * {@code OutOfMemoryError} on individual requests. Services with no {@code OutOfMemoryError} handling
 * will propagate the error to callers as 500s. At sustained 10% the service degrades noticeably but
 * typically recovers without a restart if load is reduced.
 *
 * <h2>Industry references</h2>
 *
 * <p>JVM heap expansion via anonymous mmap is documented in HotSpot internals
 * ({@code src/hotspot/os/linux/os_linux.cpp}, {@code pd_commit_memory}). The connection between
 * {@code ENOMEM} and {@code OutOfMemoryError} is described in JEP 387 (Elastic Metaspace) and in the
 * JVM Specification (chapter 6, memory areas). cgroups v2 memory accounting that causes heap
 * expansion failure is covered in the Kubernetes memory-resource documentation and in the systemd
 * cgroup integration guide.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
