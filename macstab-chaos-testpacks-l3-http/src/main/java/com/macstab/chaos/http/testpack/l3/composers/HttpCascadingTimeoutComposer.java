 ...
 *
 * <p>Applies TCP-level timeout, half-duration DNS latency, and SocketTimeoutException injection
 * to reproduce a multi-hop timeout cascade across a service call chain.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class HttpCascadingTimeoutComposer implements L3Composer<IncidentChaosHttpCascadingTimeout> {

    public HttpCascadingTimeoutComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosHttpCascadingTimeout ann) {
        ...
        final var scenario = ChaosScenario.builder(scenarioId)
                .description("HTTP cascading timeout — inject SocketTimeoutException on HTTP client methods")
                .selector(selector)
                .effect(ChaosEffect.injectException("java.net.SocketTimeoutException", "HTTP timeout cascade"))
    ...
    public List<String> describe(final IncidentChaosHttpCascadingTimeout ann) {
        return List.of(
                ...
                "jvm: SocketTimeoutException(\"HTTP timeout cascade\") on class prefix '" + ann.classPattern() + "' (METHOD_ENTER)",
