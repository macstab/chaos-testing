 ...
 *   <li>JVM: MonitorContention with {@code contendingThreads} threads holding a synthetic lock
 *       for {@code lockHoldMs} ms — models JNI critical section contention that delays GC
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>Heap is not exhausted; restarting the pod fixes the
 * issue immediately; the root cause (monitor contention delaying GC) is never identified because
 * the heap dump looks normal.
 *
...
@ChaosL3(composer = "com.macstab.chaos.jvm.testpack.l3.composers.JvmGcLockerFakeOomComposer", severity = Severity.SEVERE)
