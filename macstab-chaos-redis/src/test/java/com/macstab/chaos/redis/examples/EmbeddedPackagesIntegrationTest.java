...
class EmbeddedPackagesIntegrationTest {
  ...
  @RedisSentinel(
      ...
      sentinels = 3,
      packages = {"curl", "vim"})
  static class SentinelRedisWithPackagesTest {

    @Test
    @DisplayName("Should install packages in all Sentinel cluster containers")
    void shouldInstallPackagesInAllClusterContainers(SentinelRedis cluster) throws Exception {
      ...
      var masterCurl = cluster.master().execInContainer("which", "curl");
      assertThat(masterCurl.getExitCode()).as("Master should have curl").isZero();

      var masterVim = cluster.master().execInContainer("which", "vim");
