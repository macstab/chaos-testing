 ...
 *
 * <p>Injects an async request timeout exception, adds RECV latency to extend the drain window,
 * and adds PTHREAD_CREATE latency to delay thread join during executor shutdown — reproducing
 * the compound delay that causes the Kubernetes grace period to expire during a rolling deploy.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SpringGracefulShutdownComposer implements L3Composer<IncidentChaosSpringGracefulShutdown> {

    public SpringGracefulShutdownComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosSpringGracefulShutdown ann) {
        final List<Object> handles = new ArrayList<>();

        final String scenarioId = JvmPlanAccumulator.instance().mintScenarioId("SpringGracefulShutdown");
        ...
        handles.add(conn.apply(container,
                NetRule.latency(Endpoint.wildcard(), NetOperation.RECV, Duration.ofMillis(ann.drainMs()), 1.0)));

        final var proc = CompositeProcessChaos.standard().advanced();
        handles.add(proc.apply(container,
                ProcessRule.latency(ProcessSelector.PTHREAD_CREATE, Duration.ofMillis(ann.drainMs() / 2))));
    ...
    public List<String> describe(final IncidentChaosSpringGracefulShutdown ann) {
        return List.of(
                ...
                "jvm: AsyncRequestTimeoutException on class prefix '" + ann.classPattern() + "' (METHOD_ENTER)",
