 ...
 * intercepted by libchaos-memory, making all file-backed mapping operations succeed but take longer
 * than expected.
 *
 * <h2>What this annotation is</h2>
 *
 * L1 libchaos-memory primitive — one (selector = {@code MMAP_FILE}, effect = LATENCY) tuple. The
 * {@code MMAP_FILE} selector covers only file-backed {@code mmap} calls (those without {@code
 * MAP_ANONYMOUS}); use {@code ChaosMmapLatency} to add latency to both anonymous and file-backed
 * calls simultaneously. Unlike the errno variants, the latency primitive always delegates to the
 * kernel and the mapping succeeds; only wall-clock time is affected.
 *
 ...
 *   <li>The mapping succeeds but takes at least {@link #delayMs} ms longer than without the rule;
 *       anonymous allocations are not affected.
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 *
 * <ul>
 *   <li>All file-backed mapping operations are delayed by at least {@link #delayMs} ms; anonymous
 *       heap allocations are unaffected, allowing isolation of the file I/O stall path from the
 *       allocator path.
 *   <li>Database engines that use memory-mapped files for SST, WAL, or log segment I/O (RocksDB,
 *       LMDB, Kafka, Chronicle Queue) will see increased read and write latency proportional to the
 *       number of segment mappings established during the test; assert that operation SLOs are met
 *       or gracefully degraded.
 *   <li>Connection-pool timeouts, health-check deadlines, and replication lag SLOs may fire; assert
 *       that these surface as structured warnings rather than unhandled exceptions.
 * </ul>
 *
 * Production failure mode: under network-filesystem (NFS, CIFS) degradation or kernel-level
 * page-cache pressure combined with NUMA rebalancing, file-backed {@code mmap} calls stall for tens
 * to hundreds of milliseconds — causing database compaction threads, log-segment rotation, and
 * shared-library loading to block and exhaust wall-clock budgets across the application.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>File-backed {@code mmap} latency stalls arise from several kernel-level sources. When the page
 * cache does not contain the file's pages (cold start, cache eviction, or direct-I/O bypass), the
 * kernel must issue I/O to populate the pages before the mapping can be faulted; this I/O can take
 * tens of milliseconds on spinning disks or overloaded network storage. The {@code mmap} call
 * itself returns quickly (it only allocates a VMA), but the first access to the mapped pages
 * triggers a page fault and I/O wait — this is the "lazy" I/O cost that this annotation simulates
 ...
 * across the data pipeline. The delay injected here simulates the worst-case I/O cost at the {@code
 * mmap} call site, allowing tests to verify that SLOs hold even when new segment mappings are slow.
 *
 ...
 * This is valuable for profiling and for verifying that the fallback to {@code pread}/{@code
 * pwrite} (which does not involve {@code mmap}) correctly avoids the latency. Use {@code
 * ChaosMmapLatency} (the broad selector) when you want to stress both the allocator and the file
 * I/O path simultaneously.
 *
 ...
 * adherence when mapping is merely slow. Both are necessary for comprehensive resilience coverage
 * of applications that use memory-mapped file I/O.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
