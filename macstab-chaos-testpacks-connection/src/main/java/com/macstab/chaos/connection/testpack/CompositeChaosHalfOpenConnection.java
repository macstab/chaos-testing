 ...
 * half-open TCP connections. The local end believes the connection is established but receives a RST
 * when it next attempts to read. Applications that keep idle connections in a pool without health
 * checks will hand out dead connections to callers, causing delayed failures when the first request
 * is sent on a half-open socket.
 *
 ...
 * caller must retry, and the failed connection is returned to the pool as invalid. Multiplied across
 * many threads, this can cause a brief spike of errors before the pool evicts stale connections.
 *
 ...
 * timeout vs. application pool idle timeout mismatch is a frequently referenced StackOverflow issue.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
