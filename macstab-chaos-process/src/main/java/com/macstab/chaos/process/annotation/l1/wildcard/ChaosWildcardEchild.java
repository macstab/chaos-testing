 ...
 * observe a no-child-exists condition on any process-management call rather than only on {@code
 * waitpid}.
 *
 ...
 *   <li>When the trial fires, the interposer sets {@code errno = ECHILD} and returns {@code -1} (or
 ...
 *       (10); {@code pthread_create}/{@code posix_spawn} return {@code ECHILD} directly; {@code
 ...
 *       waiter).
 *   <li>{@code fork}/{@code posix_spawn}/{@code pthread_create} receiving ECHILD (via the wildcard)
 *       is unexpected; assert that the application's catch-all error handler logs the errno and the
 *       specific syscall name — ECHILD from a fork call indicates a wildcard rule is active and
 *       should be surfaced in diagnostic logs for operators to investigate.
 *   <li>Assert that the application does not retry on ECHILD from spawn calls — the child was never
 *       created and ECHILD does not indicate a transient condition on the spawn path; the
 *       application should escalate rather than retry.
 * </ul>
 *
 * Production failure mode: a process supervisor uses a SIGCHLD handler with a {@code waitpid(-1,
 * WNOHANG)} drain loop; the loop does not distinguish between ECHILD as loop-termination (expected)
 * and ECHILD arising before any children were spawned (unexpected); the supervisor silently
 * discards child state on unexpected ECHILD without alerting, losing track of which children
 * successfully started.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code ECHILD} from {@code waitpid} has two distinct semantics that the application must
 * distinguish: when called as {@code waitpid(-1, WNOHANG)} in a SIGCHLD handler drain loop, ECHILD
 * is the expected loop-exit condition signalling all children have been reaped; when called for a
 * specific pid ({@code waitpid(child_pid, ...)}) and ECHILD fires, it means the pid was already
 * reaped by another waiter (double-wait bug) or the pid was recycled to a different process. The
 * wildcard variant fires ECHILD on all process-management families, which tests whether the generic
 * error handler correctly propagates ECHILD from unexpected call sites.
 *
 * <p>The wildcard selector applying ECHILD to fork and posix_spawn is unusual — these calls do not
 * normally produce ECHILD in the kernel. This is an intentional property of the wildcard effect: it
 * tests whether the application's generic errno handling is correct regardless of the specific call
 * site. Applications that have a catch-all process error handler but only unit-test it for expected
 * errnos (EAGAIN, ENOMEM) may silently swallow ECHILD from unexpected call sites.
 *
 * <p>The SIGCHLD handler + drain loop pattern is the most common source of real ECHILD: the handler
 * calls {@code waitpid(-1, WNOHANG)} in a loop until ECHILD; if ECHILD fires early (before all
 * children are collected), the remaining children become permanent zombies. Applications must log a
 * warning when ECHILD fires before the expected number of children have been reaped.
 *
 ...
 * termination so moderate rates are acceptable; values above 0.05 may cause child-reaping loops to
 * exit before collecting all children, producing zombie accumulation.
 *
