 ...
 * sleeps for {@link #finalizerDelayMs()} milliseconds, then makes all of them unreachable so the GC
 * enqueues them on the finalizer queue faster than the finalizer thread can process them.
 *
 ...
 *       next GC cycle discovers them, promotes them to the finalizer queue (the {@code
 *       java.lang.ref.Finalizer} reference queue), and hands them to the single {@code Finalizer}
 *       thread.
 *   <li>The {@code Finalizer} thread processes finalizable objects one at a time, sequentially.
 *       With {@code objectCount = 1000} and {@code finalizerDelayMs = 100}, the thread requires 100
 *       seconds to drain the queue; during this time, the objects' memory cannot be reclaimed by
 *       the GC.
 ...
 *   <li><strong>GC overhead alarm triggers.</strong> The JVM's {@code GcOverheadLimit} (default 98%
 *       of time in GC with less than 2% heap reclaimed) may fire when the finalizer backlog
 ...
 *       (older {@code FileInputStream}, some JDBC drivers) release native resources in {@code
 *       finalize()}; a backed-up finalizer queue means file descriptors or database connections are
 *       not released promptly; assert that connection-pool exhaustion is handled gracefully.
 ...
 * unreachable and the object has a non-trivial {@code finalize()} method (one overriding {@code
 * Object.finalize()} with a non-empty body), the GC does not immediately free the object. Instead,
 * it wraps it in a {@code java.lang.ref.Finalizer} reference and enqueues it on a {@code
 * ReferenceQueue}. A single JVM-internal daemon thread — the {@code Finalizer} thread — dequeues
 * these references and calls {@code finalize()} on each object in turn. Only after {@code
 * finalize()} returns is the object eligible for a second GC pass that actually frees its memory.
 *
 ...
 *
 * <p>Modern Java best practice is to avoid {@code finalize()} entirely in favour of {@code
 * java.lang.ref.Cleaner} (Java 9+) or explicit {@code AutoCloseable} patterns, which do not suffer
 * from this single-thread bottleneck. This stressor is therefore most relevant for testing
