...
/**
 * Injects {@code ERRNO} on every libchaos-intercepted {@code waitpid} call inside
 ...
 * @ChaosWaitpidEinvalFailAfter(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosWaitpidEinvalFailAfters}) to bind different probabilities to
...
public @interface ChaosWaitpidEinvalFailAfter {
   ...
   * <p>Example:
   * <pre>{@code
   * @ChaosWaitpidEinvalFailAfter(id = "primary",  probability = 0.001)
   * @ChaosWaitpidEinvalFailAfter(id = "replica",  probability = 0.01)
