 ...
 *       must reduce argument size or use a file-based argument passing mechanism.
 ...
 *       argument batching or reduces the injected environment rather than crashing.
 *   <li>Assert that the application cleans up the {@code dirfd} file descriptor on {@code E2BIG}
 *       failure — the fd was opened before the exec attempt and must be explicitly closed since the
 *       exec did not proceed to replace the image and inherit fd ownership.
 * </ul>
 *
 ...
 * total environment size past {@code ARG_MAX}; the exec fails with {@code E2BIG} and the container
 * enters a crash loop with no explicit error message referencing the environment size.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>{@code execveat(dirfd, pathname, argv, envp, flags)} extends {@code execve} with a directory
 * reference: when {@code pathname} is relative, it is resolved relative to the directory opened by
 * {@code dirfd} rather than the process's current working directory. When {@code flags} includes
 * {@code AT_EMPTY_PATH} and {@code pathname} is the empty string, {@code dirfd} must refer to an
 * executable file and the kernel executes it directly (equivalent to {@code fexecve}). The {@code
 * E2BIG} failure semantics are identical to {@code execve}: the combined size of {@code argv} and
 * {@code envp} strings exceeds the {@code ARG_MAX} limit.
 *
 * <p>The {@code AT_EMPTY_PATH} pattern is used by container runtimes (runc, crun, gVisor) to
 * execute the container's entrypoint from a file descriptor rather than a path, which avoids TOCTOU
 * races between path lookup and execution. The {@code E2BIG} failure in this pattern is
 * particularly difficult to diagnose because the failing exec has no pathname in the error context
 * — the operator sees only "Argument list too long" with no binary name.
 *
 * <p>Compared with {@code execve}'s {@code E2BIG}: the failure is identical in cause and
 * remediation; the only difference is that {@code execveat} additionally requires managing the
 * {@code dirfd} lifecycle — the fd must be closed on exec failure since the exec path did not
 * proceed to the image-replacement phase where it would be inherited or closed.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 1e-2; {@code execveat} is typically called at
 * container startup and for each child spawn, so low probabilities exercise the error path.
 *
