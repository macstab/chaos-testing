...
public class NetworkChaosAnnotationSimpleExample {
  ...
  static class SentinelNetworkChaosExample {
    ...
    @Test
    void allContainersHaveNetworkTools(SentinelCluster cluster) {
      ...
      control.network().injectLatency(cluster.getMasterContainer(), Duration.ofMillis(100));

      System.out.println("✅ 100ms latency injected to master!");

      // Reset
      control.network().reset(cluster.getMasterContainer());
    }
  }
