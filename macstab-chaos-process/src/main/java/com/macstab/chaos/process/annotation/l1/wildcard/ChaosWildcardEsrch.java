 ...
 * libchaos-process — {@code waitpid}, {@code fork}, {@code execve}, {@code posix_spawn},
 * {@code pthread_create}, and their variants — simultaneously, gated by {@link #probability},
 * modelling stale-pid conditions and process-group dissolution that can affect all process
 ...
 *       runs.</li>
 *   <li>When the trial fires, the interposer sets {@code errno = ESRCH} and returns {@code -1}
 ...
 *       {@code errno = ESRCH} (3); {@code pthread_create}/{@code posix_spawn} return {@code ESRCH}
 *       directly; {@code strerror(ESRCH)}: "No such process".</li>
 * </ol>
 *
 * <h2>Observable effects and what to assert in tests</h2>
 * <ul>
 *   <li>{@code waitpid()} returns {@code -1} with {@code errno = ESRCH}; assert that the
 *       application treats ESRCH as a non-retryable diagnostic signal indicating a stale pid
 *       or dissolved process group — the process or group does not exist and will not reappear;
 *       assert that the stale pid is logged and removed from the application's child-tracking
 *       registry.</li>
 ...
 *       registry entry without retrying — a dissolved group cannot be restored by waiting.</li>
 ...
 *       should be surfaced for diagnostic purposes.</li>
 *   <li>Assert that ESRCH is distinguished from ECHILD: ESRCH means the process or group does
 *       not exist at all; ECHILD means the process exists but is not a child; the cleanup action
 *       differs — ESRCH requires removing the stale pid from the registry, ECHILD may indicate
 *       a double-wait bug requiring investigation of the wait logic.</li>
 * </ul>
 * Production failure mode: a process group manager tracks child process groups in a registry;
 * after all members of a group exit the group dissolves; subsequent group waits return ESRCH;
 * the manager treats ESRCH as a transient error (confusing it with EINTR) and retries in a loop;
 * the loop consumes CPU while the registry entry remains stale, preventing new groups from being
 * created with the same pgid until the manager is restarted.
 *
 * <h2>Deep technical dive</h2>
 * <p>{@code ESRCH} from {@code waitpid} has two distinct semantics depending on the pid argument:
 * for a specific positive pid, ESRCH means the process does not exist at all (not a child, not
 * in the system); for a negative pgid ({@code waitpid(-pgid, ...)}), ESRCH means no process in
 * that process group is a child of the caller, or the group itself does not exist. Both cases are
 * non-retryable — the process or group is gone. ESRCH is distinct from ECHILD: ECHILD fires when
 * a process exists but is not a child of the caller; ESRCH fires when the process or group does
 * not exist at all (for positive pids, the distinction depends on kernel version).
 *
 * <p>The wildcard selector fires ESRCH across all process-management families. ESRCH from
 * fork/posix_spawn/pthread_create is unusual (these normally produce EAGAIN, ENOMEM, or EPERM),
 * but the wildcard variant fires it to test whether the application's catch-all error handler
 * correctly identifies and logs ESRCH rather than conflating it with other errnos. This reveals
 * missing or incomplete errno classification in generic process-management error handling.
 *
 * <p>PID recycling produces apparent ESRCH: a child exits and is reaped; the kernel recycles its
 * pid to a new unrelated process; a late waitpid on the old pid receives ESRCH (the new process
 * is not a child); applications that track child pids across asynchronous operations must validate
 ...
 * }</pre>
 *
 * <p><strong>Probability guidance:</strong> 1e-3 to 5e-3; ESRCH from waitpid is non-retryable
 * so even low rates produce registry cleanups — start with 1e-3 to validate diagnostic logging
 * without causing excessive registry churn.
