...
/**
 * Injects {@code EPIPE} into {@code send(2)}, causing the call to return {@code -1} with {@code
 * errno = EPIPE} as if the remote peer closed its end of the connection gracefully (FIN), the
 * application continued sending data, and the remote peer responded with a TCP RST to reject the
 * unexpected data — producing the broken-pipe condition.
 *
 ...
 * send} call; when it fires the interposer returns {@code -1} with {@code errno = EPIPE} without
 ...
 *       will be terminated. Assert that the application either uses {@code MSG_NOSIGNAL} on send or
 *       installs a {@code SIGPIPE} handler (or sets it to {@code SIG_IGN}).
 ...
 *
 * <p>When the remote peer calls {@code close()} or {@code shutdown(SHUT_WR)}, it sends a TCP FIN to
 * the local socket. The local socket transitions to the CLOSE_WAIT state but can still send data.
 * If the application then calls {@code send()}, the kernel delivers the data; the remote peer, now
 * in a half-closed state and not expecting data, responds with a RST. On the next {@code send} the
 * kernel has received the RST, knows the pipe is broken, and returns {@code -1} with {@code errno =
 * EPIPE}. Simultaneously, the kernel raises {@code SIGPIPE} unless the application passed {@code
 * MSG_NOSIGNAL} in the send flags.
 *
 ...
 * now-closed receive buffer). Only the second send (after the RST is received) returns {@code
 * EPIPE}. This race is relevant to idempotency: the application cannot tell whether the first send
 * was processed before the peer closed the connection.
 *
 * <p>The JVM handles {@code SIGPIPE} at startup: it sets {@code SIGPIPE} to {@code SIG_IGN} for the
 * JVM process, so Java applications are not terminated by the signal. Instead, the error is
 * surfaced as a {@code SocketException} with the message "Broken pipe". Application code that wraps
 * socket writes in a loop should treat "Broken pipe" as a non-retriable terminal error for the
 * current connection and propagate a connection-loss event to callers.
 *
 ...
 * direct way to pass {@code MSG_NOSIGNAL} from user code; the JVM already suppresses the signal at
 * the process level, so this is only relevant for native code called via JNI.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
