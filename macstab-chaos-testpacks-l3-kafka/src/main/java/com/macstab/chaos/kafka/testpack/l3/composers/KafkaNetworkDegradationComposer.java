 ...
 */
public final class KafkaNetworkDegradationComposer
    implements L3Composer<IncidentChaosKafkaNetworkDegradation> {

  public KafkaNetworkDegradationComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosKafkaNetworkDegradation ann) {
    ...
    handles.add(
        conn.apply(
            container,
            NetRule.latency(
                Endpoint.wildcard(), NetOperation.SEND, Duration.ofMillis(ann.latencyMs()), 1.0)));
    handles.add(
        conn.apply(
            container,
            NetRule.latency(
    ...
    final String scenarioId =
        JvmPlanAccumulator.instance().mintScenarioId("KafkaNetworkDegradation");
    final var selector =
