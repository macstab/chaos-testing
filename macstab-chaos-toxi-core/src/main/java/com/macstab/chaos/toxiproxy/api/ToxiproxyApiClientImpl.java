...
/**
 * Production implementation of {@link ToxiproxyApiClient} that executes HTTP requests against
 * the Toxiproxy management API by running shell commands inside the container.
 *
 ...
 *
 * <p>HTTP commands ({@code curl}, {@code wget}, etc.) are generated by
 * {@link com.macstab.chaos.core.command.http.HttpCommandBuilder}, obtained via
 * {@link ContainerContext#http()}. This abstraction selects the appropriate HTTP client
 * for the container's OS distribution — Alpine may not have {@code curl} and uses {@code wget};
 * Debian/Ubuntu use {@code curl}. No tool names are hardcoded in this class.
 *
 * <h2>JSON Construction Without a JSON Library</h2>
 *
 * <p>Request payloads and response parsing use {@code String.format()} and regex respectively,
 * with no external JSON dependency. This is deliberate: the Toxiproxy API payloads are small,
 * structurally fixed, and well-understood. Adding Jackson or Gson for three to four request
 * payloads would be disproportionate for a test-only library. The trade-off is injection risk
 * if proxy or toxic names contain JSON special characters — callers must restrict names to
 * {@code [a-zA-Z0-9_-]}.
 *
 ...
 * void). These overloads implement a uniform exception-wrapping contract: {@link IOException}
 * is re-thrown as-is; any other {@link Exception} is wrapped as {@link IOException} with a
 * context message. This eliminates the repeated {@code try/catch} boilerplate that would otherwise
 * appear in every method. Functional interfaces ({@link ReturningApiCall} and {@link VoidApiCall})
 * allow passing lambdas to the overloads without checked exception erasure.
 *
 * <h2>Thread Safety</h2>
 *
 * <p>This class is unconditionally thread-safe. The only instance field is {@code apiUrl} (a
 * {@code String}, immutable). All mutable state during request processing is local to the method
 * call stack. Concurrent calls from multiple threads on the same instance are safe; Docker daemon
 * exec connection pool limits may throttle throughput under high concurrency.
 *
 * <h2>Stable Internal Contract</h2>
 *
 * <p>This class is the production implementation of a stable interface ({@link ToxiproxyApiClient}).
 * It is {@code final} and not intended for extension. Test doubles should implement
 * {@link ToxiproxyApiClient} directly.
 *
