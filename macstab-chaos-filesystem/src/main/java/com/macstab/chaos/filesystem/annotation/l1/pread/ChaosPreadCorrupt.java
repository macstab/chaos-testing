 ...
 *       walks the returned buffer, applying a per-byte Bernoulli trial with probability
 *       {@link #probability}; each selected byte has one randomly-chosen bit flipped.
 * </ol>
 *
 ...
 *       checksum verification catches the corruption and raises an error rather than processing
 *       the corrupted data.
 *   <li>{@code pread(2)} is commonly used by database engines for random-access page reads
 *       (each page is read at its known file offset without adjusting the file position). A
 *       corrupted page header length field causes the parser to read garbage; assert that the
 *       page checksum is verified before the page contents are trusted.
 *   <li>Applications that use memory-mapped files ({@code mmap}) are NOT affected by this
 *       injection — memory-mapped reads bypass the {@code pread} interposer. Use this annotation
 *       only to test code paths that use explicit {@code pread}-based I/O.
 *   <li>Assert that silent corruption on a index page is detected by the index consistency check
 *       at startup and triggers a rebuild rather than returning incorrect query results.
 * </ul>
 *
 ...
 *
 * <p>{@code pread(2)} reads {@code count} bytes from file descriptor {@code fd} at offset
 * {@code offset} without modifying the file's current position. It is equivalent to calling
 * {@code lseek} to the offset, calling {@code read}, and then restoring the position — but as a
 * single atomic operation. This makes it the preferred I/O primitive for multi-threaded applications
 * that read from different regions of a file concurrently, because each call is self-contained and
 * does not race with other threads' file position updates.
 *
