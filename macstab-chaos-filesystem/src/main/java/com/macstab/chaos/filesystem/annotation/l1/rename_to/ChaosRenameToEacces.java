 ...
 * causing the call to return {@code -1} with {@code errno = EACCES} as if the calling process does
 * not have write permission on the directory that would receive the new filename.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (operation = {@code RENAME_TO}, errno = {@code
 * EACCES}) tuple. The {@code RENAME_TO} operation models the permission check on the destination
 * path of {@code rename(2)}: the calling process must have write permission on the destination
 * directory to add the new name. A Bernoulli trial with probability {@link #probability} is run on
 * each intercepted {@code rename} call; when it fires the interposer returns {@code -1} with {@code
 * errno = EACCES} without performing any real kernel operation.
 *
 ...
 *       errno = EACCES}, simulating a permission failure on the destination directory.
 * </ol>
 *
 ...
 *       name to the destination directory; the source file remains at its original location and the
 *       destination is unchanged. Assert that the application treats this as a permission error and
 *       correctly identifies the destination directory as the failing resource.
 ...
 *   <li>Applications that promote work files from a staging area to a shared output directory owned
 *       by a different UID must handle {@code EACCES} on the rename to the output path; assert that
 *       the promotion failure is logged with the full destination path and the permissions of the
 *       output directory.
 *   <li>Distinguish from {@link ChaosRenameFromEacces}: a {@code RENAME_FROM} failure means the
 *       process cannot remove the old name; a {@code RENAME_TO} failure means it cannot add the new
 *       name. Applications should log which path caused the denial.
 * </ul>
 *
 ...
 * an administrator restricts write access to an output directory after deployment), when SELinux or
 * AppArmor policy prevents the process from writing to the destination context, and when the sticky
 * bit on the destination's parent directory prevents overwriting a file owned by another user.
 *
 ...
 *
 * <p>The sticky bit adds a further restriction: even with write permission on the directory, a
 * process cannot overwrite an existing destination entry if the destination file is owned by a
 * different user and the process does not have {@code CAP_FOWNER}. The kernel checks stickiness
 * after DAC — an {@code EACCES} from stickiness is indistinguishable to userspace from an {@code
 * EACCES} from missing write permission.
 *
 * <p>Java's {@code Files.move(Path, Path, ATOMIC_MOVE)} wraps {@code rename(2)} and throws {@code
 * IOException("Permission denied")} for {@code EACCES}. The {@code IOException} message does not
 * distinguish source-path from destination-path failures; applications that must route the error to
 * the right operator alert need to inspect both paths' permissions after the failure.
 *
 * <p>Compared with {@link ChaosRenameToErofs}: {@code EACCES} is a policy failure (potentially
 * resolvable by changing directory permissions or running as a different user); {@code EROFS} is a
 * storage-system failure (requires filesystem repair). Use {@code EACCES} to test the
