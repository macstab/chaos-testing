 ...
 * {@code suppress} effect applied to the {@code SCHEDULE_TICK} operation. It intercepts the tick
 * path — the moment the pool worker is about to invoke the dequeued task — and discards the
 * execution call without running the application's task body. The annotation is declared on the
 ...
 * {@code ScheduledThreadPoolExecutor}. When the interceptor fires:
 *
 * <ol>
 ...
 *   <li>The suppress effect skips the {@code run()} call entirely — the application's
 *       {@code Runnable} or {@code Callable} body does not execute.
 ...
 *       {@code null} result for {@code Runnable} tasks, and callers of {@code future.get()} unblock.
 * </ol>
 *
 ...
 *   <li>{@code ScheduledFuture.isDone()} returns {@code true} and {@code get()} returns
 *       {@code null} — the future completes without the task body having run; assert both.
 ...
 *       renewal) do not occur — assert via their observable consequences (e.g. stale metric,
 *       full cache, expired lease).
 * </ul>
 *
 ...
 *
 * <p><strong>Interception point.</strong> The tick interception point is inside
 * {@code ScheduledFutureTask#run()}, at the line that delegates to the wrapped delegate (either
 * the user's {@code Runnable.run()} or {@code Callable.call()}). The suppress effect returns
 * before that line. The {@code ScheduledFutureTask}'s own post-run bookkeeping (setting the
 * result, completing the future) executes normally — only the user's code is skipped.
 *
 * <p><strong>Future completion semantics.</strong> For one-shot tasks, the future is completed
 * with a {@code null} result (for {@code Runnable}) or the result of a zero-argument supplier
 * (for {@code Callable}). Because the suppress skips the user's {@code Callable.call()}, the
 * result is {@code null} — callers awaiting the result via {@code future.get()} receive
 * {@code null} rather than the expected computed value. Callers that do not tolerate {@code null}
 * results will fail with a {@code NullPointerException}.
 *
 * <p><strong>Periodic task re-scheduling.</strong> For {@code scheduleAtFixedRate} and
 * {@code scheduleWithFixedDelay}, the next tick is scheduled from within the task's
 * {@code run()} method (the outer wrapper, not the user's body). Because the outer wrapper's
 * re-scheduling logic executes before the suppress takes effect on the inner delegate call, the
 * next tick <em>is</em> re-scheduled even when the user's body is suppressed. This means periodic
 * tasks continue to fire (and be suppressed) on every tick — assert that no tick body executes
