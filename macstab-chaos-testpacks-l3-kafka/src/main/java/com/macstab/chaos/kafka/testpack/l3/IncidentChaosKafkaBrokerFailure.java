...
/**
 * <h2>What this is</h2>
 * <p>Simulates a complete Kafka broker failure: all new connections are refused, DNS resolution
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Critical</strong><br>Producers accumulate unacked batches until
 * {@code delivery.timeout.ms} expires; consumers lose group membership as heartbeats stop;
 * offset commit drift is possible if auto-commit fires during the outage window.
 *
 * <h2>Industry references</h2>
 * <p>Broker down → producer/consumer retry storm with exponential backoff collision is
 * a well-documented Kafka failure mode. Producers in high-throughput pipelines exhaust
 * {@code buffer.memory} before the broker recovers; consumers trigger rebalances as
 * {@code max.poll.interval.ms} is exceeded waiting for fetch responses.
 *
 * <h2>Example</h2>
 * <pre>{@code
 * @AppContainer
...
@ChaosL3(composer = "com.macstab.chaos.kafka.testpack.l3.composers.KafkaBrokerFailureComposer", severity = Severity.CRITICAL)
