...
/**
 * Delays every libchaos-memory-intercepted {@code every interposed VM syscall} call by {@link #delayMs}
 ...
 * (selector = {@code WILDCARD}, effect = LATENCY) pair. Unlike the errno variants, the
 * latency primitive always delegates to the kernel — it only adds wall-clock cost before doing so.
 *
 * <p><strong>What chaos this applies:</strong> every {@code every interposed VM syscall} call intercepted by
 ...
 * starts. The shared library interposes the libc wrappers for the {@code WILDCARD} syscall
 ...
 * applies the rule to every capable container. Use the repeatable form ({@code @ChaosWildcardLatencys})
...
public @interface ChaosWildcardLatency {
  ...
  /**
   * Latency to inject before every matching {@code every interposed VM syscall} call, in milliseconds.
