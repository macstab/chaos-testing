...
/**
 * Injects {@code ENOSYS} into every interposed time syscall ({@code clock_gettime}, {@code nanosleep},
 * {@code usleep}), causing each to return {@code -1} with {@code errno = ENOSYS} as if the kernel
 * does not implement the syscall variant requested.
 *
 * <h2>What this annotation is</h2>
 *
 * <p>L1 libchaos primitive. Encodes exactly one (selector = {@code WILDCARD}, errno = {@code ENOSYS})
 * tuple. The {@code WILDCARD} selector matches all three interposed time syscalls simultaneously —
 * equivalent to applying {@link ChaosClockGettimeEnosys}, {@link ChaosNanosleepEnosys}, and
 * {@link ChaosUsleepEnosys} in a single annotation. No runtime selector-errno validation is needed.
 *
 ...
 *   <li>Assert that the application selects a non-syscall fallback (e.g. monotonic counter,
 *       {@code CLOCK_COARSE}) or terminates with a meaningful diagnostic rather than crashing
 *       silently.
 * </ul>
 *
 * <p>In production, simultaneous {@code ENOSYS} across all time syscalls occurs on stripped or
 * embedded kernels that omit the {@code clock_gettime} VDSO and the {@code nanosleep} syscall —
 * a configuration sometimes used in ultra-minimal container base images.
 *
 ...
 * (CONFIG_POSIX_TIMERS=n) can disable the POSIX timer subsystem entirely, causing all
 * {@code clock_gettime} variants except {@code CLOCK_REALTIME} to return {@code ENOSYS}.
 * The wildcard injection simulates this condition across all three time interfaces simultaneously.
 *
 ...
 * code that calls time interfaces directly through JNI or via a native library will see the
 * {@code ENOSYS} errors; pure Java code that uses {@code System.nanoTime()} may not, depending on
 * the JVM fallback path.
 *
 ...
 *
 * <p>Sibling per-syscall annotations ({@link ChaosClockGettimeEnosys}, {@link ChaosNanosleepEnosys},
 * {@link ChaosUsleepEnosys}) allow targeted injection to a single syscall when the fall-through
 * chain of a specific library needs to be tested in isolation.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
