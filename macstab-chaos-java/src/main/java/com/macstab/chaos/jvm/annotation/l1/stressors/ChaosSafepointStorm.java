...
/**
 * Drives a continuous storm of JVM safepoints inside the target container by issuing {@code
 * System.gc()} calls on a tight interval, repeatedly stopping all application threads for
 ...
 * milliseconds. Each call forces the JVM to bring all application threads to a safepoint, producing
 * a stop-the-world pause even if the heap does not need collection.
 *
 * <h2>What chaos this applies</h2>
 *
 * <ol>
 *   <li>A background stressor thread loops, sleeping {@link #gcIntervalMs()} milliseconds between
 *       iterations, and calls {@code System.gc()} on each iteration.
 *   <li>Each {@code System.gc()} call causes the JVM to request a safepoint: the VM thread sets a
 *       safepoint-pending flag, and every application thread checks that flag at the next safepoint
 *       poll — typically at method-exit bytecodes, backward branches, or JNI entry/exit boundaries.
 *       Threads that reach the poll suspend themselves.
 ...
 *       flag and all application threads resume.
 *   <li>The cycle repeats continuously. With a 100 ms interval the application experiences at least
 *       ten stop-the-world pauses per second in addition to any pauses triggered by normal GC
 *       activity.
 * </ol>
 *
 ...
 *       Assert that SLA-bound endpoints remain within timeout even when pauses recur at {@link
 *       #gcIntervalMs()} frequency.
 ...
 *       endpoint may time out during a stop-the-world pause; assert that the probe's {@code
 *       timeoutSeconds} is sized to survive the pause duration or that the probe uses a dedicated
 *       thread that is less likely to be at a safepoint boundary.
 ...
 * calls to no-ops, which means this stressor will not trigger GC cycles — but it will still request
 * safepoints for the GC operation, depending on the JVM implementation. On most HotSpot builds,
 * {@code DisableExplicitGC} prevents the GC work but does not prevent the safepoint handshake. On
 * ZGC and Shenandoah (concurrent collectors), the "stop-the-world" phase is very short
 * (sub-millisecond), so this stressor has a less severe effect on those collectors than on G1 or
 * Parallel GC where full collections pause for much longer.
 *
