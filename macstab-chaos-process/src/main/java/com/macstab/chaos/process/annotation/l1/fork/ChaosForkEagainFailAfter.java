 ...
 *       {@code errno = EAGAIN} and returns {@code -1} without issuing the real kernel call.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 11, {@code strerror}: "Resource
 *       temporarily unavailable"; no child process is created; the calling process continues in its
 *       current state.
 * </ol>
 *
 ...
 *       return {@code -1} with {@code errno = EAGAIN}; assert that the application implements a
 *       bounded retry loop with backoff rather than an infinite retry or a hard failure, since
 *       EAGAIN from fork is a transient resource condition in production.
 ...
 *       child processes exit — assert that the application's process pool has an upper bound that
 *       is set below {@code RLIMIT_NPROC} to avoid hitting the limit in production.
 *   <li>Assert that the application does not treat post-threshold EAGAIN as a permanent failure
 *       requiring operator escalation — EAGAIN from fork is self-healing once child processes exit
 *       and the uid's process count falls below {@code RLIMIT_NPROC}.
 * </ul>
 *
 * Production failure mode: a process-isolation service forks a child for each incoming request; the
 * request rate exceeds the rate at which children exit; the uid's process count approaches {@code
 * RLIMIT_NPROC}; after N successful forks, all subsequent forks return EAGAIN; the service has no
 * upper bound on its process pool and no retry logic, so all requests during the exhaustion window
 * fail and the service enters a degraded state that persists until the process count falls.
 *
 ...
 * all subsequent forks return EAGAIN until children are reaped. Setting {@link
 * #successesBeforeFailure} to the uid's {@code RLIMIT_NPROC} minus the baseline process count
 ...
 * independent probability p of failing — this models transient kernel pressure where failures are
 * scattered across the request stream. Under FAIL_AFTER, the first N forks succeed and then all
 * subsequent forks fail until the rule is removed — this models the resource-ceiling scenario where
 * the pid table fills deterministically as the service handles its Nth request. The two patterns
 * require different application responses: transient failures call for retry; ceiling failures call
 * for load-shedding and backpressure.
 *
 * <p>The counter scope matches the container lifetime, not the test method. When the annotation is
 * at class scope, a test class can execute multiple test methods: early methods exercise the
 ...
 *
 * <p>Contrast with {@code ChaosForkEnomem} (ERRNO, ENOMEM): fork-ENOMEM indicates the kernel cannot
 * allocate memory for the child process structures (task_struct, mm_struct, kernel stack), which is
 * a different resource class from pid-table exhaustion. Applications should implement different
 * retry strategies for each: EAGAIN is always self-healing (wait for children to exit); ENOMEM may
 * persist longer under node-level memory pressure.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * number of concurrent child processes before the uid's {@code RLIMIT_NPROC} is reached; values in
 * the range 20–200 cover most process-isolation service scenarios; 0 means the pid table is full
 * from the first fork attempt.
 *
