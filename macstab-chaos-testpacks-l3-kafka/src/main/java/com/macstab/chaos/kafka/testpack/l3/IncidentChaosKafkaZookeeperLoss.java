 ...
 *   <li>Connection: CONNECT → ECONNREFUSED at {@code toxicity} — even if a cached address is used,
 *       the connection is refused; producers block waiting for metadata
 * </ul>
 *
 * <h2>How bad it is</h2>
 *
 * <p>Severity: <strong>Critical</strong><br>
 * Producers block indefinitely; no metadata is available; consumers cannot commit offsets; the
 * entire Kafka pipeline stalls.
 *
 * <h2>Industry references</h2>
 *
 * <p>ZooKeeper loss causing Kafka metadata unavailability is a well-known cluster failure mode:
 * without ZooKeeper quorum, brokers cannot elect a controller, partition leadership is frozen, and
 * producers block until delivery.timeout.ms is exceeded.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
@ChaosL3(
    composer = "com.macstab.chaos.kafka.testpack.l3.composers.KafkaZookeeperLossComposer",
