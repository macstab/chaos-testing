...
/**
 * Intercepts {@code ServerSocketChannel.accept()} and holds the calling thread for
 * {@link #delayMs()} milliseconds before the new inbound connection is handed to the Netty or
 * Undertow pipeline, simulating a slow accept loop that causes the kernel's TCP backlog queue to
 * fill and inbound connection attempts to be silently dropped or RST-ed.
 *
 ...
 *       completed TCP handshakes queue ahead of the slow accept loop; once full, the kernel silently
 *       drops SYN packets (Linux) or sends TCP RST (some OS configurations); assert that connecting
 *       clients observe {@code Connection refused} or long connection timeouts rather than fast
 *       rejection, depending on kernel configuration.
 ...
 * internal implementation. Netty's {@code NioServerSocketChannel.doReadMessages()} calls
 * {@code javaChannel().accept()} inside the boss event loop after {@code Selector.select()}
 * returns {@code OP_ACCEPT} readiness. The chaos delay fires before the actual accept syscall
 * ({@code accept4} on Linux), blocking the boss loop thread.
 *
 ...
 * kernel either drops new SYN packets (so the client retransmits) or sends RST depending on
 * {@code /proc/sys/net/ipv4/tcp_abort_on_overflow}.
 *
 * <p>Netty's boss group calls {@code accept()} in a loop until all pending connections are
 * drained or the configured {@code maxMessagesPerRead} is reached. Each iteration incurs the
 * delay, so a burst of pending connections in the backlog queue will compound the total delay
 * time: ten pending connections at 100 ms each means the boss loop is occupied for a full second
 * before returning to the selector.
 *
 ...
 * only the acceptor threads and does not directly block I/O worker threads, making the effect
 * more isolated than in Netty's boss-worker model.
 *
