...
/**
 * Injects {@code EADDRNOTAVAIL} on every libchaos-intercepted {@code bind} call inside
 * the target container, making the call fail as if the kernel returned {@code EADDRNOTAVAIL}.
 *
 ...
 * annotation class exists only because {@code EADDRNOTAVAIL} is a valid POSIX result of
 * {@code bind}.
 *
 ...
 * When it fires the interceptor returns {@code -1} and sets {@code errno = EADDRNOTAVAIL} — from
 * the application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: address not available — interface unconfigured; network-setup race.
 *
 ...
 * class. Use the repeatable form ({@code @ChaosBindEaddrnotavails}) to bind different probabilities to
