 ...
 *       specific kernel implementations when a second {@code listen} call conflicts with an existing
 *       listener on the same port via {@code SO_REUSEPORT}; assert that the application handles it
 ...
 *   <li>Assert that the application logs the failure with the port number and process information so
 *       that operators can identify which process holds the conflicting listener.
 ...
 * </ul>
 *
 * <p>In production, {@code EADDRINUSE} from {@code listen} can occur when multiple processes attempt
 * to listen on the same port without {@code SO_REUSEPORT}, when a container restart creates a new
 * process before the previous process's socket reaches TIME_WAIT expiry, and in specific kernel
 * versions where the {@code SO_REUSEPORT} validation conflicts with an existing listener.
 *
 ...
 * multiple sockets bind to the same address and port with {@code SO_REUSEPORT}, the kernel validates
 * at {@code listen} time that all sockets sharing the port have compatible options; a mismatch
 * causes {@code EADDRINUSE} from {@code listen}.
 *
 * <p>This distinction is important for server framework startup logic: frameworks that catch
 * {@code EADDRINUSE} from {@code bind} and retry with {@code SO_REUSEADDR} may not handle
 * {@code EADDRINUSE} from {@code listen} correctly, since the latter indicates a different kind of
