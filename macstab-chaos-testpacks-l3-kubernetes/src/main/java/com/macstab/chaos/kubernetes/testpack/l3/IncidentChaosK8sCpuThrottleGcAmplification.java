...
/**
 * <h2>What this is</h2>
 * <p>Simulates the CFS CPU throttle / GC amplification loop: a Kubernetes CPU limit causes the
 * CFS scheduler to throttle the JVM. The JVM has spawned N GC threads (one per core), and CFS
 * throttles all of them simultaneously. A 50 ms GC pause becomes 400 ms wall-clock time.
 * The Kubernetes liveness probe times out and kills the pod.
 *
 * <h2>Composed of</h2>
 * <ul>
 *   <li>JVM: SafepointStorm every {@code gcIntervalMs} ms — drives frequent GC safepoints that
 *       are then extended by CFS throttle into multi-hundred-millisecond pauses
 *   <li>Connection: RECV → timeout — models downstream services timing out while waiting for
 *       responses from the throttled JVM
 * </ul>
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Critical</strong><br>71% of Kubernetes deployments with CPU limits
 * experience this; liveness probes kill healthy pods under transient load, causing cascading
 * restart loops that amplify rather than heal the incident.
 *
...
@ChaosL3(composer = "com.macstab.chaos.kubernetes.testpack.l3.composers.K8sCpuThrottleGcAmplificationComposer", severity = Severity.CRITICAL)
