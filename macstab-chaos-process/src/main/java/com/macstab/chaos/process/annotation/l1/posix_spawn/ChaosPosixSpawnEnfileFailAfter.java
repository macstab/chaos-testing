...
/**
 * Injects {@code ERRNO} on every libchaos-intercepted {@code posix spawn} call inside
 ...
 * @ChaosPosixSpawnEnfileFailAfter(probability = 0.001)
 * class FaultTest {
 *   @Test
 ...
 * class. Use the repeatable form ({@code @ChaosPosixSpawnEnfileFailAfters}) to bind different probabilities to
...
public @interface ChaosPosixSpawnEnfileFailAfter {
   ...
   * <p>Example:
   * <pre>{@code
   * @ChaosPosixSpawnEnfileFailAfter(id = "primary",  probability = 0.001)
   * @ChaosPosixSpawnEnfileFailAfter(id = "replica",  probability = 0.01)
