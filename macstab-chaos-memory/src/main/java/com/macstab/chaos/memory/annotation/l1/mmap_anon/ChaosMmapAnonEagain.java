...
/**
 * L1 chaos primitive: inject {@code EAGAIN} on every libchaos-memory-intercepted
 * {@code mmap(MAP_ANONYMOUS)} call inside the container, gated by {@link #probability}.
 *
 * <p><strong>What this simulates:</strong> temporary failure (would block) — surfaces in kernels under memory pressure or rlimit exhaustion.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * production rates for {@code EAGAIN} on {@code mmap(MAP_ANONYMOUS)}; {@code 1.0} produces
