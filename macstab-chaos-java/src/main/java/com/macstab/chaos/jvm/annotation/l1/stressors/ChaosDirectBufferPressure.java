...
/**
 * Activates a self-driving JVM stressor that exhaust direct buffer memory (off-heap NIO) inside the
 * target container's JVM for the duration of the test class or test method.
 *
 ...
 *
 * <p><strong>What chaos this applies:</strong> the stressor exhaust direct buffer memory (off-heap
 * NIO). The effect persists throughout the test and is not probabilistic — it runs continuously at
 * the configured intensity until the rule is removed.
 *
 ...
 * ({@code @ChaosDirectBufferPressures}) to apply different stressor intensities to different
 * containers.
 *
