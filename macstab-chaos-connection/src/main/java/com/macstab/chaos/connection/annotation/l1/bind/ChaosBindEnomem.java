...
/**
 * Injects {@code ENOMEM} into {@code bind(2)}, causing the call to return {@code -1} with
 * {@code errno = ENOMEM} as if the kernel could not allocate the internal routing and socket
 * address structures required to register the local address.
 *
 ...
 * {@code bind} call; when it fires the interposer returns {@code -1} with {@code errno = ENOMEM}
 ...
 *   <li>Applications that retry {@code bind} on {@code ENOMEM} are reasonable — unlike
 *       {@code EINVAL}, {@code ENOMEM} is a transient condition that may resolve as other allocations
 *       are freed; assert that the retry loop has a bounded maximum duration and back-off delay.
 *   <li>Assert that the application emits a memory-pressure alert or metric when it receives
 *       {@code ENOMEM} from a socket operation, since this indicates kernel memory exhaustion rather
 *       than an application-level problem.
 ...
 * share the host kernel's memory, and a memory-hungry neighbour container triggers the OOM condition.
 *
 ...
 * kernel-side {@code ENOMEM} from a socket syscall does not indicate that the JVM heap is full.
 * The kernel and JVM use entirely separate memory allocators; a host can have ample JVM heap while
 * the kernel networking slab is exhausted. Applications that conflate these two forms of OOM and
 * apply the same recovery strategy (e.g., triggering GC on socket ENOMEM) will fail to resolve the
 * actual kernel memory pressure.
 *
 * <p>Java's NIO and {@code ServerSocket} both map {@code ENOMEM} from {@code bind} to a
 * {@code SocketException} with the message "Cannot allocate memory". Application code that uses
 * {@code e.getMessage().contains("memory")} to detect this condition is fragile; the message text
 * varies across glibc versions and JVM implementations. Prefer inspecting the native error code via
 * JNA or a custom native wrapper for production-quality error classification.
 *
