 ...
 *
 * <p>Combines anonymous mmap ENOMEM, connection ECONNRESET, and JVM OutOfMemoryError to reproduce
 * the compound failure profile of a Redis maxmemory eviction storm.
 *
...
public final class RedisOomEvictionComposer implements L3Composer<IncidentChaosRedisOomEviction> {
  ...
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosRedisOomEviction ann) {
    ...
    final var selector =
        ChaosSelector.method(
            EnumSet.of(OperationType.METHOD_EXIT), NamePattern.prefix("redis"), NamePattern.any());
    final var scenario =
        ChaosScenario.builder(scenarioId)
            ...
            .effect(
                ChaosEffect.injectException(
                    "java.lang.OutOfMemoryError", "Redis maxmemory eviction OOM"))
