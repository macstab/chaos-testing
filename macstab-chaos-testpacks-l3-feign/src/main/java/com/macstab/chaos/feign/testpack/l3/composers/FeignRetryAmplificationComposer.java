 ...
 *
 * <p>Injects ECONNREFUSED on CONNECT to trigger brownout conditions, then injects IOException
 * at METHOD_EXIT on the Feign client to activate both the Feign retry layer and any enclosing
 * Resilience4j retry, producing up to 9× upstream amplification.
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class FeignRetryAmplificationComposer implements L3Composer<IncidentChaosFeignRetryAmplification> {

    public FeignRetryAmplificationComposer() {}

    @Override
    public List<Object> apply(final GenericContainer<?> container, final IncidentChaosFeignRetryAmplification ann) {
        ...
        final var scenario = ChaosScenario.builder(id)
                .description("Feign Retry Amplification — Feign × Resilience4j = 9 upstream calls per user request")
