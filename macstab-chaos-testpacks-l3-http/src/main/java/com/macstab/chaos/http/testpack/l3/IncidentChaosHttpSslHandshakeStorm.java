 ...
 * <h2>Composed of</h2>
 * <ul>
 *   <li>Connection: CONNECT → ECONNRESET at toxicity {@code toxicity} — TCP connection resets
 *       abort the TLS handshake at the socket level, mimicking mid-handshake RST packets
 ...
 *
 * <h2>How bad it is</h2>
 * <p>Severity: <strong>Severe</strong><br>SSL failures under load cause connection-pool
 * exhaustion as handshake threads block; combined with the higher CPU cost of failed TLS
 * negotiations, the load balancer and TLS terminator can be driven into saturation.
 *
...
@ChaosL3(composer = "com.macstab.chaos.http.testpack.l3.composers.HttpSslHandshakeStormComposer", severity = Severity.SEVERE)
