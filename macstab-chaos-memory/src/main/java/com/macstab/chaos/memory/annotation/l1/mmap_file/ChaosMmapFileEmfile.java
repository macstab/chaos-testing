 ...
 *
 * L1 libchaos-memory primitive — one (selector = {@code MMAP_FILE}, errno = {@code EMFILE}) tuple.
 ...
 *
 * <ul>
 *   <li>{@code mmap} returns {@code MAP_FAILED}; {@code errno = EMFILE} (24); the application must
 *       distinguish this from {@code ENFILE} — {@code EMFILE} is a per-process limit (solvable by
 *       closing file descriptors or raising {@code RLIMIT_NOFILE}); {@code ENFILE} is a system-wide
 *       limit (requires host-level intervention).
 ...
 *       descriptor pools before retrying; assert that eviction is triggered and the retry succeeds
 *       without data loss.
 *   <li>Assert that the application logs the per-process fd count and the current {@code
 *       RLIMIT_NOFILE} soft limit alongside the error so that operators can tune the limit without
 *       a code change.
 * </ul>
 *
 * Production failure mode: a database engine or log-structured storage system accumulates open file
 * descriptors across compaction and segment-rotation cycles without adequate eviction; as the
 * per-process descriptor count approaches {@code RLIMIT_NOFILE}, subsequent file-backed {@code
 * mmap} calls begin failing with {@code EMFILE}, preventing new segments from being memory-mapped
 * and degrading read throughput to zero.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>POSIX specifies {@code EMFILE} when {@code mmap} internally needs to allocate a new file table
 * entry and the per-process count would exceed {@code RLIMIT_NOFILE}. On Linux, the {@code
 * alloc_empty_file} path in {@code do_mmap_pgoff} calls {@code get_unused_fd_flags} to allocate a
 * slot in the file descriptor table; if the slot count is at the soft limit, the kernel returns
 * {@code -EMFILE}.
 *
 * <p>This kernel mechanism differs from the anonymous-mmap case: anonymous mappings never allocate
 * a file descriptor, so they cannot produce real {@code EMFILE} from the kernel. For file-backed
 * mappings, the kernel does increment a reference count on the {@code struct file} but does not
 * consume an additional fd slot from the process's descriptor table — the existing fd passed to
 * {@code mmap} is sufficient. However, some kernel paths and certain filesystem drivers
 * (particularly those implementing custom {@code mmap} file operations) may internally allocate
 * auxiliary fds. This annotation exercises the error path for this scenario.
 *
 * <p>In practice, the most common source of {@code EMFILE} in database engines is not from {@code
 * mmap} directly, but from the {@code open(2)} calls that precede the mapping. A high
 * per-file-segment fd count combined with an inadequate {@code RLIMIT_NOFILE} configuration causes
 * the {@code open} to fail first. This annotation targets the rarer but equally important case
 * where the {@code open} succeeded but the subsequent {@code mmap} fails due to auxiliary resource
 * exhaustion.
 *
 * <p>Compared with {@code ENFILE}: {@code EMFILE} is a per-process soft limit (solvable by {@code
 * setrlimit(RLIMIT_NOFILE)}); {@code ENFILE} is the system-wide global file-table limit set by
 * {@code fs.file-max} and requires host-level kernel parameter changes. Runbooks must distinguish
 * the two: an {@code EMFILE} incident can be mitigated by the container without host access; an
 * {@code ENFILE} incident requires escalation to the platform team.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
