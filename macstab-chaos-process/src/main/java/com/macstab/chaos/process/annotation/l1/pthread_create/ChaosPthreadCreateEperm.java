...
/**
 * Injects {@code EPERM} on every libchaos-intercepted {@code pthread create} call inside the target
 ...
 * exists only because {@code EPERM} is a valid POSIX result of {@code pthread create}.
 *
 ...
 * it fires the interceptor returns {@code -1} and sets {@code errno = EPERM} — from the
 * application's perspective this is indistinguishable from a real kernel-level failure.
 * Specifically this simulates: operation not permitted — capability-dropped containers performing
 ...
 * class. Use the repeatable form ({@code @ChaosPthreadCreateEperms}) to bind different
