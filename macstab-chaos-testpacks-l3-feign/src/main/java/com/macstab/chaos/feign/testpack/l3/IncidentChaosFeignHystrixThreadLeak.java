 ...
 * <h2>What this is</h2>
 *
 * <p>Simulates the Feign + Hystrix thread-leak failure: the Hystrix fallback fires on timeout, but
 * the underlying thread remains blocked in {@code socketRead0()} indefinitely. The thread pool
 * drains one thread per timed-out request; no recovery is possible without a pod restart.
 *
 * <h2>Composed of</h2>
 *
 * <ul>
 *   <li>Connection: RECV latency of {@code latencyMs} ms at 100% toxicity — makes every Feign call
 *       exceed the Hystrix timeout threshold
 ...
 * The pool drains thread-by-thread. Once exhausted, every new request is rejected immediately. No
 * recovery without a restart.
 *
 * <h2>Industry references</h2>
 *
 * <p>Netflix/Hystrix issue #1240 — thread leak when Hystrix timeout fires but underlying socket
 * read is still blocked in native code.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
@ChaosL3(
    composer = "com.macstab.chaos.feign.testpack.l3.composers.FeignHystrixThreadLeakComposer",
