 ...
 *
 * <p>Applies RECV latency on all connections and a JVM Reactor exception to reproduce the compound
 * failure profile of a blocking call on a reactor carrier thread — health endpoint times out, pod
 * is killed. (JDriven post-mortem)
 *
 * @author Christian Schnapka - Macstab GmbH
 */
public final class SpringWebFluxReactorStarvationComposer
    implements L3Composer<IncidentChaosSpringWebFluxReactorStarvation> {

  public SpringWebFluxReactorStarvationComposer() {}

  @Override
  public List<Object> apply(
      final GenericContainer<?> container, final IncidentChaosSpringWebFluxReactorStarvation ann) {
    ...
    final String id =
        JvmPlanAccumulator.instance().mintScenarioId("SpringWebFluxReactorStarvation-exc");
    final var sc =
  ...
  public List<String> describe(final IncidentChaosSpringWebFluxReactorStarvation ann) {
    return List.of(
        ...
        "connection: RECV latency="
            + ann.latencyMs()
            + "ms (downstream I/O blocks reactor threads)",
