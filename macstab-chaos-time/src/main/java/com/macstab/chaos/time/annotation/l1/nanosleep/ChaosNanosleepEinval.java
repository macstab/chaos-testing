 ...
 * of {@code nanosleep(2)} when the {@code tv_nsec} field is not in the range [0, 999999999] or
 ...
 *
 * <ul>
 *   <li>The sleep returns immediately with an error; back-pressure loops that rely on {@code
 *       nanosleep} for pacing will spin at full CPU if they do not check the return value.
 ...
 *   <li>Assert that the application does not enter a busy-spin on {@code EINVAL} and that it falls
 *       back to a bounded sleep using a corrected duration.
 * </ul>
 *
 ...
 * the legal maximum and triggers {@code EINVAL}; this is a common off-by-one in code that computes
 * durations by dividing nanoseconds and storing the remainder incorrectly.
 *
 ...
 * rate-limiters that build {@code timespec} structs from user-provided inputs. Java's {@code
 * Thread.sleep(long)} internally calls {@code nanosleep} and passes a well-formed struct, so it is
 * not directly affected; custom JNI code and native libraries are the primary targets.
 *
 * <p>Sibling annotations: {@link ChaosNanosleepEintr} targets signal interruption; {@link
 * ChaosNanosleepEfault} targets bad pointer arguments.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
