...
/**
 * Delays every libchaos-memory-intercepted {@code every interposed VM syscall} call by {@link
 * #delayMs} milliseconds before delegating to the real kernel call.
 *
 * <p><strong>What this annotation is:</strong> an L1 chaos primitive encoding exactly one (selector
 * = {@code WILDCARD}, effect = LATENCY) pair. Unlike the errno variants, the latency primitive
 * always delegates to the kernel — it only adds wall-clock cost before doing so.
 *
 * <p><strong>What chaos this applies:</strong> every {@code every interposed VM syscall} call
 * intercepted by libchaos-memory blocks for {@link #delayMs} ms before the kernel call is issued.
 * This simulates the wall-clock cost increase that surfaces under memory-pressure stall events,
 * transparent hugepage compaction storms, and NUMA balancing passes — none of which return an errno
 * but all of which add latency to allocations and can exhaust application-level timeouts.
 *
 ...
 * starts. The shared library interposes the libc wrappers for the {@code WILDCARD} syscall family.
 ...
 * ({@code @ChaosWildcardLatencys}) to set different delays on different containers simultaneously.
 *
