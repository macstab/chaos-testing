 ...
 *
 * L1 libchaos-process primitive — one (selector = {@code EXECVE}, errno = {@code EMFILE}, effect =
 ...
 * with {@code errno = EMFILE} until the rule is removed. This models progressive file descriptor
 * exhaustion scenarios where the process accumulates leaks over time until the fd table is too full
 * to allocate the slot needed to open the exec target binary.
 *
 ...
 *       intercepted call sets {@code errno = EMFILE} and returns {@code -1} without executing.
 *   <li>The calling code receives: {@code -1} return, {@code errno} 24, {@code strerror}: "Too many
 *       open files"; the counter remains tripped until removal.
 * </ol>
 *
 ...
 *       treat it as a recoverable resource error (close leaked fds or raise the limit) rather than
 *       a permanent binary-unavailability condition.
 ...
 *       fd-leak diagnostic rather than queuing spawn requests that will all fail with EMFILE.
 *   <li>Assert that the application correctly distinguishes exec-EMFILE (fd table full — check for
 *       leaks) from exec-ENOENT (binary not found — check deployment) in its diagnostic; the
 *       remediation operator actions are entirely different for these two errors.
 * </ul>
 *
 * Production failure mode: a long-running server process spawns helper subprocesses for each
 * request but leaves open file descriptors across each fork+exec iteration due to missing {@code
 * FD_CLOEXEC} flags or incorrect {@code close} calls; after N successful execs the process's fd
 * table is exhausted and all subsequent helper invocations fail with {@code EMFILE}, silently
 * breaking the feature that depends on the helper.
 *
 * <h2>Deep technical dive</h2>
 *
 * <p>The FAIL_AFTER model for exec-EMFILE accurately captures the progressive fd-leak scenario:
 * each successful exec that leaks fds brings the process closer to its {@code RLIMIT_NOFILE} limit;
 * after exactly N execs the limit is crossed and all subsequent execs fail. The deterministic
 * threshold makes the fd-leak path reproducible in tests, enabling assertions against the exact
 * exec count at which EMFILE first occurs.
 *
 * <p>A key diagnostic tool for exec-EMFILE is the process's current fd count at the time of the
 * failure. Applications should log {@code /proc/self/fd} count alongside the {@code EMFILE} error
 * from exec. The threshold value can be calibrated by running the application under the FAIL_AFTER
 * annotation with increasing {@link #successesBeforeFailure} values until the annotation's
 * threshold coincides with the application's actual fd-leak rate.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 * number of exec calls before fd exhaustion based on the known leak rate and the process's {@code
 * RLIMIT_NOFILE}; use zero to test startup-time fd-limit validation.
 *
